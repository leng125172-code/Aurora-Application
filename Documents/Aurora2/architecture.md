...
Web 调试页 ────┘                                           │
                                                   Station Actor / Device Proxy
                                                          │ local gRPC
                                      .NET Device/Vision Host（厂商 SDK 隔离）
                                                          │
                                      AuroraCommunication / C ABI / 厂商驱动
                                                          │
                                                PLC、运动控制器等设备
```
...
| .NET Device/Vision Host | 相机/投影仪/电机厂商 SDK、帧传输、既有视觉算子隔离 | 工站状态、远程认证、UI |
...
C++ 仅保留给已有稳定算法或厂商 SDK，通过窄 C ABI 或 Device/Vision Host 隔离进程接入。新业务逻辑默认使用 Rust。
