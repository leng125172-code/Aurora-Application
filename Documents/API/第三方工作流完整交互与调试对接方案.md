...
该接口不返回状态、内部变量、错误包装、结果图片列表或 `stagedKey`。普通标量和对象
保持原生 JSON 类型；文件类结果的 `value` 是 Blob Key，前端使用 Key 调用对应 Blob
查询/下载接口。`faulted` 和 `stopped` 只展示状态错误，不查询完成结果。
...
| 文本 | `string` | string |
| 文件 Blob Key | `blob` | string |
| 结构化数据 | `object`、`array` | object/array |
| 日期与标识 | `datetime`、`guid` | string |

不会返回 `System.Int32` 等 CLR 名称。普通文本的 `valueType` 为 `string`，文件
Blob Key 的 `valueType` 为 `blob`。前端仅对 `blob` 根据最后一个路径段的扩展名
决定展示方式，且忽略大小写：
...
`string` 值即使以文件扩展名结尾也不得按文件渲染；`blob` 无扩展名时按普通文件下载。
不得根据变量名或 Display 名称猜测文件类型。服务端原样返回 Blob Key，不返回临时 Key
或下载 URL。结束节点若直接选择 `Mat` 或 `PointCloudData`，前端应提示“请先使用存
Blob 算子”，避免内嵌大型二进制数据。
