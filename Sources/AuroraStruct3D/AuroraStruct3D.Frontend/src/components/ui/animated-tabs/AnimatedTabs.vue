...
const selectedValue = computed(() => (isControlled.value ? props.modelValue : uncontrolledValue.value))
const id = useId()
...
<template>
    <div class="flex h-full w-full flex-col" @mouseenter="pointerHovering = true" @mouseleave="pointerHovering = false">
        <div
            :class="
                cn(
        ...
        <div class="relative mt-24 min-h-0 flex-1">
            ...
            >
                <template #content="{ tab, active: slotActive, hovering: slotHovering }">
                    <slot name="content" :tab="tab" :active="slotActive" :hovering="slotHovering">
                        <component
                            :is="tab.content"
                            v-if="tab.content"
