...
const dataUrlMask = computed(() => `url("data:image/svg+xml,${encodeURIComponent(svgMask.value)}")`)

function updateSvgMask() {
    const responsiveFontSize = typeof props.fontSize === 'number' ? `${props.fontSize}vw` : props.fontSize
...
watch(
    () => [props.fontSize, props.fontWeight, props.textAnchor, props.dominantBaseline, props.fontFamily],
    updateSvgMask
)
