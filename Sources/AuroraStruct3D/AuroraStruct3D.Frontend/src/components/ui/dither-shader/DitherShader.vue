...
function applyDithering(ctx: CanvasRenderingContext2D, displayWidth: number, displayHeight: number, time: number = 0) {
        ...
        for (let x = 0; x < displayWidth; x += effectivePixelSize) {
            ...
            switch (props.ditherMode) {
                ...
                case 'crosshatch': {
                    ...
                    const line2 = (x - y + props.gridSize * 4) % (props.gridSize * 2) < props.gridSize ? 1 : 0
            ...
            switch (props.colorMode) {
                ...
                case 'custom': {
                    if (parsedCustomPalette.value.length === 2) {
                        const shouldBeDark = luminance < ditherThreshold
                        outputColor = shouldBeDark ? parsedCustomPalette.value[0] : parsedCustomPalette.value[1]
                    } else {
...
<template>
    <div ref="containerRef" :class="cn(`relative h-full w-full`, props.class)">
        <canvas
            ref="canvasRef"
