...
function calculateContrastRatios(color: HsvaColor) {
    ...
    const darkModeRatio =
        (Math.max(whiteTextLuminance, darkLuminance) + 0.05) / (Math.min(whiteTextLuminance, darkLuminance) + 0.05)
    // For light mode: black text against the light-composited color
    const lightModeRatio =
        (Math.max(blackTextLuminance, lightLuminance) + 0.05) / (Math.min(blackTextLuminance, lightLuminance) + 0.05)
    ...
    <div class="flex items-center justify-between gap-4 pt-2 select-none">
        <div class="flex items-center gap-4">
            ...
            <div class="flex flex-col justify-between">
                <span class="text-muted-foreground text-xs text-nowrap whitespace-nowrap">Contrast Ratio</span>
                <span class="text-sm">{{ currentContrastRatio }}</span>
            </div>
        </div>
        <div class="flex items-center justify-end gap-1 *:select-none">
            <UiBadge :variant="isAccessible.aa ? 'default' : 'secondary'" class="gap-1">
                <Icon :name="isAccessible.aa ? 'lucide:check' : 'lucide:x'" :size="12" />
                AA
            </UiBadge>
            <UiBadge :variant="isAccessible.aaa ? 'default' : 'secondary'" class="gap-1">
                <Icon :name="isAccessible.aaa ? 'lucide:check' : 'lucide:x'" :size="12" />
                AAA
            </UiBadge>
        </div>
    </div>
