...
import { type SerialPortOperationLogDto, getSerialPortLogs } from '@/api/serial-ports'
...
const logItems = ref<SerialPortOperationLogDto[]>([])
...
async function loadLogs(): Promise<void> {
    ...
    try {
        const result = await getSerialPortLogs({
            serialPortConfigId: logSelectedId.value,
...
<template>
    <div class="flex flex-col gap-4">
        <h1 class="text-2xl font-bold tracking-tight">{{ t('serialPort.logsTitle') }}</h1>
        ...
        <AppCard :beam="false">
            <div class="flex flex-col gap-3 border-b border-border/40 px-3 py-2">
                ...
                >
                    <span class="whitespace-nowrap text-sm text-muted-foreground">{{ t('serialPort.port') }}</span>
                    <Select
            ...
            <DataTable :value="logItems" :loading="logLoading" striped-rows size="small" data-key="id">
                <template #empty>
                    <div class="py-6 text-center text-sm text-muted-foreground">
                        {{ logSelectedId ? t('serialPort.logNoLogs') : t('serialPort.logSelectPortHint') }}
                    </div>
                </template>
                ...
                <Column field="isSuccess" :header="t('camera.logResult')" style="min-width: 5rem">
                    <template #body="{ data }">
                        <Tag
                            ...
                            :value="data.isSuccess ? t('serialPort.logSuccess') : t('serialPort.logFailed')"
                            class="!text-xs"
                        />
                    </template>
                </Column>
