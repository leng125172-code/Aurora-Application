...
const realtimeStatusText: Record<RealtimeSubscriptionStatus, string> = {
    stopped: '未订阅', starting: '订阅中', active: '已订阅', reconnecting: '重连中', error: '订阅失败',
}
const realtimeStatusClass: Record<RealtimeSubscriptionStatus, string> = {
    stopped: 'text-muted-foreground', starting: 'text-amber-600', active: 'text-emerald-600', reconnecting: 'text-amber-600', error: 'text-destructive',
}
...
async function stopRealtime() {
    ...
    try { await unsubscribePlcTags(id, activeSubscriptionId) } catch { /* 离线时服务端会清理跟踪记录 */ }
}
...
hub.onreconnecting(() => { realtimeStatus.value = 'reconnecting' })
hub.onreconnected(async () => {
    subscriptionId.value = undefined
    try { await startRealtime() } catch { /* 下次页面操作再重试，避免前端重连风暴 */ }
})
hub.onclose(() => {
    subscriptionId.value = undefined
    ...
    <div class="space-y-5 p-1 sm:p-2">
        ...
        <div class="grid gap-5 xl:grid-cols-2">
            <section class="space-y-3 rounded-lg border p-4">
                <div class="flex items-center justify-between"><h2 class="font-semibold">OPC UA 节点浏览</h2><Button size="small" severity="secondary" outlined label="刷新" @click="browse(parentAddress)" /></div>
                <Button v-if="parentAddress" size="small" text label="返回根节点" @click="browse()" />
                <div class="max-h-80 overflow-auto rounded border">
                    <Button v-for="node in nodes" :key="node.address" size="small" severity="secondary" text class="!flex !w-full !justify-between !rounded-none !border-b !p-2 !text-left" @click="selectNode(node)">
                ...
                <div class="grid gap-2 md:grid-cols-2">
                    ...
                    <InputText v-model="newTag.address" size="small" placeholder="NodeId，例如 ns=2;s=Tag1" class="md:col-span-2" />
                    <Select v-model="newTag.dataType" size="small" :options="dataTypeOptions" option-label="label" option-value="value" />
                    <Button size="small" label="加入点位表" @click="addTag" />
                </div>
            </section>
            <section class="space-y-3 rounded-lg border p-4">
                <div class="flex items-center justify-between"><div><h2 class="font-semibold">实时点位</h2><div class="text-xs" :class="realtimeStatusClass[realtimeStatus]">订阅状态：{{ realtimeStatusText[realtimeStatus] }}</div></div><div class="flex gap-2"><Button size="small" severity="secondary" outlined :label="subscriptionId ? '停止订阅' : '开始订阅'" @click="subscriptionId ? stopRealtime() : startRealtime()" /><Button size="small" severity="secondary" outlined label="批量读取" @click="readAll" /></div></div>
                <div class="max-h-[32rem] overflow-auto">
                    <div v-for="tag in tags" :key="tag.id" class="grid grid-cols-[1fr_1fr_auto] items-center gap-2 border-b py-2">
                        ...
                        <div>
                            ...
                            <details v-if="presentPlcValue(values[tag.id]?.engineeringValue).details.length" class="mt-1 text-xs">
                                ...
                                <dl class="mt-1 grid grid-cols-[auto_1fr] gap-x-3 gap-y-1 rounded bg-muted p-2">
                                    <template v-for="detail in presentPlcValue(values[tag.id]?.engineeringValue).details" :key="detail.label">
                                        <dt class="text-muted-foreground">{{ detail.label }}</dt><dd class="break-all">{{ detail.value }}</dd>
                                    </template>
                                </dl>
                                <details v-if="presentPlcValue(values[tag.id]?.engineeringValue).rawJson" class="mt-1">
                                    ...
                                    <pre class="mt-1 max-h-48 overflow-auto rounded bg-muted p-2 text-[10px] leading-4">{{ presentPlcValue(values[tag.id]?.engineeringValue).rawJson }}</pre>
                                </details>
                            </details>
                        </div>
                        <div class="flex gap-1"><template v-if="(tag.access & PlcTagAccess.Write) !== 0"><InputText v-model="writeValues[tag.id]" size="small" class="w-24" /><Button size="small" severity="secondary" outlined label="写入" @click="write(tag)" /></template><Button size="small" severity="danger" text label="删除" @click="removeTag(tag)" /></div>
                    </div>
