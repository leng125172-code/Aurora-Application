<template>
    <div class="relative min-h-full">
        <!-- 背景装饰 -->
        <InteractiveGridPattern class="absolute inset-0 opacity-20 pointer-events-none" :width="40" :height="40" />
    ...
    return groups.value
        .map((g) => {
            const eps = g.endpoints.filter((ep) => {
                ...
                const matchText =
                    !q || ep.path.toLowerCase().includes(q) || (ep.operation.summary ?? '').toLowerCase().includes(q)
...
function methodCount(method: string): number {
    return groups.value.reduce((s, g) => s + g.endpoints.filter((e) => e.method === method).length, 0)
}
