...
import { type MotorOperationLogDto, getMotorLogs } from '@/api/motors'
...
const logItems = ref<MotorOperationLogDto[]>([])
...
async function loadLogs(): Promise<void> {
    ...
    try {
        const result = await getMotorLogs({
            motorAxisId: logSelectedId.value,
...
<template>
    <div class="flex flex-col gap-4">
        <h1 class="text-2xl font-bold tracking-tight">{{ t('motor.logsTitle') }}</h1>
        ...
        <AppCard :beam="false">
            <div class="flex flex-col gap-3 border-b border-border/40 px-3 py-2">
                ...
                >
                    <span class="whitespace-nowrap text-sm text-muted-foreground">{{ t('motor.axis') }}</span>
                    <Select
                        v-model="logSelectedId"
                        :options="store.motors"
                        option-label="name"
                        option-value="id"
                        :placeholder="t('motor.selectAxis')"
                        size="small"
            ...
            <DataTable :value="logItems" :loading="logLoading" striped-rows size="small" data-key="id">
                <template #empty>
                    <div class="py-6 text-center text-sm text-muted-foreground">
                        {{ logSelectedId ? t('motor.logNoLogs') : t('motor.logSelectAxisHint') }}
                    </div>
                </template>
                ...
                <Column field="operationType" :header="t('camera.logOperation')" style="min-width: 7rem">
                    <template #body="{ data }">
                        <span class="text-xs">{{ data.operationType }}</span>
                    </template>
                </Column>
                <Column field="isSuccess" :header="t('camera.logResult')" style="min-width: 5rem">
                    <template #body="{ data }">
                        <Tag
                            ...
                            :value="data.isSuccess ? t('motor.logSuccess') : t('motor.logFailed')"
                            class="!text-xs"
                        />
                    </template>
                </Column>
                <Column field="roundTripMs" :header="t('camera.logDuration')" style="min-width: 5rem">
                    <template #body="{ data }">
                        <span class="tabular-nums text-xs">
                            {{ data.roundTripMs >= 0 ? `${data.roundTripMs} ms` : '—' }}
                        </span>
                    </template>
                </Column>
                <Column field="commandCode" :header="t('motor.logCmdCode')" style="min-width: 6rem">
                    <template #body="{ data }">
                        <span class="text-xs font-mono">{{ data.commandCode ?? '—' }}</span>
                    </template>
                </Column>
