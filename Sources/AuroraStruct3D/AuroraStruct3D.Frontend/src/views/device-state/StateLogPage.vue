...
<template>
    <div class="flex flex-col gap-4 p-4">
        <h1 class="text-lg font-semibold">{{ t('deviceState.stateLog') }}</h1>
        ...
        <div class="flex flex-wrap items-center gap-3">
            ...
            <button class="rounded border px-3 py-1 text-sm text-muted-foreground hover:bg-muted/50" @click="onReset">
                {{ t('deviceState.reset') }}
            </button>
            ...
            <table class="w-full min-w-[800px] text-sm">
                ...
                <tbody>
                    <tr v-if="loading" class="h-20">
                        <td colspan="7" class="text-center text-muted-foreground">{{ t('deviceState.loading') }}</td>
                    </tr>
                    ...
                    <tr v-for="item in items" :key="item.id" class="border-b transition-colors hover:bg-muted/30">
                        ...
                        <td class="max-w-xs px-3 py-2">
                            <span v-if="item.errorMessage" class="truncate text-destructive" :title="item.errorMessage">
                                {{ item.errorMessage }}
                            </span>
