...
async function loadCameras(): Promise<void> {
    ...
    try {
        ...
        cameras.value = result.items.filter((c) => c.isEnabled)
    } finally {
        loading.value = false
    }
}
...
async function toggleLed(): Promise<void> {
    if (!props.selectedProjectorId) return
    ledBusy.value = true
    try {
        if (ledIsOn.value) {
            await ledOff(props.selectedProjectorId)
            ledIsOn.value = false
        } else {
            await ledOn(props.selectedProjectorId)
            ledIsOn.value = true
        }
    } catch (e: unknown) {
        toast.error(e instanceof Error ? e.message : String(e))
...
async function doTakeIntrinsic(cam: CameraDeviceDto): Promise<void> {
    ...
    try {
        const photo = await takeIntrinsicPhoto({
            ...
            projectorDeviceId: props.selectedProjectorId ?? undefined,
        })
        // 追加到列表头部（最新的在前）
...
async function doTakeExtrinsic(cam: CameraDeviceDto): Promise<void> {
    ...
    try {
        const photo = await takeExtrinsicPhoto({
            calibProjectId: props.project.id,
            cameraDeviceId: cam.id,
            projectorDeviceId: props.selectedProjectorId,
        })
...
onMounted(async () => {
    await Promise.all([loadBoardConfig(), loadCameras()])
})
</script>
                ...
                <div v-if="expandedCameraId === cam.id" class="border-t border-border/40 bg-background/20">
                    <div class="grid grid-cols-1 gap-4 p-4 lg:grid-cols-3">
                        ...
                        <div class="flex flex-col gap-3">
                            <!-- 投影仪 LED 控制 -->
                            <div
                                v-if="props.selectedProjectorId"
