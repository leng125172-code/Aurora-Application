...
import type { TreeOutput } from '#/api-client/index';
...
const getParentKey = (
  ...
  for (const node of tree) {
    if (node.children) {
      if (
        node.children.some((item: { key: number | string }) => item.key === key)
      ) {
        parentKey = node.key;
...
// 获取所有节点的 key
const getAllKeys = (nodes: any[]): (number | string)[] => {
...
const unAddRolesTableEvents = {
  ...
  checkboxAll:({ records }: { records: { name: string }[] }) => {
    selectRoles.value = records;
  }
};
...
<template>
  <Page :auto-content-height="true">
    ...
    <div
      ...
      class="fixed z-50 rounded-md bg-white py-2 shadow-md"
      :style="{
