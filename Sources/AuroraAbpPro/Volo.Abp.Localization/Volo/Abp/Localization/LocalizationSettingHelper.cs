...
public static class LocalizationSettingHelper
{
    ...
    /// <returns></returns>
    public static (string cultureName, string uiCultureName) ParseLanguageSetting(
    ...
    )
    {
        ...
        if (
            splitted.Length == 2
            && CultureHelper.IsValidCultureCode(splitted[0])
            && CultureHelper.IsValidCultureCode(splitted[1])
        )
        {
