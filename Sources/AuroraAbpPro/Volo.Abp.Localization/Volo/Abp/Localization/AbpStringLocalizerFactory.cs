...
public class AbpStringLocalizerFactory : IStringLocalizerFactory, IAbpStringLocalizerFactory
{
    ...
    )
    {
        ...
        IStringLocalizer GetOrCreateLocalizer()
        {
            ...
            return LocalizerCache
                .GetOrAdd(resourceName, _ => CreateStringLocalizerCacheItem(resource))
                .Localizer;
        }
    ...
    )
    {
        ...
        return new StringLocalizerCacheItem(
            new AbpDictionaryBasedStringLocalizer(
                resource,
                resource
                    .BaseResourceNames.Select(x =>
                        CreateByResourceNameOrNullInternal(x, lockCache: false)
                    )
                    .Where(x => x != null)
                    .ToList()!,
    ...
    )
    {
        ...
        return new StringLocalizerCacheItem(
            new AbpDictionaryBasedStringLocalizer(resource, baseLocalizers, AbpLocalizationOptions)
        );
    }
    ...
    {
        services.Replace(
            ServiceDescriptor.Singleton<IStringLocalizerFactory, AbpStringLocalizerFactory>()
        );
