...
public class InboxProcessor : IInboxProcessor, ITransientDependency
{
    ...
    protected virtual async Task RunAsync()
    {
        ...
        )
        {
                ...
                while (true)
                {
                    ...
                    foreach (var waitingEvent in waitingEvents)
                    {
                        Logger.LogInformation(
                            $"Start processing the incoming event with id = {waitingEvent.Id:N}"
                        );
                        ...
                        try
                        {
                            ...
                            Logger.LogInformation(
                                $"Processed the incoming event with id = {waitingEvent.Id:N}"
                            );
                        }
                        catch (Exception e)
                        {
                            Logger.LogError(
                                e,
                                $"Event with id = {waitingEvent.Id:N} processing failed."
                            );

                            if (
                                EventBusBoxesOptions.InboxProcessorFailurePolicy
                            ...
                            if (
                                EventBusBoxesOptions.InboxProcessorFailurePolicy
                                == InboxProcessorFailurePolicy.RetryLater
                            )
                            {
                                using (
                                    var uow = UnitOfWorkManager.Begin(
                                ...
                                )
                                {
                                    ...
                                    if (
                                        waitingEvent.RetryCount
                                        >= EventBusBoxesOptions.InboxProcessorMaxRetryCount
                                    )
                                    {
                                        Logger.LogWarning(
                                            $"Event with id = {waitingEvent.Id:N} has exceeded the maximum retry count. Marking it as discarded."
                                        );

                                        await Inbox.RetryLaterAsync(
                                            waitingEvent.Id,
                                            waitingEvent.RetryCount,
                                            null
                                        );
                                    ...
                                    waitingEvent.NextRetryTime = GetNextRetryTime(
                                        waitingEvent.RetryCount,
                                        EventBusBoxesOptions.InboxProcessorRetryBackoffFactor
                                    );

                                    Logger.LogInformation(
                                        $"Event with id = {waitingEvent.Id:N} will retry later. "
                            ...
                            if (
                                EventBusBoxesOptions.InboxProcessorFailurePolicy
                                == InboxProcessorFailurePolicy.Discard
                            )
                            {
                                using (
                                    var uow = UnitOfWorkManager.Begin(
                                ...
                                {
                                    Logger.LogInformation(
                                        $"Event with id = {waitingEvent.Id:N} will be discarded."
                                    );
            ...
            else
            {
                ...
                {
                    await Task.Delay(
                        EventBusBoxesOptions.DistributedLockWaitDuration,
                        StoppingToken
                    );
                }
    ...
    {
        return await Inbox.GetWaitingEventsAsync(
            EventBusBoxesOptions.InboxWaitingEventMaxCount,
            EventBusBoxesOptions.InboxProcessorFilter,
            StoppingToken
        );
    }
