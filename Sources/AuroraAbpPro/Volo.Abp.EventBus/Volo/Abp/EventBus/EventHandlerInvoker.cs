...
public class EventHandlerInvoker : IEventHandlerInvoker, ISingletonDependency
{
    ...
    public async Task InvokeAsync(IEventHandler eventHandler, object eventData, Type eventType)
    {
        var cacheItem = _cache.GetOrAdd(
            $"{eventHandler.GetType().FullName}-{eventType.FullName}",
            _ =>
            {
                ...
                {
                    item.Local = (IEventHandlerMethodExecutor?)
                ...
                {
                    item.Distributed = (IEventHandlerMethodExecutor?)
                        Activator.CreateInstance(
                            typeof(DistributedEventHandlerMethodExecutor<>).MakeGenericType(
                                eventType
                            )
                        );
                }
        ...
        if (cacheItem.Local == null && cacheItem.Distributed == null)
        {
            throw new AbpException(
                "The object instance is not an event handler. Object type: "
