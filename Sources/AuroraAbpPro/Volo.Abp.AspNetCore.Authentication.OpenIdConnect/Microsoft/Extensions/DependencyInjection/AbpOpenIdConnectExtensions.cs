...
public static class AbpOpenIdConnectExtensions
{
        ...
        builder.Services.Configure<AbpClaimsPrincipalFactoryOptions>(options =>
        {
            ...
            if (!openIdConnectOptions.Authority.IsNullOrEmpty())
            {
                options.RemoteRefreshUrl =
                    openIdConnectOptions.Authority.RemovePostFix("/") + options.RemoteRefreshUrl;
            }
        });
            ...
            options =>
            {
                ...
                options.Events.OnTokenValidated = async (context) =>
                {
                    var logger = context.HttpContext.RequestServices.GetRequiredService<
                        ILogger<AbpAspNetCoreAuthenticationOpenIdConnectModule>
                    >();
                    var client =
                        context.HttpContext.RequestServices.GetRequiredService<IOpenIdLocalUserCreationClient>();
                    try
                    {
                    ...
                    if (
                        CultureHelper.IsValidCultureCode(culture)
                    ...
                    )
                    {
                        context.Response.OnStarting(() =>
                        {
                            logger.LogInformation(
                                $"Setting culture and ui-culture to the response. culture: {culture}, ui-culture: {uiCulture}"
                            );
                    ...
                    {
                        logger.LogWarning(
                            $"Invalid culture or ui-culture parameter in the OpenIdConnect response. culture: {culture}, ui-culture: {uiCulture}"
                        );
                    }
                };
    ...
    private static void SetAbpTenantId(AuthorizationCodeReceivedContext receivedContext)
    {
        ...
        {
            receivedContext.TokenEndpointRequest?.SetParameter(
                tenantKey,
