...
namespace StackExchange.Profiling.Data
{
    ...
    public class MvcDiagnosticListener : IMiniProfilerDiagnosticListener
    {
        ...
        private readonly ConcurrentDictionary<(string, string), string> _descriptorNameCache =
            new();
        ...
        private object? StartAction<T>(string label, T descriptor)
            where T : ActionDescriptor
        {
            var profiler = MiniProfiler.Current;
            var stepName = GetName(
                label,
            ...
            CurrentTiming.Value = new StackTiming(
                descriptor,
                profiler != null
                    ? new Timing(profiler, profiler.Head, stepName, null, null, debugStackShave: 4)
                    : null,
                CurrentTiming.Value
            );
            return null;
        ...
        private object? StartActionResult<T>(T result)
            where T : IActionResult
        {
            var profiler = MiniProfiler.Current;
            var stepName = result switch
            {
                ViewResult vr => vr.ViewName.HasValue()
                    ? GetName("View", vr.ViewName)
                    : nameof(ViewResult),
                ContentResult cr => cr.ContentType.HasValue()
                    ? GetName("Content", cr.ContentType)
                    : nameof(ContentResult),
                ObjectResult or => or.DeclaredType != null
                    ? GetName("Object", or.DeclaredType.Name)
                    : nameof(ObjectResult),
                StatusCodeResult scr => scr.StatusCode > 0
                    ? GetName("Status Code", scr.StatusCode.ToString())
                    : nameof(StatusCodeResult),
                JsonResult jr => jr.ContentType.HasValue()
                    ? GetName("JSON", jr.ContentType)
                    : nameof(JsonResult),
        ...
        private object? StartFilter<T>(string label, T filter)
            where T : IFilterMetadata
        {
            ...
            if (profiler?.Options is MiniProfilerOptions opts && opts.EnableMvcFilterProfiling)
            {
                var stepName = GetName(
                    label,
                    filter.GetType().Name,
                    name => name.EndsWith("Attribute") ? name[..^"Attribute".Length] : name
                );
                CurrentTiming.Value = new StackTiming(
                    filter,
                    new Timing(
        ...
        private object? StartView<T>(T state, string label, string viewName)
            where T : class
        {
            var profiler = MiniProfiler.Current;
            if (profiler?.Options is MiniProfilerOptions opts && opts.EnableMvcViewProfiling)
            {
                ...
                var stepName = GetName(
                    label,
                    viewName,
                    name => name?.StartsWith("/Views/") == true ? name["/Views/".Length..] : name
                );
                CurrentTiming.Value = new StackTiming(
                    state,
                    new Timing(
        ...
        private object? StartHandler<T>(T state, HandlerMethodDescriptor handler)
            where T : class
        {
            var profiler = MiniProfiler.Current;
        ...
        private object? Complete<T>(T state)
            where T : class
        {
        ...
        public void OnNext(KeyValuePair<string, object?> kv)
        {
            try
            {
                _ = kv.Value switch
                {
                    ...
                    BeforeControllerActionMethodEventData data => StartAction(
                        "Controller Action",
                        data.ActionContext.ActionDescriptor
                    ),
                    AfterControllerActionMethodEventData data => Complete(
                        data.ActionContext.ActionDescriptor
                    ),
                    // ActionResultEvent
                    ...
                    BeforeAuthorizationFilterOnAuthorizationEventData data => StartFilter(
                        "Auth Filter",
                        data.Filter
                    ),
                    ...
                    // ViewComponent
                    BeforeViewComponentEventData data => StartView(
                        data.ViewComponentContext,
                        "Component (Invoke)",
                        data.ViewComponentContext.ViewComponentDescriptor.ShortName
                    ),
                    ...
                    BeforePageFilterOnPageHandlerSelectionEventData data => StartFilter(
                        "Filter (Selection)",
                        data.Filter
                    ),
                    AfterPageFilterOnPageHandlerSelectionEventData data => Complete(data.Filter),
                    // PageFilterOnPageHandlerSelected
                    BeforePageFilterOnPageHandlerSelectedEventData data => StartFilter(
                        "Filter (Selected)",
                        data.Filter
                    ),
