...
public abstract class BundleManagerBase : IBundleManager
{
    ...
    public virtual async Task<IReadOnlyList<BundleFile>> GetScriptBundleFilesAsync(
        string bundleName
    )
    {
    ...
    protected virtual async Task<IReadOnlyList<BundleFile>> GetBundleFilesAsync(
    ...
    private BundleCacheItem AddToBundleCache(
        string bundleName,
        IBundler bundler,
    ...
    )
    {
        var bundleRelativePath =
            ...
            + bundleName
            + "."
            + bundleFiles.JoinAsString("|").ToMd5()
            + "."
            + bundler.FileExtension;

        return BundleCache.GetOrAdd(
            bundleRelativePath,
            () =>
            {
    ...
    )
    {
        lock (cacheValue.WatchDisposeHandles)
        {
            ...
            {
                var watchDisposeHandle = GetFileProvider()
                    .Watch(file)
    ...
    protected virtual async Task<List<BundleFile>> GetBundleFilesAsync(
    ...
    {
        return new BundleConfigurationContext(
            ServiceProvider,
            GetFileProvider(),
            Options.Parameters
        );
    ...
    protected virtual List<IBundleContributor> GetContributors(
        BundleConfigurationCollection bundles,
        string bundleName
    )
    {
