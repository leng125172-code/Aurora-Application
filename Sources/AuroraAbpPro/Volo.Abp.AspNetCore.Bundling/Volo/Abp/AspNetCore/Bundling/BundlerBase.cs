...
public abstract class BundlerBase : IBundler, ITransientDependency
{
    ...
    public BundleResult Bundle(IBundlerContext context)
    {
        Logger.LogInformation(
            $"Bundling {context.BundleRelativePath} ({context.ContentFiles.Count} files)"
        );
        ...
        Logger.LogInformation(
            $"Bundled {context.BundleRelativePath} ({bundleContent.Length} bytes)"
        );

        return new BundleResult(bundleContent);
    }

    private void AddFileToBundle(
        IBundlerContext context,
        StringBuilder bundleContentBuilder,
        string fileName
    )
    {
    ...
    private string GetFileContentConsideringMinification(IBundlerContext context, string fileName)
    {
        ...
        if (minFileInfo != null)
        {
            ...
            Logger.LogDebug(
                $"  > Using the pre-minified file: {minFileInfo.Name} ({fileContent.Length} bytes)"
            );
            return fileContent;
        }
    ...
    private string GetAndMinifyFileContent(IBundlerContext context, string fileName)
    {
        ...
        try
        {
            ...
            Logger.LogInformation(
                $"  > Minified {fileName} ({nonMinifiedSize} bytes -> {fileContent.Length} bytes)"
            );
        ...
        {
            Logger.LogWarning(
                $"Unable to minify the file: {fileName}. Return file content without minification.",
                ex
            );
        }
