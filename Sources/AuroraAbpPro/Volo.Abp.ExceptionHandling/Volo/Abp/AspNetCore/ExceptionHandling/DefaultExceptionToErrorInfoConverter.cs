        ...
        ITransientDependency
{
    ...
    )
    {
        ...
        if (
            exception is AbpRemoteCallException remoteCallException
            && remoteCallException.Error != null
        )
        {
        ...
        if (exception is IHasValidationErrors)
        {
            ...
            {
                errorInfo.Details = GetValidationErrorNarrative(
                    (exception as IHasValidationErrors)!
                );
            }
    ...
    )
    {
        if (exception is ILocalizeErrorMessage localizeErrorMessageException)
        {
            ...
            {
                errorInfo.Message = localizeErrorMessageException.LocalizeMessage(
                    new LocalizationContext(scope.ServiceProvider)
                );
            }
        ...
        var localizationResourceType = LocalizationOptions.ErrorCodeNamespaceMappings.GetOrDefault(
            codeNamespace
        );
    ...
    protected virtual RemoteServiceErrorInfo CreateEntityNotFoundError(
        ...
        if (exception.EntityType != null)
        {
            var message =
                exception.Id != null
                    ...
                    : string.Format(
                        L["EntityNotFoundErrorMessageWithoutId"],
                        exception.EntityType.Name
                    );
    ...
    protected virtual void AddExceptionToDetails(
        Exception exception,
    ...
    protected virtual RemoteServiceValidationErrorInfo[] GetValidationErrorInfos(
    ...
    )
    {
        ...
        {
            var validationError = new RemoteServiceValidationErrorInfo(
