...
namespace Hangfire
{
    public static class HangfireServiceCollectionExtensions
    {
        ...
        )
        {
            ...
            services.TryAddSingletonChecked<IBackgroundJobClientFactory>(static x =>
                x.GetService<DefaultClientManagerFactory>()
            );
            services.TryAddSingletonChecked<IBackgroundJobClientFactoryV2>(static x =>
                x.GetService<DefaultClientManagerFactory>()
            );
            services.TryAddSingletonChecked<IRecurringJobManagerFactory>(static x =>
                x.GetService<DefaultClientManagerFactory>()
            );
            services.TryAddSingletonChecked<IRecurringJobManagerFactoryV2>(static x =>
                x.GetService<DefaultClientManagerFactory>()
            );

            services.TryAddSingletonChecked(static x =>
                x.GetService<IBackgroundJobClientFactory>().GetClient(x.GetService<JobStorage>())
            );
            services.TryAddSingletonChecked(static x =>
                x.GetService<IBackgroundJobClientFactoryV2>()
                    .GetClientV2(x.GetService<JobStorage>())
            );

            services.TryAddSingletonChecked(static x =>
                x.GetService<IRecurringJobManagerFactory>().GetManager(x.GetService<JobStorage>())
            );
            services.TryAddSingletonChecked(static x =>
                x.GetService<IRecurringJobManagerFactoryV2>()
                    .GetManagerV2(x.GetService<JobStorage>())
            );
        ...
        )
        {
            ...
            return AddHangfireServerInner(
                services,
                null,
                null,
                (provider, options) => optionsAction(options)
            );
        }
        ...
        )
        {
            ...
            {
                var options =
                    provider.GetService<BackgroundJobServerOptions>()
                    ?? new BackgroundJobServerOptions();
                return CreateBackgroundJobServerHostedService(
                    provider,
                    storage,
                    additionalProcesses,
                    options
