        ...
        builder.Services.Configure<AbpClaimsPrincipalFactoryOptions>(options =>
        {
            ...
            if (!jwtBearerOption.Authority.IsNullOrEmpty())
            {
                options.RemoteRefreshUrl =
                    jwtBearerOption.Authority.RemovePostFix("/") + options.RemoteRefreshUrl;
            }
        });
        ...
        return builder.AddJwtBearer(
            ...
            options =>
            {
                ...
                options.Events.OnChallenge = async eventContext =>
                {
                    ...
                    var tokenUnauthorizedErrorInfo =
                        eventContext.HttpContext.RequestServices.GetRequiredService<AbpAspNetCoreTokenUnauthorizedErrorInfo>();
                    if (
                        string.IsNullOrEmpty(tokenUnauthorizedErrorInfo.Error)
