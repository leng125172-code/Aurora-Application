...
public class FeatureDefinitionSerializer : IFeatureDefinitionSerializer, ITransientDependency
{
    ...
    public Task<FeatureDefinitionRecord> SerializeAsync(
        ...
        FeatureGroupDefinition featureGroup
    )
    {
        using (CultureHelper.Use(CultureInfo.InvariantCulture))
        {
            var featureRecord = new FeatureDefinitionRecord(
                ...
                featureGroup?.Name,
                feature.Name,
                feature.Parent?.Name ?? string.Empty,
                LocalizableStringSerializer.Serialize(feature.DisplayName) ?? string.Empty,
                LocalizableStringSerializer.Serialize(feature.Description) ?? string.Empty,
                feature.DefaultValue ?? string.Empty,
