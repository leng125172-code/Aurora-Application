...
public class StaticFeatureSaver : IStaticFeatureSaver, ITransientDependency
{
        ...
        )
        {
            if (commonLockHandle == null)
            {
                ...
                throw new AbpException(
                    "Could not acquire distributed lock for saving static features!"
                );
            }
            ...
            )
            {
                try
                {
                    var hasChangesInGroups = await UpdateChangedFeatureGroupsAsync(
                        featureGroupRecords
                    );
    ...
    )
    {
        ...
        var featureGroupRecordsInDatabase = (
            await FeatureGroupRepository.GetListAsync()
        ...
        var deletedRecords = FeatureOptions.DeletedFeatureGroups.Any()
            ? featureGroupRecordsInDatabase
                .Values.Where(x => FeatureOptions.DeletedFeatureGroups.Contains(x.Name))
                .ToArray()
    ...
    )
    {
        ...
        var featureRecordsInDatabase = (await FeatureRepository.GetListAsync()).ToDictionary(x =>
            x.Name
        );
        ...
        if (FeatureOptions.DeletedFeatures.Any())
        {
            deletedRecords.AddRange(
                featureRecordsInDatabase.Values.Where(x =>
                    FeatureOptions.DeletedFeatures.Contains(x.Name)
                )
            );
        }
        ...
        if (FeatureOptions.DeletedFeatureGroups.Any())
        {
            deletedRecords.AddIfNotContains(
                featureRecordsInDatabase.Values.Where(x =>
                    FeatureOptions.DeletedFeatureGroups.Contains(x.GroupName)
                )
            );
        }
    ...
    )
    {
        ...
        stringBuilder.AppendLine(
            JsonSerializer.Serialize(featureGroupRecords, jsonSerializerOptions)
        );
