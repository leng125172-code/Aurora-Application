...
public abstract class AbpTagHelperResourceService : ITransientDependency
{
    ...
    )
    {
        ...
        foreach (var bundleFile in bundleFiles)
        {
            ...
            else
            {
                ...
                {
                    Logger.LogError(
                        $"Could not find the bundle file '{bundleFile.FileName}' for the bundle '{bundleName}'!"
                    );
                    AddErrorScript(
                        viewContext,
                        tagHelper,
        ...
        stopwatch.Stop();
        Logger.LogDebug(
            $"Added bundle '{bundleName}' to the page in {stopwatch.Elapsed.TotalMilliseconds:0.00} ms."
        );
    }
    ...
    protected abstract void AddHtmlTag(
    ...
    protected virtual void AddErrorScript(
        ViewContext viewContext,
        TagHelper tagHelper,
        TagHelperContext context,
        TagHelperOutput output,
    ...
    )
    {
        output.Content.AppendHtml(
            $"<script>console.log(\"%cCould not find the bundle file '{file.FileName}' for the bundle '{bundleName}'!\", 'background: yellow; color: black; font-size:20px;');</script>{Environment.NewLine}"
        );
    }
