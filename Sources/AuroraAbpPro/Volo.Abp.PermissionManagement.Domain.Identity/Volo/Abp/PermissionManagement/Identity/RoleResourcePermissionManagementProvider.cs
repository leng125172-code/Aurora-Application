...
public class RoleResourcePermissionManagementProvider : ResourcePermissionManagementProvider
{
    ...
    public RoleResourcePermissionManagementProvider(
        ...
        : base(resourcepPrmissionGrantRepository, guidGenerator, currentTenant)
    {
        UserRoleFinder = userRoleFinder;
    }

    public override async Task<ResourcePermissionValueProviderGrantInfo> CheckAsync(
        string name,
        string resourceName,
        string resourceKey,
        string providerName,
    ...
    {
        var multipleGrantInfo = await CheckAsync(
            new[] { name },
            resourceName,
            resourceKey,
            providerName,
    ...
    public override async Task<MultipleResourcePermissionValueProviderGrantInfo> CheckAsync(
        string[] names,
        string resourceName,
        string resourceKey,
        string providerName,
    ...
    )
    {
        using (ResourcePermissionGrantRepository.DisableTracking())
        {
            ...
            {
                resourcePermissionGrants.AddRange(
            ...
            if (
                providerName == UserResourcePermissionValueProvider.ProviderName
