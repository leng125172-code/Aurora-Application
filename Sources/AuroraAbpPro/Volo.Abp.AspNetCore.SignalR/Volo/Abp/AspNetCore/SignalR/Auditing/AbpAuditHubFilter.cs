...
public class AbpAuditHubFilter : IHubFilter
{
    ...
    {
        var options = invocationContext
            .ServiceProvider.GetRequiredService<IOptions<AbpAuditingOptions>>()
            .Value;
        ...
        var hasError = false;
        var auditingManager =
            invocationContext.ServiceProvider.GetRequiredService<IAuditingManager>();
        using (var saveHandle = auditingManager.BeginScope())
        {
            ...
            {
                if (
                    await ShouldWriteAuditLogAsync(
                ...
                )
                {
                    var unitOfWorkManager =
                        invocationContext.ServiceProvider.GetRequiredService<IUnitOfWorkManager>();
    ...
    )
    {
        ...
        if (
            ...
            && !serviceProvider.GetRequiredService<ICurrentUser>().IsAuthenticated
        )
        {
            return false;
        }
