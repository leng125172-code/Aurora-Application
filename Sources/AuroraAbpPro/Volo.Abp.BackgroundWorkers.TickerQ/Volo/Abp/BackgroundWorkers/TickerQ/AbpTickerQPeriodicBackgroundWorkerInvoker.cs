...
public class AbpTickerQPeriodicBackgroundWorkerInvoker
{
    ...
    public AbpTickerQPeriodicBackgroundWorkerInvoker(
    ...
    )
    {
        ...
        switch (worker)
        {
            case AsyncPeriodicBackgroundWorkerBase:
            {
                ...
                var method = workerType.GetMethod(
                    "DoWorkAsync",
                ...
                var instanceParam = Expression.Parameter(
                    typeof(AsyncPeriodicBackgroundWorkerBase),
                    "worker"
                );
                var contextParam = Expression.Parameter(
                    typeof(PeriodicBackgroundWorkerContext),
                    "context"
                );
                var call = Expression.Call(
                    Expression.Convert(instanceParam, workerType),
                    method,
                    contextParam
                );
                var lambda = Expression.Lambda<
                    Func<AsyncPeriodicBackgroundWorkerBase, PeriodicBackgroundWorkerContext, Task>
                >(call, instanceParam, contextParam);
            ...
            case PeriodicBackgroundWorkerBase:
            {
                ...
                var instanceParam = Expression.Parameter(
                    typeof(PeriodicBackgroundWorkerBase),
                    "worker"
                );
                var contextParam = Expression.Parameter(
                    typeof(PeriodicBackgroundWorkerContext),
                ...
                var lambda = Expression.Lambda<
                    Action<PeriodicBackgroundWorkerBase, PeriodicBackgroundWorkerContext>
                >(call, instanceParam, contextParam);
