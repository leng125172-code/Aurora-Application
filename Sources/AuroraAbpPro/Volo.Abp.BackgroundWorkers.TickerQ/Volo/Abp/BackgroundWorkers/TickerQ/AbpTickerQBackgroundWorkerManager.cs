        ...
        if (worker is AsyncPeriodicBackgroundWorkerBase or PeriodicBackgroundWorkerBase)
        {
            ...
            if (period == null && cronExpression.IsNullOrWhiteSpace())
            {
                throw new AbpException(
                    $"Both 'Period' and 'CronExpression' are not set for {worker.GetType().FullName}. You must set at least one of them."
                );
            }

            cronExpression = cronExpression ?? GetCron(period!.Value);
            var name =
                BackgroundWorkerNameAttribute.GetNameOrNull(worker.GetType())
            ...
            AbpTickerQFunctionProvider.Functions.TryAdd(
                ...
                (
                    ...
                    {
                        var workerInvoker = new AbpTickerQPeriodicBackgroundWorkerInvoker(
                            worker,
                            serviceProvider
                        );
                        await workerInvoker.DoWorkAsync(
                            tickerFunctionContext,
                            tickerQCancellationToken
                        );
                    }
                )
            );

            AbpTickerQBackgroundWorkersProvider.BackgroundWorkers.Add(
                name!,
