    ...
    where TCacheItem : class
{
    ...
    {
        return await InternalCache.GetOrCreateAsync(
            key,
            factory,
            optionsFactory,
            hideErrors,
    ...
    where TCacheKey : notnull
{
    ...
    )
    {
        ...
        async Task RemoveRealCache()
        {
            ...
            try
            {
                await HybridCache.RemoveAsync(keyArray.Select(NormalizeKey), token);
            }
            catch (Exception ex)
            {
                if (hideErrors == true)
    ...
    protected virtual IHybridCacheSerializer<TCacheItem> ResolveSerializer()
    {
        ...
        if (serializer is null)
        {
            throw new InvalidOperationException(
                $"No {nameof(IHybridCacheSerializer<TCacheItem>)} configured for type '{typeof(TCacheItem).Name}'"
            );
        }
