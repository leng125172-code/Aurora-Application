    ...
    where TCacheItem : class
{
    ...
    {
        return InternalCache.GetOrAddAsync(
            key,
            factory,
            optionsFactory,
            hideErrors,
    ...
    {
        return InternalCache.GetOrAddManyAsync(
            keys,
            factory,
            optionsFactory,
            hideErrors,
    ...
    where TCacheKey : notnull
{
    ...
    protected virtual string NormalizeKey(TCacheKey key)
    {
        return KeyNormalizer.NormalizeKey(
            new DistributedCacheKeyNormalizeArgs(key.ToString()!, CacheName, IgnoreMultiTenancy)
        );
    }
    ...
    protected virtual void SetDefaultOptions()
    {
        ...
        //IgnoreMultiTenancy
        IgnoreMultiTenancy = typeof(TCacheItem).IsDefined(
            typeof(IgnoreMultiTenancyAttribute),
            true
        );
    ...
    )
    {
        ...
        var resultMap = result.Where(x => x.Value != null).ToDictionary(x => x.Key, x => x.Value);
    ...
    )
    {
        ...
        try
        {
            foreach (var item in items)
            {
                await SetAsync(item.Key, item.Value, options, false, considerUow, token: token);
            }
        }
        catch (Exception ex)
        {
    ...
    )
    {
        if (itemBytes.Length != itemKeys.Length)
        {
            throw new AbpException(
                "count of the item bytes should be same with the count of the given keys"
            );
        }
        ...
        {
            result.Add(
                new KeyValuePair<TCacheKey, TCacheItem?>(itemKeys[i], ToCacheItem(itemBytes[i]))
            );
        }
    ...
    private static KeyValuePair<TCacheKey, TCacheItem?>[] ToCacheItemsWithDefaultValues(
        TCacheKey[] keys
    )
    {
