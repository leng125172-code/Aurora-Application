...
namespace StackExchange.Profiling.Storage
{
    ...
    public abstract class DatabaseStorageBase : IAsyncStorage, IDatabaseStorageConnectable
    {
        ...
        public abstract IEnumerable<Guid> List(
            int maxResults,
            DateTime? start = null,
        ...
        /// <param name="orderBy">Result order.</param>
        /// <returns>The list of GUID keys.</returns>
        public abstract Task<IEnumerable<Guid>> ListAsync(
            int maxResults,
            DateTime? start = null,
            DateTime? finish = null,
            ListResultsOrder orderBy = ListResultsOrder.Descending
        ...
        protected void ConnectTimings(
            MiniProfiler? profiler,
            List<Timing> timings,
            List<ClientTiming> clientTimings
        )
        {
            if (profiler?.RootTimingId.HasValue == true && timings.Count > 0)
            {
                ...
                if (rootTiming != null)
                {
                    ...
                    var timingsLookupByParent = timings.ToLookup(
                        x => (Guid)x.ParentTimingId!,
                        x => x
                    );
        ...
        private void PopulateChildTimings(
            Timing parent,
            ILookup<Guid, Timing> timingsLookupByParent
        )
        {
            if (timingsLookupByParent.Contains(parent.Id))
            {
                foreach (
                    var timing in timingsLookupByParent[parent.Id].OrderBy(x => x.StartMilliseconds)
                )
                {
