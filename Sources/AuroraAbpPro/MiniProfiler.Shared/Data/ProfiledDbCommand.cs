    ...
    public partial class ProfiledDbCommand : DbCommand
    {
        ...
        private static Action<IDbCommand, bool>? GetBindByName(Type commandType)
        {
            if (commandType == null)
                return null; // GIGO
            if (
                Link<Type, Action<IDbCommand, bool>>.TryGet(
                    bindByNameCache,
                    commandType,
                    out var action
                )
            ...
            var prop = commandType.GetProperty(
                "BindByName",
            ...
            )
            {
                ...
                il.Emit(OpCodes.Ret);
                action =
                    (Action<IDbCommand, bool>)
                        method.CreateDelegate(typeof(Action<IDbCommand, bool>));
            }

            // cache it
            Link<Type, Action<IDbCommand, bool>>.TryAdd(
                ref bindByNameCache,
                commandType,
                ref action!
            );
        ...
        /// </summary>
        protected virtual DbDataReader CreateDbDataReader(
            DbDataReader original,
        ...
        [
            ...
            Obsolete($"Please use {nameof(WrappedCommand)}", false)
        ]
