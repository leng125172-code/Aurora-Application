...
namespace StackExchange.Profiling.Helpers
{
    ...
    public static class StackTraceSnippet
    {
        ...
        public static string Get(MiniProfilerBaseOptions options)
        {
            ...
            for (int i = 0; i < frames.Length; i++)
            {
                ...
                if (
                    stackLength >= options.StackMaxLength
                    // ASP.NET: no need to continue up the chain
                    || method.Name == "System.Web.HttpApplication.IExecutionStep.Execute"
                    || (
                        ModuleNames.GetOrAdd(method.Module, m => m.Name)
                            == "Microsoft.AspNetCore.Mvc.Core.dll"
                        && method.DeclaringType?.Name == "ObjectMethodExecutor"
                    )
                )
                {
                    frames[i] = null;
                    ...
                    || options.ExcludedAssemblies.Contains(
                        AssemblyNames.GetOrAdd(
                            method.Module.Assembly,
                            a => a.GetName().Name ?? "Unknown Assembly"
                        )
                    )
        ...
        internal static string HtmlPrettify(string stackTrace, int? commonStart = null)
        {
            string GetBetween(Capture prev, Capture next) =>
                stackTrace.Substring(
                    ...
                    next.Index - (prev.Index + prev.Length)
                );
            ...
            for (var mi = 0; mi < matches.Count; mi++)
            {
                ...
                if (sb.Length == 0 && isAsync && leadIn.Index > pos)
                {
                    sb.Append("<span class=\"stack stack-row\">")
                        ...
                        .AppendHtmlEncode(
                            stackTrace
                                .Substring(pos, leadIn.Index - pos)
                                .Trim(NewLine_CarriageReturn)
                        )
                        .Append("</span>")
                        .Append("</span>");
                    pos += sb.Length;
                ...
                if (
                    (nextEndStack > -1 && nextEndStack < m.Index + m.Length + 3)
                    || (!isAsync && nextIsAsync)
                )
                {
                ...
                if (paramTypes.Count > 0)
                {
                    ...
                    sb.Append("<span class=\"stack parens\">")
                        .AppendHtmlEncode(
                            allParams.Value.Substring(last.Index + last.Length - allParams.Index)
                        )
                        .Append("</span>");
                }
                else
    ...
    internal static class StackTraceExtensions
    {
        ...
        private static readonly Regex _genericTypeRegex = new Regex(
            $@"(?<BaseClass>{StackTraceUtils.NoSpace}+)`(?<ArgCount>\d+)"
        );
        ...
        private static readonly Dictionary<string, string[]> _commonGenerics = new()
        {
            ...
            ["Microsoft.CodeAnalysis.Diagnostics.CodeBlockStartAnalysisContext`1"] = new[]
            {
                "TLanguageKindEnum",
            },
            ...
            ["Microsoft.CodeAnalysis.Semantics.OperationVisitor`2"] = new[]
            {
                "TArgument",
                "TResult",
            },
            ...
            ["System.Collections.Immutable.ImmutableSortedDictionary`2"] = new[]
            {
                "TKey",
                "TValue",
            },
            ...
            ["System.Data.Linq.SqlClient.Implementation.ObjectMaterializer`1"] = new[]
            {
                "TDataReader",
            },
        ...
        internal static StringBuilder AppendHtmlEncode(this StringBuilder sb, string s) =>
            ...
            void AppendArgs(string[] tArgs)
            {
                ...
                if (tArgs.Length > 5)
                {
                    sb.Append("<span class=\"stack generic-type\">")
                        ...
                        .Append("</span>")
                        .Append(',')
                        .Append("<span class=\"stack generic-type\">")
                        .Append(tArgs[1])
                        .Append("</span>")
                        .Append(',')
                        .Append("<span class=\"stack generic-type\">")
