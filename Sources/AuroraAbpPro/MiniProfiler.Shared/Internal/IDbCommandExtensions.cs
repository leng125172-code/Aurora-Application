...
namespace StackExchange.Profiling.Internal
{
    ...
    public static class IDbCommandExtensions
    {
        ...
        public static CustomTiming? GetTiming(
            this IDbCommand command,
            string commandType,
            MiniProfiler? profiler,
            string customType = "sql"
        )
        {
            if (command == null || profiler == null)
                return null;

            var commandText =
                profiler.Options.SqlFormatter?.GetFormattedSql(command)
                ?? command.GetReadableCommand();
        ...
        {
            if ((command?.Parameters?.Count ?? 0) == 0)
                return null;
