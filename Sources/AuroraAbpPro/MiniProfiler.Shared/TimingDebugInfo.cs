...
namespace StackExchange.Profiling
{
    ...
    public class TimingDebugInfo
    {
        ...
        public string RichHtmlStack =>
            StackTraceUtils.HtmlPrettify(RawStack.ToString(), CommonStackStart);
        ...
        internal TimingDebugInfo(Timing parent, int debugStackShave = 0)
        {
            ...
            if (parent.ParentTiming?.DebugInfo?.RawStack is StackTrace parentStack)
            {
                ...
                int myIndex,
                    parentIndex;
                for (
                    myIndex = RawStack.FrameCount - 1, parentIndex = parentStack.FrameCount - 1;
                ...
                )
                {
                    ...
                    if (
                        myFrame.GetILOffset() == parentFrame.GetILOffset()
                        && myFrame.GetMethod() == parentFrame.GetMethod()
                    )
                    {
