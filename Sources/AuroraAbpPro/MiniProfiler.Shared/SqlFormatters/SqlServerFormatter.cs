...
namespace StackExchange.Profiling.SqlFormatters
{
    ...
    {
        private static readonly Regex CommandSpacing = new Regex(
            @",([^\s])",
        ...
        protected static readonly Dictionary<
            DbType,
            Func<SqlTimingParameter, string>
        ...
        private static Func<SqlTimingParameter, string> GetWithLenFormatter(string native)
        {
            ...
            return p =>
            {
                ...
                return capture
                    + "("
                    + (p.Size > 8000 ? "max" : p.Size.ToString(CultureInfo.InvariantCulture))
                    + ")";
            };
        }
        ...
        public virtual string FormatSql(
            string commandText,
            List<SqlTimingParameter>? parameters,
            IDbCommand? command
        )
        {
        ...
        private void GenerateStoreProcedureCall(
            string commandText,
            List<SqlTimingParameter>? parameters,
        ...
        private void GenerateSelectStatement(
            StringBuilder buffer,
            List<SqlTimingParameter>? parameters
        )
        {
            ...
            var parametersToSelect = parameters
                .Where(x =>
                    x.Direction == nameof(ParameterDirection.InputOutput)
                    || x.Direction == nameof(ParameterDirection.Output)
                )
                .Select(x => EnsureParameterPrefix(x.Name) + " AS " + RemoveParameterPrefix(x.Name))
                .ToList();

            var returnValueParameter = parameters.SingleOrDefault(x =>
                x.Direction == nameof(ParameterDirection.ReturnValue)
            );
            if (returnValueParameter != null)
            {
                parametersToSelect.Insert(
                    0,
                    EnsureParameterPrefix(returnValueParameter.Name) + " AS ReturnValue"
                );
            }

            if (parametersToSelect.Count == 0)
                return;

            buffer.Append("\nSELECT ").Append(string.Join(", ", parametersToSelect)).Append(';');
        }

        private static SqlTimingParameter? GetReturnValueParameter(
            List<SqlTimingParameter>? parameters
        )
        {
        ...
        private void GenerateStoredProcedureParameters(
            StringBuilder buffer,
            List<SqlTimingParameter>? parameters
        )
        {
            ...
            foreach (var parameter in parameters)
            {
                ...
                buffer
                    .Append(' ')
                    .Append(EnsureParameterPrefix(parameter.Name))
                    .Append(" = ")
                    .Append(EnsureParameterPrefix(parameter.Name));

                // Output and InputOutput directions treated equally on the database side.
                if (
                    parameter.Direction == nameof(ParameterDirection.Output)
            ...
            if (parameters?.Count > 0)
            {
                ...
                foreach (var parameter in parameters)
                {
                    ...
                    if (
                        ...
                        && parameter.Direction != nameof(ParameterDirection.ReturnValue)
                    )
                    {
