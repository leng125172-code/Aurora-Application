        ...
        public string GetServerTimingHeader()
        {
            ...
            foreach (var t in GetTimingHierarchy())
            {
                ...
                foreach (var ct in t.CustomTimings)
                {
                    if (ct.Value?.Count > 0)
                    {
                        ...
                        summary[ct.Key] =
                            (summary.TryGetValue(ct.Key, out decimal cur) ? cur : 0) + ctTotal;
                    }
                }
            }
            ...
            foreach (var category in summary)
            {
                sb.Append(category.Key)
                    ...
                    .Append("\"")
                    .Append(";dur=")
                    .Append(category.Value.ToString("0.##", NumberFormatInfo.InvariantInfo))
                    .Append(',');
            }
            sb.Append("total;desc=\"Total\";dur=")
                .Append(total.ToString("0.##", NumberFormatInfo.InvariantInfo));
            return sb.ToString();
        }
    }
}
