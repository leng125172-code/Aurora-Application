...
namespace Hangfire
{
    ...
    {
        [Obsolete(
            "请改用 IServiceCollection.AddHangfireServer 方法在 ConfigureServices 中配置。将于 2.0.0 版本移除。"
        )]
        ...
        )
        {
            ...
            options =
                options
                ?? services.GetService<BackgroundJobServerOptions>()
            ...
            services.RegisterHangfireServer(
                HangfireServiceCollectionExtensions.GetInternalServices(
                    services,
                    out var factory,
                    out var stateChanger,
                    out var performer
                )
