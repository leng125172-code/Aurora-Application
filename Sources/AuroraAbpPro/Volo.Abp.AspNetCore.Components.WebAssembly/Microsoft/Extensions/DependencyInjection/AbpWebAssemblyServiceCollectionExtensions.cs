...
public static class AbpWebAssemblyServiceCollectionExtensions
{
    ...
    {
        var webAssemblyHostEnvironment =
            services.GetSingletonInstanceOrNull<IWebAssemblyHostEnvironment>();

        if (webAssemblyHostEnvironment == null)
        {
            return new EmptyWebAssemblyHostEnvironment() { Environment = Environments.Development };
        }
