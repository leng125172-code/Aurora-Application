...
public class PersistentComponentStateAbpAccessTokenProvider : IAbpAccessTokenProvider
{
    ...
    public PersistentComponentStateAbpAccessTokenProvider(
    ...
    public virtual Task<string?> GetTokenAsync()
    {
        ...
        AccessToken = _persistentComponentState.TryTakeFromJson<PersistentAccessToken>(
            PersistentAccessToken.Key,
            out var token
        )
            ? token?.AccessToken
            : null;
