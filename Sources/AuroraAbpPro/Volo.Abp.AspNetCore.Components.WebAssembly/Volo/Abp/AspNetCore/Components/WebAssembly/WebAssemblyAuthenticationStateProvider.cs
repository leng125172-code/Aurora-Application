...
/// </summary>
public class WebAssemblyAuthenticationStateProvider<
    ...
    where TAccount : RemoteUserAccount
{
    ...
    public override async Task<AuthenticationState> GetAuthenticationStateAsync()
    {
        ...
        var applicationConfigurationDto =
            await WebAssemblyCachedApplicationConfigurationClient.GetAsync();
        if (
            state.User.Identity != null
            && state.User.Identity.IsAuthenticated
            && !applicationConfigurationDto.CurrentUser.IsAuthenticated
        )
        {
    ...
    protected virtual async Task TryRevokeOldAccessTokensAsync()
    {
        ...
        foreach (var accessToken in revokeAccessTokens)
        {
            ...
            if (!accessToken.IsNullOrWhiteSpace() && !currentAccessToken.IsNullOrWhiteSpace())
            {
                ...
                var currentSessionId = handler
                    .ReadJwtToken(currentAccessToken)
                    ?.Claims?.FirstOrDefault(x => x.Type == AbpClaimTypes.SessionId);
                var sessionId = handler
            ...
            var result = await httpClient.RevokeTokenAsync(
                new TokenRevocationRequest
                {
                    Address =
                        authority.EnsureEndsWith('/')
