...
public class TenantResolver : ITenantResolver, ITransientDependency
{
    ...
    public TenantResolver(
        IOptions<AbpTenantResolveOptions> options,
    ...
    public virtual async Task<TenantResolveResult> ResolveTenantIdOrNameAsync()
    {
        ...
        using (var serviceScope = ServiceProvider.CreateScope())
        {
            ...
            {
                Logger.LogDebug(
                    "Trying to resolve tenant through '{TenantResolverName}'...",
                    tenantResolver.Name
                );
                ...
                if (context.HasResolvedTenantOrHost())
                {
                    ...
                    Logger.LogDebug(
                        "Tenant resolved by '{TenantResolverName}' as '{TenantIdOrName}'.",
                        tenantResolver.Name,
        ...
        if (
            result.TenantIdOrName.IsNullOrEmpty()
        ...
        )
        {
            ...
            Logger.LogDebug(
                "No tenant resolved. Using fallback tenant as '{FallbackTenant}'.",
                result.TenantIdOrName
            );
        }
