...
public class AbpTimeZoneMiddleware : AbpMiddlewareBase, ITransientDependency
{
    public override async Task InvokeAsync(HttpContext context, RequestDelegate next)
    {
        ...
        var currentTimezoneProvider =
            context.RequestServices.GetRequiredService<ICurrentTimezoneProvider>();
    ...
    protected virtual async Task<string?> GetTimezoneFromRequestAsync(HttpContext context)
    {
        var timeZoneSources = new Func<HttpContext, Task<string?>>[]
        {
            ctx =>
                Task.FromResult(
                    ctx.Request.Headers[TimeZoneConsts.DefaultTimeZoneKey].FirstOrDefault()
                ),
            ctx =>
                Task.FromResult<string?>(
                    ctx.Request.Query[TimeZoneConsts.DefaultTimeZoneKey].ToString()
                ),
            async ctx =>
                ctx.Request.HasFormContentType
                    ? (await ctx.Request.ReadFormAsync())[TimeZoneConsts.DefaultTimeZoneKey]
                        .ToString()
                    : null,
            ctx =>
                Task.FromResult(ctx.Request.Cookies[TimeZoneConsts.DefaultTimeZoneKey]?.ToString()),
        };
