...
public static class CookieAuthenticationOptionsExtensions
{
    ...
    /// </summary>
    public static CookieAuthenticationOptions CheckTokenExpiration(
        ...
        options.Events.OnValidatePrincipal = async principalContext =>
        {
            ...
            )
            {
                logger.LogInformation(
                    "The access_token expires within {AdvanceSeconds}s; signing out.",
                    advance.Value.TotalSeconds
                );
            ...
            if (
                principalContext.Properties.IssuedUtc != null
                && DateTimeOffset.UtcNow.Subtract(principalContext.Properties.IssuedUtc.Value)
                    > validationInterval
            )
            {
                logger.LogInformation(
                    "Checking access_token activity every {Seconds} seconds.",
                    validationInterval.Value.TotalSeconds
                );
                ...
                if (!accessToken.IsNullOrWhiteSpace())
                {
                    ...
                    var introspectionEndpoint = openIdConnectOptions
                        .Configuration
                        ?.IntrospectionEndpoint;
                    if (
                        introspectionEndpoint.IsNullOrWhiteSpace()
                        && !openIdConnectOptions.Authority.IsNullOrWhiteSpace()
                    )
                    {
                        introspectionEndpoint =
                    ...
                    {
                        logger.LogWarning(
                            "No introspection endpoint configured. Skipping token activity check."
                        );
                        await InvokePreviousHandlerAsync(principalContext, previousHandler);
                        return;
                    }
                    ...
                    if (response.IsError)
                    {
                        ...
                        await SignOutAndInvokePreviousHandlerAsync(
                            principalContext,
                            previousHandler
                        );
                        return;
                ...
                {
                    logger.LogError(
                        "The access_token is not found in the cookie properties. Ensure SaveTokens of OpenIdConnectOptions is true."
                    );
    ...
    private static async Task<OpenIdConnectOptions> GetOpenIdConnectOptions(
        CookieValidatePrincipalContext principalContext,
        string oidcAuthenticationScheme
    )
    {
        var openIdConnectOptions = principalContext
            .HttpContext.RequestServices.GetRequiredService<IOptionsMonitor<OpenIdConnectOptions>>()
            .Get(oidcAuthenticationScheme);
        var cancellationTokenProvider =
            principalContext.HttpContext.RequestServices.GetRequiredService<ICancellationTokenProvider>();
        if (
            openIdConnectOptions.Configuration == null
            && openIdConnectOptions.ConfigurationManager != null
        )
        {
            openIdConnectOptions.Configuration =
                await openIdConnectOptions.ConfigurationManager.GetConfigurationAsync(
    ...
    private static Task InvokePreviousHandlerAsync(
        CookieValidatePrincipalContext principalContext,
        Func<CookieValidatePrincipalContext, Task>? previousHandler
    )
    {
        return previousHandler != null ? previousHandler(principalContext) : Task.CompletedTask;
    }

    private static async Task SignOutAndInvokePreviousHandlerAsync(
        CookieValidatePrincipalContext principalContext,
        Func<CookieValidatePrincipalContext, Task>? previousHandler
    )
    {
