...
public class AbpAuditingMiddleware : AbpMiddlewareBase, ITransientDependency
{
    ...
    {
        if (
            await ShouldSkipAsync(context, next)
            || !AuditingOptions.IsEnabled
            || IsIgnoredUrl(context)
        )
        {
            await next(context);
            return;
    ...
    private bool IsIgnoredUrl(HttpContext context)
    {
        ...
        if (
            !AuditingOptions.IsEnabledForIntegrationServices
            && context.Request.Path.Value.StartsWith(
                $"/{AbpAspNetCoreConsts.DefaultIntegrationServiceApiPrefix}/",
        ...
        if (
            AspNetCoreAuditingOptions.IgnoredUrls.Any(x =>
    ...
    )
    {
        ...
        if (
            !AuditingOptions.IsEnabledForGetRequests
            && (
                string.Equals(
