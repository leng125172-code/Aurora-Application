...
public class WebContentFileProvider : IWebContentFileProvider, ISingletonDependency
{
    ...
    public virtual IChangeToken Watch(string filter)
    {
        ...
        return new CompositeChangeToken(
            new[] { _fileProvider.Watch(_rootPath + filter), _fileProvider.Watch(filter) }
        );
    }
    ...
    {
        return Options.AllowedExtraWebContentFolders.Any(s =>
    ...
    {
        return Options.AllowedExtraWebContentFileExtensions.Any(e =>
            path.EndsWith(e, StringComparison.OrdinalIgnoreCase)
        );
    }
}
