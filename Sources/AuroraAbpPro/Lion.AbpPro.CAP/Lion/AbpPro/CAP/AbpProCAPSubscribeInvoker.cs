...
public class AbpProCAPSubscribeInvoker : ISubscribeInvoker
{
    ...
    public AbpProCAPSubscribeInvoker(
    ...
    )
    {
        ...
        for (var i = 0; i < parameterDescriptors.Count; i++)
        {
            ...
            {
                executeParameters[i] = GetCapProvidedParameter(
                    parameterDescriptor,
                    message,
                    cancellationToken
                );
            ...
            else
            {
                if (message.Value != null)
                {
                    ...
                    {
                        executeParameters[i] = _serializer.Deserialize(
                            message.Value,
        ...
        {
            var capHeader =
                executeParameters.FirstOrDefault(x => x is CapHeader) as AbpProCapHeader;
            return new ConsumerExecutedResult(
                resultObj,
                message.GetId(),
                callbackName,
                capHeader?.ResponseHeader
            );
        }
    }

    private static object GetCapProvidedParameter(
