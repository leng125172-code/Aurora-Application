...
namespace Lion.AbpPro.NotificationManagement.Notifications
{
    ...
    public class NotificationAppService : NotificationManagementAppService, INotificationAppService
    {
        ...
        /// 发送正常广播消息
        /// </summary>
        public virtual async Task SendBroadCastInformationMessageAsync(
            SendBroadCastMessageInput input
        )
        {
            await _messageManager.SendMessageAsync(
                input.Title,
        ...
        /// 分页获取消息
        /// </summary>
        public virtual async Task<PagedResultDto<PagingNotificationOutput>> PageNotificationAsync(
            PagingNotificationInput input
        )
        {
            var totalCount = await _notificationManager.GetPagingCountAsync(
                input.Title,
                input.Content,
                input.SenderUserId,
            ...
            var list = await _notificationManager.GetPagingListAsync(
                input.Title,
                input.Content,
                input.SenderUserId,
                input.SenderUserName,
            ...
            // }
            return new PagedResultDto<PagingNotificationOutput>(
                totalCount,
                list.Adapt<List<PagingNotificationOutput>>()
            );
        }
        ...
        public virtual async Task<
            PagedResultDto<PagingNotificationSubscriptionOutput>
        > PageNotificationSubscriptionAsync(PagingNotificationSubscriptionInput input)
        {
            var totalCount = await _notificationSubscriptionManager.GetPagingCountAsync(
            ...
            var list = await _notificationSubscriptionManager.GetPagingListAsync(
                input.NotificationId,
                input.ReceiverUserId,
                input.ReceiverUserName,
                input.StartReadTime,
            ...
            var result = new PagedResultDto<PagingNotificationSubscriptionOutput>(
                totalCount,
                list.Adapt<List<PagingNotificationSubscriptionOutput>>()
            );
            // 获取消息内容
            if (totalCount > 0)
            {
                var notifications = await _notificationManager.GetListAsync(
                    list.Select(e => e.NotificationId).Distinct().ToList()
                );
                foreach (var item in result.Items)
                {
                    var notification = notifications.FirstOrDefault(e =>
                        e.Id == item.NotificationId
                    );
                    if (notification != null)
                    {
