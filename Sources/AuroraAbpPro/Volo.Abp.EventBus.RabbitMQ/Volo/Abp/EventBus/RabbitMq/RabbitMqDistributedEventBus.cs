        ...
        ISingletonDependency
{
        ...
        GetOrCreateHandlerFactories(typeof(TEvent))
            .Locking(factories =>
            {
                factories.RemoveAll(factory =>
                {
                    ...
                    var actionHandler =
                        singleInstanceFactory.HandlerInstance as ActionEventHandler<TEvent>;
    ...
    protected override void AddToUnitOfWork(
    ...
    )
    {
        using (
            var channel = await (
                await ConnectionPool.GetAsync(AbpRabbitMqEventBusOptions.ConnectionName)
            ).CreateChannelAsync(
    ...
    protected virtual async Task EnsureExchangeExistsAsync(IChannel channel)
    {
        ...
        {
            using (
                var temporaryChannel = await (
                    await ConnectionPool.GetAsync(AbpRabbitMqEventBusOptions.ConnectionName)
                ).CreateChannelAsync()
            )
            {
                await temporaryChannel.ExchangeDeclarePassiveAsync(
                    AbpRabbitMqEventBusOptions.ExchangeName
                );
            }
        }
    ...
    protected virtual void SetEventMessageHeaders(
    ...
    )
    {
        ...
        foreach (
            var handlerFactory in HandlerFactories.Where(hf =>
