...
public class ChannelPool : IChannelPool, ISingletonDependency
{
    ...
    public virtual async Task<IChannelAccessor> AcquireAsync(
    ...
    )
    {
        ...
        else
        {
            using (await Semaphore.LockAsync())
            {
                ...
                {
                    poolItem = new ChannelPoolItem(
                        await CreateChannelAsync(channelName, connectionName)
                    );
    ...
    protected virtual async Task<IChannel> CreateChannelAsync(
        string channelName,
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        foreach (var poolItem in Channels.Values)
        {
            ...
            remainingWaitDuration =
                remainingWaitDuration > poolItemDisposeStopwatch.Elapsed
