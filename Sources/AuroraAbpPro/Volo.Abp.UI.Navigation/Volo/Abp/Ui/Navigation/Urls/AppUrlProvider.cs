...
public class AppUrlProvider : IAppUrlProvider, ITransientDependency
{
    ...
    {
        return await MultiTenantUrlProvider.GetUrlAsync(await GetConfiguredUrl(appName, urlName));
    }
    ...
    public virtual async Task<bool> IsRedirectAllowedUrlAsync(string url)
    {
        ...
        var allow = redirectAllowedUrls.Any(x =>
            url.StartsWith(x, StringComparison.CurrentCultureIgnoreCase)
            || UrlHelpers.IsSubdomainOf(url, x)
        );
        if (!allow)
        {
            Logger.LogError(
                $"Invalid RedirectUrl: {url}, Use {nameof(AppUrlProvider)} to configure it!"
            );
        }
        return allow;
    }
