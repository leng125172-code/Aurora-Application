...
{
    public MySqlCapTransaction(IDispatcher dispatcher)
        : base(dispatcher) { }
...
public static class CapTransactionExtensions
{
    ...
    )
    {
        ...
        publisher.Transaction = ActivatorUtilities.CreateInstance<MySqlCapTransaction>(
    ...
    )
    {
        ...
        return Task.FromResult<IDbContextTransaction>(
            new CapEFDbTransaction(publisher.Transaction)
        );
    }
    ...
    public static ICapTransaction BeginTransaction(
    ...
    /// <param name="cancellationToken"></param>
    /// <returns>The <see cref="ICapTransaction" /> object.</returns>
    public static ValueTask<ICapTransaction> BeginTransactionAsync(
        this IDbConnection dbConnection,
        ICapPublisher publisher,
        bool autoCommit = false,
        CancellationToken cancellationToken = default
    ...
    {
        if (dbConnection.State == ConnectionState.Closed)
            ((DbConnection)dbConnection).OpenAsync(cancellationToken).GetAwaiter().GetResult();

        var dbTransaction = ((DbConnection)dbConnection)
            .BeginTransactionAsync(isolationLevel, cancellationToken)
            .GetAwaiter()
            .GetResult();
        publisher.Transaction = ActivatorUtilities.CreateInstance<MySqlCapTransaction>(
