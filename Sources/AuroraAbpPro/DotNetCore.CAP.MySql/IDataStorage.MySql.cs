...
public class MySqlDataStorage : IDataStorage
{
    ...
    )
    {
        ...
        var opResult = await connection
            .ExecuteNonQueryAsync(sql, sqlParams: sqlParams)
    ...
    public async Task ChangePublishStateAsync(
    ...
    public async Task<MediumMessage> StoreMessageAsync(
        string name,
        Message content,
        object? transaction = null
    )
    {
        ...
        object[] sqlParams =
        {
            ...
            new MySqlParameter(
                "@ExpiresAt",
                message.ExpiresAt.HasValue ? message.ExpiresAt.Value : DBNull.Value
            ),
    ...
    public async Task StoreReceivedExceptionMessageAsync(string name, string group, string content)
    {
        object[] sqlParams =
        {
            ...
            new MySqlParameter(
                "@ExpiresAt",
                DateTime.Now.AddSeconds(_capOptions.Value.FailedMessageExpiredAfter)
            ),
    ...
    public async Task<MediumMessage> StoreReceivedMessageAsync(
        string name,
        string group,
        Message message
    )
    {
        ...
        object[] sqlParams =
        {
            ...
            new MySqlParameter(
                "@ExpiresAt",
                mdMessage.ExpiresAt.HasValue ? mdMessage.ExpiresAt.Value : DBNull.Value
            ),
            new MySqlParameter("@StatusName", nameof(StatusName.Scheduled)),
        };
    ...
    private async Task ChangeMessageStateAsync(
        string tableName,
        MediumMessage message,
        StatusName state,
        object? transaction = null
    )
    {
        ...
        if (transaction is DbTransaction dbTransaction)
        {
            ...
            await connection
                .ExecuteNonQueryAsync(sql, dbTransaction, sqlParams)
                .ConfigureAwait(false);
        }
        else
        {
    ...
    private async Task<IEnumerable<MediumMessage>> GetMessagesOfNeedRetryAsync(
        string tableName,
        TimeSpan lookbackSeconds
    )
    {
