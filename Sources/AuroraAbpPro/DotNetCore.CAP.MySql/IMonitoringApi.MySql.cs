...
(
                    ...
                    while (await reader.ReadAsync())
                    {
                        ...
                        messages.Add(
                            new MessageDto
                            {
                                ...
                                Group =
                                    queryDto.MessageType == MessageType.Subscribe
                                        ? reader.GetString(index++)
                                        : default,
                                Content = reader.GetString(index++),
                                Retries = reader.GetInt32(index++),
                                Added = reader.GetDateTime(index++),
                                ExpiresAt = reader.IsDBNull(index++)
                                    ? null
    ...
    private async ValueTask<int> GetNumberOfMessage(string tableName, string statusName)
    {
        ...
        return await connection
            .ExecuteScalarAsync<int>(sqlQuery, new MySqlParameter("@State", statusName))
            .ConfigureAwait(false);
    }

    private Task<Dictionary<DateTime, int>> GetHourlyTimelineStats(
        string tableName,
        string statusName
    )
    {
