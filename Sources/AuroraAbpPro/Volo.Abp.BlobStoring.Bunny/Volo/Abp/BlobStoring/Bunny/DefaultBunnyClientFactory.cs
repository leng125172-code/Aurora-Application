...
public class DefaultBunnyClientFactory : IBunnyClientFactory, ITransientDependency
{
    ...
    )
    {
        using (var client = _httpClientFactory.CreateClient("BunnyApiClient"))
        {
            ...
            var response = await client.PostAsync("https://api.bunny.net/storagezone", content);
    ...
    protected virtual async Task<BunnyStorageZoneModel?> GetStorageZoneAsync(
        string accessKey,
        string containerName
    )
    {
        using (var client = _httpClientFactory.CreateClient("BunnyApiClient"))
        {
            ...
            return zones?.FirstOrDefault(x =>
                x.Name.Equals(containerName, StringComparison.OrdinalIgnoreCase) && !x.Deleted
            );
        }
    }
}
