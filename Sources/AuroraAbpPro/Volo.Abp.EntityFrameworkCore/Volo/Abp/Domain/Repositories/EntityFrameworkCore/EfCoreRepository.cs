    ...
    where TEntity : class, IEntity
{
    ...
    )
    {
        ...
        var savedEntity = (
            await GetDbSetInternal(dbContext)
                .AddAsync(entity, GetCancellationToken(cancellationToken))
        ).Entity;
    ...
    {
        return EntityName != null ? dbContext.Set<TEntity>(EntityName) : dbContext.Set<TEntity>();
    }
    ...
    )
    {
        return includeDetails
            ...
            : await (await GetQueryableAsync()).ToListAsync(
                GetCancellationToken(cancellationToken)
            );
    }
    ...
    {
        return await (await GetQueryableAsync()).LongCountAsync(
            GetCancellationToken(cancellationToken)
        );
    }
