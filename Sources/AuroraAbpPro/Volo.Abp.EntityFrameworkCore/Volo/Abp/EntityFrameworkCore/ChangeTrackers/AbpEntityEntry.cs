...
public class AbpEntityEntry
{
    ...
    public bool IsModified
    {
        get
        {
            return _isModified
                ...
                || NavigationEntries.Any(n => n.IsModified);
        }
    ...
    public AbpEntityEntry(string id, EntityEntry entityEntry)
    {
        ...
        NavigationEntries = EntityEntry
            .Navigations.Select(x => new AbpNavigationEntry(x, x.Metadata.Name))
            .ToList();
    }

    public void UpdateNavigation(EntityEntry entityEntry, AbpNavigationEntry navigationEntry)
    {
        if (IsModified || EntityEntry.State == EntityState.Modified || navigationEntry.IsModified)
        {
            return;
        }
...
public class AbpNavigationEntry
{
    ...
    public AbpNavigationEntry(NavigationEntry navigationEntry, string name)
    {
        ...
        {
            OriginalValue =
                navigationEntry is CollectionEntry
                    ? new List<object>()
