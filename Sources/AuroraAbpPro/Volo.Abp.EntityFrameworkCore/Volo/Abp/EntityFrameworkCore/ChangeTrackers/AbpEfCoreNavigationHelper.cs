...
#pragma warning disable EF1001
        ...
        var internalEntityEntityEntry = stateManager.TryGetEntry(
            entityEntry.Entity,
        ...
        foreach (var foreignKey in foreignKeys)
        {
            ...
            var navigationEntry =
                abpEntityEntry.NavigationEntries.FirstOrDefault(x =>
            ...
            if (
                !abpEntityEntry.IsModified
                && (!checkEntityEntryState || IsEntityEntryChanged(entityEntry))
            )
            {
        ...
        foreach (var skipNavigation in skipNavigations)
        {
            ...
            foreach (var joinEntry in stateManager.Entries)
            {
                if (
                    ...
                    || stateManager.FindPrincipal(joinEntry, foreignKey)
                        != internalEntityEntityEntry
                )
                {
                    continue;
                }
...
#pragma warning restore EF1001
    ...
    public virtual bool IsEntityEntryModified(EntityEntry entityEntry)
    {
        ...
        return EntityEntries.TryGetValue(entryId, out var abpEntityEntry)
            && abpEntityEntry.IsModified;
    }

    public virtual bool IsNavigationEntryModified(
        EntityEntry entityEntry,
        int? navigationEntryIndex = null
    )
    {
        var entryId = GetEntityEntryIdentity(entityEntry);
        if (entryId == null)
        ...
        var navigationEntryProperty = abpEntityEntry.NavigationEntries.ElementAtOrDefault(
            navigationEntryIndex.Value
        );
        return navigationEntryProperty != null && navigationEntryProperty.IsModified;
    }

    public virtual AbpNavigationEntry? GetNavigationEntry(
        EntityEntry entityEntry,
