...
public class AbpDbContextOptions
{
    ...
    public void ConfigureDefaultConvention(
        [NotNull] Action<DbContext, ModelConfigurationBuilder> action,
    ...
    public void ConfigureConventions<TDbContext>(
        [NotNull] Action<TDbContext, ModelConfigurationBuilder> action
    )
        where TDbContext : AbpDbContext<TDbContext>
    {
        ...
        if (actions == null)
        {
            ...
            {
                new Action<DbContext, ModelConfigurationBuilder>(
                    (dbContext, builder) => action((TDbContext)dbContext, builder)
                ),
            };
            return;
        }
    ...
    public void ConfigureDefaultOnModelCreating(
        [NotNull] Action<DbContext, ModelBuilder> action,
        bool overrideExisting = false
    )
    {
        Check.NotNull(action, nameof(action));
    ...
    public void ConfigureOnModelCreating<TDbContext>(
        [NotNull] Action<TDbContext, ModelBuilder> action
    )
        where TDbContext : AbpDbContext<TDbContext>
    {
        Check.NotNull(action, nameof(action));
        ...
        if (actions == null)
        {
            ...
            {
                new Action<DbContext, ModelBuilder>(
                    (dbContext, builder) => action((TDbContext)dbContext, builder)
                ),
            };
            return;
    ...
    public void ConfigureDefaultOnConfiguring(
        [NotNull] Action<DbContext, DbContextOptionsBuilder> action,
        bool overrideExisting = false
    )
    {
        Check.NotNull(action, nameof(action));
    ...
    public void ConfigureOnConfiguring<TDbContext>(
        [NotNull] Action<TDbContext, DbContextOptionsBuilder> action
    )
        where TDbContext : AbpDbContext<TDbContext>
    {
        Check.NotNull(action, nameof(action));
        ...
        if (actions == null)
        {
            ...
            {
                new Action<DbContext, DbContextOptionsBuilder>(
                    (dbContext, builder) => action((TDbContext)dbContext, builder)
                ),
            };
            return;
    ...
    )
    {
        ...
        while (true)
        {
            var foundType = DbContextReplacements.LastOrDefault(x =>
                x.Key.Type == replacementType && x.Key.MultiTenancySide.HasFlag(multiTenancySides)
            );
