    ...
    where TDbContext : DbContext
{
    ...
    public IOptions<AbpEntityChangeOptions> EntityChangeOptions =>
        LazyServiceProvider.LazyGetRequiredService<IOptions<AbpEntityChangeOptions>>();
    ...
    public AbpEfCoreNavigationHelper AbpEfCoreNavigationHelper =>
        LazyServiceProvider.LazyGetRequiredService<AbpEfCoreNavigationHelper>();

    public IOptions<AbpDbContextOptions> Options =>
        LazyServiceProvider.LazyGetRequiredService<IOptions<AbpDbContextOptions>>();

    public IOptions<AbpEfCoreGlobalFilterOptions> GlobalFilterOptions =>
        LazyServiceProvider.LazyGetRequiredService<IOptions<AbpEfCoreGlobalFilterOptions>>();

    private static readonly MethodInfo ConfigureBasePropertiesMethodInfo =
        typeof(AbpDbContext<TDbContext>).GetMethod(
    ...
    protected override void OnConfiguring(DbContextOptionsBuilder optionsBuilder)
    {
        ...
        foreach (
            var onConfiguringAction in Options.Value.OnConfiguringActions.GetOrDefault(
                typeof(TDbContext)
            ) ?? []
        )
        {
            onConfiguringAction
                .As<Action<DbContext, DbContextOptionsBuilder>>()
                .Invoke(this, optionsBuilder);
        }
    }
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        foreach (
            var onModelCreatingAction in Options.Value.OnModelCreatingActions.GetOrDefault(
                typeof(TDbContext)
            ) ?? []
        )
        {
    ...
    protected override void ConfigureConventions(ModelConfigurationBuilder configurationBuilder)
    {
        ...
        foreach (
            var conventionAction in Options.Value.ConventionActions.GetOrDefault(typeof(TDbContext))
                ?? []
        )
        {
            conventionAction
                .As<Action<DbContext, ModelConfigurationBuilder>>()
                .Invoke(this, configurationBuilder);
        }
    }
    ...
    )
    {
        try
        {
            ...
            if (entityChangeList != null)
            {
                ...
                {
                    Logger.LogDebug(
                        $"Added {entityChangeList.Count} entity changes to the current audit log"
                    );
                }
            }
    ...
    protected virtual Task PublishEventsForChangedEntityOnSaveChangeAsync()
    {
        foreach (var entityEntry in AbpEfCoreNavigationHelper.GetChangedEntityEntries())
        {
            ...
            {
                var ignoredEntity = EntityChangeOptions.Value.IgnoredNavigationEntitySelectors.Any(
                    selector => selector.Predicate(entityEntry.Entity.GetType())
                );
                var onlyForeignKeyModifiedEntity =
                    entityEntry.State == EntityState.Modified
                    && IsOnlyForeignKeysModified(entityEntry);
                if (
                    (entityEntry.State == EntityState.Unchanged && ignoredEntity)
                    || onlyForeignKeyModifiedEntity && ignoredEntity
                )
                {
                    continue;
                }
                ...
                if (
                    ...
                    && EntityChangeOptions.Value.IgnoredUpdateAggregateRootSelectors.All(selector =>
                        !selector.Predicate(entityEntry.Entity.GetType())
                    )
                ...
                if (
                    entityEntry.Entity is ISoftDelete
                    && entityEntry.Entity.As<ISoftDelete>().IsDeleted
                )
                {
                ...
                GetAllPropertyEntries(entityEntry)
                    .Any(x =>
                        x.IsModified
                        && (
    ...
    {
        if (
            initializationContext.UnitOfWork.Options.Timeout.HasValue
        ...
        switch (entry.State)
        {
            ...
            case EntityState.Modified:
                ...
                )
                {
                    ...
                    var modifiedProperties = GetAllPropertyEntries(entry)
                        .Where(x => x.IsModified)
                        .ToList();
                    var disableAuditingAttributes = modifiedProperties
                        .Select(x =>
                            x.Metadata.PropertyInfo?.GetCustomAttribute<DisableAuditingAttribute>()
                        )
                        .ToList();
                    ...
                    && EntityChangeOptions.Value.IgnoredNavigationEntitySelectors.All(selector =>
                        !selector.Predicate(entry.Entity.GetType())
                    )
                ...
                )
                {
                    if (
                        EntityChangeOptions.Value.UpdateAggregateRootWhenNavigationChanges
                        && EntityChangeOptions.Value.IgnoredUpdateAggregateRootSelectors.All(
                            selector => !selector.Predicate(entry.Entity.GetType())
                        )
                    )
                    {
    ...
    protected virtual IEnumerable<PropertyEntry> GetAllComplexPropertyEntries(
        IEnumerable<ComplexPropertyEntry> complexPropertyEntries
    )
    {
        foreach (var complexPropertyEntry in complexPropertyEntries)
        {
            ...
            if (
                complexPropertyInfo != null
                && complexPropertyInfo.IsDefined(typeof(DisableAuditingAttribute), true)
            )
            {
                continue;
            }
            ...
            foreach (
                var nestedPropertyEntry in GetAllComplexPropertyEntries(
                    complexPropertyEntry.ComplexProperties
                )
            )
            {
    ...
    {
        return GetAllPropertyEntries(entry)
            .Where(x => x.IsModified)
            .All(x =>
                x.Metadata.IsForeignKey()
                && (
                    x.CurrentValue == null
                    || x.OriginalValue?.ToString() == x.CurrentValue?.ToString()
                )
            );
    }
    ...
    protected virtual void HandlePropertiesBeforeSave()
    {
        ...
        {
            foreach (
                var entry in AbpEfCoreNavigationHelper
                    .GetChangedEntityEntries()
                    .Where(x => x.State == EntityState.Unchanged)
                    .Where(x =>
                        EntityChangeOptions.Value.IgnoredNavigationEntitySelectors.All(selector =>
                            !selector.Predicate(x.Entity.GetType())
                        )
                    )
                    .Where(x =>
                        EntityChangeOptions.Value.IgnoredUpdateAggregateRootSelectors.All(
                            selector => !selector.Predicate(x.Entity.GetType())
                        )
                    )
            )
            {
        ...
        foreach (var entry in ChangeTracker.Entries().ToList())
        {
            ...
            if (distributedEvents != null && distributedEvents.Any())
            {
                eventReport.DistributedEvents.AddRange(
                    distributedEvents.Select(eventRecord => new DomainEventEntry(
                        entry.Entity,
                        eventRecord.EventData,
                        eventRecord.EventOrder
                    ))
    ...
    protected virtual void HandleExtraPropertiesOnSave(EntityEntry entry)
    {
        ...
        var efMappedProperties = ObjectExtensionManager
            .Instance.GetProperties(entityType)
    ...
    protected virtual void ApplyAbpConceptsForDeletedEntity(EntityEntry entry)
    {
        ...
        {
            originalExtraProperties = entry.OriginalValues.GetValue<ExtraPropertyDictionary>(
        ...
        if (concurrencyStamp != null && entry.Entity is IHasConcurrencyStamp)
        {
            ObjectHelper.TrySetProperty(
                entry.Entity.As<IHasConcurrencyStamp>(),
        ...
        {
            ObjectHelper.TrySetProperty(
                entry.Entity.As<IHasExtraProperties>(),
                x => x.ExtraProperties,
    ...
    protected virtual bool IsHardDeleted(EntityEntry entry)
    {
        var hardDeletedEntities =
            UnitOfWorkManager?.Current?.Items.GetOrDefault(UnitOfWorkItemNames.HardDeletedEntities)
            as HashSet<IEntity>;
        ...
        where TEntity : class
    {
        ...
        foreach (
            var property in mutableEntityType
                .GetProperties()
                .Where(property =>
        ...
        where TEntity : class
    {
        ...
        if (
            idPropertyBuilder.Metadata.PropertyInfo!.IsDefined(
        ...
        where TEntity : class
    {
        ...
        if (typeof(ISoftDelete).IsAssignableFrom(typeof(TEntity)))
        {
            var softDeleteColumnName =
                entityTypeBuilder
                    .Metadata.FindProperty(nameof(ISoftDelete.IsDeleted))
                    ?.GetColumnName()
                ?? "IsDeleted";
            expression = e =>
                !IsSoftDeleteFilterEnabled || !EF.Property<bool>(e, softDeleteColumnName);
            if (UseDbFunction())
            {
                expression = e =>
                    AbpEfCoreDataFilterDbFunctionMethods.SoftDeleteFilter(
        ...
        if (typeof(IMultiTenant).IsAssignableFrom(typeof(TEntity)))
        {
            var multiTenantColumnName =
                entityTypeBuilder
                    .Metadata.FindProperty(nameof(IMultiTenant.TenantId))
                    ?.GetColumnName()
                ?? "TenantId";
            ...
            {
                multiTenantFilter = e =>
                    AbpEfCoreDataFilterDbFunctionMethods.MultiTenantFilter(
