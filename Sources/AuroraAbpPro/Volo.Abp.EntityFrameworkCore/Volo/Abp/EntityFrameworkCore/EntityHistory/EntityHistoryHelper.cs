...
public class EntityHistoryHelper : IEntityHistoryHelper, ITransientDependency
{
    ...
    protected virtual EntityChangeInfo? CreateEntityChangeOrNull(EntityEntry entityEntry)
    {
        ...
        switch (entityEntry.State)
        {
            ...
            case EntityState.Modified:
                changeType = IsDeleted(entityEntry)
        ...
        if (
            entityId == null
            && changeType != EntityChangeType.Created
            && !EntityHelper.IsValueObject(entity)
        )
        {
            return null;
        }
    ...
    protected virtual string? GetEntityId(object entityAsObj)
    {
        ...
        throw new AbpException(
            $"Entities should implement the {typeof(IEntity).AssemblyQualifiedName} interface or {typeof(ValueObject).AssemblyQualifiedName} class! "
    ...
    protected virtual List<EntityPropertyChangeInfo> GetPropertyChanges(EntityEntry entityEntry)
    {
        ...
        foreach (
            var (navigationEntry, index) in entityEntry.Navigations.Select((value, i) => (value, i))
        )
        {
            ...
            if (
                navigationEntry.Metadata.TargetEntityType.IsMappedToJson()
                && navigationEntry is ReferenceEntry referenceEntry
            ...
            )
            {
                foreach (var propertyChange in GetPropertyChanges(referenceEntry.TargetEntry))
                {
                    propertyChange.PropertyName =
                        $"{referenceEntry.Metadata.Name}.{propertyChange.PropertyName}";
            ...
            {
                var abpNavigationEntry = AbpEfCoreNavigationHelper.GetNavigationEntry(
                    entityEntry,
                    index
                );
                ...
                propertyChanges.Add(
                    new EntityPropertyChangeInfo
                    {
                        ...
                        PropertyTypeFullName = navigationEntry
                            .Metadata.ClrType.GetFirstGenericArgumentIfNullable()
                            .FullName!,
                        OriginalValue = GetNavigationPropertyValue(
                            abpNavigationEntry?.OriginalValue,
                            isCollection
                        ),
                        NewValue = GetNavigationPropertyValue(
                            abpNavigationEntry?.CurrentValue,
                            isCollection
                        ),
                    }
                );
    ...
    )
    {
        ...
        foreach (var propertyEntry in complexPropertyEntry.Properties)
        {
            if (ShouldSavePropertyHistory(propertyEntry, isCreated || isDeleted) && !isSoftDeleted)
            {
                var propertyType = DeterminePropertyTypeFromEntry(
    ...
    /// </returns>
    protected virtual Type DeterminePropertyTypeFromEntry(
    ...
    protected virtual string? GetNavigationPropertyValue(object? entity, bool isCollection)
    {
        switch (entity)
        {
            ...
            case IEnumerable enumerable:
                ...
                var serializedKeysEnumerable =
                    keysList.Count == 1 && !isCollection
                        ? keysList.First()
                        : JsonSerializer.Serialize(keysList);

                return serializedKeysEnumerable.TruncateWithPostfix(
                    EntityPropertyChangeInfo.MaxValueLength
                );
    ...
    protected virtual bool ShouldSavePropertyHistory(PropertyEntry propertyEntry, bool defaultValue)
    {
        ...
        if (
            propertyEntry.OriginalValue is ExtraPropertyDictionary originalValue
        ...
        )
        {
            ...
            if (
                !originalValue.Select(x => x.Value).SequenceEqual(currentValue.Select(x => x.Value))
            )
            {
                return true;
            }
    ...
    {
        if (
            entityType.IsAssignableTo<IHasCreationTime>()
        ...
        if (
            entityType.IsAssignableTo<IMayHaveCreator>()
        ...
        if (
            entityType.IsAssignableTo<IMustHaveCreator>()
        ...
        if (
            entityType.IsAssignableTo<IHasModificationTime>()
        ...
        if (
            entityType.IsAssignableTo<IModificationAuditedObject>()
        ...
        if (
            entityType.IsAssignableTo<ISoftDelete>()
        ...
        if (
            entityType.IsAssignableTo<IHasDeletionTime>()
        ...
        if (
            entityType.IsAssignableTo<IDeletionAuditedObject>()
    ...
    public virtual void UpdateChangeList(List<EntityChangeInfo> entityChanges)
    {
        foreach (var entityChange in entityChanges)
        {
            ...
            foreach (var foreignKey in foreignKeys)
            {
                ...
                {
                    var propertyEntry = entityEntry.Property(property.Name);
                    var propertyChange = entityChange.PropertyChanges.FirstOrDefault(pc =>
                        pc.PropertyName == property.Name
                    );
                        ...
                        )
                        {
                            ...
                            entityChange.PropertyChanges.Add(
                                new EntityPropertyChangeInfo
                                {
                                    ...
                                    OriginalValue = JsonSerializer.Serialize(
                                        propertyEntry.OriginalValue!
                                    ),
                                    PropertyName = property.Name,
                                    PropertyTypeFullName = property
                    ...
                    if (propertyChange.OriginalValue == propertyChange.NewValue)
                    {
                        ...
                        {
                            // Update foreign key
                            propertyChange.NewValue = newValue.TruncateWithPostfix(
                                EntityPropertyChangeInfo.MaxValueLength
                            );
                        }
                    }
