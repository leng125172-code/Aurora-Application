    ...
    where TDbContext : DbContext, IEfCoreDbContext
{
    ...
    protected virtual async Task LockAndApplyDatabaseMigrationsAsync()
    {
        Logger.LogInformation(
            $"Trying to acquire the distributed lock for database migration: {DatabaseName}."
        );

        var schemaMigrated = false;

        await using (
            var handle = await DistributedLock.TryAcquireAsync("DatabaseMigration_" + DatabaseName)
        )
        {
            if (handle is null)
            {
                Logger.LogInformation(
                    $"Distributed lock could not be acquired for database migration: {DatabaseName}. Operation cancelled."
                );
                return;
            }

            Logger.LogInformation(
                $"Distributed lock is acquired for database migration: {DatabaseName}..."
            );
        ...
        Logger.LogInformation(
            $"Distributed lock has been released for database migration: {DatabaseName}..."
        );
    }
    ...
    protected virtual async Task TryAsync(Func<Task> task, int maxTryCount = 3)
    {
        ...
        catch (Exception ex)
        {
            ...
            Logger.LogWarning(
                ex,
                $"{ex.GetType().Name} has been thrown. The operation will be tried {maxTryCount} times more."
            );

            await Task.Delay(
                RandomHelper.GetRandom(MinValueToWaitOnFailure, MaxValueToWaitOnFailure)
            );
