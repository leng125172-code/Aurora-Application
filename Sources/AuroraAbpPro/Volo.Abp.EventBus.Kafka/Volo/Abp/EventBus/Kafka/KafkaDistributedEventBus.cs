...
public class KafkaDistributedEventBus : DistributedEventBusBase, ISingletonDependency
{
    ...
    protected override async Task PublishToEventBusAsync(Type eventType, object eventData)
    {
        ...
        {
            headers.Add(
                EventBusConsts.CorrelationIdHeaderName,
    ...
    )
    {
        ...
        if (result.Status != PersistenceStatus.Persisted)
        {
            throw new AbpException(
                $"Failed to publish event '{outgoingEvent.EventName}' to topic '{AbpKafkaEventBusOptions.TopicName}'. Status: {result.Status}"
            );
        }
    ...
    )
    {
        ...
        {
            throw new AbpException(
                $"Failed to publish event '{eventName}' to topic '{topicName}'. Status: {result.Status}"
            );
        }
    }
