    ...
    public class SettingAppService : BasicManagementAppService, ISettingAppService
    {
        ...
        public virtual async Task<List<SettingOutput>> GetAsync()
        {
            ...
            var settings = allSettings
                .Where(e => e.Properties.ContainsKey(BasicManagementSettings.Group.Default))
                .ToList();

            var settingOutput = settings
                .GroupBy(e => e.Properties[BasicManagementSettings.Group.Default].ToString())
                .Select(s => new SettingOutput
                {
                    Group = s.Key,
            ...
            foreach (var item in settingOutput)
            {
                ...
                foreach (var itemDefinition in currentSettings)
                {
                    ...
                    var type = itemDefinition
                        .Properties.FirstOrDefault(f =>
                            f.Key == AbpProSettingConsts.ControlType.Default
                        )
                        .Value.ToString();

                    item.SettingItemOutput.Add(
                        new SettingItemOutput(
                            itemDefinition.Name,
