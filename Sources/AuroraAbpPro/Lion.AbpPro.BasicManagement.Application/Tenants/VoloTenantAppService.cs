...
namespace Lion.AbpPro.BasicManagement.Tenants
{
    ...
    public class VoloTenantAppService : BasicManagementAppService, IVoloTenantAppService
    {
        ...
        [AllowAnonymous]
        public virtual async Task<FindTenantResultDto> FindTenantByNameAsync(
            FindTenantByNameInput input
        )
        {
        ...
        public async Task<
            PagedResultDto<PageTenantConnectionStringOutput>
        > PageConnectionStringsAsync(PageTenantConnectionStringInput input)
        {
            ...
            {
                items = items
                    .Where(e => e.Value.ToLower().Contains(input.Value.ToLower()))
                    .ToList();
            }
            result.Items = items;
        ...
        public async Task AddOrUpdateConnectionStringAsync(AddOrUpdateConnectionStringInput input)
        {
            ...
            if (
                AbpTenantManagementDbProperties.ConnectionStringName.ToLower()
                    == input.Name.ToLower()
                || AbpBackgroundJobsDbProperties.ConnectionStringName.ToLower()
                    == input.Name.ToLower()
                || AbpFeatureManagementDbProperties.ConnectionStringName.ToLower()
                    == input.Name.ToLower()
                || AbpSettingManagementDbProperties.ConnectionStringName.ToLower()
                    == input.Name.ToLower()
            )
            {
                throw new BusinessException(
                    BasicManagementErrorCodes.NotSupportSetConnectionString
                );
            }
            var tenant = await _tenantRepository.FindAsync(input.Id, true);
            ...
            var connectionString = tenant.ConnectionStrings.FirstOrDefault(e =>
                e.Value == input.Name
            );
