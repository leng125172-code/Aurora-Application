...
public class ConnectionPool : IConnectionPool, ISingletonDependency
{
    ...
    private readonly ConcurrentDictionary<
        string,
        Lazy<ServiceBusAdministrationClient>
    > _adminClients;
        ...
        return _clients
            .GetOrAdd(
                ...
                new Lazy<ServiceBusClient>(() =>
                {
                    ...
                    if (
                        !config.FullyQualifiedNamespace.IsNullOrWhiteSpace()
                        && config.TokenCredential != null
                    )
                    {
                    ...
                    throw new InvalidOperationException(
                        $"{connectionName} does not have a valid Service Bus connection configuration."
                    );
                })
            )
            .Value;
    ...
    {
        connectionName ??= AzureServiceBusConnections.DefaultConnectionName;
        return _adminClients
            .GetOrAdd(
                connectionName,
                new Lazy<ServiceBusAdministrationClient>(() =>
                {
                    var config = _options.Connections.GetOrDefault(connectionName);
                    if (!config.ConnectionString.IsNullOrWhiteSpace())
                    {
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        {
            Logger.LogDebug(
                $"Disposed admin connection pool with no admin connection in the pool."
            );
            return;
        }

        Logger.LogInformation(
            $"Disposing admin connection pool ({_adminClients.Count} admin connections)."
        );
        _adminClients.Clear();
    }
}
