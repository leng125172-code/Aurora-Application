...
public class SendBroadCastMessageInput : IValidatableObject
{
    ...
    public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        var localization = validationContext.GetRequiredService<
            IStringLocalizer<NotificationManagementResource>
        >();

        if (Title.IsNullOrWhiteSpace())
        {
            yield return new ValidationResult(
                localization[NotificationManagementErrorCodes.MessageTitle],
                new[] { nameof(Title) }
            );
        }
        ...
        {
            yield return new ValidationResult(
                localization[NotificationManagementErrorCodes.MessageContent],
                new[] { nameof(Content) }
            );
        }
    }
