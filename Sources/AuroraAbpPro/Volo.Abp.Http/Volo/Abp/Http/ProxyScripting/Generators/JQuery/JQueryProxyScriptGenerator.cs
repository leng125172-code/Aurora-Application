...
public class JQueryProxyScriptGenerator : IProxyScriptGenerator, ITransientDependency
{
    ...
    public JQueryProxyScriptGenerator(
        IOptions<DynamicJavaScriptProxyOptions> dynamicJavaScriptProxyOptions
    )
    {
    ...
    )
    {
        ...
        if (versionParam != null)
        {
            ...
            else
            {
                ...
                {
                    var apiVersionVariable = ProxyScriptingJsFuncHelper.GetParamNameInJsFunc(
                        apiVersion
                    );
                    script.AppendLine(
                        $"      {apiVersionVariable} = {apiVersionVariable} ? {apiVersionVariable} : '{version}';"
                    );
                }
            }
        }
        ...
        var ajaxParamsIsFromForm = action.Parameters.Any(x =>
            x.BindingSourceId == ParameterBindingSources.Form
        );
        var dataType =
            action.ReturnValue.Type == ReturnValueApiDescriptionModel.Create(typeof(string)).Type
                ? "{ dataType: 'text' }, "
                : string.Empty;
        script.AppendLine(
            ajaxParamsIsFromForm
                ? "      }, $.extend(true, {}, "
                    + dataType
    ...
    )
    {
        ...
        foreach (var action in actions.Values)
        {
            ...
            actionsByName
                .GetOrAdd(actionName, () => new List<ActionApiDescriptionModel>())
                .Add(action);
        }
