...
internal class InMemoryStorage : IDataStorage
{
    ...
    public Task ChangePublishStateAsync(
    ...
    public Task<MediumMessage> StoreMessageAsync(
        string name,
        Message content,
        object? dbTransaction = null
    )
    ...
    )
    {
        ...
        if (table == nameof(PublishedMessages))
        {
            var ids = PublishedMessages
                .Values.Where(x => x.ExpiresAt < timeout)
                .Select(x => x.DbId)
                .Take(batchCount);
    ...
    )
    {
        IEnumerable<MediumMessage> result = PublishedMessages
            .Values.Where(x =>
                x.Retries < _capOptions.Value.FailedRetryCount
    ...
    {
        var deleteResult = ReceivedMessages.TryRemove(
    ...
    {
        var deleteResult = PublishedMessages.TryRemove(
            id.ToString(CultureInfo.InvariantCulture),
            out _
        );
        return Task.FromResult(deleteResult ? 1 : 0);
