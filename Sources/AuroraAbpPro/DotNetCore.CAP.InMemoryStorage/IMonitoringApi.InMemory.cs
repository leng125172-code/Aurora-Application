...
internal class InMemoryMonitoringApi : IMonitoringApi
{
    ...
    public Task<StatisticsDto> GetStatisticsAsync()
    {
        var stats = new StatisticsDto
        {
            ...
            PublishedFailed = InMemoryStorage.PublishedMessages.Values.Count(x =>
                x.StatusName == StatusName.Failed
            ),
            ReceivedFailed = InMemoryStorage.ReceivedMessages.Values.Count(x =>
                x.StatusName == StatusName.Failed
            ),
            PublishedDelayed = InMemoryStorage.PublishedMessages.Values.Count(x =>
        ...
        if (queryDto.MessageType == MessageType.Publish)
        {
            ...
            if (!string.IsNullOrEmpty(queryDto.StatusName))
                expression = expression.Where(x =>
                    x.StatusName.ToString()
                        .Equals(queryDto.StatusName, StringComparison.InvariantCultureIgnoreCase)
                );
            ...
            return Task.FromResult(
                new PagedQueryResult<MessageDto>
                {
    ...
    {
        return new ValueTask<int>(
            InMemoryStorage.PublishedMessages.Values.Count(x =>
                x.StatusName == StatusName.Succeeded
            )
        );
    }
    ...
    private Task<IDictionary<DateTime, int>> GetHourlyTimelineStats(
        MessageType type,
        string statusName
    )
    {
        ...
        if (type == MessageType.Publish)
            valuesMap = InMemoryStorage
                .PublishedMessages.Values.Where(x => x.StatusName.ToString() == statusName)
                .GroupBy(x => x.Added.ToString("yyyy-MM-dd-HH"))
                .ToDictionary(x => x.Key, x => x.Count());
        else
            valuesMap = InMemoryStorage
                .ReceivedMessages.Values.Where(x => x.StatusName.ToString() == statusName)
                .GroupBy(x => x.Added.ToString("yyyy-MM-dd-HH"))
                .ToDictionary(x => x.Key, x => x.Count());
