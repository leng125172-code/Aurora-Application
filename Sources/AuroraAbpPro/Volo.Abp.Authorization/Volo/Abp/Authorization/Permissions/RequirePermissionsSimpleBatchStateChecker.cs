    ...
    where TState : IHasSimpleStateCheckers<TState>
{
    ...
    private static readonly AsyncLocal<RequirePermissionsSimpleBatchStateChecker<TState>> _current =
        new AsyncLocal<RequirePermissionsSimpleBatchStateChecker<TState>>();
            ...
            if (model != null)
            {
                ...
                else
                {
                    result[model.State] = grantResult.Result.Any(x =>
                        model.Permissions.Contains(x.Key)
