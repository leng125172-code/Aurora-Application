...
public class ResourcePermissionPopulator : ITransientDependency
{
    ...
    public virtual async Task PopulateAsync<TResource>(
        ...
        where TResource : IHasResourcePermissions
    {
        ...
        foreach (var resource in resources)
        {
            ...
            var results = await ResourcePermissionChecker.IsGrantedAsync(
                resopurcePermissions.Select(x => x.Name).ToArray(),
                resourceName,
                resourceKey
            );
            foreach (var resopurcePermission in resopurcePermissions)
            {
                ...
                var hasPermission =
                    results.Result.TryGetValue(resopurcePermission.Name, out var granted)
