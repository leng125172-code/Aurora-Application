...
public class ResourcePermissionChecker : IResourcePermissionChecker, ITransientDependency
{
    ...
    )
    {
        ...
        var multiTenancySide =
            claimsPrincipal?.GetMultiTenancySide() ?? CurrentTenant.GetMultiTenancySide();
        ...
        var isGranted = false;
        var context = new ResourcePermissionValueCheckContext(
            permission,
            claimsPrincipal,
            resourceName,
            resourceKey
        ...
        {
            if (
                context.Permission.Providers.Any()
