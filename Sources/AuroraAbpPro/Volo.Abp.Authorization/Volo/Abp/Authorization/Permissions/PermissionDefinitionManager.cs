...
public class PermissionDefinitionManager : IPermissionDefinitionManager, ITransientDependency
{
    ...
    public virtual async Task<PermissionDefinition?> GetOrNullAsync(string name)
    {
        ...
        return await _staticStore.GetOrNullAsync(name) ?? await _dynamicStore.GetOrNullAsync(name);
    }

    public virtual async Task<PermissionDefinition> GetResourcePermissionAsync(
        string resourceName,
        string name
    )
    {
        ...
        if (permission == null)
        {
            throw new AbpException(
                $"Undefined resource permission: {name} for resource: {resourceName}"
            );
        }
    ...
    public virtual async Task<PermissionDefinition?> GetResourcePermissionOrNullAsync(
        string resourceName,
        string name
    )
    {
    ...
    public virtual async Task<IReadOnlyList<PermissionDefinition>> GetPermissionsAsync()
    {
        ...
        return staticPermissions
            .Concat(dynamicPermissions.Where(d => !staticPermissionNames.Contains(d.Name)))
            .ToImmutableList();
    }
    ...
    public virtual async Task<IReadOnlyList<PermissionDefinition>> GetResourcePermissionsAsync()
    {
        ...
        /* We prefer static permissions over dynamics */
        return staticResourcePermissions
            .Concat(
                dynamicResourcePermissions.Where(d =>
                    !staticResourcePermissionNames.Contains(d.Name)
                )
            )
            .ToImmutableList();
    }
