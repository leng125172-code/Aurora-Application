    ...
    public class RelationalDiagnosticListener : IMiniProfilerDiagnosticListener
    {
        ...
        public void OnNext(KeyValuePair<string, object?> kv)
        {
            ...
            if (key == RelationalEventId.CommandExecuting.Name)
            {
                ...
                {
                    var timing = data.Command.GetTiming(
                        data.ExecuteMethod + (data.IsAsync ? " (Async)" : null),
            ...
            {
                if (
                    val is CommandExecutedEventData data
                    && _commands.TryRemove(data.CommandId, out var current)
                )
                {
            ...
            {
                if (
                    val is CommandErrorEventData data
                    && _commands.TryRemove(data.CommandId, out var command)
                )
                {
            ...
            {
                if (
                    val is DataReaderDisposingEventData data
                    && _readers.TryRemove(data.CommandId, out var reader)
                )
                {
                    reader.Stop();
                }
            ...
            else if (key == RelationalEventId.ConnectionOpening.Name)
            {
                ...
                if (
                    val is ConnectionEventData data
                    && (profiler?.Options == null || profiler.Options.TrackConnectionOpenClose)
                )
                {
                    var timing = profiler.CustomTiming(
                        "sql",
            ...
            {
                if (
                    val is ConnectionEndEventData data
                    && _opening.TryRemove(data.ConnectionId, out var openingTiming)
                )
                {
                    openingTiming?.Stop();
                }
            ...
            else if (key == RelationalEventId.ConnectionClosed.Name)
            {
                if (
                    val is ConnectionEndEventData data
                    && _closing.TryRemove(data.ConnectionId, out var closingTiming)
                )
                {
                    closingTiming?.Stop();
                }
