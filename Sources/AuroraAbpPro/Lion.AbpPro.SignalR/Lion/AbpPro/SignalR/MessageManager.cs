...
public class MessageManager : IMessageManager, ITransientDependency
{
    ...
    )
    {
        ...
        if (messageType == MessageType.Common)
        {
            ...
            await _hubContext
                .Clients.Users([receiverUserId.ToString()])
                .ReceiveTextMessageAsync(
                    new SendNotificationDto(messageId, title, content, messageType, messageLevel)
                );
        }

        if (messageType == MessageType.BroadCast)
        {
            await _hubContext.Clients.All.ReceiveBroadCastMessageAsync(
                new SendNotificationDto(messageId, title, content, messageType, messageLevel)
            );
        }
        ...
        if (isPersistent)
        {
            await _localEventBus.PublishAsync(
                new CreatedNotificationLocalEvent(
                    messageId,
                    tenantId,
                    title,
                    content,
