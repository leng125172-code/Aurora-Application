...
public class AwsBlobProvider : BlobProviderBase, ITransientDependency
{
    ...
    public override async Task SaveAsync(BlobProviderSaveArgs args)
    {
        ...
        using (var amazonS3Client = await GetAmazonS3Client(args))
        {
            if (
                ...
                && await BlobExistsAsync(amazonS3Client, containerName, blobName)
            )
            {
    ...
    public override async Task<bool> DeleteAsync(BlobProviderDeleteArgs args)
    {
        ...
        using (var amazonS3Client = await GetAmazonS3Client(args))
        {
            ...
            await amazonS3Client.DeleteObjectAsync(
    ...
    )
    {
        if (!await AmazonS3Util.DoesS3BucketExistV2Async(amazonS3Client, containerName))
        {
            await amazonS3Client.PutBucketAsync(
                new PutBucketRequest { BucketName = containerName }
            );
        }
    }
    ...
    protected virtual string GetContainerName(BlobProviderArgs args)
    {
        ...
        return configuration.ContainerName.IsNullOrWhiteSpace()
            ? args.ContainerName
            : BlobNormalizeNamingService.NormalizeContainerName(
                args.Configuration,
