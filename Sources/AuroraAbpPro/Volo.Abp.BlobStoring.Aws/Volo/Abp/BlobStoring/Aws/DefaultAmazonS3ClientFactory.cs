...
public class DefaultAmazonS3ClientFactory : IAmazonS3ClientFactory, ITransientDependency
{
    ...
    )
    {
        ...
        if (configuration.UseTemporaryFederatedCredentials)
        {
            return new AmazonS3Client(
                await GetTemporaryFederatedCredentialsAsync(configuration),
                region
            );
        }
        ...
        Check.NotNullOrWhiteSpace(
            ...
            nameof(configuration.SecretAccessKey)
        );
    ...
    {
        var temporaryCredentialsCache = await Cache.GetAsync(
        ...
        if (temporaryCredentialsCache == null)
        {
            ...
            if (
                !configuration.AccessKeyId.IsNullOrEmpty()
                && !configuration.SecretAccessKey.IsNullOrEmpty()
            )
            {
            ...
            using (stsClient)
            {
                ...
                temporaryCredentialsCache = await SetTemporaryCredentialsCache(
                    configuration,
                    credentials
                );
            }
    ...
    )
    {
        ...
        if (temporaryCredentialsCache == null)
        {
            ...
            using (stsClient)
            {
                ...
                var federationTokenResponse = await stsClient.GetFederationTokenAsync(
        ...
        await Cache.SetAsync(
            ...
            new DistributedCacheEntryOptions
            {
                AbsoluteExpirationRelativeToNow = TimeSpan.FromSeconds(
                    configuration.DurationSeconds - 10
                ),
            }
        );
