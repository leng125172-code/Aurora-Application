...
public class EfCoreIdentitySessionRepository
    : EfCoreRepository<IIdentityDbContext, IdentitySession, Guid>,
        IIdentitySessionRepository
{
        ...
        return await (await GetDbSetAsync())
            ...
            .OrderBy(
                sorting.IsNullOrWhiteSpace()
                    ? $"{nameof(IdentitySession.LastAccessed)} desc"
                    : sorting
            )
    ...
    {
        await DeleteAsync(
            x => x.UserId == userId && x.Id != exceptSessionId,
    ...
    {
        await DeleteAsync(
            x => x.UserId == userId && x.Device == device && x.Id != exceptSessionId,
            cancellationToken: cancellationToken
        );
    }
    ...
    )
    {
        ...
        await DeleteDirectAsync(
            x => x.LastAccessed == null || x.LastAccessed < now.Subtract(inactiveTimeSpan),
            cancellationToken: cancellationToken
        );
    }
}
