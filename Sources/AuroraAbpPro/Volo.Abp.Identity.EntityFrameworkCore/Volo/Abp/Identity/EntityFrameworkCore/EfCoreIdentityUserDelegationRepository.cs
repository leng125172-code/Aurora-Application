        ...
        IIdentityUserDelegationRepository
{
    ...
    )
    {
        ...
        return await (await GetDbSetAsync())
            .AsNoTracking()
            .Where(x => x.TargetUserId == targetUserId && x.StartTime <= now && x.EndTime >= now)
            .ToListAsync(cancellationToken: cancellationToken);
    }
    ...
    )
    {
        ...
        return await (await GetDbSetAsync())
            .AsNoTracking()
            .FirstOrDefaultAsync(
                x => x.Id == id && x.StartTime <= now && x.EndTime >= now,
