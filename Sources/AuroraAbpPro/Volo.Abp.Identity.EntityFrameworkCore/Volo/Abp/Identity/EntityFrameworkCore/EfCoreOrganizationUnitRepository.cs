        ...
        IOrganizationUnitRepository
{
    ...
    )
    {
        return await (await GetDbSetAsync())
            .IncludeDetails(includeDetails)
            .OrderBy(
                sorting.IsNullOrEmpty() ? nameof(OrganizationUnit.CreationTime) + " desc" : sorting
            )
    ...
    )
    {
        ...
        var query =
            from organizationRole in dbContext.Set<OrganizationUnitRole>()
            join organizationUnit in dbContext.OrganizationUnits.IncludeDetails(includeDetails)
                on organizationRole.OrganizationUnitId equals organizationUnit.Id
    ...
    )
    {
        ...
        var query =
            ...
            join role in dbContext.Roles.IncludeDetails(includeDetails)
                on organizationRole.RoleId equals role.Id
    ...
    {
        var query = await CreateGetMembersFilteredQueryAsync(
            organizationUnit,
            filter,
            includeChildren
        );
    ...
    )
    {
        ...
        var userIdsInOrganizationUnit = dbContext
            .Set<IdentityUserOrganizationUnit>()
            .Where(uou => uou.OrganizationUnitId == organizationUnit.Id)
            .Select(uou => uou.UserId);

        var query = dbContext.Users.Where(u => !userIdsInOrganizationUnit.Contains(u.Id));
    ...
    )
    {
        ...
        return await dbContext
            .Users.Where(u => !userIdsInOrganizationUnit.Contains(u.Id))
            .WhereIf(
    ...
    )
    {
        ...
        var ouMembersQuery = await dbContext
            .Set<IdentityUserOrganizationUnit>()
