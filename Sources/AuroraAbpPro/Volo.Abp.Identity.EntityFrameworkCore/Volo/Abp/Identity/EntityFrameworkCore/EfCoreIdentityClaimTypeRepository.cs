...
public class EfCoreIdentityClaimTypeRepository
    : EfCoreRepository<IIdentityDbContext, IdentityClaimType, Guid>,
        IIdentityClaimTypeRepository
{
    public EfCoreIdentityClaimTypeRepository(
        ...
        var identityClaimTypes = await (await GetDbSetAsync())
            ...
            .OrderBy(
                sorting.IsNullOrWhiteSpace()
                    ? nameof(IdentityClaimType.CreationTime) + " desc"
                    : sorting
            )
    ...
    public virtual async Task<List<IdentityClaimType>> GetListByNamesAsync(
