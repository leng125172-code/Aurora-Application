...
public class EfCoreIdentityRoleRepository
    : EfCoreRepository<IIdentityDbContext, IdentityRole, Guid>,
        IIdentityRoleRepository
{
    ...
    {
        var roles = await GetListInternalAsync(
            sorting,
            maxResultCount,
            skipCount,
            filter,
        ...
        var userCount = await (await GetDbContextAsync())
        ...
        return roles
            .Select(role => new IdentityRoleWithUserCount(
                role,
                userCount.FirstOrDefault(x => x.RoleId == role.Id)?.Count ?? 0
            ))
            .ToList();
    }
    ...
    public virtual async Task RemoveClaimFromAllRolesAsync(
        string claimType,
        bool autoSave = false,
        CancellationToken cancellationToken = default
    )
    ...
    )
    {
        return await (await GetDbSetAsync())
            ...
            .OrderBy(
                sorting.IsNullOrWhiteSpace() ? nameof(IdentityRole.CreationTime) + " desc" : sorting
            )
