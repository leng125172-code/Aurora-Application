...
public class EfCoreIdentityLinkUserRepository
    : EfCoreRepository<IIdentityDbContext, IdentityLinkUser, Guid>,
        IIdentityLinkUserRepository
{
    public EfCoreIdentityLinkUserRepository(
    ...
    )
    {
        return await (await GetDbSetAsync())
            ...
            .FirstOrDefaultAsync(
                x =>
                    x.SourceUserId == sourceLinkUserInfo.UserId
                        && x.SourceTenantId == sourceLinkUserInfo.TenantId
                        && x.TargetUserId == targetLinkUserInfo.UserId
                        && x.TargetTenantId == targetLinkUserInfo.TenantId
                    || x.TargetUserId == sourceLinkUserInfo.UserId
                        && x.TargetTenantId == sourceLinkUserInfo.TenantId
                        && x.SourceUserId == targetLinkUserInfo.UserId
                        && x.SourceTenantId == targetLinkUserInfo.TenantId,
    ...
    )
    {
        var query = (await GetDbSetAsync())
            ...
            .Where(x =>
                ...
                || x.TargetUserId == linkUserInfo.UserId
                    && x.TargetTenantId == linkUserInfo.TenantId
            );
