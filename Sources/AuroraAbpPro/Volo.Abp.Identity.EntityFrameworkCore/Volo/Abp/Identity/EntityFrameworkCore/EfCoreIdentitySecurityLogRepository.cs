...
public class EfCoreIdentitySecurityLogRepository
    : EfCoreRepository<IIdentityDbContext, IdentitySecurityLog, Guid>,
        IIdentitySecurityLogRepository
{
    public EfCoreIdentitySecurityLogRepository(
        ...
        return await query
            .OrderBy(
                sorting.IsNullOrWhiteSpace()
                    ? $"{nameof(IdentitySecurityLog.CreationTime)} desc"
                    : sorting
            )
    ...
    )
    {
        return (await GetDbSetAsync())
            ...
            .WhereIf(
                endTime.HasValue,
                securityLog => securityLog.CreationTime < endTime.Value.AddDays(1).Date
            )
            .WhereIf(
            ...
            .WhereIf(
                ...
                securityLog => securityLog.UserName == userName
            )
            .WhereIf(
                !clientId.IsNullOrWhiteSpace(),
                securityLog => securityLog.ClientId == clientId
            )
            .WhereIf(
                !correlationId.IsNullOrWhiteSpace(),
                securityLog => securityLog.CorrelationId == correlationId
            )
            .WhereIf(
                !clientIpAddress.IsNullOrWhiteSpace(),
                securityLog => securityLog.ClientIpAddress == clientIpAddress
            );
    }
}
