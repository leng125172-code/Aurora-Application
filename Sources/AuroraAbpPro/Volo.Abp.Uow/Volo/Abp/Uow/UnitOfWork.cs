...
public class UnitOfWork : IUnitOfWork, ITransientDependency
{
    ...
    protected List<
        KeyValuePair<UnitOfWorkEventRecord, Predicate<UnitOfWorkEventRecord>?>
    > DistributedEventWithPredicates { get; } =
        new List<KeyValuePair<UnitOfWorkEventRecord, Predicate<UnitOfWorkEventRecord>?>>();
    protected List<UnitOfWorkEventRecord> DistributedEvents { get; } =
        new List<UnitOfWorkEventRecord>();

    protected List<
        KeyValuePair<UnitOfWorkEventRecord, Predicate<UnitOfWorkEventRecord>?>
    > LocalEventWithPredicates { get; } =
        new List<KeyValuePair<UnitOfWorkEventRecord, Predicate<UnitOfWorkEventRecord>?>>();
    ...
    public virtual async Task CompleteAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            while (LocalEvents.Any() || DistributedEvents.Any())
            {
                ...
                {
                    var distributedEventsToBePublished = DistributedEvents
    ...
    public virtual void AddDatabaseApi(string key, IDatabaseApi api)
    {
        ...
        if (_databaseApis.ContainsKey(key))
        {
            throw new AbpException(
                "This unit of work already contains a database API for the given key."
            );
        }
    ...
    public virtual void AddTransactionApi(string key, ITransactionApi api)
    {
        ...
        {
            throw new AbpException(
                "This unit of work already contains a transaction API for the given key."
            );
        }
    ...
    {
        LocalEventWithPredicates.Add(
            new KeyValuePair<UnitOfWorkEventRecord, Predicate<UnitOfWorkEventRecord>?>(
                eventRecord,
                replacementSelector
            )
        );
    ...
    {
        DistributedEventWithPredicates.Add(
            new KeyValuePair<UnitOfWorkEventRecord, Predicate<UnitOfWorkEventRecord>?>(
                eventRecord,
                replacementSelector
            )
    ...
    protected virtual List<UnitOfWorkEventRecord> GetEventsRecords(
        List<
            KeyValuePair<UnitOfWorkEventRecord, Predicate<UnitOfWorkEventRecord>?>
        > eventWithPredicates
    )
