...
public static class SerilogToEsExtensions
{
    ...
    )
    {
        ...
        loggerConfiguration.ReadFrom.Configuration(configuration).Enrich.FromLogContext();
        var writeToSections = configuration.GetSection("Serilog:WriteTo").GetChildren();
        var elasticSection = writeToSections.FirstOrDefault(s =>
            s.GetValue<string>("Name") == "Elastic"
        );

        var writeToElasticSearch =
            elasticSection?.GetSection("Args").GetValue<bool>("Enabled") ?? false;
        ...
        var applicationName =
            elasticSection?.GetSection("Args").GetValue<string>("ApplicationName") ?? string.Empty;
        ...
        loggerConfiguration
            ...
            .WriteTo.Elasticsearch(
                BuildElasticSearchSinkOptions(esUrl, indexFormat, esUserName, esPassword)
            );
