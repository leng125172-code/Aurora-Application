...
public static class ApplicationBuilderExtensions
{
    ...
    public static IApplicationBuilder UseAbpProConsul(this IApplicationBuilder app)
    {
        ...
        var consulOptions = app
            .ApplicationServices.GetRequiredService<IOptions<AbpProConsulOptions>>()
            .Value;
        ...
        var consulServiceRegistration = new AgentServiceRegistration
        {
            ...
            Check = new AgentServiceCheck
            {
                DeregisterCriticalServiceAfter = TimeSpan.FromSeconds(
                    consulOptions.DeregisterCriticalServiceAfter
                ), //服务停止多久后注销
    ...
    public static IApplicationBuilder UseAbpProRequestLocalization(this IApplicationBuilder app)
    {
        return app.UseAbpRequestLocalization(options =>
        {
            ...
            options.RequestCultureProviders.Add(
                new AbpProAcceptLanguageHeaderRequestCultureProvider()
            );
        });
    }
    ...
    public static IApplicationBuilder UseAbpProSwaggerUI(
        this IApplicationBuilder app,
        string endpoint,
        string name
    )
    {
        var swaggerOptions = app
            .ApplicationServices.GetRequiredService<IOptions<AbpProSwaggerOptions>>()
            .Value;
    ...
    {
        var miniProfilerOptions = app
            .ApplicationServices.GetRequiredService<IOptions<AbpProMiniProfilerOptions>>()
            .Value;
        if (!miniProfilerOptions.Enabled)
            return app;
        app.UseMiniProfiler();
        return app;
    }
    ...
    {
        var abpProMultiTenancyOptions = app
            .ApplicationServices.GetRequiredService<IOptions<AbpProMultiTenancyOptions>>()
            .Value;
        if (!abpProMultiTenancyOptions.Enabled)
            return app;
        app.UseMultiTenancy();
        return app;
    }
    ...
    {
        var corsOptions = app
            .ApplicationServices.GetRequiredService<IOptions<AbpProCorsOptions>>()
            .Value;
    ...
    {
        var auditOptions = app
            .ApplicationServices.GetRequiredService<IOptions<AbpProAuditOptions>>()
            .Value;
    ...
    /// </summary>
    /// <returns></returns>
    public static IApplicationBuilder UseAbpProRequestResponseEncrypt(this IApplicationBuilder app)
    {
        app.UseMiddleware<RequestEncryptMiddleware>();
