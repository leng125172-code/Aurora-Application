...
public class AbpProAcceptLanguageHeaderRequestCultureProvider
    ...
    )
    {
        ...
        try
        {
            ...
            if (
                culture.Buffer != null
                && (
                    culture.Buffer.StartsWith("zh-CN")
                    || culture.Buffer.StartsWith("zh-cn")
                    || culture.Buffer.StartsWith("zh-HK")
                    || culture.Buffer.StartsWith("zh-TW")
                    || culture.Buffer.StartsWith("zh-MO")
                )
            )
            {
