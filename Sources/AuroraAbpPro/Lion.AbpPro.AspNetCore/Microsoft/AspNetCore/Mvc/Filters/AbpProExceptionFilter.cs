...
public class AbpProExceptionFilter : AbpExceptionFilter
{
    ...
    {
        var exceptionHandlingOptions = context
            .GetRequiredService<IOptions<AbpExceptionHandlingOptions>>()
            .Value;
        var exceptionToErrorInfoConverter =
            context.GetRequiredService<IExceptionToErrorInfoConverter>();
        var remoteServiceErrorInfo = exceptionToErrorInfoConverter.Convert(
            context.Exception,
            options =>
            {
    ...
    private async Task DefaultHandlerAsync(ExceptionContext context)
    {
        ...
        if (context.Exception is AbpAuthorizationException)
        {
            await context
                ...
                .HandleAsync(
                    context.Exception.As<AbpAuthorizationException>(),
                    context.HttpContext
                );
        }
        else
    ...
    private bool IsWrapResult(ExceptionContext context)
    {
        if (
            context
                ...
                .ControllerTypeInfo.GetCustomAttributes(typeof(WrapResultAttribute), true)
                .Any()
        )
        {
            return true;
        ...
        if (
            context
                .ActionDescriptor.GetMethodInfo()
                .GetCustomAttributes(typeof(WrapResultAttribute), true)
                .Any()
        )
        {
            return true;
    ...
    private WrapResult<object> SimplifyMessage(ExceptionContext context)
    {
        ...
        switch (context.Exception)
        {
            ...
            case AbpValidationException validationException:
                var errorMessage =
                    localizer["Lion.AbpPro:ParameterValidationFailed"]
                    + ";"
                    + validationException.ValidationErrors.JoinAsString(";");
            ...
            default:
            {
                if (context.Exception is IHasErrorCode codeException)
                {
                    var exceptionConverter =
                        context.GetRequiredService<IAbpProExceptionConverter>();
                    var message = exceptionConverter.TryToLocalizeExceptionMessage(
                        context.Exception
                    );
