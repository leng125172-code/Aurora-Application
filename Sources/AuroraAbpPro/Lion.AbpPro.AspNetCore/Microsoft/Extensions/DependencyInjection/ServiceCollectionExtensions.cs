...
public static class ServiceCollectionExtensions
{
    ...
    {
        var consulOptions = service
            .BuildServiceProvider()
            .GetRequiredService<IOptions<AbpProConsulOptions>>()
            .Value;
        if (!consulOptions.Enabled)
            return service;

        service.AddSingleton<IConsulClient>(p => new ConsulClient(config =>
        {
            config.Address = new Uri(consulOptions.ServiceUrl);
        }));
    ...
    {
        var corsOptions = service
            .BuildServiceProvider()
            .GetRequiredService<IOptions<AbpProCorsOptions>>()
            .Value;
    ...
    {
        var antiForgeryOptions = service
            .BuildServiceProvider()
            .GetRequiredService<IOptions<AbpProAntiForgeryOptions>>()
            .Value;
        service.Configure<AbpAntiForgeryOptions>(options =>
        {
            options.AutoValidate = antiForgeryOptions.Enabled;
        });
        return service;
    }
    ...
    /// 添加swagger
    /// </summary>
    public static IServiceCollection AddAbpProSwagger(
        this IServiceCollection service,
        string name,
        string version = "v1"
    )
    {
        var swaggerOptions = service
            .BuildServiceProvider()
            .GetRequiredService<IOptions<AbpProSwaggerOptions>>()
            .Value;
        if (!swaggerOptions.Enabled)
            return service;

        // 允许 Swashbuckle 发现 Minimal API（MapGet/MapPost）注册的端点
        ...
        service.AddSwaggerGen(options =>
        {
            ...
            options.DocumentFilter<CapDashboardSwaggerFilter>();
            options.DocumentFilter<HangfireDashboardSwaggerFilter>();
            options.DocumentFilter<MiniProfilerSwaggerFilter>();
    ...
    public static IServiceCollection AddAbpProMiniProfiler(this IServiceCollection service)
    {
        var options = service
            .BuildServiceProvider()
            .GetRequiredService<IOptions<AbpProMiniProfilerOptions>>()
            .Value;
        if (!options.Enabled)
            return service;
        service
            .AddMiniProfiler(opt => opt.RouteBasePath = options.RouteBasePath)
            .AddEntityFramework();
        return service;
    }
    ...
    {
        var multiTenancyOptions = service
            .BuildServiceProvider()
            .GetRequiredService<IOptions<AbpProMultiTenancyOptions>>()
            .Value;
    ...
    {
        var jwtOptions = service
            .BuildServiceProvider()
            .GetRequiredService<IOptions<AbpProJwtOptions>>()
            .Value;
        var cookieOptions = service
            .BuildServiceProvider()
            .GetRequiredService<IOptions<AbpProCookieOptions>>()
            .Value;
            ...
            .AddJwtBearer(options =>
            {
                ...
                options.Events = new JwtBearerEvents
                {
                    OnMessageReceived = currentContext =>
                    {
                        ...
                        {
                            accessToken = currentContext.HttpContext.Request.Headers[
                                "Authorization"
                            ];
                        ...
                        currentContext.Request.Headers.Append(
                            "Authorization",
                            $"Bearer {accessToken}"
                        );
    ...
    {
        var auditOptions = service
            .BuildServiceProvider()
            .GetRequiredService<IOptions<AbpProAuditOptions>>()
            .Value;
        service.Configure<AbpAuditingOptions>(options =>
        {
