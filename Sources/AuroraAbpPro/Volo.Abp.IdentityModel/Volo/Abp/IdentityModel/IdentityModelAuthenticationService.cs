...
public class IdentityModelAuthenticationService
        ...
        ITransientDependency
{
    ...
    protected virtual async Task<string?> GetAccessTokenOrNullAsync(string? identityClientName)
    {
        ...
        {
            Logger.LogWarning(
                $"Could not find {nameof(IdentityClientConfiguration)} for {identityClientName}. Either define a configuration for {identityClientName} or set a default configuration."
            );
            return null;
        }
    ...
    )
    {
        using (var httpClient = HttpClientFactory.CreateClient(HttpClientName))
        {
            ...
            switch (configuration.GrantType)
            {
                ...
                default:
                    throw new AbpException(
                        "Grant type was not implemented: " + configuration.GrantType
                    );
            }
        }
    }
    ...
    )
    {
        ...
        for (var i = 0; i < ((response.ExpiresIn ?? 300) / response.Interval + 1); i++)
        {
            ...
            var tokenResponse = await httpClient.RequestDeviceTokenAsync(
    ...
    protected virtual Task AddParametersToRequestAsync(
        IdentityClientConfiguration configuration,
        ProtocolRequest request
    )
    {
        foreach (
            var pair in configuration.Where(p =>
                p.Key.StartsWith("[o]", StringComparison.OrdinalIgnoreCase)
            )
        )
        {
