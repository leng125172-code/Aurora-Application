...
{
    private static readonly List<string> ExcludeDirectory = new List<string>()
    {
        "node_modules",
        ".git",
        ".idea",
            ...
            if (projectPath.EndsWith("package.json"))
            {
                ...
                if (frameworkType != JavaScriptFrameworkType.None)
                {
                    ...
                    Logger.LogInformation(
                        $"Installing dependencies for {frameworkName} project: {projectDirectory}"
                    );
                    NpmHelper.RunYarn(projectDirectory);
                }
            }
    ...
    private JavaScriptFrameworkType DetectFrameworkTypeFromPackageJson(string packageJsonFilePath)
    {
        ...
        catch (Exception ex)
        {
            Logger.LogWarning(
                ex,
                "Failed to parse package.json at {PackageJsonFilePath}",
                packageJsonFilePath
            );
            return JavaScriptFrameworkType.None;
        }
    ...
    private List<string> FindAllProjects(string directory)
    {
        ...
        var angularFiles = Directory
            .GetFiles(directory, "angular.json", SearchOption.AllDirectories)
            .Where(file =>
                ExcludeDirectory.All(x =>
                    file.IndexOf(
                        x + Path.DirectorySeparatorChar,
        ...
        var packageJsonFiles = Directory
            ...
            .Where(packageJsonFile =>
            {
                ...
                if (
                    Directory
                        .GetFiles(packageJsonDirectory, "*.csproj", SearchOption.TopDirectoryOnly)
                        .Any()
                )
                {
                    return false;
    ...
    private async Task CleanAndCopyResources(string fileDirectory)
    {
        ...
        var resourceMapping = new ResourceMapping { Clean = new List<string> { LibsDirectory } };
        ...
        foreach (var mappingFile in mappingFiles)
        {
            using (var reader = File.OpenText(mappingFile))
            {
                ...
                mapping
                    ...
                    .ForEach(x =>
                    {
                        resourceMapping.Aliases.AddIfNotContains(
                ...
                mapping
                    ...
                    .ForEach(x =>
                    {
                        resourceMapping.Mappings.AddIfNotContains(
                            new KeyValuePair<string, string>(x.Key, x.Value)
                        );
                    });
            }
    ...
    private List<FileMatchResult> FindFiles(string directory, params string[] patterns)
    {
        ...
        return result
            .Files.Select(x => new FileMatchResult(Path.Combine(directory, x.Path), x.Stem))
            .ToList();
    }
