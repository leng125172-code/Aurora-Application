...
public abstract class ProjectCreationCommandBase
{
    ...
    protected async Task<ProjectBuildArgs> GetProjectBuildArgsAsync(
    ...
    )
    {
        ...
        var publicWebSite =
            ...
            && commandLineArgs.Options.ContainsKey(Options.PublicWebSite.Long);
        if (publicWebSite)
        {
        ...
        if (gitHubAbpLocalRepositoryPath != null)
        {
            Logger.LogInformation(
                $"GitHub Abp Local Repository Path: {gitHubAbpLocalRepositoryPath}"
            );
        }
        ...
        {
            Logger.LogInformation(
                $"GitHub Volo Local Repository Path: {gitHubVoloLocalRepositoryPath}"
            );
        }

        var templateSource = commandLineArgs.Options.GetOrNull(
            Options.TemplateSource.Short,
            Options.TemplateSource.Long
        );
        ...
        var outputFolder = commandLineArgs.Options.GetOrNull(
            Options.OutputFolder.Short,
            Options.OutputFolder.Long
        );
        ...
        {
            var slnPath = commandLineArgs.Options.GetOrNull(
                Options.MainSolution.Short,
                Options.MainSolution.Long
            );

            if (slnPath == null)
            {
                slnPath = Directory
                    .GetFiles(outputFolderRoot, "*.sln")
                    .Concat(Directory.GetFiles(outputFolderRoot, "*.slnx"))
                    .FirstOrDefault();
            }
            ...
            outputFolder = MicroserviceServiceTemplateBase.CalculateTargetFolder(
                outputFolderRoot,
                solutionName.ProjectName
            );
            uiFramework =
                uiFramework == UiFramework.NotSpecified
                    ? FindMicroserviceSolutionUiFramework(outputFolderRoot)
                    : uiFramework;
        }
        else
        {
        ...
        {
            connectionString = ConnectionStringProvider.GetByDbms(
        ...
        var theme =
            uiFramework == UiFramework.None
                ? (Theme?)null
                : GetThemeByTemplateOrNull(commandLineArgs, template);
        var themeStyle = theme.HasValue
            ? GetThemeStyleOrNull(commandLineArgs, theme.Value)
            : (ThemeStyle?)null;

        var skipCache =
            commandLineArgs.Options.ContainsKey(Options.SkipCache.Long)
            || commandLineArgs.Options.ContainsKey(Options.SkipCache.Short);

        var trustUserVersion =
            !version.IsNullOrEmpty()
                && commandLineArgs.Options.ContainsKey(Options.TrustUserVersion.Long)
            || commandLineArgs.Options.ContainsKey(Options.TrustUserVersion.Short);
    ...
    protected void ExtractProjectZip(ProjectBuildResult project, string outputFolder)
    {
        EventBus.PublishAsync(
            new ProjectCreationProgressEvent { Message = "Unzipping the solution" },
            false
        );
    ...
    protected void OpenRelatedWebPage(
    ...
    )
    {
        ...
        if (AppTemplateBase.IsAppTemplate(template))
        {
            ...
            OpenThanksPage(
                projectArgs.UiFramework,
                projectArgs.DatabaseProvider,
                isTiered
                    || commandLineArgs.Options.ContainsKey("separate-identity-server")
                    || commandLineArgs.Options.ContainsKey("separate-auth-server"),
                isCommercial
            );
        }
    ...
    {
        uiFramework =
            uiFramework == UiFramework.NotSpecified || uiFramework == UiFramework.None
                ? UiFramework.Mvc
                : uiFramework;
        databaseProvider =
            databaseProvider == DatabaseProvider.NotSpecified
                ? DatabaseProvider.EntityFrameworkCore
                : databaseProvider;

        var tieredYesNo = tiered ? "yes" : "no";
        var url =
            $"https://abp.io/project-created-success?ui={uiFramework:g}&db={databaseProvider:g}&tiered={tieredYesNo}&commercial={(commercial ? "yes" : "no")}";
    ...
    protected static string GetConnectionString(CommandLineArgs commandLineArgs)
    {
        var connectionString = commandLineArgs.Options.GetOrNull(
            Options.ConnectionString.Short,
            Options.ConnectionString.Long
        );
    ...
    protected virtual DatabaseProvider GetDatabaseProvider(CommandLineArgs commandLineArgs)
    {
        var optionValue = commandLineArgs.Options.GetOrNull(
            Options.DatabaseProvider.Short,
            Options.DatabaseProvider.Long
        );
        ...
        if (
            optionValue.Equals("ef", StringComparison.InvariantCultureIgnoreCase)
            || optionValue.Equals(
                "entityframeworkcore",
        ...
        if (
            optionValue.Equals("mongo", StringComparison.InvariantCultureIgnoreCase)
            || optionValue.Equals("mongodb", StringComparison.InvariantCultureIgnoreCase)
        )
        {
        ...
        throw new CliUsageException(
            ExceptionMessageHelper.GetInvalidOptionExceptionMessage("Database Provider")
        );
    }

    protected virtual async Task RunGraphBuildForMicroserviceServiceTemplate(
        ProjectBuildArgs projectArgs
    )
    {
        if (MicroserviceServiceTemplateBase.IsMicroserviceServiceTemplate(projectArgs.TemplateName))
        {
            await EventBus.PublishAsync(
                new ProjectCreationProgressEvent { Message = "Building the microservice solution" },
                false
            );
    ...
    {
        if (
            AppTemplateBase.IsAppTemplate(projectArgs.TemplateName)
        ...
        )
        {
            ...
            await EventBus.PublishAsync(
                new ProjectCreationProgressEvent { Message = "Installing client-side packages" },
                false
            );
    ...
    protected virtual async Task RunBundleInternalAsync(ProjectBuildArgs projectArgs)
    {
        ...
        var message =
            isWebassembly || isModuleTemplate
        ...
        await EventBus.PublishAsync(new ProjectCreationProgressEvent { Message = message }, false);
        ...
        if (
            isWebassembly
            && Directory.GetFiles(path, "*.Blazor.Client.csproj", SearchOption.AllDirectories).Any()
        )
        {
        ...
        if (isModuleTemplate)
        {
            ...
            if (
                Directory
                    .GetFiles(path, "*.Blazor.Host.Client.csproj", SearchOption.AllDirectories)
                    .Any()
            )
            {
    ...
    protected virtual bool ShouldRunBundleCommand(ProjectBuildArgs projectArgs)
    {
        ...
        if (
            MicroserviceServiceTemplateBase.IsMicroserviceTemplate(projectArgs.TemplateName)
            && projectArgs.UiFramework is UiFramework.Blazor
        )
        {
            return true;
        }
        ...
        if (
            ModuleTemplateBase.IsModuleTemplate(projectArgs.TemplateName)
            && projectArgs.UiFramework != UiFramework.None
        )
        {
            return true;
        }
    ...
    protected async Task CreateInitialMigrationsAsync(ProjectBuildArgs projectArgs)
    {
        ...
        switch (projectArgs.TemplateName)
        {
            ...
            case AppProTemplate.TemplateName:
                efCoreProjectPath = Directory
                    .GetFiles(
                        projectArgs.OutputFolder,
            ...
            case AppNoLayersProTemplate.TemplateName:
                efCoreProjectPath =
                    Directory
                        ...
                        .FirstOrDefault()
                    ?? Directory
                        .GetFiles(projectArgs.OutputFolder, "*.csproj", SearchOption.AllDirectories)
                        .FirstOrDefault();
        ...
        await EventBus.PublishAsync(
            new ProjectCreationProgressEvent { Message = "Creating the initial DB migration" },
            false
        );

        if (!isModuleTemplate)
        {
            await InitialMigrationCreator.CreateAsync(
                Path.GetDirectoryName(efCoreProjectPath),
                isLayeredTemplate
            );
        }
        else
        {
            var hostProjectsWithEfCore = Directory
                .GetFiles(projectArgs.OutputFolder, "*.csproj", SearchOption.AllDirectories)
    ...
    protected Task CreateOpenIddictPfxFilesAsync(ProjectBuildArgs projectArgs)
    {
        ...
        moduleDirectory = Path.GetDirectoryName(
            projectArgs.SolutionName.CompanyName == null
                ? moduleDirectory.Replace(
                    "MyCompanyName.MyProjectName",
                    projectArgs.SolutionName.ProjectName
                )
                : moduleDirectory
                    .Replace("MyCompanyName", projectArgs.SolutionName.CompanyName)
                    .Replace("MyProjectName", projectArgs.SolutionName.ProjectName)
        );
        ...
        if (Directory.Exists(moduleDirectory))
        {
            ...
            CmdHelper.RunCmd(
                $"dotnet dev-certs https -ep openiddict.pfx -p {RandomizeAuthServerPassPhraseStep.RandomOpenIddictPassword}",
                moduleDirectory
            );
        }
        else
        {
            Logger.LogWarning(
                $"Couldn't find the module directory to create openiddict.pfx file: {moduleDirectory}"
            );
        }
        return Task.CompletedTask;
    }
    ...
    protected virtual DatabaseManagementSystem GetDatabaseManagementSystem(
        CommandLineArgs commandLineArgs
    )
    {
        var optionValue = commandLineArgs.Options.GetOrNull(
            Options.DatabaseManagementSystem.Short,
            Options.DatabaseManagementSystem.Long
        );
        ...
        switch (optionValue.ToLowerInvariant())
        {
            ...
            default:
                throw new CliUsageException(
                    ExceptionMessageHelper.GetInvalidOptionExceptionMessage(
    ...
    protected virtual MobileApp GetMobilePreference(
        CommandLineArgs commandLineArgs,
        string template
    )
    {
        var optionValue = commandLineArgs.Options.GetOrNull(
            Options.Mobile.Short,
            Options.Mobile.Long
        );
        ...
        switch (optionValue)
        {
            ...
            case "react-native"
                when template
                    is AppProTemplate.TemplateName
                        or MicroserviceProTemplate.TemplateName:
            ...
            default:
                throw new CliUsageException(
                    ExceptionMessageHelper.GetInvalidOptionExceptionMessage("Mobile App")
                );
        }
    }

    protected virtual UiFramework GetUiFramework(
        CommandLineArgs commandLineArgs,
        string template = "app"
    )
    {
        ...
        var optionValue = commandLineArgs.Options.GetOrNull(
            Options.UiFramework.Short,
            Options.UiFramework.Long
        );
        ...
        switch (optionValue)
        {
            ...
            default:
                throw new CliUsageException(
                    ExceptionMessageHelper.GetInvalidOptionExceptionMessage("UI Framework")
                );
        }
    }

    protected virtual Theme? GetThemeByTemplateOrNull(
        CommandLineArgs commandLineArgs,
        string template = "app"
    )
    {
        ...
        return template switch
        {
            ...
            or AppNoLayersProTemplate.TemplateName
            or MicroserviceProTemplate.TemplateName => GetAppProTheme(),
            _ => null,
        };
    ...
    protected void ConfigureAngularJsonForThemeSelection(ProjectBuildArgs projectArgs)
    {
        ...
        {
            var angularFolderPath =
                projectArgs.TemplateName == MicroserviceProTemplate.TemplateName
                    ? projectArgs.OutputFolder.EnsureEndsWith(Path.DirectorySeparatorChar)
                        + "apps"
            ...
            AngularThemeConfigurer.Configure(
                new AngularThemeConfigurationArgs(
    ...
    protected virtual async Task ConfigureAngularAfterMicroserviceServiceCreatedAsync(
        ProjectBuildArgs projectArgs,
        string template
    )
    {
    ...
    )
    {
        ...
        var commandBuilder = new StringBuilder(
            $"npx ng g @abp/ng.schematics:create-lib --package-name {libraryName}"
        );

        commandBuilder.Append(
            $" --is-secondary-entrypoint {isSecondaryEndpoint.ToString().ToLower()}"
        );
