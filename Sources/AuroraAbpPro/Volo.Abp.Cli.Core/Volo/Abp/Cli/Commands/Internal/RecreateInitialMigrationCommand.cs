...
public class RecreateInitialMigrationCommand : IConsoleCommand, ITransientDependency
{
    ...
    public virtual Task ExecuteAsync(CommandLineArgs commandLineArgs)
    {
        ...
        foreach (var csprojFile in csprojFiles)
        {
            ...
            {
                Logger.LogError(
                    "Build failed for project {Project}. Skipping migration recreation.",
                    csprojFile
                );
                continue;
            }
            ...
            {
                CmdHelper.RunCmd(
                    $"dotnet ef migrations add Initial --context MyProjectNameDbContext",
                    workingDirectory: projectDir
                );
                CmdHelper.RunCmd(
                    $"dotnet ef migrations add Initial --context MyProjectNameTenantDbContext --output-dir TenantMigrations",
                    workingDirectory: projectDir
                );
            }
