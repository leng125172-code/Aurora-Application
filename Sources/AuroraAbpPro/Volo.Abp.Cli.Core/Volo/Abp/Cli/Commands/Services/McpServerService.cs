...
public class McpServerService : ITransientDependency
{
    ...
    {
        public const string InvalidResponseFormat =
            "The tool execution completed but returned an invalid response format. Please try again.";
    ...
    private static CallToolResult CreateErrorResult(string errorMessage)
    {
        ...
        {
            Content = new List<ContentBlock> { new TextContentBlock { Text = errorMessage } },
            IsError = true,
        };
    }
    ...
    )
    {
        ...
        var tool = new AbpMcpServerTool(
            ...
            (context, cancellationToken) =>
                HandleToolInvocationAsync(name, context, cancellationToken)
        );
    ...
    )
    {
        _mcpLogger.Debug(
            LogSource,
            $"Tool '{toolName}' called with arguments: {context.Params.Arguments}"
        );
    ...
    private CallToolResult TryDeserializeResult(string resultJson, string toolName)
    {
        ...
        catch (Exception ex)
        {
            _mcpLogger.Error(
                LogSource,
                $"Failed to deserialize response as CallToolResult: {ex.Message}"
            );

            var logResponse =
                resultJson.Length <= MaxLogResponseLength
                    ? resultJson
