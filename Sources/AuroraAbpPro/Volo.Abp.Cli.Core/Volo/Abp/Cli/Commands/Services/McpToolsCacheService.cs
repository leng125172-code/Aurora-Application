...
public class McpToolsCacheService : ITransientDependency
{
    ...
    public async Task<List<McpToolDefinition>> GetToolDefinitionsAsync()
    {
        ...
        _mcpLogger.Info(
            LogSource,
            $"Successfully fetched and cached {tools.Count} tool definitions"
        );
        return tools;
    }
    ...
    private async Task<bool> IsCacheValidAsync()
    {
        try
        {
            ...
            var lastFetchTimeString = await _memoryService.GetAsync(
                CliConsts.MemoryKeys.McpToolsLastFetchDate
            );
    ...
    private async Task<List<McpToolDefinition>> LoadFromCacheAsync()
    {
        try
        {
            ...
            var tools = JsonSerializer.Deserialize<List<McpToolDefinition>>(
                json,
