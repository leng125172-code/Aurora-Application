...
public class InitialMigrationCreator : ITransientDependency
{
    ...
    public async Task<bool> CreateAsync(string targetProjectFolder, bool layeredTemplate = true)
    {
        ...
        var dbContextName =
            tenantDbContextName != null ? FindDbContextName(targetProjectFolder) : null;

        var migrationOutput = AddMigrationAndGetOutput(
            targetProjectFolder,
            dbContextName,
            "Migrations"
        );
        var tenantMigrationOutput =
            tenantDbContextName != null
                ? AddMigrationAndGetOutput(
                    targetProjectFolder,
                    tenantDbContextName,
                    "TenantMigrations"
                )
        ...
        var migrationSuccess =
            CheckMigrationOutput(migrationOutput) && CheckMigrationOutput(tenantMigrationOutput);
        ...
        {
            Logger.LogError(
                "Creating initial migrations process is failed! Details:"
                    + Environment.NewLine
                    + migrationOutput
                    + Environment.NewLine
                    + tenantMigrationOutput
                    + Environment.NewLine
            );
        }
    ...
    private string FindTenantDbContextName(string projectFolder)
    {
        var tenantDbContext =
            Directory
                .GetFiles(
                    projectFolder,
                    "*TenantMigrationsDbContext.cs",
                    SearchOption.AllDirectories
                )
                .FirstOrDefault()
            ?? Directory
                .GetFiles(projectFolder, "*TenantDbContext.cs", SearchOption.AllDirectories)
                .FirstOrDefault();
    ...
    private string FindDbContextName(string projectFolder)
    {
        var dbContext =
            Directory
                .GetFiles(projectFolder, "*MigrationsDbContext.cs", SearchOption.AllDirectories)
                .FirstOrDefault(fp => !fp.EndsWith("TenantMigrationsDbContext.cs"))
            ?? Directory
                .GetFiles(projectFolder, "*DbContext.cs", SearchOption.AllDirectories)
    ...
    private string AddMigrationAndGetOutput(
