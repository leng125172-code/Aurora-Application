...
public class NewCommand : ProjectCreationCommandBase, IConsoleCommand, ITransientDependency
{
    ...
    public async Task ExecuteAsync(CommandLineArgs commandLineArgs)
    {
        ...
        await _telemetryService.AddActivityAsync(
            activityName,
            o =>
            {
        ...
        Logger.LogInformation(
            $"'{projectName}' has been successfully created to '{projectArgs.OutputFolder}'"
        );
        ...
        var skipInstallLibs =
            commandLineArgs.Options.ContainsKey(Options.SkipInstallingLibs.Long)
        ...
        var skipBundling =
            commandLineArgs.Options.ContainsKey(Options.SkipBundling.Long)
    ...
    private async Task CheckCreatedRequirements(ProjectBuildArgs projectArgs)
    {
        ...
        if (projectArgs.ExtraProperties.ContainsKey("PreRequirements:Redis"))
        {
            ...
            finally
            {
                if (!isConnected)
                {
                    requirementWarningMessages.Add(
        ...
        {
            requirementWarningMessages.AddFirst(
                "NOTICE: The following tools are required to run your solution:"
            );

            await EventBus.PublishAsync(
                new ProjectPostRequirementsCheckedEvent
                {
    ...
    public string GetUsageInfo()
    {
        ...
        sb.AppendLine(
            "-sib|--skip-installing-libs                      (Doesn't run `abp install-libs` command after project creation)"
        );
        sb.AppendLine(
            "-sb|--skip-bundling                             (Doesn't run `abp bundle` command after Blazor Wasm project creation)"
        );
        sb.AppendLine(
            "-sc|--skip-cache                                (Always download the latest from our server and refresh their templates folder cache)"
        );
        sb.AppendLine("");
