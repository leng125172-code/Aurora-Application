...
public class SuiteCommand : IConsoleCommand, ITransientDependency
{
    ...
    public async Task ExecuteAsync(CommandLineArgs commandLineArgs)
    {
        ...
        var preview =
            commandLineArgs.Options.ContainsKey(Options.Preview.Short)
        ...
        switch (operationType)
        {
            ...
            case null:
                ...
                _abpSuitePort = await _suiteAppSettingsService.GetSuitePortAsync(
                    currentSuiteVersionAsString
                );
    ...
    {
        var entityFile = args.Options.GetOrNull(
            Options.Crud.Entity.Short,
            Options.Crud.Entity.Long
        );
        var solutionFile = args.Options.GetOrNull(
            Options.Crud.Solution.Short,
            Options.Crud.Solution.Long
        );

        if (
            entityFile.IsNullOrEmpty()
            || !entityFile.EndsWith(".json")
            || !File.Exists(entityFile)
            || solutionFile.IsNullOrEmpty()
            || !(solutionFile.EndsWith(".sln") || solutionFile.EndsWith(".slnx"))
        )
        {
        ...
        var IsSolutionBuilt = Convert.ToBoolean(
            await IsSolutionBuiltResponse.Content.ReadAsStringAsync()
        );
        ...
        if (!response.IsNullOrWhiteSpace())
        {
            ...
            if (response.Contains("Commercial.SuiteTemplates.dll"))
            {
                Logger.LogInformation(
                    "The solution should be built before generating an entity! Use `dotnet build` to build your solution."
                );
            }
        }
        else
    ...
    {
        var latestPreviewVersionInfo =
            await _packageVersionCheckerService.GetLatestVersionOrNullAsync(
                packageId: SuitePackageName,
                includeReleaseCandidates: true
            );

        return latestPreviewVersionInfo.Version.IsPrerelease
            ? latestPreviewVersionInfo.Version.ToString()
            : null;
    }
    ...
    private object GetTargetSolutionOrNull(CommandLineArgs commandLineArgs)
    {
        return commandLineArgs.Options.GetOrNull(
            ...
            ?? Directory
                .GetFiles(Directory.GetCurrentDirectory(), "*.sln", SearchOption.TopDirectoryOnly)
                .Concat(
                    Directory.GetFiles(
