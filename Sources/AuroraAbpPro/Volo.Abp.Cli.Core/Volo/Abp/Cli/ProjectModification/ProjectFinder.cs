...
{
    [CanBeNull]
    public static string FindNuGetTargetProjectFile(
        string[] projectFiles,
    ...
    )
    {
        ...
        switch (target)
        {
            ...
            case NuGetPackageTarget.BlazorWebAssembly:
                var BlazorWebAssemblyTargetProject = FindProjectEndsWith(
                    projectFiles,
                    assemblyNames,
                    ".Blazor.Client"
                );
                return
                    BlazorWebAssemblyTargetProject != null
                    && !BlazorProjectTypeChecker.IsBlazorServerProject(
                        BlazorWebAssemblyTargetProject
                    )
                    ? BlazorWebAssemblyTargetProject
                    : null;
            case NuGetPackageTarget.BlazorServer:
                var BlazorServerTargetProject = FindProjectEndsWith(
                    projectFiles,
                    assemblyNames,
                    ".Blazor"
                );
                return
                    BlazorServerTargetProject != null
    ...
    )
    {
        for (var i = 0; i < assemblyNames.Length; i++)
        {
            ...
            if (
                assemblyName.EndsWith(postfix, StringComparison.OrdinalIgnoreCase)
