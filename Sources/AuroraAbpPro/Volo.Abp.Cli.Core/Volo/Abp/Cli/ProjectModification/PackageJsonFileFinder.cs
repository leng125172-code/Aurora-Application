...
public class PackageJsonFileFinder : ITransientDependency
{
    public List<string> Find(string rootDirectory)
    {
        ...
        return Directory
            .GetFiles(
                rootDirectory.EnsureEndsWith(slash),
                "*package.json",
    ...
    protected virtual bool IsWithProjectFile(string path)
    {
        ...
        return Directory
                .GetFiles(directory, "*.csproj", searchOption: SearchOption.TopDirectoryOnly)
                .Any() || File.Exists(Path.Combine(directory, "angular.json"));
    }
}
