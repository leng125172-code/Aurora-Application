...
public class LocalReferenceConverter : ITransientDependency
{
    ...
    public async Task ConvertAsync([NotNull] string directory, [NotNull] List<string> localPaths)
    {
    ...
    private async Task ConvertProjectToLocalReferences(
        string targetProject,
        List<string> localProjects
    )
    {
        var xmlDocument = new XmlDocument() { PreserveWhitespace = true };
        xmlDocument.Load(GenerateStreamFromString(File.ReadAllText(targetProject)));

        var matchedNodes = xmlDocument.SelectNodes(
        ...
        foreach (XmlNode matchedNode in matchedNodes)
        {
            ...
            var localProject = localProjects.Find(x =>
                x.EndsWith($"\\{packageName}.csproj") || x.EndsWith($"/{packageName}.csproj")
            );
