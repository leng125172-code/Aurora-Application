...
public class ProjectNpmPackageAdder : ITransientDependency
{
    ...
    public async Task AddNpmPackageAsync(
        string directory,
    ...
    )
    {
        ...
        Logger.LogInformation(
            $"Installing '{npmPackage.Name}' package to the project '{packageJsonFilePath}'..."
        );
    ...
    protected virtual async Task<bool> DownloadAngularSourceCode(
    ...
    public async Task AddMvcPackageAsync(
        string directory,
        NpmPackageInfo npmPackage,
        string version = null,
    ...
    )
    {
        var packageJsonFilePath = Path.Combine(directory, "package.json");
        if (
            !File.Exists(packageJsonFilePath)
    ...
    public async Task RemoveMvcPackageAsync(
        string directory,
        NpmPackageInfo npmPackage,
        bool skipInstallingLibs = false
    )
    {
        ...
        Logger.LogInformation(
            $"Removing '{npmPackage.Name}' package from the project '{packageJsonFilePath}'..."
        );
    ...
    private async Task<NpmPackageInfo> FindNpmPackageInfoAsync(string packageName)
    {
        ...
        using (
            var response = await client.GetAsync(url, CliHttpClientFactory.GetCancellationToken())
        )
        {
