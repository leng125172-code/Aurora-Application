...
public class NpmPackagesUpdater : ITransientDependency
{
    ...
    )
    {
        ...
        {
            var updated = await UpdatePackagesInFile(
                file,
                includePreviews,
        ...
        {
            var fileDirectory = Path.GetDirectoryName(file.Key)
    ...
    )
    {
        ...
        {
            var updated = await TryUpdatingPackage(
                filePath,
                abpPackage,
                includePreviews,
                includeReleaseCandidates,
    ...
    )
    {
        ...
        if (!specifiedVersion.IsNullOrWhiteSpace())
        {
            if (IsLeptonXPackage(package) && !specifiedLeptonXVersion.IsNullOrWhiteSpace())
            {
                ...
                if (
                    SemanticVersion.Parse(specifiedLeptonXVersion)
            ...
            else
            {
                ...
                if (
                    SemanticVersion.Parse(specifiedVersion)
                    <= SemanticVersion.Parse(currentVersion.RemovePreFix("~", "^"))
                )
                {
                    return false;
        ...
        else
        {
            if (includePreRc && !includeReleaseCandidates)
            {
                version = await GetLatestVersion(
                    package,
                    includePreRc: true,
                    workingDirectory: filePath.RemovePostFix("package.json")
                );
            }
            else if (
                (
                    ...
                    || (
                        !switchToStable
                        && (currentVersion != null && currentVersion.Contains("-preview"))
                    )
                ) && !includeReleaseCandidates
            )
            {
    ...
    )
    {
        ...
        if (includePreRc)
        {
            ...
            newVersion = versionList
                .Where(f => f.Contains(filterKey))
                .OrderBy(o => o)
                .FirstOrDefault();
        }
    ...
    protected virtual List<JProperty> GetAbpPackagesFromPackageJson(JObject fileObject)
    {
        ...
        foreach (var dependencyListName in dependencyList)
        {
            ...
            abpPackages.AddRange(
                properties
                    .Where(p =>
                        p.Name.StartsWith("@abp/")
    ...
    protected virtual List<string> GetPackageVersionList(
        JProperty package,
        string workingDirectory = null
    )
    {
        ...
        return JsonConvert
            .DeserializeObject<string[]>(versionListAsJson)
            .OrderByDescending(SemanticVersion.Parse, new VersionComparer())
            .ToList();
    }
