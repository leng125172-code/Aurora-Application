...
public class PackageSourceManager : ITransientDependency
{
    ...
    public void Add(
        ...
        string sourceKey,
        string sourceValue,
        params string[] packageMappingPatterns
    )
    {
    ...
    protected virtual void ConfigurePackageSourceMappings(
        XmlDocument doc,
        string sourceKey,
        string[] packageMappingPatterns
    )
    {
        ...
        if (packageMappingNodes == null)
        {
            ...
            Logger.LogWarning(
                $"<packageSourceMapping> node not found in 'NuGet.Config' file. Skipping adding patterns for '{sourceKey}' source."
            );
            return;
        }
