...
public class EfCoreMigrationManager : ITransientDependency
{
    ...
    public void AddMigration(string dbMigrationsCsprojFile, string module)
    {
        ...
        var dbContextName =
            tenantDbContextName != null ? FindDbContextName(dbMigrationsProjectFolder) : null;

        if (!string.IsNullOrEmpty(tenantDbContextName))
        {
            RunAddMigrationCommand(
    ...
    )
    {
        CmdHelper.RunCmd(
            ...
            exitCode: out var buildExitCode
        );
        if (buildExitCode != 0)
        {
            Logger.LogWarning(
                "Dotnet build failed for project folder {ProjectFolder}. Skipping EF Core migration command.",
                dbMigrationsProjectFolder
            );
            return;
        }
