...
public class NugetPackageToLocalReferenceConverter : ITransientDependency
{
    ...
    private void ConvertToLocalReference(
        string folder,
        List<NugetPackageInfoWithModuleName> nugetPackageList,
    ...
    )
    {
        ...
        foreach (var projectFile in projectFiles)
        {
            ...
            using (var stream = StreamHelper.GenerateStreamFromString(content))
            {
                ...
                doc.Load(stream);
                var convertedProject = ProcessReferenceNodes(
                    folder,
                    doc,
                    nugetPackageList,
                    localPathPrefix,
    ...
    private string ProcessReferenceNodes(
        string folder,
        XmlDocument doc,
        List<NugetPackageInfoWithModuleName> nugetPackageList,
        string localPathPrefix,
    ...
    )
    {
        var nodes = doc.SelectNodes(
            $"/Project/ItemGroup/PackageReference[starts-with(@Include, '{modulePrefix}')]"
        );
        ...
        foreach (XmlNode oldNode in nodes)
        {
            ...
            var moduleName = nugetPackageList
                .FirstOrDefault(n => n.NugetPackage.Name == oldNodeIncludeValue)
                ?.ModuleName;

            if (moduleName == null)
            {
                var localProject = GetProjectFilesUnder(folder)
                    .FirstOrDefault(f => f.EndsWith($"{oldNodeIncludeValue}.csproj"));

                if (localProject != null)
                {
                    moduleName = Directory
                        .GetParent(
                            Directory.GetParent(Path.GetDirectoryName(localProject)).FullName
                        )
                        .Name;

                    if (
                        oldNodeIncludeValue.EndsWith(
                            ".test",
    ...
    public List<NugetPackageInfoWithModuleName> GetNugetPackages(ModuleWithMastersInfo module)
    {
        ...
        list.AddRange(
            module.NugetPackages.Select(n => new NugetPackageInfoWithModuleName
            {
