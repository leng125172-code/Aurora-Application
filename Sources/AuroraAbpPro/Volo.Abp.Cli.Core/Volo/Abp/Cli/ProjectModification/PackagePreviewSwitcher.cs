...
public class PackagePreviewSwitcher : ITransientDependency
{
    ...
    private async Task SwitchProjectsToPreview(List<string> projects)
    {
        foreach (var project in projects)
        {
            ...
            await _npmPackagesUpdater.Update(folder, false, true);
        }
    }
    ...
    private async Task SwitchSolutionsToPreview(List<string> solutionPaths)
    {
        foreach (var solutionPath in solutionPaths)
        {
            ...
            await _npmPackagesUpdater.Update(solutionFolder, false, true);

            if (solutionAngularFolder != null)
            {
                await _npmPackagesUpdater.Update(solutionAngularFolder, false, true);
            }
        }
    }
    ...
    private async Task SwitchProjectsToStable(List<string> projects)
    {
        foreach (var project in projects)
        {
            ...
            await _nugetPackagesVersionUpdater.UpdateProjectAsync(project, false, false, true);

            await _npmPackagesUpdater.Update(folder, false, false, true);
        }
    }
    ...
    private async Task SwitchSolutionsToStable(List<string> solutionPaths)
    {
        foreach (var solutionPath in solutionPaths)
        {
            ...
            await _npmPackagesUpdater.Update(solutionFolder, false, false, true);

            if (solutionAngularFolder != null)
            {
                await _npmPackagesUpdater.Update(solutionAngularFolder, false, false, true);
            }
        }
    }
    ...
    private async Task SwitchProjectsToNightlyPreview(List<string> projects)
    {
        foreach (var project in projects)
        {
            ...
            _packageSourceManager.Add(
                FindSolutionFolder(project) ?? folder,
                "ABP Nightly",
                "https://www.myget.org/F/abp-nightly/api/v3/index.json",
                "Volo.*"
            );

            await _nugetPackagesVersionUpdater.UpdateSolutionAsync(project, true);

            await _npmPackagesUpdater.Update(folder, true);
        }
    }
    ...
    private async Task SwitchSolutionsToNightlyPreview(List<string> solutionPaths)
    {
        foreach (var solutionPath in solutionPaths)
        {
            ...
            {
                await _nugetPackagesVersionUpdater.UpdateSolutionAsync(solutionPath, true);
            }

            await _npmPackagesUpdater.Update(solutionFolder, true);

            if (solutionAngularFolder != null)
            {
                await _npmPackagesUpdater.Update(solutionAngularFolder, true);
            }
        }
    }
    ...
    private async Task SwitchNpmPackageVersionsOfProjectsToPreRc(List<string> projects)
    {
        foreach (var project in projects)
        {
            ...
            await _npmPackagesUpdater.Update(folder, includePreRc: true);
        }
    }
    ...
    private async Task SwitchNpmPackageVersionsOfSolutionsToPreRc(List<string> solutionPaths)
    {
        foreach (var solutionPath in solutionPaths)
        {
            ...
            await _npmPackagesUpdater.Update(solutionFolder, includePreRc: true);

            if (solutionAngularFolder != null)
            {
                await _npmPackagesUpdater.Update(solutionAngularFolder, includePreRc: true);
            }
        }
    }
    ...
    {
        return Directory
            .GetFiles(GetDirectory(commandLineArgs), "*.sln", SearchOption.AllDirectories)
            .Concat(
                Directory.GetFiles(
                    GetDirectory(commandLineArgs),
                    "*.slnx",
    ...
    {
        return Directory
            .GetFiles(GetDirectory(commandLineArgs), "*.csproj", SearchOption.AllDirectories)
            .ToList();
    }

    private string GetDirectory(CommandLineArgs commandLineArgs)
    {
        return commandLineArgs.Options.GetOrNull(
                Options.SolutionDirectory.Short,
                Options.SolutionDirectory.Long
            )
    ...
    {
        var upperAngularPath = Path.Combine(
            Directory.GetParent(solutionFolder)?.FullName ?? "",
            "angular"
        );
    ...
    private string FindSolutionFolder(string projectFile)
    {
        ...
        do
        {
            ...
            if (
                Directory
                    .GetFiles(targetFolder, "*.sln", SearchOption.TopDirectoryOnly)
                    .Concat(
                        Directory.GetFiles(targetFolder, "*.slnx", SearchOption.TopDirectoryOnly)
                    )
                    .Any()
            )
            {
