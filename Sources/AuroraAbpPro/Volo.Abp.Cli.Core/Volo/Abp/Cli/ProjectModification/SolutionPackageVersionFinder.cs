...
public class SolutionPackageVersionFinder : ITransientDependency
{
    ...
    public string FindByCsprojVersion(
        string solutionFile,
    ...
    )
    {
        ...
        foreach (var projectFile in projectFilesUnderSrc)
        {
            ...
            if (
                TryParseVersionFromCsprojViaXmlDocument(
                    content,
                    out var s,
                    packagePrefix,
                    excludedKeywords,
    ...
    )
    {
        ...
        using (var stream = StreamHelper.GenerateStreamFromString(content))
        {
            ...
            var nodes = doc.SelectNodes(
                $"/Project/ItemGroup/PackageReference[starts-with(@Include, '{packagePrefix}')]"
            );
            ...
            foreach (XmlNode node in nodes!)
            {
                ...
                if (
                    !excludedKeywords.IsNullOrWhiteSpace()
                    && excludedKeywords.Split(',').Any(ek => packageId!.Contains(ek))
                )
                {
                    continue;
                }
    ...
    public static bool TryParseVersionFromCsprojFile(
        string csprojContent,
        out string version,
        string packagePrefix = "Volo.Abp",
        string excludedKeywords = "LeptonX",
        string includedKeywords = null
    )
    {
        return TryParseVersionFromCsprojViaXmlDocument(
            csprojContent,
            out version,
            packagePrefix,
            excludedKeywords,
    ...
    public static bool TryParseSemanticVersionFromCsprojFile(
        string csprojContent,
