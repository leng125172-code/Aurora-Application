...
public class ChangeThemeStep : ProjectBuildPipelineStep
{
    ...
    {
        var defaultThemeName = context.BuildArgs.TemplateName
            is AppTemplate.TemplateName
                or AppNoLayersTemplate.TemplateName
            ? LeptonXLite
            : LeptonX;
        ...
        ChangeThemeForAngularProjects(
            ...
            defaultThemeName,
            Basic,
            GetAngularPackageName(context.BuildArgs.Theme!.Value),
            GetAngularPackageName(Theme.Basic)
        );
    }
    ...
    protected virtual void ChangeToLeptonTheme(ProjectBuildContext context)
    {
        ...
        ChangeThemeForAngularProjects(
            context,
            oldThemeName: LeptonX,
            Lepton,
            GetAngularPackageName(Theme.LeptonX),
            GetAngularPackageName(Theme.Lepton)
        );

        ConfigureLeptonManagementPackagesForNoLayersMvc(
            context,
            "/MyCompanyName.MyProjectName.Mvc/MyCompanyName.MyProjectName.csproj",
            new[] { "Web", "HttpApi", "Application" }
        );
    ...
    private static void ChangeThemeToBasicForBlazorProjects(
    ...
    )
    {
        ...
        ReplaceAllKeywords(context, "MyProjectNameBlazorModule.cs", defaultThemeName, Basic);

        ReplaceAllKeywords(context, "MyProjectNameBlazorClientModule.cs", defaultThemeName, Basic);

        ReplaceAllKeywords(context, "Routes.razor", defaultThemeName, Basic);

        ReplaceAllKeywords(context, "App.razor", defaultThemeName, Basic);
        ...
        ReplaceAllKeywords(context, "MyProjectNameHostModule.cs", defaultThemeName, Basic);
    }

    private static void ChangeThemeForAngularProjects(
        ProjectBuildContext context,
        string oldThemeName,
        string newThemeName,
    ...
    )
    {
        ...
        RemoveLinesByStatement(context, "/angular/src/app/app.module.ts", "SideMenuLayoutModule");
        ...
        ReplaceAllKeywords(context, "/angular/src/app/app.module.ts", "HttpErrorComponent, ", "");
    ...
    private static void ConfigureLeptonManagementPackagesForNoLayersMvc(
        ProjectBuildContext context,
        string targetProjectPath,
        IEnumerable<string> projectNames
    )
    {
        var file = context.Files.FirstOrDefault(f =>
            !f.Name.Contains("Test")
            && f.Name.Contains(targetProjectPath)
            && f.Name.Contains(".csproj")
        );
        if (file == null)
        {
            return;
        ...
        foreach (var projectName in projectNames)
        {
            ...
            AddProjectReference(
                file,
                $@"..\..\..\..\lepton-theme\src\Volo.Abp.LeptonTheme.Management.{projectName}\Volo.Abp.LeptonTheme.Management.{projectName}.csproj"
            );
            AddModuleDependency(
                moduleFile,
                projectName,
                $"LeptonThemeManagement{ConvertProjectNameToModuleName($"{projectName}")}Module"
            );
        }
    }
    ...
    private static void ChangeThemeToLeptonForMvcProjects(ProjectBuildContext context)
    {
        ...
        foreach (var projectName in projectNames)
        {
            ...
            ReplaceAllKeywords(context, moduleFile.Name, LeptonX, Lepton);

            RemoveLinesByStatement(context, moduleFile.Name, "Volo.Abp.LeptonX.Shared;");
    ...
    private static void AddLeptonThemeManagementReferenceToProjects(ProjectBuildContext context)
    {
        ...
        var microserviceServiceProjects = new Dictionary<string, string>
        {
            ...
            {
                "Domain.Shared",
                "MyCompanyName.MyProjectName.AdministrationService.Domain.Shared.csproj"
            },
            {
                "Application",
                "MyCompanyName.MyProjectName.AdministrationService.Application.csproj"
            },
            {
                "Application.Contracts",
                "MyCompanyName.MyProjectName.AdministrationService.Application.Contracts.csproj"
            },
            { "HttpApi", "MyCompanyName.MyProjectName.AdministrationService.HttpApi.csproj" },
            {
                "HttpApi.Client",
                "MyCompanyName.MyProjectName.AdministrationService.HttpApi.Client.csproj"
            },
    ...
    private static void AddUiProjectToProjects(
    ...
    private static void AddLeptonThemeManagementReference(
        ProjectBuildContext context,
        KeyValuePair<string, string> projectInfo
    )
    {
        var reference =
            $@"..\..\..\..\..\lepton-theme\src\Volo.Abp.LeptonTheme.Management.{projectInfo.Key}\Volo.Abp.LeptonTheme.Management.{projectInfo.Key}.csproj";
        var projectFile = context.Files.FirstOrDefault(f =>
            !f.Name.Contains("Test")
            && f.Name.Contains(projectInfo.Value)
            && f.Name.Contains(".csproj")
        );
    ...
    private static void AddModuleDependency(
        FileEntry moduleFile,
        string projectName,
        string dependency,
        bool underManagementFolder = true
    )
    {
            ...
            if (lines[i].Contains("namespace MyCompanyName.MyProjectName"))
            {
                lines[i - 1] =
                    ...
                    + (
                        ...
                        + (underManagementFolder ? ".Management." : ".")
                        + (projectNames.Any(p => p == projectName) ? projectName : "")
                    )
                        .TrimEnd('.')
                        .EnsureEndsWith(';')
                    + Environment.NewLine;
    ...
    private static void ReplacePackageReferenceWithProjectReference(
        ProjectBuildContext context,
    ...
    )
    {
        ...
        var lineIndex = lines.FindIndex(line =>
            line.Contains("PackageReference") && line.Contains(packageReference)
        );
        if (lineIndex == -1)
        {
            return;
    ...
    private static void ReplaceImportPackage(
        ProjectBuildContext context,
        string filePath,
    ...
    private static void RemoveLinesByStatement(
        ProjectBuildContext context,
        string filePath,
        string statement
    )
    ...
    private static void ChangeModuleImportBetweenStatements(
        ProjectBuildContext context,
        string filePath,
        ...
        using (var stream = StreamHelper.GenerateStreamFromString(file.Content))
        {
            ...
            else
            {
                for (var i = 0; i < itemGroupNodes.Count; i++)
                {
                    ...
                    {
                        if (
                            itemGroupNodes[i].ChildNodes[j].Name != "ProjectReference"
                            || itemGroupNodes[i].ChildNodes[j].NodeType != XmlNodeType.Element
                        )
                        {
                            continue;
                        }
    ...
    private static FileEntry ConvertProjectFileToModuleFile(
        ProjectBuildContext context,
        FileEntry projectFile
    )
    {
        var splittedProjectFileName = projectFile.Name.RemovePostFix("/").Split("/");

        splittedProjectFileName = splittedProjectFileName
            .Take(splittedProjectFileName.Length - 1)
            .ToArray();
        ...
        return context.Files.FirstOrDefault(f =>
            f.Name.Contains(splittedProjectFileName.Last() + "/" + fileName)
    ...
    {
        foreach (
            var file in context.Files.Where(x => x.Name.Contains(oldFolderName) && x.IsDirectory)
        )
        {
            new MoveFolderStep(file.Name, file.Name.Replace(oldFolderName, newFolderName)).Execute(
                context
            );
        }
    }

    private static void ReplaceAllKeywords(
        ProjectBuildContext context,
    ...
    private static void ChangeThemeToBasicForMvcProjects(
        ProjectBuildContext context,
        string defaultThemeName
    )
    {
        ...
        foreach (var projectName in projectNames)
        {
            ...
            ReplaceAllKeywords(context, moduleFile.Name, defaultThemeName, Basic);
    ...
    private static void ChangeThemeToBasicForBlazorServerProjects(
        ProjectBuildContext context,
        string defaultThemeName
    )
    {
        ...
        foreach (var project in projects)
        {
            ...
            ReplaceAllKeywords(context, $"_Host.cshtml", defaultThemeName, Basic);
    ...
    private static void ChangeThemeToBasicForBlazorWebAppProjects(
        ProjectBuildContext context,
        string defaultThemeName
    )
    {
        ...
        foreach (var project in projects)
        {
            ...
            ReplaceAllKeywords(context, $"{project.Value}.cs", defaultThemeName, Basic + "Theme");
    ...
    private static void ChangeThemeToLeptonForBlazorServerProjects(ProjectBuildContext context)
    {
        ...
        foreach (var projectName in projectNames)
        {
            ...
            ReplaceAllKeywords(context, $"/Pages/_Host.cshtml", LeptonX, Lepton);
    ...
    private static void ChangeThemeToLeptonForBlazorWebAppProjects(ProjectBuildContext context)
    {
        ...
        foreach (var projectName in projectNames)
        {
            ...
            ReplaceAllKeywords(context, $"MyProjectNameBlazorModule.cs", LeptonX, Lepton);

            ReplaceAllKeywords(context, $"MyProjectNameBlazorClientModule.cs", LeptonX, Lepton);

            ReplaceAllKeywords(context, "Routes.razor", LeptonX, Lepton);

            ReplaceAllKeywords(context, "App.razor", LeptonX, Lepton);
    ...
    private static void ChangeThemeToLeptonForNoLayersBlazorServerProjects(
        ProjectBuildContext context
    )
    {
