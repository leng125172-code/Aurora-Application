...
public class RemoveProjectFromSolutionStep : ProjectBuildPipelineStep
{
    ...
    )
    {
        ...
        if (
            solutionFilePathWithoutFileExtension != null
            && solutionFilePathWithoutFileExtension.EndsWith(".sln")
        )
        {
            _solutionFilePathWithoutFileExtension =
                solutionFilePathWithoutFileExtension.RemovePostFix(".sln");
        }
        else if (
            solutionFilePathWithoutFileExtension != null
            && solutionFilePathWithoutFileExtension.EndsWith(".slnx")
        )
        {
            _solutionFilePathWithoutFileExtension =
                solutionFilePathWithoutFileExtension.RemovePostFix(".slnx");
        }
        else
        {
    ...
    public override void Execute(ProjectBuildContext context)
    {
        ...
        var solutionFile =
            context.FindFile(_solutionFilePathWithoutFileExtension + ".sln")
    ...
    private string FindProjectKey(List<string> solutionFileLines)
    {
        foreach (var solutionFileLine in solutionFileLines)
        {
            ...
            {
                var curlyBracketStartIndex = solutionFileLine.LastIndexOf(
                    "{",
                    StringComparison.OrdinalIgnoreCase
                );
                var curlyBracketEndIndex = solutionFileLine.LastIndexOf(
                    "}",
                    StringComparison.OrdinalIgnoreCase
                );
                return solutionFileLine.Substring(
                    curlyBracketStartIndex + 1,
    ...
    private void SetSolutionAndProjectPathsIfNull(ProjectBuildContext context)
    {
        if (_solutionFilePathWithoutFileExtension == null)
        {
            _solutionFilePathWithoutFileExtension =
                context
                    .FindFile("/aspnet-core/MyCompanyName.MyProjectName.sln")
                    ?.Name.RemovePostFix(".sln")
                ?? context
                    .FindFile("/aspnet-core/MyCompanyName.MyProjectName.slnx")
                    ?.Name.RemovePostFix(".slnx")
                ?? context.FindFile("/MyCompanyName.MyProjectName.sln")?.Name.RemovePostFix(".sln")
                ?? context
                    .FindFile("/MyCompanyName.MyProjectName.slnx")
                    ?.Name.RemovePostFix(".slnx")
                ?? context
                    .FindFile("/MyCompanyName.MyProjectName.MicroserviceName.sln")
                    ?.Name.RemovePostFix(".sln")
                ?? context
                    .FindFile("/MyCompanyName.MyProjectName.MicroserviceName.slnx")
                    ?.Name.RemovePostFix(".slnx");
        }
        if (_projectFolderPath == null)
        {
            _projectFolderPath =
                context.FindFile("/aspnet-core/src/" + _projectName.EnsureEndsWith('/'))?.Name
