...
public class CreateAppSettingsSecretsStep : ProjectBuildPipelineStep
{
    ...
    public override void Execute(ProjectBuildContext context)
    {
        ...
        foreach (var appSettingsFile in appSettingsFiles)
        {
            context.Files.Add(
                new FileEntry(
                    appSettingsFile.Name.Replace(
                        CliConsts.AppSettingsJsonFileName,
                        CliConsts.AppSettingsSecretJsonFileName
                    ),
        ...
        var projectFiles = context
            .Files.Where(x =>
                x.Name.EndsWith(".csproj") && x.Content.Contains(AppSettingsPlaceholder)
            )
            .ToList();
    ...
    private static string ReplaceAppSettingsSecretsPlaceholder(string content)
    {
        ...
        var appSettingsRemoveLine = content
            .SplitToLines()
            .FirstOrDefault(l =>
                l.Contains("None")
                && l.Contains("Remove")
        ...
        {
            var prefix = appSettingsRemoveLine
                .Split("\"")
                .Select(s => s.Trim())
        ...
        var replaceContent =
            $"<None Remove=\"{path + CliConsts.AppSettingsSecretJsonFileName}\" />{Environment.NewLine}"
