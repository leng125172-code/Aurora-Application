...
public class ProjectReferenceReplaceStep : ProjectBuildPipelineStep
{
    ...
    private bool IsBranchName(string versionOrBranchName)
    {
        ...
        if (
            versionOrBranchName[0].IsIn('v', 'V')
    ...
    private abstract class ProjectReferenceReplacer
    {
        ...
        protected ProjectReferenceReplacer(ProjectBuildContext context, string projectName)
        {
            _entries = context.Files;
            _isMicroserviceServiceTemplate =
                MicroserviceServiceTemplateBase.IsMicroserviceServiceTemplate(
                    context.Template?.Name
                );
            _projectName = projectName;
            CentralPackageManagement = context.Files.Any(x =>
                x.Name.EndsWith("Directory.Packages.props")
            );
        }
        ...
        private string ProcessReferenceNodes(XmlDocument doc, string content)
        {
            ...
            foreach (XmlNode oldNode in nodes)
            {
                ...
                if (
                    _entries.Any(e =>
                        e.Name.EndsWith(
                            GetProjectNameWithExtensionFromProjectReference(oldNodeIncludeValue)
                        )
                    )
                )
                {
        ...
        protected abstract XmlElement GetNewReferenceNode(
            XmlDocument doc,
            string oldNodeIncludeValue
        );
        ...
        public class NugetReferenceReplacer : ProjectReferenceReplacer
        {
            ...
            protected override XmlElement GetNewReferenceNode(
                XmlDocument doc,
                string oldNodeIncludeValue
            )
            {
                ...
                var versionAttr = doc.CreateAttribute(
                    CentralPackageManagement ? "VersionOverride" : "Version"
                );
        ...
        public class LocalProjectPathReferenceReplacer : ProjectReferenceReplacer
        {
            ...
            public LocalProjectPathReferenceReplacer(
                ProjectBuildContext context,
                string projectName,
            ...
            private string SetGithubPath(string includeValue)
            {
                ...
                {
                    if (
                        includeValue.StartsWith(
                            "abp\\",
                            StringComparison.InvariantCultureIgnoreCase
                        )
                    )
                    {
                        return _gitHubAbpLocalRepositoryPath.EnsureEndsWith('\\')
                            + includeValue.Substring("abp\\".Length);
                    }

                    return _gitHubVoloLocalRepositoryPath.EnsureEndsWith('\\')
                        + "abp\\"
                        + includeValue;
                }
