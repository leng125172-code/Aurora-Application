...
public class AppModuleDatabaseManagementSystemChangeStep : ProjectBuildPipelineStep
{
    ...
    {
        var dbContextFactoryFiles = context.Files.Where(f =>
            f.Name.EndsWith("DbContextFactory.cs", StringComparison.OrdinalIgnoreCase)
        );
    ...
    {
        var efCoreProjectFiles = context.Files.Where(f =>
            f.Name.EndsWith(".csproj", StringComparison.OrdinalIgnoreCase)
        );
        foreach (var efCoreProjectFile in efCoreProjectFiles)
        {
            efCoreProjectFile?.ReplaceText(
                "Volo.Abp.EntityFrameworkCore.SqlServer",
                newPackageName
            );
        }

        var efCoreModuleClasses = context.Files.Where(f =>
            f.Name.EndsWith("Module.cs", StringComparison.OrdinalIgnoreCase)
        );
