...
public class ConnectionStringChangeStep : ProjectBuildPipelineStep
{
    ...
    {
        var appSettingsJsonFiles = context
            .Files.Where(f =>
        ...
        var newConnectionString =
            $"\"{DefaultConnectionStringKey}\": \"{context.BuildArgs.ConnectionString.Replace(@"\\", @"\").Replace(@"\", @"\\")}\"";

        foreach (var appSettingsJson in appSettingsJsonFiles)
        {
            try
            {
                var appSettingJsonContentWithoutBom = StringHelper.ConvertFromBytesWithoutBom(
                    appSettingsJson.Bytes
                );
            ...
            {
                Console.WriteLine(
                    "Cannot change the connection string in "
                        + appSettingsJson.Name
                        + ". Error: "
                        + ex.Message
                );
            }
