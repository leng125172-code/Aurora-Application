...
public class DatabaseManagementSystemChangeStep : ProjectBuildPipelineStep
{
    ...
    public override void Execute(ProjectBuildContext context)
    {
        switch (context.BuildArgs.DatabaseManagementSystem)
        {
            case DatabaseManagementSystem.MySQL:
                ChangeEntityFrameworkCoreDependency(
                    context,
    ...
    {
        var dbContextFactoryFile =
            context.Files.FirstOrDefault(f =>
                f.Name.EndsWith(
                    $"{(_hasDbMigrations ? "Migrations" : string.Empty)}DbContextFactoryBase.cs",
                    StringComparison.OrdinalIgnoreCase
                )
            )
            ?? context.Files.FirstOrDefault(f =>
                f.Name.EndsWith(
                    $"{(_hasDbMigrations ? "Migrations" : string.Empty)}DbContextFactory.cs",
                    StringComparison.OrdinalIgnoreCase
                )
            );

        dbContextFactoryFile?.ReplaceText(
    ...
    private void ChangeEntityFrameworkCoreDependency(
        ...
        string newModuleNamespace,
        string newModuleClass
    )
    {
        var efCoreProjectFile = context.Files.FirstOrDefault(f =>
            f.Name.EndsWith("EntityFrameworkCore.csproj", StringComparison.OrdinalIgnoreCase)
        );
        efCoreProjectFile?.ReplaceText("Volo.Abp.EntityFrameworkCore.SqlServer", newPackageName);

        var efCoreModuleClass = context.Files.FirstOrDefault(f =>
            f.Name.EndsWith("EntityFrameworkCoreModule.cs", StringComparison.OrdinalIgnoreCase)
        );
        efCoreModuleClass?.ReplaceText(
    ...
    private void ChangeUseSqlServer(
        ProjectBuildContext context,
        string newUseMethodForEfModule,
        string newUseMethodForDbContext = null
    )
    {
