...
public static class FileEntryExtensions
{
    ...
    {
        if (
            !file.Content.Contains("<TEMPLATE-REMOVE")
            && !file.Content.Contains("</TEMPLATE-REMOVE>")
        )
        {
            return;
    ...
    private static void RemoveMarkedTemplateCode(
        this FileEntry file,
        List<string> symbols,
    ...
    )
    {
        ...
        for (int i = 0; i < lines.Length; i++)
        {
            if (lines[i].Contains("<TEMPLATE-REMOVE"))
            {
                ...
                {
                    sectionShouldBeRemoved = parsedMarker.Symbols.Any(s =>
                        !parsedMarker.IsNegativeCondition
                ...
                {
                    sectionShouldBeRemoved = !parsedMarker.Symbols.All(s =>
                        !parsedMarker.IsNegativeCondition
                            ? symbols.Contains(s, StringComparer.InvariantCultureIgnoreCase)
                            : !symbols.Contains(s, StringComparer.InvariantCultureIgnoreCase)
                                == false
