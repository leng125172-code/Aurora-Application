...
public class RemoveUnnecessaryPortsStep : ProjectBuildPipelineStep
{
    ...
    {
        var httpApiHostAppSettings = context.Files.FirstOrDefault(f =>
            f.Name.EndsWith(".HttpApi.Host/appsettings.json")
        );
        ...
        foreach (var line in lines)
        {
            ...
            {
                var jsonValue = line.Contains("CorsOrigins")
                    ? appJson["CorsOrigins"]?.ToString()
                    : appJson["RedirectAllowedUrls"]?.ToString();
                if (!jsonValue.IsNullOrWhiteSpace())
                {
                    newLine = line.Replace(
                        ...
                        string.Join(
                            ",",
                            jsonValue.Split(',').Where(x => !portsToRemove.Contains(x))
                        )
                    );
                }
            }
