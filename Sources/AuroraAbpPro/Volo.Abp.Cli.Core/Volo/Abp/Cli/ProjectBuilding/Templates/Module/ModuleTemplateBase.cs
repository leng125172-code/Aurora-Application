...
public abstract class ModuleTemplateBase : TemplateInfo
{
    ...
    private void DeleteUnrelatedProjects(
        ProjectBuildContext context,
    ...
    )
    {
        ...
        steps.Add(
            new RemoveProjectFromSolutionStep("MyCompanyName.MyProjectName.Blazor.WebAssembly")
        );
    ...
    )
    {
        steps.Add(
            new RemoveFolderStep(
                "/aspnet-core/host/MyCompanyName.MyProjectName.AuthServer/Migrations"
            )
        );
        steps.Add(
            new RemoveFolderStep(
                "/aspnet-core/host/MyCompanyName.MyProjectName.Blazor.Server.Host/Migrations"
            )
        );
        steps.Add(
            new RemoveFolderStep(
                "/aspnet-core/host/MyCompanyName.MyProjectName.Web.Unified/Migrations"
            )
        );
        if (context.BuildArgs.TemplateName == ModuleProTemplate.TemplateName)
        {
            steps.Add(
                new RemoveFolderStep(
                    "/aspnet-core/host/MyCompanyName.MyProjectName.HttpApi.Host/Migrations"
                )
            );
        }
    }
    ...
    private void CleanupFolderHierarchy(
        ProjectBuildContext context,
        List<ProjectBuildPipelineStep> steps
    )
    {
