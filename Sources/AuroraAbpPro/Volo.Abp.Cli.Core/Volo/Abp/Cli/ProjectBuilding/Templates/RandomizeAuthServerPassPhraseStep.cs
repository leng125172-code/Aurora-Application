...
public class RandomizeAuthServerPassPhraseStep : ProjectBuildPipelineStep
{
    ...
    private const string KestrelCertificatesDefaultPassword =
    ...
    private const string ProductionEncryptionAndSigningCertificate =
        "AddProductionEncryptionAndSigningCertificate(\"openiddict.pfx\", \"00000000-0000-0000-0000-000000000000\");";
    private const string ReadmeCallout = "`00000000-0000-0000-0000-000000000000`";

    private static readonly string RandomPassword = Guid.NewGuid().ToString("D");
    public static readonly string RandomOpenIddictPassword = Guid.NewGuid().ToString("D");

    public override void Execute(ProjectBuildContext context)
    {
        var files = context
            .Files.Where(x => !x.IsDirectory)
            .Where(x =>
                x.Name.EndsWith(".cs")
            ...
            .Where(x =>
                x.Content.IndexOf(DefaultPassword, StringComparison.InvariantCultureIgnoreCase) >= 0
            )
            .ToList();
        ...
        foreach (var file in files)
        {
            ...
            for (var i = 0; i < lines.Length; i++)
            {
                if (lines[i].Contains(KestrelCertificatesDefaultPassword))
                {
                    lines[i] = lines[i]
                        .Replace(
                            KestrelCertificatesDefaultPassword,
                            KestrelCertificatesDefaultPassword.Replace(
                                DefaultPassword,
                                RandomPassword
                            )
                        );
                ...
                if (lines[i].Contains(LocalhostPfx))
                {
                    lines[i] = lines[i]
                        .Replace(
                            LocalhostPfx,
                            LocalhostPfx.Replace(DefaultPassword, RandomPassword)
                        );
                }
                ...
                if (lines[i].Contains(DotnetDevCerts))
                {
                    lines[i] = lines[i]
                        .Replace(
                            DotnetDevCerts,
                            DotnetDevCerts.Replace(DefaultPassword, RandomOpenIddictPassword)
                        );
                }
                ...
                if (lines[i].Contains(ReadmeCallout))
                {
                    lines[i] = lines[i]
                        .Replace(
                            ReadmeCallout,
                            ReadmeCallout.Replace(DefaultPassword, RandomOpenIddictPassword)
                        );
                }
                ...
                if (lines[i].Contains(ProductionEncryptionAndSigningCertificate))
                {
                    lines[i] = lines[i]
                        .Replace(
                            ...
                            ProductionEncryptionAndSigningCertificate.Replace(
                                DefaultPassword,
