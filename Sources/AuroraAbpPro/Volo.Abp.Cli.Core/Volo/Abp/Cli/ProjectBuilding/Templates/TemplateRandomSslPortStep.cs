...
public class TemplateRandomSslPortStep : ProjectBuildPipelineStep
{
    ...
    public override void Execute(ProjectBuildContext context)
    {
        ...
        var appSettings = context
            .Files.Where(x =>
                !x.IsDirectory
                && x.Name.EndsWith("appSettings.json", StringComparison.InvariantCultureIgnoreCase)
            )
            .ToList();

        var angularEnvironments = context
            .Files.Where(x =>
                !x.IsDirectory
                && (
                    x.Name.EndsWith(
        ...
        var reactNativeEnvironments = context
            .Files.Where(x =>
                !x.IsDirectory
                && x.Name.EndsWith(
                    $"{MobileApp.ReactNative.GetFolderName()}/Environment.js",
                    StringComparison.InvariantCultureIgnoreCase
                )
            )
            .ToList();
        ...
        if (AppTemplateBase.IsAppTemplate(context.Template.Name))
        {
            // no tiered
            if (
                launchSettings.Count == 1
                && launchSettings
                    .First()
                    .Name.Equals(
                        "/aspnet-core/src/MyCompanyName.MyProjectName.Web/Properties/launchSettings.json",
                        StringComparison.InvariantCultureIgnoreCase
                    )
            )
            {
                ...
                dbMigrator?.SetContent(
                    dbMigrator.Content.Replace("https://localhost:44302", "https://localhost:44303")
                );
                ...
                web?.SetContent(
                    web.Content.Replace("https://localhost:44301", "https://localhost:44303")
                );

                var consoleTestApp = appSettings.FirstOrDefault(x =>
                    x.Name.Equals(
                        "/aspnet-core/test/MyCompanyName.MyProjectName.HttpApi.Client.ConsoleTestApp/appsettings.json",
                        StringComparison.InvariantCultureIgnoreCase
                    )
                );
                consoleTestApp?.SetContent(
                    consoleTestApp.Content.Replace(
            ...
            foreach (var launchSetting in launchSettings)
            {
                ...
                for (var i = 0; i < launchSettingLines.Length; i++)
                {
                    if (launchSettingLines[i].Contains($"{buildInUrl}"))
                    {
                        launchSettingLines[i] = launchSettingLines[i]
                            .Replace($"{buildInUrl}", $"{buildInUrlWithoutPort}:{newPort}");
                    }
                ...
                foreach (var appSetting in appSettings)
                {
                    ...
                    for (var i = 0; i < appSettingLines.Length; i++)
                    {
                        ...
                        {
                            appSettingLines[i] = appSettingLines[i]
                                .Replace(buildInUrl, $"{buildInUrlWithoutPort}:{newPort}");
                        }
                    }
                ...
                foreach (var environment in angularEnvironments)
                {
                    ...
                    for (var i = 0; i < environmentLines.Length; i++)
                    {
                        ...
                        {
                            environmentLines[i] = environmentLines[i]
                                .Replace(buildInUrl, $"{buildInUrlWithoutPort}:{newPort}");
                        }
                    }
                ...
                foreach (var environment in reactNativeEnvironments)
                {
                    ...
                    var buildInUrlWithoutPortHttp = buildInUrlWithoutPort.Replace(
                        "https://",
                        "http://"
                    );
                    ...
                    for (var i = 0; i < environmentLines.Length; i++)
                    {
                        ...
                        {
                            environmentLines[i] = environmentLines[i]
                                .Replace(buildInUrlHttp, $"{buildInUrlWithoutPortHttp}:{newPort}");
                        }
                    }
                    environment.SetLines(environmentLines);
                }
    ...
    private string GetRandomPort(IReadOnlyCollection<string> excludePort = null)
    {
        var ports = Enumerable
            .Range(_minSslPort, _maxSslPort - _minSslPort + 1)
            .Select(p => p.ToString());

        return RandomHelper.GetRandomOfList(
            excludePort != null ? ports.Except(excludePort).ToList() : ports.ToList()
        );
    }
