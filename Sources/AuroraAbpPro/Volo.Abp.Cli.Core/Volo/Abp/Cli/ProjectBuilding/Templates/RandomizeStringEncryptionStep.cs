...
public class RandomizeStringEncryptionStep : ProjectBuildPipelineStep
{
    ...
    public override void Execute(ProjectBuildContext context)
    {
        var appSettings = context
            ...
            .Where(x =>
                x.Content.IndexOf("StringEncryption", StringComparison.InvariantCultureIgnoreCase)
                >= 0
            )
            .ToList();
        ...
        foreach (var appSetting in appSettings)
        {
            ...
            {
                if (
                    appSettingLines[i].Contains("DefaultPassPhrase")
                    && appSettingLines[i].Contains(DefaultPassPhrase)
                )
                {
                    appSettingLines[i] = appSettingLines[i]
                        .Replace(DefaultPassPhrase, randomPassPhrase);
                }
            }
