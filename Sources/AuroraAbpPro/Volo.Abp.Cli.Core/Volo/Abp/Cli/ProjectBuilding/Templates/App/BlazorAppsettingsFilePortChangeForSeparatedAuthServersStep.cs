...
public class BlazorAppsettingsFilePortChangeForSeparatedAuthServersStep : ProjectBuildPipelineStep
{
    public override void Execute(ProjectBuildContext context)
    {
        var appsettingsFile = context.Files.FirstOrDefault(x =>
            ...
            && (
                x.Name.EndsWith(
                    "aspnet-core/src/MyCompanyName.MyProjectName.Blazor/wwwroot/appsettings.json",
                    StringComparison.InvariantCultureIgnoreCase
                )
                || x.Name.EndsWith(
                    "aspnet-core/src/MyCompanyName.MyProjectName.Blazor.Client/wwwroot/appsettings.json",
                    StringComparison.InvariantCultureIgnoreCase
                )
            )
        );
        ...
        for (var i = 1; i < lines.Length; i++)
        {
            ...
            if (line.Contains("Authority") && line.Contains("localhost"))
            {
                ...
                previousLine.Contains("AbpAccountPublic")
