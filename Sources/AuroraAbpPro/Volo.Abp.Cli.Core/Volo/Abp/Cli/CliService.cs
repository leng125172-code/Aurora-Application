...
public class CliService : ITransientDependency
{
        ...
        try
        {
            ...
            if (ShouldLogNewVersionInfo(latestVersionInfo, currentCliVersion))
            {
                if (
                    ...
                    && !latestVersionInfo.Version.IsPrerelease
                )
                {
                    latestVersionInfo =
                        await PackageVersionCheckerService.GetLatestStableVersionFromGithubAsync();
    ...
    private bool ShouldLogNewVersionInfo(
        LatestVersionInfo latestVersionInfo,
        SemanticVersion currentCliVersion
    )
    {
    ...
    private string GetToolPath(Assembly assembly)
    {
        ...
        return assembly.Location.Substring(
            0,
            assembly.Location.IndexOf(".store", StringComparison.Ordinal)
        );
    }
    ...
    private void LogNewVersionInfo(
    ...
    )
    {
        ...
        switch (updateChannel)
        {
            ...
            case UpdateChannel.Prerelease:
                Logger.LogWarning(
                    $"dotnet tool update {toolPathArg} Volo.Abp.Cli --version {latestVersion}"
                );
                break;
            ...
            case UpdateChannel.Development:
                ...
                Logger.LogWarning(
                    $"dotnet tool install {toolPathArg} Volo.Abp.Cli --add-source https://www.myget.org/F/abp-nightly/api/v3/index.json --version {latestVersion}"
                );
                break;
            default:
