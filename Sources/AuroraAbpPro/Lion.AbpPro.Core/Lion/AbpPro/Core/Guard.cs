...
public static class Guard
{
    ...
    public static void LessThan<T>(T value, string valueName, T target, bool canEqual = false)
    ...
    /// <param name="target">要比较的值。</param>
    /// <param name="canEqual">是否可等于。</param>
    public static void GreaterThan<T>(T value, string valueName, T target, bool canEqual = false)
        where T : IComparable<T>
    {
        var flag = canEqual ? value.CompareTo(target) >= 0 : value.CompareTo(target) > 0;
        if (!flag)
    ...
    public static string DirectoryExists(string directory, string parameterName)
    {
    ...
    /// <param name="valueName">参数名称。</param>
    public static string FileExists(string filename, string valueName)
    {
