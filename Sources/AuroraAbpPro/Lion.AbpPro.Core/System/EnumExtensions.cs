...
public static class EnumExtensions
{
    ...
    private static void Each(this Type enumType, Action<string, string, string, object> action)
    {
        ...
        foreach (var name in arr)
        {
            ...
            {
                var attr =
                    Attribute.GetCustomAttribute(fieldInfo, typeof(DescriptionAttribute), false)
    ...
    private static List<KeyValuePair<string, string>> GetEnumTypeValueList<T>()
    {
        ...
        typeof(T).Each(
            (name, value, description, enumObj) =>
    ...
    public static List<KeyValuePair<string, string>> GetEnumTypeValueNameList<T>()
    {
        ...
        typeof(T).Each(
            (name, value, description, enumObj) =>
                items.Add(new KeyValuePair<string, string>(name, description))
        );
        return items;
    }
        ...
        where TModel : Enum
    {
        ...
        foreach (TModel model in modelArray)
            keyValuePairList.Add(
                new KeyValuePair<string, string>(model.ToString(), model.ToString())
            );
    ...
    /// </summary>
    /// <typeparam name="TModel">指定枚举</typeparam>
    public static List<KeyValuePair<string, TModel>> GetEnumKeyValueList<TModel>()
        where TModel : Enum
    {
        ...
        foreach (KeyValuePair<string, TModel> keyValuePair in enumTypeList)
            keyValuePairList.Add(
                new KeyValuePair<string, TModel>(keyValuePair.Key, keyValuePair.Value)
            );
        return keyValuePairList;
    }
    ...
    public static List<KeyValuePair<string, string>> GetEntityDoubleStringKeyValueList<TModel>()
    {
        ...
        foreach (KeyValuePair<string, string> keyValuePair in enumTypeList)
            keyValuePairList.Add(
                new KeyValuePair<string, string>(keyValuePair.Key, keyValuePair.Value)
            );
        return keyValuePairList;
    }
    ...
    public static List<KeyValuePair<string, int>> GetEntityStringIntKeyValueList<TModel>()
    {
        ...
        foreach (KeyValuePair<string, string> keyValuePair in enumTypeList)
            keyValuePairList.Add(
                new KeyValuePair<string, int>(keyValuePair.Key, Convert.ToInt32(keyValuePair.Value))
            );
        return keyValuePairList;
    }
        ...
        foreach (KeyValuePair<string, string> keyValuePair in enumTypeList)
            keyValuePairList.Add(
                new KeyValuePair<int, int>(
                    Convert.ToInt32(keyValuePair.Key),
                    Convert.ToInt32(keyValuePair.Value)
                )
            );
        return keyValuePairList;
    }
