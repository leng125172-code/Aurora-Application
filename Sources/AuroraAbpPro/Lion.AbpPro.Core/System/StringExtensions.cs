...
public static class StringExtensions
{
    ...
    public static bool IsIdentityCardId(this string value)
    {
        ...
        if (value.Length == 15)
        {
            ...
            return DateTime.TryParse(
                string.Format("{0}-{1}-{2}", "19" + array[2], array[3], array[4]),
                out _
            );
        }
    ...
    public static double GetSimilarityWith(
        this string source,
        string target,
        bool ignoreCase = false
    )
    {
        ...
        //获取交集数量
        int q = sourceChars.Intersect(targetChars).Count(),
            s = sourceChars.Length - q,
            r = targetChars.Length - q;
    ...
    public static string PascalToKebabCase(this string value)
    {
        ...
        return Regex
            .Replace(value, "(?<!^)([A-Z][a-z]|(?<=[a-z])[A-Z])", "-$1", RegexOptions.Compiled)
            .Trim()
            .ToLower();
    }
