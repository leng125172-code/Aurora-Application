...
public class LanguageManagementDataSeedContributor : ITransientDependency, IDataSeedContributor
{
    ...
    public async Task SeedAsync(DataSeedContext context)
    {
        ...
        {
            var defaultLanguage = await _settingManager.GetOrNullDefaultAsync(
                LanguageManagementConsts.SettingDefaultLanguage
            );
            using (_softDeleteFilter.Disable())
            {
                ...
                foreach (var language in _localizationOptions.Languages)
                {
                    if (
                        !languages.Any(e =>
                            e.CultureName == language.CultureName
                            && e.UiCultureName == language.UiCultureName
                        )
                    )
                    {
