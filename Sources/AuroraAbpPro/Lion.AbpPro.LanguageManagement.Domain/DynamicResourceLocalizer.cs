...
public class DynamicResourceLocalizer : IDynamicResourceLocalizer, ISingletonDependency
{
    ...
    public DynamicResourceLocalizer(
    ...
    )
    {
        ...
        {
            dictionary[keyValuePair.Key] = new LocalizedString(
                keyValuePair.Key,
                keyValuePair.Value
            );
        }
    ...
    protected virtual async Task<LanguageTextCacheItem> GetCacheLanguageText(
        LocalizationResourceBase resource,
        string cultureName
    )
    {
        var result = await _distributedCache.GetOrAddAsync(
    ...
    protected virtual async Task<LanguageTextCacheItem> CreateCacheLanguageText(
        LocalizationResourceBase resource,
        string cultureName
    )
    {
        ...
        {
            var languageTexts = await scope
                .ServiceProvider.GetRequiredService<ILanguageTextManager>()
