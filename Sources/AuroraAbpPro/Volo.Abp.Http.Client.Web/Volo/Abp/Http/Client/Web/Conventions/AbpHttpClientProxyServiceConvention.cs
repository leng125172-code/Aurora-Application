...
public class AbpHttpClientProxyServiceConvention : AbpServiceConvention
{
    ...
    protected override IList<ControllerModel> GetControllers(ApplicationModel application)
    {
        return application
            .Controllers.Where(c =>
                !AbpHttpClientProxyHelper.IsClientProxyService(c.ControllerType)
            )
            .ToList();
    }
    ...
    {
        return application
            .Controllers.Where(c => AbpHttpClientProxyHelper.IsClientProxyService(c.ControllerType))
            .ToList();
    }
    ...
    protected override void ConfigureParameters(ControllerModel controller)
    {
        foreach (var action in controller.Actions)
        {
            foreach (var prm in action.Parameters)
            {
                ...
                if (
                    StaticProxyingOptions.BindingFromQueryTypes.Contains(
                        prm.ParameterInfo.ParameterType
                    )
                )
                {
                    prm.BindingInfo = BindingInfo.GetBindingInfo(
                        new[] { new FromQueryAttribute() }
                    );
                }
            }
        }
    ...
    protected virtual bool ShouldBeRemove(
        ApplicationModel application,
        ControllerModel controllerModel
    )
    {
        return application
            .Controllers.Where(x => x.ControllerType != controllerModel.ControllerType)
            .Any(x =>
                FindAppServiceInterfaceType(x) == FindAppServiceInterfaceType(controllerModel)
            );
    }
    ...
    protected virtual void ConfigureClientProxySelector(ControllerModel controller)
    {
        ...
        if (moduleApiDescription != null && !moduleApiDescription.RootPath.IsNullOrWhiteSpace())
        {
            ...
            controller.RouteValues.Add(
                new KeyValuePair<string, string?>("area", moduleApiDescription.RootPath)
            );
        }
    ...
    )
    {
        try
        {
            ...
            {
                Logger.LogWarning(
                    $"Could not find ApiDescriptionModel for action: {action.ActionName} in controller: {controller.ControllerName}, May be the generate-proxy.json is not up to date."
                );
                return;
                ;
            }
                ...
                var abpServiceSelectorModel = new SelectorModel
                {
                    ...
                    ActionConstraints =
                    {
                        new HttpMethodActionConstraint(
                            new[] { actionApiDescriptionModel.HttpMethod! }
                        ),
                    },
                };
        ...
        {
            Logger.LogError(
                $"An exception has occured while configuring client proxy selector. Controller: {controller.ControllerName} ({controller.ControllerType.AssemblyQualifiedName}), Action: {action.ActionName}"
            );
            throw;
        }
    }
    ...
    protected virtual void ConfigureClientProxyApiExplorer(ControllerModel controller)
    {
        if (ControllerWithAttributeRoute.Contains(controller))
        {
            ...
            {
                controller.ApiExplorer.IsVisible = IsVisibleRemoteService(
    ...
    protected virtual ModuleApiDescriptionModel? FindModuleApiDescriptionModel(
        ControllerModel controller
    )
    {
        ...
        {
            if (
                moduleApiDescription.Controllers.Values.Any(x =>
                    x.Interfaces.Any(t => t.Type == appServiceType.FullName)
                )
            )
            {
    ...
    protected virtual ControllerApiDescriptionModel? FindControllerApiDescriptionModel(
        ControllerModel controller
    )
    {
        var appServiceType = FindAppServiceInterfaceType(controller);
        ...
        var applicationApiDescriptionModel = ClientProxyApiDescriptionFinder.GetApiDescription();
        foreach (
            var controllerApiDescription in applicationApiDescriptionModel.Modules.Values.SelectMany(
                x => x.Controllers.Values
            )
        )
        {
    ...
    protected virtual ActionApiDescriptionModel? FindActionApiDescriptionModel(
        ControllerModel controller,
        ActionModel action
    )
    {
