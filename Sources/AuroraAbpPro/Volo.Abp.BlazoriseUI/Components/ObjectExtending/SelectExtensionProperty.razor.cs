    ...
    where TEntity : IHasExtraProperties
{
    ...
    public int? SelectedValue
    {
        get
        {
            return Entity.GetProperty<int?>(
                PropertyInfo.Name,
                Nullable.GetUnderlyingType(PropertyInfo.Type!) != null ? null : 0
            );
        }
    ...
    protected virtual List<SelectItem<int?>> GetSelectItemsFromEnum()
    {
        ...
        foreach (var enumValue in enumType.GetEnumValues())
        {
            selectItems.Add(
                ...
                {
                    Value = (int)enumValue,
                    Text = AbpEnumLocalizer.GetString(
                        enumType,
                        enumValue,
                        new[] { StringLocalizerFactory.CreateDefaultOrNull() }
                    ),
                }
            );
        }
