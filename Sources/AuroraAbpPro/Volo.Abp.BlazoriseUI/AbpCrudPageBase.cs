    ...
    where TUpdateViewModel : class, new()
{
    ...
    protected virtual async Task<List<TableColumn>> GetExtensionTableColumnsAsync(
        string moduleName,
        string entityType
    )
    {
        ...
        foreach (var propertyInfo in properties)
        {
            ...
            if (propertyInfo.IsAvailableToClients && propertyInfo.UI.OnTable.IsVisible)
            {
                if (propertyInfo.Name.EndsWith("_Text"))
                {
                    ...
                    var lookupPropertyDefinition = properties.SingleOrDefault(t =>
                        t.Name == lookupPropertyName
                    )!;
                    tableColumns.Add(
                        new TableColumn
                        {
                            Title = lookupPropertyDefinition.GetLocalizedDisplayName(
