...
public class LdapSettingProvider : ILdapSettingProvider, ITransientDependency
{
    ...
    {
        return (await SettingProvider.GetOrNullAsync(LdapSettingNames.Ldaps))?.To<bool>()
            ?? default;
    }
    ...
    {
        return (await SettingProvider.GetOrNullAsync(LdapSettingNames.ServerPort))?.To<int>()
            ?? default;
    }
