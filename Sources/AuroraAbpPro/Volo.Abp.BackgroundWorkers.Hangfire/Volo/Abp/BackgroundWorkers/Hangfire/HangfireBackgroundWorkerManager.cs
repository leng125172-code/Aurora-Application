...
public class HangfireBackgroundWorkerManager : BackgroundWorkerManager, ISingletonDependency
{
    ...
    )
    {
        ...
        var abpHangfireOptions = ServiceProvider
            .GetRequiredService<IOptions<AbpHangfireOptions>>()
            .Value;
        ...
        switch (worker)
        {
            case IHangfireBackgroundWorker hangfireBackgroundWorker:
            {
                ...
                var queueName = hangfireBackgroundWorker.Queue.IsNullOrWhiteSpace()
                    ? defaultQueue
                ...
                if (!JobStorage.Current.HasFeature(JobStorageFeatures.JobQueueProperty))
                {
                    ...
                    RecurringJob.AddOrUpdate(
                        ...
                        () =>
                            ((IHangfireBackgroundWorker)unProxyWorker).DoWorkAsync(
                                cancellationToken
                            ),
            ...
            case AsyncPeriodicBackgroundWorkerBase or PeriodicBackgroundWorkerBase:
            {
                ...
                var workerAdapter = (
                    ServiceProvider.GetRequiredService(
                        typeof(HangfirePeriodicBackgroundWorkerAdapter<>).MakeGenericType(
                            ProxyHelper.GetUnProxiedType(worker)
                        )
                    ) as IHangfireBackgroundWorker
                )!;
                ...
                var queueName = workerAdapter.Queue.IsNullOrWhiteSpace()
                    ? defaultQueue
    ...
    private static readonly MethodInfo? GetRecurringJobIdMethodInfo =
        typeof(RecurringJob).GetMethod(
            "GetRecurringJobId",
            BindingFlags.NonPublic | BindingFlags.Static
        );

    protected virtual string? GetRecurringJobId(
        IBackgroundWorker worker,
        Expression<Func<Task>> methodCall
    )
    {
        ...
        {
            recurringJobId =
                $"HangfirePeriodicBackgroundWorkerAdapter<{worker.GetType().Name}>.DoWorkAsync";
        }
