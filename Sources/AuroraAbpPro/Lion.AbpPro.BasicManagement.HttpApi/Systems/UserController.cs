...
namespace Lion.AbpPro.BasicManagement.Systems
{
    ...
    public class UserController : BasicManagementController, IUserAppService
    {
        ...
        [HttpPost("canUseTwoFactor")]
        [SwaggerOperation(summary: "是否启用双因素验证", Tags = new[] { "Users" })]
        public Task<bool> CanUseTwoFactorAsync()
        {
            return _userAppService.CanUseTwoFactorAsync();
        }

        [HttpPost("getQRCode")]
        [SwaggerOperation(summary: "获取双因素二维码", Tags = new[] { "Users" })]
        public Task<GetQRCodeOutput> GetQRCodeAsync()
        {
            return _userAppService.GetQRCodeAsync();
        }

        [HttpPost("enabledTwoFactor")]
        [SwaggerOperation(summary: "开启双因素验证", Tags = new[] { "Users" })]
        public Task EnabledTwoFactorAsync(EnabledTwoFactorInput input)
        {
            return _userAppService.EnabledTwoFactorAsync(input);
        }

        [HttpPost("disabledTwoFactor")]
        [SwaggerOperation(summary: "关闭双因素验证", Tags = new[] { "Users" })]
        public Task DisabledTwoFactorAsync(DisabledTwoFactorInput input)
        {
            return _userAppService.DisabledTwoFactorAsync(input);
        }

        [HttpPost("resetTwoFactor")]
        [SwaggerOperation(summary: "重置双因素验证", Tags = new[] { "Users" })]
        public Task ResetTwoFactorAsync(ResetTwoFactorInput input)
        {
            return _userAppService.ResetTwoFactorAsync(input);
        }
