...
{
    private const string HttpContextItemsCacheKeyFormat =
        "ApplicationConfigurationDto_{0}_{1}_CacheKey";
    ...
    protected virtual async Task<ApplicationConfigurationDto> GetRemoteConfigurationAsync()
    {
        ...
        if (
            !CultureHelper.IsCompatibleCulture(config.Localization.CurrentCulture.Name, cultureName)
        )
        {
    ...
    protected virtual string GetHttpContextItemsCacheKey()
    {
        return string.Format(
            ...
            HttpContextItemsCacheKeyFormat,
            CurrentUser.Id?.ToString("N") ?? "Anonymous",
