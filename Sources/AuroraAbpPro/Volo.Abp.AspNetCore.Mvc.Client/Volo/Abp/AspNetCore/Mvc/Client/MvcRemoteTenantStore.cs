...
public class MvcRemoteTenantStore : ITenantStore, ITransientDependency
{
    ...
    public async Task<TenantConfiguration?> FindAsync(string normalizedName)
    {
        ...
        if (
            httpContext != null
            && httpContext.Items[cacheKey]
                is TenantConfigurationCacheItem tenantConfigurationInHttpContext
        )
        {
        ...
        if (tenantConfiguration?.Value == null)
        {
            ...
            {
                Logger.LogWarning(
                    $"Tenant with name '{normalizedName}' was found, but not present in the distributed cache, "
    ...
    public async Task<TenantConfiguration?> FindAsync(Guid id)
    {
        ...
        if (tenantConfiguration?.Value == null)
        {
            ...
            {
                Logger.LogWarning(
                    $"Tenant with ID '{id}' was found, but not present in the distributed cache, "
                        + "Ensure all applications use the same distributed cache and the same cache key prefix"
                );
            }
        }
    ...
    public TenantConfiguration? Find(string normalizedName)
    {
        ...
        if (tenantConfiguration?.Value == null)
        {
            var tenant = AsyncHelper.RunSync(async () =>
