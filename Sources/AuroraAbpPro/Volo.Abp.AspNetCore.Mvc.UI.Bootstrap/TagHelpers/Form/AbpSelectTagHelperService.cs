...
public class AbpSelectTagHelperService : AbpTagHelperService<AbpSelectTagHelper>
{
    ...
    protected virtual async Task<string> GetFormInputGroupAsHtmlAsync(
    ...
    )
    {
        ...
        return TagHelper.FloatingLabel
            ? selectAsHtml
                + Environment.NewLine
                + label
                + Environment.NewLine
    ...
    protected virtual async Task<TagHelperOutput> GetSelectTagAsync(
        TagHelperContext context,
        TagHelperOutput output,
        TagHelperContent childContent
    )
    {
        ...
        else if (!TagHelper.AutocompleteSelectedItemName.IsNullOrEmpty())
        {
            ...
            {
                new SelectListItem(
                    TagHelper.AutocompleteSelectedItemName,
                    TagHelper.AutocompleteSelectedItemValue,
                    false
                ),
            };
        }

        var selectTagHelperOutput = await selectTagHelper.ProcessAndGetOutputAsync(
    ...
    protected virtual void AddAutocompleteAttributes(TagHelperOutput output)
    {
        if (!TagHelper.AutocompleteApiUrl.IsNullOrEmpty())
        {
            ...
            output.Attributes.Add(
                ...
                TagHelper.AutocompleteItemsPropertyName
            );
            output.Attributes.Add(
                "data-autocomplete-display-property",
                TagHelper.AutocompleteDisplayPropertyName
            );
            output.Attributes.Add(
                "data-autocomplete-value-property",
                TagHelper.AutocompleteValuePropertyName
            );
            output.Attributes.Add(
                "data-autocomplete-filter-param-name",
                TagHelper.AutocompleteFilterParamName
            );
            output.Attributes.Add(
                "data-autocomplete-selected-item-name",
                TagHelper.AutocompleteSelectedItemName
            );
            output.Attributes.Add(
                "data-autocomplete-selected-item-value",
                TagHelper.AutocompleteSelectedItemValue
            );
            ...
            output.Attributes.Add(
                "data-autocomplete-parent-selector",
                TagHelper.AutocompleteParentSelector
            );
        }
    }
    ...
    protected virtual List<SelectListItem> GetSelectItems(
        TagHelperContext context,
        TagHelperOutput output
    )
    {
    ...
    protected virtual string GetRequiredSymbol(TagHelperContext context, TagHelperOutput output)
    {
        ...
        return
            TagHelper.AspFor.ModelExplorer.GetAttribute<RequiredAttribute>() != null
            || isHaveRequiredAttribute
            ? "<span> * </span>"
            : "";
    }
    ...
    protected virtual void AddInfoTextId(TagHelperOutput inputTagHelperOutput)
    {
        ...
        var infoText = _tagHelperLocalizer.GetLocalizedText(
            idAttr.Value + "InfoText",
            TagHelper.AspFor.ModelExplorer
        );
    ...
    )
    {
        ...
        var localizedText = _tagHelperLocalizer.GetLocalizedText(
            text,
            TagHelper.AspFor.ModelExplorer
        );
    ...
    protected virtual List<SelectListItem> GetSelectItemsFromEnum(
        TagHelperContext context,
        TagHelperOutput output,
    ...
    )
    {
        ...
        var containerLocalizer = _tagHelperLocalizer.GetLocalizerOrNull(
            explorer.Container.ModelType.Assembly
        );

        foreach (var enumValue in enumType!.GetEnumValuesAsUnderlyingType())
        {
            var localizedMemberName = _abpEnumLocalizer.GetString(
                enumType,
                enumValue,
                new[] { containerLocalizer, _stringLocalizerFactory.CreateDefaultOrNull() }!
            );
            selectItems.Add(
                new SelectListItem { Value = enumValue.ToString(), Text = localizedMemberName }
            );
        }
    ...
    protected virtual async Task<string> GetValidationAsHtmlAsync(
        TagHelperContext context,
        TagHelperOutput output,
        TagHelperOutput inputTag
    )
