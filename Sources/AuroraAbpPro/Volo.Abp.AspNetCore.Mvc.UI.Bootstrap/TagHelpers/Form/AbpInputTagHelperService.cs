...
public class AbpInputTagHelperService : AbpTagHelperService<AbpInputTagHelper>
{
    ...
    public AbpInputTagHelperService(
    ...
    public override async Task ProcessAsync(TagHelperContext context, TagHelperOutput output)
    {
        ...
        {
            TagHelper.ViewContext.CheckBoxHiddenInputRenderMode = TagHelper
    ...
    protected virtual async Task<(string, bool)> GetFormInputGroupAsHtmlAsync(
    ...
    )
    {
        ...
        var validation = isCheckBox
            ? ""
            : await GetValidationAsHtmlAsync(context, output, inputTag);

        return (
            GetContent(context, output, label, inputHtml, validation, info, isCheckBox),
            isCheckBox
        );
    }
    ...
    {
        var innerContent =
            isCheckbox || TagHelper.FloatingLabel ? inputHtml + label : label + inputHtml;
    ...
    )
    {
        ...
        var mb =
            !isHidden && TagHelper.AddMarginBottomClass
                ? (isCheckbox ? "mb-2" : "mb-3")
                : string.Empty;
        return "<div class=\""
            + (isCheckbox ? $"custom-checkbox custom-control {mb} form-check" : $"{mb}")
            + "\">"
            + Environment.NewLine
            + innerHtml
            + Environment.NewLine
    ...
    protected virtual async Task<(TagHelperOutput, bool)> GetInputTagHelperOutputAsync(
        TagHelperContext context,
        TagHelperOutput output
    )
    {
    ...
    private void AddFormControlClass(
        TagHelperContext context,
        TagHelperOutput output,
        bool isCheckbox,
    ...
    protected virtual void AddDisabledAttribute(TagHelperOutput inputTagHelperOutput)
    {
        if (
            ...
            && (
                TagHelper.IsDisabled
                || TagHelper.AspFor.ModelExplorer.GetAttribute<DisabledInput>() != null
            )
        )
        {
    ...
    protected virtual void AddReadOnlyAttribute(TagHelperOutput inputTagHelperOutput)
    {
        if (
            ...
            && (
                TagHelper.IsReadonly != false
                || TagHelper.AspFor.ModelExplorer.GetAttribute<ReadOnlyInput>() != null
            )
        )
        {
    ...
    protected virtual void AddPlaceholderAttribute(TagHelperOutput inputTagHelperOutput)
    {
        ...
        {
            var placeholderLocalized = _tagHelperLocalizer.GetLocalizedText(
                attribute?.Value ?? TagHelper.AspFor.Name,
    ...
    protected virtual bool IsInputCheckbox(
        TagHelperContext context,
        TagHelperOutput output,
