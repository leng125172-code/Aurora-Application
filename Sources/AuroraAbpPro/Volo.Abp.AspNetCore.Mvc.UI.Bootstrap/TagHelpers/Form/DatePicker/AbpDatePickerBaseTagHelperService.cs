...
public abstract class AbpDatePickerBaseTagHelperService<TTagHelper>
    ...
    where TTagHelper : AbpDatePickerBaseTagHelper<TTagHelper>
{
    ...
    protected AbpDatePickerBaseTagHelperService(
    ...
    protected virtual T? GetAttribute<T>()
        where T : Attribute
    {
        return GetAttributeAndModelExpression<T>(out _);
    }

    protected abstract T? GetAttributeAndModelExpression<T>(out ModelExpression? modelExpression)
        where T : Attribute;
    ...
    public override async Task ProcessAsync(TagHelperContext context, TagHelperOutput output)
    {
        ...
        var optionsAttribute = GetAttributeAndModelExpression<DatePickerOptionsAttribute>(
            out var modelExpression
        );
        if (optionsAttribute != null)
        {
            TagHelper.SetDatePickerOptions(
                optionsAttribute.GetDatePickerOptions(modelExpression!.ModelExplorer)!
            );
        }
        ...
        var openButtonContent = TagHelper.OpenButton
            ...
            : "";
        var clearButtonContent =
            TagHelper.ClearButton == true
            || (!TagHelper.ClearButton.HasValue && TagHelper.AutoUpdateInput != true)
                ? await ProcessButtonAndGetContentAsync(
                    context,
                    output,
                    "times",
                    "clear",
                    visible: !TagHelper.SingleOpenAndClearButton
                )
                : "";
        ...
        var inputGroup = new TagHelperOutput(
            "div",
        ...
        var abpDatePickerTag = new TagHelperOutput(
            TagName,
            GetBaseTagAttributes(context, output, TagHelper),
            (_, _) => Task.FromResult<TagHelperContent>(new DefaultTagHelperContent())
        );
    ...
    {
        if (
            inputTagHelperOutput.Attributes.ContainsName("readonly") == false
    ...
    protected virtual TagHelperAttributeList GetInputAttributes(
    ...
    )
    {
        ...
        var tagHelperAttributes = output
            .Attributes.Where(a => !a.Name.StartsWith(groupPrefix))
            .ToList();
    ...
    protected virtual void LeaveOnlyGroupAttributes(
        TagHelperContext context,
        TagHelperOutput output
    )
    {
    ...
    protected virtual string SurroundInnerHtmlAndGet(
        TagHelperContext context,
        TagHelperOutput output,
        string innerHtml
    )
    ...
    protected virtual void AddGroupToFormGroupContents(
        TagHelperContext context,
        string propertyName,
        string html,
        int order
    ...
    {
        var list =
            context.GetValue<List<FormGroupItem>>(FormGroupContents) ?? new List<FormGroupItem>();

        if (
            !list.Any(igc =>
                igc.HtmlContent.Contains("id=\"" + propertyName.Replace('.', '_') + "\"")
            )
        )
        {
            list.Add(
    ...
    protected TagHelperAttributeList ConvertDatePickerOptionsToAttributeList(
    ...
    )
    {
        ...
        if (options.ShowDropdowns == false)
        {
            attrList.Add(
                "data-show-dropdowns",
                options.ShowDropdowns.ToString()!.ToLowerInvariant()
            );
        }
        ...
        {
            attrList.Add(
                "data-time-picker24-hour",
                options.TimePicker24Hour.ToString()!.ToLowerInvariant()
            );
        }
        ...
        {
            attrList.Add(
                "data-time-picker-seconds",
                options.TimePickerSeconds.ToString()!.ToLowerInvariant()
            );
        }
        ...
        {
            attrList.Add(
                "data-linked-calendars",
                options.LinkedCalendars.ToString()!.ToLowerInvariant()
            );
        }
        ...
        {
            attrList.Add(
                "data-auto-update-input",
                options.AutoUpdateInput.ToString()!.ToLowerInvariant()
            );
        }
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        {
            attrList.Add(
                "data-always-show-calendars",
                options.AlwaysShowCalendars.ToString()!.ToLowerInvariant()
            );
        }
        ...
        if (options.ShowCustomRangeLabel == false)
        {
            attrList.Add(
                ...
                options.ShowCustomRangeLabel.ToString()!.ToLowerInvariant()
            );
        }
        ...
        {
            attrList.Add(
                "data-single-open-and-clear-button",
                options.SingleOpenAndClearButton.ToString().ToLowerInvariant()
            );
        }
    ...
    protected TagHelperAttributeList GetBaseTagAttributes(
        TagHelperContext context,
        TagHelperOutput output,
        IAbpDatePickerOptions options
    )
    ...
    protected virtual bool IsOutputHidden(TagHelperOutput inputTag)
    {
        return inputTag.Attributes.Any(a =>
            a.Name.ToLowerInvariant() == "type"
            && a.Value?.ToString()?.ToLowerInvariant() == "hidden"
        );
    }

    protected virtual string GetInfoAsHtml(
        TagHelperContext context,
        TagHelperOutput output,
    ...
    )
    {
        ...
        {
            var infoAttribute = GetAttributeAndModelExpression<InputInfoText>(
                out var modelExpression
            );
    ...
    protected virtual async Task<string> GetLabelAsHtmlAsync(
        TagHelperContext context,
        TagHelperOutput output,
        TagHelperOutput inputTag
    )
    {
        ...
        if (string.IsNullOrEmpty(TagHelper.Label))
        {
            return await GetLabelAsHtmlUsingTagHelperAsync(context, output)
                + GetRequiredSymbol(context, output);
        }
    ...
    protected virtual string GetRequiredSymbol(TagHelperContext context, TagHelperOutput output)
    {
        ...
        return GetAttribute<RequiredAttribute>() != null || isHaveRequiredAttribute
            ? "<span> * </span>"
            : "";
    }
    ...
    protected virtual async Task<string> GetLabelAsHtmlUsingTagHelperAsync(
        TagHelperContext context,
        TagHelperOutput output
    )
    {
    ...
    protected virtual async Task<string> ProcessButtonAndGetContentAsync(
        TagHelperContext context,
        TagHelperOutput output,
        string icon,
        string type,
    ...
    )
    {
        ...
        return await abpButtonTagHelper.RenderAsync(
            attributes,
            context,
            Encoder,
            "button",
    ...
    public virtual string GetInfoText()
    {
        ...
        if (infoAttribute != null)
        {
            return TagHelperLocalizer.GetLocalizedText(
                infoAttribute.Text,
    ...
    protected virtual void AddPlaceholderAttribute(TagHelperOutput inputTagHelperOutput)
    {
        ...
        if (attribute != null)
        {
            var placeholderLocalized = TagHelperLocalizer.GetLocalizedText(
                attribute.Value,
                modelExpression!.ModelExplorer
            );
    ...
    protected virtual void AddFormControls(
        TagHelperContext context,
        TagHelperOutput output,
    ...
    {
        if (
            inputTagHelperOutput.Attributes.ContainsName("disabled") == false
    ...
    protected virtual Task<string> GetValidationAsHtmlAsync(
        TagHelperContext context,
        TagHelperOutput output
    )
    {
    ...
    protected virtual async Task<string> GetValidationAsHtmlByInputAsync(
        TagHelperContext context,
        TagHelperOutput output,
    ...
    )
    {
        ...
        if (
            !output.Attributes.TryGetAttribute("name", out var nameAttribute)
            || nameAttribute == null
        ...
        return await validationMessageTagHelper.RenderAsync(
            attributeList,
            context,
            Encoder,
            "span",
