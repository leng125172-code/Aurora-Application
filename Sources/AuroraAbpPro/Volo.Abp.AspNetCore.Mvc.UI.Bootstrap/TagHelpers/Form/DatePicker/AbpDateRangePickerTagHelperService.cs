...
public class AbpDateRangePickerTagHelperService
    : AbpDatePickerBaseTagHelperService<AbpDateRangePickerTagHelper>
{
    ...
    protected override T? GetAttributeAndModelExpression<T>(out ModelExpression? modelExpression)
        where T : class
    {
        modelExpression = new[] { TagHelper.AspForStart, TagHelper.AspForEnd }.FirstOrDefault(x =>
    ...
    public override async Task ProcessAsync(TagHelperContext context, TagHelperOutput output)
    {
        ...
        {
            var startDateAttributes = new TagHelperAttributeList
            {
                { "data-hidden-datepicker", "true" },
            ...
            if (Clock.SupportsMultipleTimezone)
            {
                ...
                {
                    StartDateTagHelper.Format = "{0:O}";
                    StartDateTagHelper.Value = Clock
            ...
            StartDateTagHelperOutput = await StartDateTagHelper.ProcessAndGetOutputAsync(
                startDateAttributes,
                context,
                "input"
            );
        ...
        {
            var endDateAttributes = new TagHelperAttributeList
            {
                { "data-hidden-datepicker", "true" },
                { "data-end-date", "true" },
                { "type", "hidden" },
    ...
    protected override void AddBaseTagAttributes(TagHelperAttributeList attributes)
    {
        if (
            ...
            && SupportedInputTypes.TryGetValue(
                TagHelper.AspForStart.Metadata.ModelType,
                out var convertFuncStart
            )
        )
        {
        ...
        if (
            ...
            && SupportedInputTypes.TryGetValue(
                TagHelper.AspForEnd.Metadata.ModelType,
                out var convertFuncEnd
            )
        )
        {
