...
public class AbpPaginationTagHelperService : AbpTagHelperService<AbpPaginationTagHelper>
{
    ...
    protected virtual async Task SetContentAsHtmlAsync(
    ...
    protected virtual async Task<string> GetPagesAsync(
        TagHelperContext context,
        TagHelperOutput output
    )
    {
    ...
    protected virtual async Task<string> GetPageAsync(
        TagHelperContext context,
        TagHelperOutput output,
        PageItem page
    )
    {
        ...
        pageHtml.AppendLine(
            "<li class=\"page-item "
                + (TagHelper.Model.CurrentPage == page.Index ? "active" : "")
                + "\">"
        );
        ...
        {
            pageHtml.AppendLine(
                await RenderAnchorTagHelperLinkHtmlAsync(
                    context,
                    output,
                    page.Index.ToString(),
                    page.Index.ToString()
    ...
    protected virtual async Task<string> GetPreviousButtonAsync(
        TagHelperContext context,
        TagHelperOutput output
    )
    {
        ...
        var currentPage =
            TagHelper.Model.CurrentPage > 1 ? (TagHelper.Model.CurrentPage - 1).ToString() : "1";
        return "<li class=\"page-item "
            + (TagHelper.Model.CurrentPage <= 1 ? "disabled" : "")
            + "\">\r\n"
            + (
                await RenderAnchorTagHelperLinkHtmlAsync(
                    context,
    ...
    protected virtual async Task<string> GetNextButton(
        TagHelperContext context,
        TagHelperOutput output
    )
    {
        ...
        return "<li class=\"page-item "
            + (TagHelper.Model.CurrentPage >= TagHelper.Model.TotalPageCount ? "disabled" : "")
            + "\">\r\n"
            + (
                await RenderAnchorTagHelperLinkHtmlAsync(
                    context,
    ...
    protected virtual async Task<string> RenderAnchorTagHelperLinkHtmlAsync(
        TagHelperContext context,
        TagHelperOutput output,
        string currentPage,
    ...
    )
    {
        ...
        var tagHelperOutput = await anchorTagHelper.ProcessAndGetOutputAsync(
            attributeList,
            context,
            "a",
    ...
    protected virtual string GetOpeningTags(TagHelperContext context, TagHelperOutput output)
    {
        ...
        var pagerInfo =
            (TagHelper.ShowInfo ?? false)
                ? "    <div class=\"col-sm-12 col-md-5\"> "
                    + _encoder.Encode(
                        localizer[
                            "PagerInfo{0}{1}{2}",
                            TagHelper.Model.ShowingFrom,
                            TagHelper.Model.ShowingTo,
                            TagHelper.Model.TotalItemsCount
                        ]
                    )
                    + "</div>\r\n"
                : "";
    ...
    {
        var hrefAttribute = attributeList.FirstOrDefault(x =>
            x.Name.Equals("href", StringComparison.OrdinalIgnoreCase)
        );
        ...
        if (hrefAttribute != null)
        {
            ...
            var routeValue =
                $"currentPage={currentPage}{(TagHelper.Model.Sort.IsNullOrWhiteSpace() ? "" : "&sort=" + TagHelper.Model.Sort)}";
