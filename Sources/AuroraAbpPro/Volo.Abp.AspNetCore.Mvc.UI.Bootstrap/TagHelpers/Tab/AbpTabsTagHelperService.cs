...
public class AbpTabsTagHelperService : AbpTagHelperService<AbpTabsTagHelper>
{
    ...
    public override async Task ProcessAsync(TagHelperContext context, TagHelperOutput output)
    {
        ...
        var finalContent = CombineHeadersAndContents(
            context,
            output,
            surroundedHeaders,
            surroundedContents
    ...
    protected virtual string CombineHeadersAndContents(
    ...
    protected virtual string SurroundHeaders(
        TagHelperContext context,
        TagHelperOutput output,
        string headers
    )
    ...
    protected virtual string SurroundContents(
        TagHelperContext context,
        TagHelperOutput output,
        string contents
    )
    ...
    protected virtual void SetActiveTab(List<TabItem> items)
    {
        ...
        foreach (var tabItem in items)
        {
            if (tabItem.Active)
            {
                tabItem.Content = tabItem.Content.Replace(
                    AbpTabItemShowActivePlaceholder,
                    " show active"
                );
                tabItem.Header = tabItem
                    .Header.Replace(AbpTabItemActivePlaceholder, " active")
                    .Replace(AbpTabItemSelectedPlaceholder, "true");
            }
            else
            {
                tabItem.Content = tabItem.Content.Replace(AbpTabItemShowActivePlaceholder, "");
                tabItem.Header = tabItem
                    .Header.Replace(AbpTabItemActivePlaceholder, "")
                    .Replace(AbpTabItemSelectedPlaceholder, "false");
            }
        }
    }
    ...
    protected virtual string GetConents(
        TagHelperContext context,
        TagHelperOutput output,
        List<TabItem> items
    )
    ...
    protected virtual List<TabItem> InitilizeFormGroupContentsContext(
        TagHelperContext context,
        TagHelperOutput output
    )
    {
    ...
    protected virtual int GetHeaderColumnSize()
    {
        return
            ...
            || TagHelper.VerticalHeaderSize == ColumnSize.Auto
            || TagHelper.VerticalHeaderSize == ColumnSize._
            ? (int)ColumnSize._3
    ...
    protected virtual string SetTabItemNameIfNotProvided(string content, int index)
    {
        return content.Replace(
            ...
            HtmlGenerator.Encode(TagHelper.Name ?? string.Empty) + "_" + index
        );
    }
}
