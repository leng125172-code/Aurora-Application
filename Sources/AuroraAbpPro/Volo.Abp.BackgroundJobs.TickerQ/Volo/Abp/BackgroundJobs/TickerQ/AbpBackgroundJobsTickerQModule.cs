...
public class AbpBackgroundJobsTickerQModule : AbpModule
{
    private static readonly MethodInfo GetTickerFunctionDelegateMethod =
        typeof(AbpBackgroundJobsTickerQModule).GetMethod(
            nameof(GetTickerFunctionDelegate),
    ...
    public override void OnApplicationInitialization(ApplicationInitializationContext context)
    {
        var abpBackgroundJobOptions = context.ServiceProvider.GetRequiredService<
            IOptions<AbpBackgroundJobOptions>
        >();
        var abpBackgroundJobsTickerQOptions = context.ServiceProvider.GetRequiredService<
            IOptions<AbpBackgroundJobsTickerQOptions>
        >();
        var tickerFunctionDelegates =
        ...
        {
            var genericMethod = GetTickerFunctionDelegateMethod.MakeGenericMethod(
                jobConfiguration.ArgsType
            );
        ...
        var abpTickerQFunctionProvider =
            context.ServiceProvider.GetRequiredService<AbpTickerQFunctionProvider>();
        foreach (var functionDelegate in tickerFunctionDelegates)
        {
            abpTickerQFunctionProvider.Functions.TryAdd(
                functionDelegate.Key,
    ...
    private static TickerFunctionDelegate GetTickerFunctionDelegate<TArgs>(Type argsType)
    {
        return async (cancellationToken, serviceProvider, context) =>
        {
            ...
            using (var scope = serviceProvider.CreateScope())
            {
                ...
                var args = await TickerRequestProvider.GetRequestAsync<TArgs>(
                    context,
                    cancellationToken
                );
