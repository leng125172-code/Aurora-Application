...
public class AuditingHelper : IAuditingHelper, ITransientDependency
{
    ...
    )
    {
        ...
        if (classType != null)
        {
            var shouldAudit = AuditingInterceptorRegistrar.ShouldAuditTypeByDefaultOrNull(
                classType,
    ...
    public virtual bool IsEntityHistoryEnabled(Type entityType, bool defaultValue = false)
    {
        ...
        foreach (
            var propertyInfo in entityType.GetProperties(
    ...
    {
        return AuditingDisabledState.BeginScope(
            AuditingDisabledScopeKey,
            new AuditingDisabledState(true)
        );
    }
    ...
    protected virtual string SerializeConvertArguments(IDictionary<string, object?> arguments)
    {
        try
        {
            ...
            foreach (var argument in arguments)
            {
                if (
                    argument.Value != null
                    && Options.IgnoredTypes.Any(t => t.IsInstanceOfType(argument.Value))
                )
                {
