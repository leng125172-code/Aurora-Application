                ...
                var settings = new JsonSerializerOptions()
                {
                    ...
                    TypeInfoResolver = new DefaultJsonTypeInfoResolver()
                    {
                        Modifiers =
                        {
                            ...
                            {
                                if (
                                    Options.IgnoredTypes.Any(ignoredType =>
                                        ignoredType.IsAssignableFrom(jsonTypeInfo.Type)
                                    )
                                    || jsonTypeInfo
                                        .Type.GetCustomAttributes(
                                            typeof(DisableAuditingAttribute),
                                            false
                                        )
                                        .Any()
                                )
                                ...
                                foreach (var property in jsonTypeInfo.Properties)
                                {
                                    if (
                                        Options.IgnoredTypes.Any(ignoredType =>
                                            ignoredType.IsAssignableFrom(property.PropertyType)
                                        )
                                    )
                                    {
                                    ...
                                    if (
                                        property.PropertyType.DeclaringType != null
                                        && property.PropertyType.DeclaringType.IsDefined(
                                            typeof(DisableAuditingAttribute)
                                        )
                                    )
                                    {
