...
[DependsOn(typeof(AbpAspNetCoreComponentsMauiBlazorModule), typeof(AbpAspNetCoreBundlingModule))]
public class AbpAspNetCoreComponentsMauiBlazorBundlingModule : AbpModule
{
    ...
    {
        var bundlingOptions = context
            .ServiceProvider.GetRequiredService<IOptions<AbpBundlingOptions>>()
            .Value;
        var logger = context.ServiceProvider.GetRequiredService<
            ILogger<AbpAspNetCoreComponentsMauiBlazorBundlingModule>
        >();
        ...
        var mauiBlazorContentFileProvider =
            context.ServiceProvider.GetRequiredService<IMauiBlazorContentFileProvider>();
        var dynamicFileProvider =
            context.ServiceProvider.GetRequiredService<IDynamicFileProvider>();
        if (!bundlingOptions.GlobalAssets.GlobalStyleBundleName.IsNullOrWhiteSpace())
        {
            ...
            foreach (var file in styleFiles)
            {
                ...
                if (!bundleManager.IsBundlingEnabled())
                {
                    ...
                    styles +=
                        $"/*{file.FileName}*/{Environment.NewLine}{fileContent}{Environment.NewLine}{Environment.NewLine}";
                }
                else
                {
        ...
        {
            var scriptFiles = await bundleManager.GetScriptBundleFilesAsync(
                bundlingOptions.GlobalAssets.GlobalScriptBundleName
            );
            ...
            foreach (var file in scriptFiles)
            {
                ...
                {
                    scripts +=
                        $"{fileContent.EnsureEndsWith(';')}{Environment.NewLine}{Environment.NewLine}";
                }
                else
                {
                    scripts +=
                        $"//{file.FileName}{Environment.NewLine}{fileContent.EnsureEndsWith(';')}{Environment.NewLine}{Environment.NewLine}";
                }
            }
