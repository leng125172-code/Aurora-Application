...
public class AbpEntityFrameworkCoreSqliteModule : AbpModule
{
    ...
    public override void ConfigureServices(ServiceConfigurationContext context)
    {
        ...
        if (sqliteOptions.BusyTimeout.HasValue)
        {
            ...
            {
                options.ConfigureDefaultOnConfiguring(
                    (dbContext, dbContextOptionsBuilder) =>
                    {
                        if (
                            dbContextOptionsBuilder.Options.Extensions.Any(extension =>
                                extension is SqliteOptionsExtension
                            )
                        )
                        {
                            dbContextOptionsBuilder.AddInterceptors(
                                new SqliteBusyTimeoutSaveChangesInterceptor(
                                    sqliteOptions.BusyTimeout.Value
                                )
                            );
                        }
                    },
