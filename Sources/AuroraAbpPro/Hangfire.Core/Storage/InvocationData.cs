...
namespace Hangfire.Storage
{
    public class InvocationData
    {
        private static readonly ConcurrentDictionary<
            MethodDeserializerCacheKey,
            MethodDeserializerCacheValue
        > MethodDeserializerCache = new();

        private static readonly ConcurrentDictionary<
            MethodSerializerCacheKey,
            MethodSerializerCacheValue
        > MethodSerializerCache = new();

        private static readonly ConcurrentDictionary<
            Tuple<Func<Type, string>, string>,
            string[]
        > ParameterTypesDeserializerCache = new();

        [Obsolete(
            "Please use IGlobalConfiguration.UseTypeResolver instead. Will be removed in 2.0.0."
        )]
        ...
        [Obsolete(
            "Please use IGlobalConfiguration.UseTypeSerializer instead. Will be removed in 2.0.0."
        )]
        ...
        [Obsolete(
            "Please use DeserializeJob() method instead. Will be removed in 2.0.0 for clarity."
        )]
        ...
        [Obsolete(
            "Please use SerializeJob(Job) method instead. Will be removed in 2.0.0 for clarity."
        )]
        ...
        public static InvocationData DeserializePayload(string payload)
        {
            ...
            try
            {
                ...
                if (jobPayload == null)
                {
                    throw new InvalidOperationException(
                        "Deserialize<JobPayload> returned `null` for a non-null payload."
                    );
                }
            }
            catch (Exception ex) when (ex.IsCatchableExceptionType())
            ...
            {
                throw new InvalidOperationException(
                    "Deserialize<InvocationData> returned `null` for a non-null payload."
                );
            }

            if (data.Type == null || data.Method == null)
            {
                data = SerializationHelper.Deserialize<InvocationData>(
                    payload,
        ...
        public string SerializePayload(bool excludeArguments = false)
        {
            if (GlobalConfiguration.HasCompatibilityLevel(CompatibilityLevel.Version_170))
            {
                ...
                var arguments = excludeArguments
                    ? null
                    : SerializationHelper.Deserialize<string[]>(Arguments);
        ...
        private static string[] DeserializeParameterTypesArray(
            Func<Type, string> typeSerializer,
            string parameterTypes
        )
        {
            return ParameterTypesDeserializerCache.GetOrAdd(
                Tuple.Create(typeSerializer, parameterTypes),
                static tuple =>
                {
                    try
                    {
                        ...
                        when (outerException.IsCatchableExceptionType())
                    {
                        try
                        {
        ...
        internal static string[] SerializeArguments(
                    ...
                    return new MethodSerializerCacheValue
                    {
                        ...
                        ParameterTypes = SerializationHelper.Serialize(
                            key.Method.GetParameters()
                                .Select(x => key.TypeSerializer(x.ParameterType))
                                .ToArray()
                        ),
                    };
                }
        ...
        )
        {
            var entry = MethodDeserializerCache.GetOrAdd(
                new MethodDeserializerCacheKey
                {
        ...
        private static object DeserializeArgument(string argument, Type type)
        {
            ...
            catch (Exception jsonException) when (jsonException.IsCatchableExceptionType())
            {
                if (type == typeof(object))
                {
                    ...
                    && ParseDateTimeArgument(argument, out var dateTime)
                )
                {
                    value = dateTime;
                }
