...
namespace Hangfire
{
    public sealed class CaptureCultureAttribute : JobFilterAttribute, IClientFilter, IServerFilter
    {
        ...
        public void OnCreating(CreatingContext context)
        {
            ...
            if (
                CaptureDefault == false
                && currentUICulture.Name.Equals(DefaultUICultureName, StringComparison.Ordinal)
            )
            {
                // Don't set the 'CurrentUICulture' job parameter when it's equal to the default one
            }
            else if (
                GlobalConfiguration.HasCompatibilityLevel(CompatibilityLevel.Version_180)
        ...
        public void OnPerforming(PerformingContext context)
        {
            ...
            var uiCultureName =
                context.GetJobParameter<string>("CurrentUICulture", allowStale: true)
                ?? cultureName;
            ...
            catch (CultureNotFoundException ex)
            {
                ...
                _logger.WarnException(
                    $"Unable to set CurrentCulture for job {context.BackgroundJob.Id} due to an exception",
                    ex
                );
            }
            ...
            catch (CultureNotFoundException ex)
            {
                // TODO: Make this overridable, and start with throwing an exception
                _logger.WarnException(
                    $"Unable to set CurrentUICulture for job {context.BackgroundJob.Id} due to an exception",
                    ex
                );
            }
        }
