...
namespace Hangfire
{
    public class ContinuationsSupportAttribute
        : JobFilterAttribute,
            IElectStateFilter,
            IApplyStateFilter
    {
        internal static readonly HashSet<string> KnownFinalStates = new HashSet<string>
        {
        ...
        internal static List<Continuation> DeserializeContinuations(string serialized)
        {
            ...
            {
                continuations = SerializationHelper.Deserialize<List<Continuation>>(
                    serialized,
        ...
        private void AddContinuation(ElectStateContext context, AwaitingState awaitingState)
        {
            ...
            using (connection.AcquireDistributedJobLock(parentId, AddJobLockTimeout))
            {
                ...
                {
                    var startImmediately = ShouldStartContinuation(
                        currentState.Name,
                    ...
                    {
                        if (
                            SucceededState.StateName.Equals(
                                currentState.Name,
                        ...
                        {
                            if (
                                !currentState.Data.TryGetValue(
                                    "Exception",
                                    out var antecedentException
                                )
                            )
                            {
        ...
        private void ExecuteContinuationsIfExist(ElectStateContext context)
        {
            ...
            foreach (var continuation in continuations)
            {
                ...
                var currentState = GetContinuationState(
                    context,
                    continuation.JobId,
                    ContinuationStateFetchTimeout
                );
                ...
                else
                {
                    try
                    {
                        nextState = SerializationHelper.Deserialize<IState>(
        ...
        private bool ShouldStartContinuation(
            string antecedentStateName,
        ...
        )
        {
            ...
            if (
                options.HasFlag(JobContinuationOptions.OnlyOnDeletedState)
                && DeletedState.StateName.Equals(
                    antecedentStateName,
                    StringComparison.OrdinalIgnoreCase
                )
        ...
        )
        {
            connection.SetJobParameter(
                jobId,
                "Continuations",
                SerializationHelper.Serialize(continuations)
            );
        }
        ...
        private static List<Continuation> GetContinuations(ElectStateContext context, string jobId)
        {
            ...
            {
                context.BackgroundJob.ParametersSnapshot.TryGetValue(
                    "Continuations",
                    out serialized
                );
            }
            ...
            {
                serialized = context.Connection.GetJobParameter(
                    jobId ?? context.BackgroundJob.Id,
                    "Continuations"
                );
            }
        ...
        void IApplyStateFilter.OnStateUnapplied(
