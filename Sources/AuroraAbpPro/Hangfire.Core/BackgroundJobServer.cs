...
#pragma warning restore 618
        ...
        [Obsolete(
            "Create your own BackgroundJobServer-like type and pass custom services to it. This constructor will be removed in 2.0.0."
        )]
        ...
        )
        {
            ...
            processes.AddRange(
                GetRequiredProcesses(filterProvider, activator, factory, performer, stateChanger)
            );
            ...
            {
                _logger.Warn(
                    $"These queues fail to match the naming format: {String.Join(", ", wrongQueues.Select(static x => $"'{x}'"))}. A queue name must consist of lowercase letters, digits, underscore, and dash characters only."
                );
            }
        ...
        [Obsolete(
            "This method is a stub. There is no need to call the `Start` method. Will be removed in version 2.0.0."
        )]
        public void Start() { }
        ...
        )
        {
            ...
            {
                filterProvider =
                    filterProvider ?? _options.FilterProvider ?? JobFilterProviders.Providers;
                activator = activator ?? _options.Activator ?? JobActivator.Current;

                factory = new BackgroundJobFactory(filterProvider);
                performer = new BackgroundJobPerformer(
                    filterProvider,
                    activator,
                    _options.TaskScheduler
                );
            ...
            processes.Add(
                new Worker(_options.Queues, performer, stateChanger).UseBackgroundPool(
                    _options.WorkerCount,
                    _options.WorkerThreadConfigurationAction
                )
            );
            ...
            if (!_options.IsLightweightServer)
            {
                ...
                processes.Add(
                    new RecurringJobScheduler(
                        factory,
                        _options.SchedulePollingInterval,
                        timeZoneResolver
                    )
                    {
                        TaskScheduler = _options.TaskScheduler,
...
#pragma warning disable 618
                ServerCheckInterval =
                    _options.ServerWatchdogOptions?.CheckInterval ?? _options.ServerCheckInterval,
                ServerTimeout =
                    _options.ServerWatchdogOptions?.ServerTimeout ?? _options.ServerTimeout,
#pragma warning restore 618
