        ...
        )
        {
            ...
            if (changedFields.Count > 0)
            {
                transaction.SetRangeInHash(
                    $"recurring-job:{recurringJob.RecurringJobId}",
                    changedFields
                );
            }

            var score = recurringJob.NextExecution.HasValue
                ? JobHelper.ToTimestamp(recurringJob.NextExecution.Value)
                : -1.0D;

            if (logger != null && logger.IsTraceEnabled())
            {
                logger.Trace(
                    $"Recurring job '{recurringJob.RecurringJobId}' is being updated. RecurringJob: ({recurringJob}), Changes: ({String.Join(";", changedFields.Select(static x => $"{x.Key}:{x.Value}"))})"
                );
            }

            transaction.AddToSet("recurring-jobs", recurringJob.RecurringJobId, score);
        }
