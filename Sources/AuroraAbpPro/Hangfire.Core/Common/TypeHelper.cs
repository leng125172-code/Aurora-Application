...
namespace Hangfire.Common
{
    ...
    {
        private static readonly ConcurrentDictionary<Type, string> DefaultTypeSerializerCache =
            new ConcurrentDictionary<Type, string>();
        private static readonly ConcurrentDictionary<
            Type,
            string
        > SimpleAssemblyTypeSerializerCache = new ConcurrentDictionary<Type, string>();

        private static readonly ConcurrentDictionary<string, Type> DefaultTypeResolverCache =
            new ConcurrentDictionary<string, Type>();
        private static readonly ConcurrentDictionary<
            string,
            Type
        > IgnoredAssemblyVersionTypeResolverCache = new ConcurrentDictionary<string, Type>();

        private static readonly Assembly CoreLibrary = typeof(int).GetTypeInfo().Assembly;
        private static readonly AssemblyName MscorlibAssemblyName = new AssemblyName(
        ...
        private static readonly Regex PublicKeyTokenRegex = new Regex(
            @", PublicKeyToken=\w+",
            RegexOptions.Compiled,
            TimeSpan.FromSeconds(5)
        );
        ...
        {
            return SimpleAssemblyTypeSerializerCache.GetOrAdd(
                type,
                static value =>
                {
        ...
        {
            return DefaultTypeResolverCache.GetOrAdd(
                typeName,
                static name =>
                {
#if NETSTANDARD1_3
        ...
        {
            return IgnoredAssemblyVersionTypeResolverCache.GetOrAdd(
                typeName,
                static value =>
                {
        ...
        private static void SerializeType(
            Type type,
        ...
        )
        {
            ...
            if (type != typeof(object) && type != typeof(string) && !typeInfo.IsPrimitive)
            {
                ...
                if (typeForwardedFrom != null)
                {
                    ...
                    var delimiterIndex = assemblyName.IndexOf(
                        ",",
                    ...
                    assemblyName =
                        delimiterIndex >= 0
                            ? assemblyName.Substring(0, delimiterIndex)
                            : assemblyName;
                }
                else
                {
        ...
        private static Assembly AssemblyResolver(string assemblyString)
        {
            ...
            if (
                assemblyName.Name.Equals("System.Console", StringComparison.OrdinalIgnoreCase)
