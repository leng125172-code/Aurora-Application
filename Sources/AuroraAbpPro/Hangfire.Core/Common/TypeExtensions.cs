...
namespace Hangfire.Common
{
    ...
    {
        private static readonly Regex GenericArgumentsRegex = new Regex(
            @"`[1-9]\d*",
        ...
        private static bool TypesMatchRecursive(
        ...
        )
        {
            if (parameterType.IsGenericParameter)
            {
                ...
                if (
                    genericArguments[position] != null
                    && genericArguments[position].GetTypeInfo() != actualType
                )
                {
                    return false;
                }
            ...
            if (parameterType.ContainsGenericParameters)
            {
                if (parameterType.IsArray)
                {
                    ...
                    return TypesMatchRecursive(
                        parameterElementType.GetTypeInfo(),
                        actualElementType.GetTypeInfo(),
                        genericArguments
                    );
                }
                ...
                for (var i = 0; i < parameterType.GenericTypeArguments.Length; i++)
                {
                    ...
                    if (
                        !TypesMatchRecursive(
                            parameterGenericArgument.GetTypeInfo(),
                            actualGenericArgument.GetTypeInfo(),
                            genericArguments
                        )
                    )
                    {
        ...
        )
        {
            ...
            typeName = GenericArgumentsRegex.Replace(
                typeName,
                match =>
                {
                    var currentGenericArgumentNumbers = int.Parse(
                    ...
                    var currentArguments = string.Join(
                        ",",
                        genericArguments
                            .Take(currentGenericArgumentNumbers)
                            .Select(ToGenericTypeString)
                    );
                    genericArguments = genericArguments
                        .Skip(currentGenericArgumentNumbers)
                        .ToArray();
