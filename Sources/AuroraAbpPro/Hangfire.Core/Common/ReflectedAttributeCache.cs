...
namespace Hangfire.Common
{
    internal static class ReflectedAttributeCache
    {
        private static readonly ConcurrentDictionary<
            TypeInfo,
            ReadOnlyCollection<JobFilterAttribute>
        > TypeFilterAttributeCache =
            new ConcurrentDictionary<TypeInfo, ReadOnlyCollection<JobFilterAttribute>>();

        private static readonly ConcurrentDictionary<
            MethodInfo,
            ReadOnlyCollection<JobFilterAttribute>
        > MethodFilterAttributeCache =
        ...
        public static ICollection<JobFilterAttribute> GetMethodFilterAttributes(
            ...
            where TMemberInfo : MemberInfo
        {
            ...
            return lookup.GetOrAdd(
                memberInfo,
                static mi => new ReadOnlyCollection<TAttribute>(
                    mi.GetCustomAttributes<TAttribute>(inherit: true).ToArray()
                )
            );
        }
    }
