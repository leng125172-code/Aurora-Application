...
namespace Hangfire
{
    internal sealed class RecurringJobEntity
    {
        ...
        )
        {
            ...
            RecurringJobId =
                recurringJobId ?? throw new ArgumentNullException(nameof(recurringJobId));

            if (
                recurringJob.TryGetValue("Queue", out var queue)
            ...
            if (
                recurringJob.TryGetValue("LastJobId", out var lastJobId)
            ...
            if (
                recurringJob.TryGetValue("LastExecution", out var lastExecution)
                && !String.IsNullOrWhiteSpace(lastExecution)
            )
            {
            ...
            if (
                recurringJob.TryGetValue("NextExecution", out var nextExecution)
                && !String.IsNullOrWhiteSpace(nextExecution)
            )
            {
            ...
            if (
                recurringJob.TryGetValue("CreatedAt", out var createdAt)
            ...
            if (recurringJob.TryGetValue("Misfire", out var misfireStr))
            {
                MisfireHandling = (MisfireHandlingMode)
                    Enum.Parse(typeof(MisfireHandlingMode), misfireStr);
            ...
            if (
                recurringJob.TryGetValue("V", out var version)
            ...
            if (
                recurringJob.TryGetValue("RetryAttempt", out var attemptString)
            ...
            if (
                recurringJob.TryGetValue("Error", out var error)
        ...
        private IReadOnlyDictionary<string, string> GetChangedFields(DateTime now)
        {
            ...
            if (
                (_recurringJob.TryGetValue("TimeZoneId", out var timeZoneId) ? timeZoneId : null)
                != TimeZoneId
            )
            {
            ...
            var serializedLastExecution = LastExecution.HasValue
                ? JobHelper.SerializeDateTime(LastExecution.Value)
                : null;

            if (
                (
                    _recurringJob.TryGetValue("LastExecution", out var lastExecution)
                        ? lastExecution
                        : null
            ...
            var serializedNextExecution = NextExecution.HasValue
            ...
            if (
                (
                    _recurringJob.TryGetValue("LastJobId", out var last)
            ...
            if (
                (
                    !_recurringJob.ContainsKey("Misfire")
                    && MisfireHandling != MisfireHandlingMode.Relaxed
                )
                || (
                    _recurringJob.TryGetValue("Misfire", out var misfire)
                    && misfire != misfireHandlingValue
                )
            )
            {
            ...
            if (
                (
                    _recurringJob.TryGetValue("Error", out var error)
                    && !String.IsNullOrWhiteSpace(error)
                        ? error
                        : null
                ) != Error
            ...
            if (
                (
                    _recurringJob.TryGetValue("RetryAttempt", out var retryAttempt)
                        ? retryAttempt
                        : null
                ) != retryAttemptValue
            )
