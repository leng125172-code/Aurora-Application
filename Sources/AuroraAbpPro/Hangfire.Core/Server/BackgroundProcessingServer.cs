...
namespace Hangfire.Server
{
    ...
    public sealed class BackgroundProcessingServer : IBackgroundProcessingServer
    {
        ...
        public BackgroundProcessingServer(
            ...
            : this(
                new BackgroundServerProcess(storage, dispatcherBuilders, options, properties),
                options
            ) { }
        ...
        )
        {
            ...
            _shutdownRegistration = AspNetShutdownDetector
                .GetShutdownToken()
                .Register(OnAspNetShutdown);
        }
        ...
        private static IBackgroundProcessDispatcherBuilder[] GetProcesses(
            [NotNull] IEnumerable<IBackgroundProcess> processes
        )
        {
            if (processes == null)
        ...
        private IBackgroundDispatcher CreateDispatcher()
        {
            ...
            return new BackgroundDispatcher(execution, RunServer, execution, ThreadFactory);
        }
        ...
        private static IEnumerable<Thread> ThreadFactory(ThreadStart threadStart)
        {
            yield return new Thread(threadStart)
            {
                IsBackground = true,
                Name =
                    $"{nameof(BackgroundServerProcess)} #{Interlocked.Increment(ref _lastThreadId)}",
            };
        }
