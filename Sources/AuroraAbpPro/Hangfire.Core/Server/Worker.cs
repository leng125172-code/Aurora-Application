...
namespace Hangfire.Server
{
    ...
    public class Worker : IBackgroundProcess
    {
        ...
        public static readonly string TransactionalAcknowledgePrefix =
            JobStorageFeatures.TransactionalAcknowledgePrefix;

        private static readonly ConcurrentDictionary<Guid, string> WorkerGuidCache = new();
        private static readonly string[] EligibleWorkerStates = new[]
        {
        ...
        public Worker()
            : this(EnqueuedState.DefaultQueue) { }
        ...
        public Worker(
            ...
            : this(
                ...
                performer,
                stateChanger,
                jobInitializationTimeout: TimeSpan.FromMinutes(1),
                maxStateChangeAttempts: 10
            ) { }
        ...
        public void Execute(BackgroundProcessContext context)
        {
            ...
            using (
                var fetchedJob = connection.FetchNextJob(_queues.ToArray(), context.StoppingToken)
            )
            {
                ...
                try
                {
                    ...
                    using (
                        var timeoutCts = new CancellationTokenSource(_jobInitializationWaitTimeout)
                    )
                    using (
                    ...
                    )
                    {
                        var processingState = new ProcessingState(
                            context.ServerId,
                            WorkerGuidCache.GetOrAdd(
                                context.ExecutionId,
                                static guid => guid.ToString()
                            )
                        );
                    ...
                    var transactionalAck = context.Storage.HasFeature(
                        JobStorageFeatures.Transaction.RemoveFromQueue(fetchedJob.GetType())
                    );
                ...
                catch (Exception ex) when (ex.IsCatchableExceptionType())
                {
                    ...
                    {
                        var action = requeueOnException
                            ? "It will be re-queued"
                            : "It will be removed from queue later";
                        _logger.Warn(
                            $"Worker stop requested while processing background job '{fetchedJob.JobId}'. {action}."
                        );
                    }
        ...
        )
        {
            ...
            var failedStateContext = new StateChangeContext(
                ...
                {
                    Reason =
                        $"Failed to change state to a '{state.Name}' one due to an exception after {_maxStateChangeAttempts} retry attempts",
                },
                expectedStates,
                disableFilters: true,
                completeJob,
        ...
        private void Requeue(IFetchedJob fetchedJob)
        {
            ...
            catch (Exception ex) when (ex.IsCatchableExceptionType())
            {
                _logger.WarnException(
                    $"Failed to immediately re-queue the background job '{fetchedJob.JobId}'. Next invocation may be delayed, if invisibility timeout is used",
                    ex
                );
            }
        }
        ...
        )
        {
            ...
            try
            {
                ...
                using (
                    var jobToken = new ServerJobCancellationToken(
                        connection,
                        backgroundJob.Id,
                        context.ServerId,
                        WorkerGuidCache.GetOrAdd(
                ...
                )
                {
                    ...
                    return !performContext.Items.TryGetValue(
                        BackgroundJobPerformer.ContextCanceledKey,
                        out var filter
                    )
                        ? (IState)
                            new SucceededState(result, (long)latency, duration.ElapsedMilliseconds)
            ...
            {
                return new FailedState(ex.InnerException, context.ServerId) { Reason = ex.Message };
            }
            catch (Exception ex) when (ex.IsCatchableExceptionType())
            {
