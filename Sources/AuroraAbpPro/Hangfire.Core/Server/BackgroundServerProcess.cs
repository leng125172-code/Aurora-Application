...
namespace Hangfire.Server
{
    internal sealed class BackgroundServerProcess : IBackgroundServerProcess
    {
        ...
        )
        {
            ...
            void HandleStopRestartSignal() =>
                Interlocked.CompareExchange(ref stoppedAt, Stopwatch.StartNew(), null);
            void HandleStoppingSignal() =>
                _logger.Info($"{GetServerTemplate(serverId)} caught stopping signal...");
            void HandleStoppedSignal() =>
                _logger.Info($"{GetServerTemplate(serverId)} caught stopped signal...");
            void HandleShutdownSignal() =>
                _logger.Warn($"{GetServerTemplate(serverId)} caught shutdown signal...");
            ...
            using (
                var restartStoppingCts = CancellationTokenSource.CreateLinkedTokenSource(
                    stoppingToken,
                    restartCts.Token
                )
            )
            using (
                var restartStoppedCts = CancellationTokenSource.CreateLinkedTokenSource(
                    stoppedToken,
                    restartCts.Token
                )
            )
            using (
                var restartShutdownCts = CancellationTokenSource.CreateLinkedTokenSource(
                    shutdownToken,
                    restartCts.Token
                )
            )
            ...
            using (restartCts.Token.Register(HandleRestartSignal))
            {
                ...
                try
                {
                    ...
                    using (
                        var heartbeat = CreateHeartbeatProcess(context, () => restartCts.Cancel())
                    )
                    {
                        ...
                        // in 2.0.
                        heartbeat
                            .WaitAsync(Timeout.InfiniteTimeSpan, shutdownToken)
                            .GetAwaiter()
                            .GetResult();
                    }
                }
        ...
        private IBackgroundDispatcher CreateHeartbeatProcess(
        ...
        )
        {
            ...
            return new ServerHeartbeatProcess(
        ...
        {
            yield return new ServerWatchdog(
                _options.ServerCheckInterval,
                _options.ServerTimeout
            ).UseBackgroundPool(threadCount: 1);
            yield return new ServerJobCancellationWatcher(
                _options.CancellationCheckInterval
            ).UseBackgroundPool(threadCount: 1);
        ...
        private IEnumerable<IBackgroundProcessDispatcherBuilder> GetStorageComponents()
        {
            if (_storage.HasFeature(Storage.JobStorageFeatures.ProcessesInsteadOfComponents))
            {
                return _storage
                    ...
                    .Select(static process => process.UseBackgroundPool(threadCount: 1));
            }

#pragma warning disable CS0618 // Type or member is obsolete
            return _storage
                .GetComponents()
                .Select(static component => new ServerProcessDispatcherBuilder(
#pragma warning restore CS0618 // Type or member is obsolete
                    component,
                    threadStart =>
                        BackgroundProcessExtensions.DefaultThreadFactory(
                            1,
        ...
        private void CreateServer(BackgroundServerContext context)
        {
            ...
            _logger.Info(
                $"{GetServerTemplate(context.ServerId)} successfully announced in {stopwatch.Elapsed.TotalMilliseconds} ms"
            );
        }
        ...
        private void ServerDelete(BackgroundServerContext context, Stopwatch stoppedAt)
        {
            ...
            {
                _logger.Trace(
                    $"{GetServerTemplate(context.ServerId)} is reporting itself as stopped..."
                );
            ...
            catch (Exception ex) when (ex.IsCatchableExceptionType())
            {
                _logger.WarnException(
                    $"{GetServerTemplate(context.ServerId)} there was an exception, server may not be removed",
                    ex
                );
            }
        }
        ...
        private void StartDispatchers(
            BackgroundServerContext context,
            ICollection<IBackgroundDispatcher> dispatchers
        )
        {
            ...
            _logger.Info(
                $"{GetServerTemplate(context.ServerId)} is starting the registered dispatchers: {String.Join(", ", _dispatcherBuilders.Select(static builder => $"{builder}"))}..."
            );
