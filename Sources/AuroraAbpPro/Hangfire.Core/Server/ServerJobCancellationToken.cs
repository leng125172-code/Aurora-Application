...
namespace Hangfire.Server
{
    internal sealed class ServerJobCancellationToken : IJobCancellationToken, IDisposable
    {
        private static readonly ConcurrentDictionary<
            string,
            ConcurrentDictionary<ServerJobCancellationToken, object>
        > WatchedServers =
        ...
        [System.Diagnostics.CodeAnalysis.SuppressMessage(
            "SonarLint",
            "S4275:GettersAndSettersShouldAccessTheExpectedFields",
            Justification = "Bad property naming for backwards compatibility."
        )]
        ...
        public bool IsAborted
        {
            get
            {
                ...
                {
                    CheckDisposed();
                    return _cancellationTokenHolder.IsValueCreated
                        && _cancellationTokenHolder.Value.IsAborted;
                }
            }
        }
        ...
        public void ThrowIfCancellationRequested()
        {
            lock (_syncRoot)
            {
                ...
                if (
                    _cancellationTokenHolder.IsValueCreated
                    && _cancellationTokenHolder.Value.IsAborted
                )
                {
        ...
        {
            WatchedServers.TryAdd(
                serverId,
                new ConcurrentDictionary<ServerJobCancellationToken, object>()
            );
        }
        ...
        private bool IsJobStateChanged(IStorageConnection connection)
        {
            ...
            if (
                ...
                || !serverId.Equals(_serverId, StringComparison.OrdinalIgnoreCase)
            )
            {
                return true;
            }
            ...
            if (
                !state.Data.TryGetValue("WorkerId", out var workerId)
                || !workerId.Equals(_workerId, StringComparison.OrdinalIgnoreCase)
            )
            {
                return true;
            }
