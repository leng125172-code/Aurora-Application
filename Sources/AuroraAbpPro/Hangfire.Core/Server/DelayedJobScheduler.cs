...
namespace Hangfire.Server
{
    ...
    public class DelayedJobScheduler : IBackgroundProcess
    {
        ...
        private readonly ConcurrentDictionary<Type, bool> _isBatchingAvailableCache =
        ...
        internal Func<int, TimeSpan> RetryDelayFunc { get; set; } =
        ...
        {
            return UseConnectionDistributedLock(
                context.Storage,
                connection =>
                {
                    var jobsProcessed = 0;

                    var now = !context.Storage.HasFeature(
                    ...
                    if (IsBatchingAvailable(context.Storage, connection))
                    {
                        ...
                        ).GetFirstByLowestScoreFromSet("schedule", 0, timestamp, BatchSize);
                        ...
                        if (entries != null)
                        {
                            foreach (var entry in entries)
                            {
                                ...
                                if (colonIndex < 0)
                                    toBeSequentiallyEnqueued.Add(entry);
                                else
                                    toBeTransactionallyEnqueued.Add(
                                        Tuple.Create(entry, colonIndex)
                                    );
                    ...
                    else
                    {
                        ...
                        {
                            _logger.Warn(
                                "Parallel execution is configured but can't be used, because current storage implementation doesn't support batching."
                            );
                        ...
                        for (var i = 0; i < BatchSize; i++)
                        {
                            ...
                            var entry = connection.GetFirstByLowestScoreFromSet(
                                "schedule",
                                0,
                                timestamp
                            );
        ...
        {
            if (colonIndex < 0)
                throw new ArgumentOutOfRangeException(nameof(colonIndex));
        ...
        )
        {
            ...
            for (var retryAttempt = 0; retryAttempt < maxRetryAttempts; retryAttempt++)
            {
                try
                {
                    var appliedState = _stateChanger.ChangeState(
                    ...
                    {
                        _logger.Debug(
                            $"Failed to change state of a scheduled background job '{jobId}'"
                        );
                        ...
                        using (connection.AcquireDistributedJobLock(jobId, TimeSpan.FromSeconds(5)))
                        {
                            ...
                            if (
                                jobData == null
                                || !ScheduledState.StateName.Equals(
                                    jobData.State,
                            ...
                            )
                            {
                                ...
                                _logger.Warn(
                                    $"Background job '{jobId}' removed from the schedule, because it's expired or its state was changed"
                                );
                            }
                        }
                    }
            ...
            _stateChanger.ChangeState(
                new StateChangeContext(
                    ...
                    {
                        Reason =
                            $"Failed to change state to the '{EnqueuedState.StateName}' one due to an exception after {MaxStateChangeAttempts} retry attempts",
                    },
                    new[] { ScheduledState.StateName },
                    disableFilters: true,
                    context.StoppingToken,
        ...
        {
            if (
                storage.HasFeature(JobStorageFeatures.Connection.BatchedGetFirstByLowest)
                || storage.HasFeature("BatchedGetFirstByLowestScoreFromSet")
            ) // FROM RCs
            {
                return true;
            }
        ...
        private T UseConnectionDistributedLock<T>(
            JobStorage storage,
        ...
        )
        {
            ...
            try
            {
                ...
                when (e.Resource.EndsWith(resource, StringComparison.Ordinal))
            {
