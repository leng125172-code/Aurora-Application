...
namespace Hangfire.Server
{
    ...
    {
        private static readonly TimeSpan CheckForShutdownTimerInterval = TimeSpan.FromMilliseconds(
            250
        );

        [SuppressMessage(
            "SonarLint",
            "S2930:IDisposablesShouldBeDisposed",
            Justification = "Has static lifetime, disposed on process shutdown."
        )]
        ...
        private static void EnsureInitialized()
        {
            ...
            catch (Exception ex) when (ex.IsCatchableExceptionType())
            {
                GetLogger()
                    .ErrorException(
                        "Failed to initialize shutdown triggers for ASP.NET application.",
                        ex
                    );
            }
        }
        ...
        private static void Cancel(string reason)
        {
            ...
            catch (AggregateException ag)
            {
                GetLogger()
                    .ErrorException(
                        "One or more exceptions were thrown during app pool shutdown: ",
                        ag
                    );
            }
        }
        ...
        private static void RegisterForStopListeningEvent(ref bool success)
        {
            try
            {
                ...
                var stopEvent = hostingEnvironmentType.GetEvent(
                    "StopListening",
                ...
                GetLogger()
                    .Debug(
                        "HostingEnvironment.StopListening shutdown trigger initialized successfully."
                    );
                success = true;
            }
            catch (Exception ex) when (ex.IsCatchableExceptionType())
            {
                GetLogger()
                    .DebugException(
                        "Unable to initialize HostingEnvironment.StopListening shutdown trigger",
                        ex
                    );
            }
        }
        ...
        private static void InitializeShutdownReason(ref bool success)
        {
            try
            {
                var hostingEnvironment = Type.GetType(
                    "System.Web.Hosting.HostingEnvironment, System.Web, Version=4.0.0.0, Culture=neutral, PublicKeyToken=b03f5f7f11d50a3a",
                    false
                );
                ...
                var shutdownReason = hostingEnvironment.GetProperty(
                    "ShutdownReason",
                    BindingFlags.Static | BindingFlags.Public
                );
                if (shutdownReason == null)
                ...
                GetLogger()
                    .Debug(
                        "HostingEnvironment.ShutdownReason shutdown trigger initialized successfully."
                    );
                success = true;
                ...
                string ShutdownReasonFunc()
                {
                    ...
                    catch (Exception ex) when (ex.IsCatchableExceptionType())
                    {
                        GetLogger()
                            .TraceException(
                                "Unable to call the HostingEnvironment.ShutdownReason property due to an exception.",
                                ex
                            );
                    }
            ...
            catch (Exception ex) when (ex.IsCatchableExceptionType())
            {
                GetLogger()
                    .DebugException(
                        "Unable to initialize HostingEnvironment.ShutdownReason shutdown trigger",
                        ex
                    );
            }
        }
        ...
        private static void InitializeMgdHasConfigChanged(ref bool success)
        {
            try
            {
                var type = Type.GetType(
                    "System.Web.Hosting.UnsafeIISMethods, System.Web, Version=4.0.0.0, Culture=neutral, PublicKeyToken=b03f5f7f11d50a3a",
                    false
                );
                if (type == null)
                    return;
                ...
                _checkConfigChangedFunc =
                    (Func<bool>)Delegate.CreateDelegate(typeof(Func<bool>), methodInfo);

                GetLogger()
                    .Debug(
                        "UnsafeIISMethods.MgdHasConfigChanged shutdown trigger initialized successfully."
                    );
                success = true;
            }
            catch (Exception ex) when (ex.IsCatchableExceptionType())
            {
                GetLogger()
                    .DebugException(
                        "Unable to initialize UnsafeIISMethods.MgdHasConfigChanged shutdown trigger",
                        ex
                    );
            }
        }
        ...
        private static void InitializeDisposingHttpRuntime(ref bool success)
        {
            ...
            {
                var type = Type.GetType(
                    "System.Web.HttpRuntime, System.Web, Version=4.0.0.0, Culture=neutral, PublicKeyToken=b03f5f7f11d50a3a",
                    false
                );
                if (type == null)
                    return;

                var theRuntimeInfo = type.GetField(
                    "_theRuntime",
                    BindingFlags.NonPublic | BindingFlags.Static
                );
                if (theRuntimeInfo == null)
                    return;

                var disposingHttpRuntimeInfo = type.GetField(
                    "_disposingHttpRuntime",
                    BindingFlags.NonPublic | BindingFlags.Instance
                );
                if (disposingHttpRuntimeInfo == null)
                    return;

                var theRuntime = CreateGetStaticFieldDelegate<object>(theRuntimeInfo);
                var disposingHttpRuntime = CreateGetFieldDelegate<bool>(
                    disposingHttpRuntimeInfo,
                    type
                );
                ...
                GetLogger()
                    .Debug(
                        "HttpRuntime._disposingHttpRuntime shutdown trigger initialized successfully."
                    );
                success = true;
            }
            catch (Exception ex)
            {
                GetLogger()
                    .DebugException(
                        "Unable to initialize HttpRuntime._disposingHttpRuntime shutdown trigger",
                        ex
                    );
            }
        }
