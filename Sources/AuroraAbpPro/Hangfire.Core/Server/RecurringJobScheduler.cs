...
namespace Hangfire.Server
{
    ...
    public class RecurringJobScheduler : IBackgroundProcess
    {
        ...
        /// <exception cref="ArgumentNullException"><paramref name="factory"/> is null.</exception>
        public RecurringJobScheduler([NotNull] IBackgroundJobFactory factory, TimeSpan pollingDelay)
        ...
        public void Execute(BackgroundProcessContext context)
        {
            ...
            else
            {
                var now = _nowFactory();
                context.Wait(
                    now.AddMilliseconds(-now.Millisecond).AddSeconds(-now.Second).AddMinutes(1)
                        - now
                );
            }
        }
            ...
            return UseConnectionDistributedLock(
                ...
                connection =>
                {
                    ...
                    if (IsBatchingAvailable(context.Storage, connection))
                    {
                        ...
                        ).GetFirstByLowestScoreFromSet("recurring-jobs", 0, timestamp, BatchSize);
                    ...
                    else
                    {
                        ...
                        for (var i = 0; i < BatchSize; i++)
                        {
                            ...
                            var recurringJobId = connection.GetFirstByLowestScoreFromSet(
                                "recurring-jobs",
                                0,
                                timestamp
                            );
        ...
        private void ScheduleRecurringJob(
            ...
            IStorageConnection connection,
            string recurringJobId,
            RecurringJobEntity recurringJob,
            DateTime now
        )
        {
                ...
                try
                {
                    ...
                    var executions = recurringJob.ScheduleNext(
                        ...
                        recurringJob.LastExecution
                            ?? recurringJob.CreatedAt?.AddSeconds(-1)
                            ?? now.AddSeconds(-1),
                        now,
                        precision
                    );
                    ...
                    foreach (var execution in executions)
                    {
                        ...
                        {
                            _logger.Debug(
                                $"Recurring job '{recurringJobId}' execution at '{execution}' has been canceled."
                            );
                        }
                    }
        ...
        )
        {
            var errorString = error.ToStringWithOriginalStackTrace(
                States.FailedState.MaxLinesInExceptionDetails,
            ...
            else
            {
                _logger.ErrorException(
                    $"Recurring job '{recurringJobId}' can't be scheduled due to an error and will be disabled.",
                    error
                );
        ...
        {
            _logger.Debug(
                $"Recurring job '{recurringJobId}' doesn't exist and will be removed from schedule."
            );
