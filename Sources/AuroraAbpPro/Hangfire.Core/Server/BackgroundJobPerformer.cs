    ...
    public class BackgroundJobPerformer : IBackgroundJobPerformer
    {
        ...
        public object Perform(PerformContext context)
        {
            ...
            catch (Exception ex) when (ex.IsCatchableExceptionType())
            {
                ...
                InvokeServerExceptionFilters(
                    exceptionContext,
                    filterInfo.ServerExceptionFiltersReversed
                );
        ...
        private object PerformJobWithFilters(
            ...
            JobFilterInfo.FilterCollection<IServerFilter> filters
        )
        {
            var preContext = new PerformingContext(context);
            var enumerator = filters.GetEnumerator();

            return InvokeNextServerFilter(
                ref enumerator,
                _innerPerformer,
                context,
                preContext
        ...
        )
        {
            ...
            try
            {
                postContext = InvokeNextServerFilter(
                    ref enumerator,
                    innerPerformer,
                    context,
                    preContext
            ...
            {
                wasError = true;
                postContext = new PerformedContext(preContext, null, false, ex);
        ...
        private static void InvokeOnPerforming(KeyValuePair<IServerFilter, PerformingContext> ctx)
        {
            ...
            catch (Exception filterException) when (filterException.IsCatchableExceptionType())
            {
                CoreBackgroundJobPerformer.HandleJobPerformanceException(
                    filterException,
                    ctx.Value.CancellationToken,
                    ctx.Value.BackgroundJob
                );
            }
        }
        ...
        )
        {
            foreach (var filter in filters)
            {
                context.Profiler.InvokeMeasured(
                    new KeyValuePair<IServerExceptionFilter, ServerExceptionContext>(
                        filter,
                        context
                    ),
        ...
        private static void InvokeOnServerException(
            KeyValuePair<IServerExceptionFilter, ServerExceptionContext> ctx
        )
        {
            try
            {
