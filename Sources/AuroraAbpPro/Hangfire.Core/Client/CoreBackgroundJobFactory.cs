...
namespace Hangfire.Client
{
    internal sealed class CoreBackgroundJobFactory : IBackgroundJobFactory
    {
        ...
        public BackgroundJob Create(CreateContext context)
        {
            ...
            if (
                context.Job.Queue != null
                && !context.Storage.HasFeature(JobStorageFeatures.JobQueueProperty)
            )
            {
        ...
        private BackgroundJob CreateBackgroundJobTwoSteps(
        ...
        )
        {
            ...
            var jobId = RetryOnException(
                ref attemptsLeft,
                static (_, ctx) =>
                    ctx.Context.Connection.CreateExpiredJob(
                        ctx.Context.Job,
                        ctx.Parameters,
                        ctx.CreatedAt,
                        ctx.ExpireIn
                    ),
                new JobCreateContext
                {
                    Context = context,
                        ...
                        if (attempt > 0)
                        {
                            ...
                            if (data == null)
                                throw new InvalidOperationException(
                                    $"Was unable to initialize a background job '{ctx.BackgroundJob.Id}', because it doesn't exists."
                                );
        ...
        private void RetryOnException<TContext>(
            ref int attemptsLeft,
            Action<int, TContext> action,
            TContext context
        )
        {
            RetryOnException(
        ...
        private TResult RetryOnException<TContext, TResult>(
            ref int attemptsLeft,
            Func<int, TContext, TResult> action,
            TContext context
        )
        {
