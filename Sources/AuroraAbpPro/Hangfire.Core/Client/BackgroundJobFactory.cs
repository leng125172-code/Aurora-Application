    ...
    public class BackgroundJobFactory : IBackgroundJobFactory
    {
        ...
        )
        {
            ...
            preContext.Profiler.InvokeMeasured(
                ...
                InvokeOnCreating,
                static ctx =>
                    $"OnCreating for {ctx.Value.Job.Type.FullName}.{ctx.Value.Job.Method.Name}"
            );
            ...
            try
            {
                postContext = InvokeNextClientFilter(
                    ref enumerator,
                    innerFactory,
                    context,
                    preContext
        ...
        private static void InvokeExceptionFilters(
            ClientExceptionContext context,
            JobFilterInfo.ReversedFilterCollection<IClientExceptionFilter> filters
        )
        {
            foreach (var filter in filters)
            {
                context.Profiler.InvokeMeasured(
                    new KeyValuePair<IClientExceptionFilter, ClientExceptionContext>(
                        filter,
                        context
                    ),
                    InvokeOnClientException,
                    static ctx =>
                        $"OnClientException for {ctx.Value.Job.Type.FullName}.{ctx.Value.Job.Method.Name}"
                );
            }
        }

        private static void InvokeOnClientException(
            KeyValuePair<IClientExceptionFilter, ClientExceptionContext> x
        )
        {
            try
            {
