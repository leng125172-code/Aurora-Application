...
namespace Hangfire.Processing
{
    ...
    public sealed class BackgroundTaskScheduler : TaskScheduler, IDisposable
    {
        ...
        public BackgroundTaskScheduler(int threadCount)
            : this(
                threadStart => DefaultThreadFactory(threadStart, threadCount),
        ...
        )
        {
            ...
            if (_threads == null || _threads.Length == 0)
            {
                throw new ArgumentException(
                    ...
                    nameof(threadFactory)
                );
            }

            if (
                _threads.Any(static thread =>
                    thread == null || (thread.ThreadState & ThreadState.Unstarted) == 0
                )
            )
            {
                throw new ArgumentException(
        ...
        protected override bool TryExecuteTaskInline(Task task, bool taskWasPreviouslyQueued)
        {
            ...
            if (!_ourThreadIds.Contains(Environment.CurrentManagedThreadId))
                return false;
        ...
        private static IEnumerable<Thread> DefaultThreadFactory(
            ...
            try
            {
                ...
                while (Volatile.Read(ref _disposed) == 0)
                {
                    ...
                    catch (ObjectDisposedException)
                    {
                        ...
                        when (ex is ThreadAbortException || ex is ThreadInterruptedException)
                    {
        ...
        private void InvokeUnhandledExceptionHandler(Exception exception)
        {
            ...
            {
                Trace.WriteLine(
                    "Unexpected exception caught in exception handler itself."
                        + Environment.NewLine
                        + ex
                );
            }
