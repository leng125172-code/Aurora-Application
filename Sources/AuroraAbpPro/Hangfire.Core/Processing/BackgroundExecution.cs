...
namespace Hangfire.Processing
{
    internal sealed class BackgroundExecution : IBackgroundExecution
    {
...
#endif
                {
                    ...
                    while (true)
                    {
                        ...
                        catch (ThreadAbortException) when (AppDomainUnloadMonitor.IsUnloading)
                        {
                            ...
                            when (ex.CancellationToken.Equals(_stopToken) || StopRequested)
                        {
...
#endif
                {
                    ...
                    while (true)
                    {
                        ...
                        try
                        {
                            ...
                            {
                                if (
                                    !await HandleDelayAsync(executionId, nextDelay)
                                        .ConfigureAwait(true)
                                )
                                {
                                    // Inability to handle the delay means that execution was
        ...
        {
            _logger.Debug(
                $"{GetExecutionLoopTemplate(executionId)} has started in {_createdAt.Elapsed.TotalMilliseconds} ms"
            );
        ...
        private void LogUnableWait(Guid executionId, TimeSpan delay, Exception ex)
        {
            _logger.FatalException(
                $"{GetExecutionLoopTemplate(executionId)} was unable to wait for '{delay}' delay due to an exception. Execution will be stopped.",
                ex
            );
        }
        ...
        private void NormalizeDelay(ref TimeSpan retryDelay)
        {
            ...
            {
                _logger.Warn(
                    $"{GetExecutionTemplate()} adjusted the retry delay from {retryDelay} to {FallbackRetryDelay}"
                );
        ...
        private void HandleException(Guid executionId, Exception exception, out TimeSpan delay)
        {
            ...
            )
            {
                ...
                catch (Exception ex) when (ex.IsCatchableExceptionType())
                {
                    ...
                    _logger.ErrorException(
                        $"{GetExecutionLoopTemplate(executionId)} was unable to reset thread abort request due to an exception. Background execution can be prematurely stopped.",
                        ex
                    );
                }
            }
            ...
            catch (Exception ex) when (ex.IsCatchableExceptionType())
            {
                _logger.WarnException(
                    $"Was unable to add the ExecutionId property to the exception object, please see inner exception for details. Original exception: ${exception.GetType()} (${exception.Message})",
                    ex
                );
            }
            ...
            _logger.DebugException(
                $"{GetExecutionLoopTemplate(executionId)} caught an exception and will be retried in {delay}",
                exception
            );
        }
        ...
        private void HandleStop(Guid executionId)
        {
            ...
            _logger.Debug(
                $"{GetExecutionLoopTemplate(executionId)} stopped in {stoppedAt?.Elapsed.TotalMilliseconds ?? 0} ms"
            );
        }
        ...
        {
            _logger.WarnException(
                $"{GetExecutionLoopTemplate(executionId)} caught ThreadAbortException, see inner exception for details",
                exception
            );
        }
#endif
        ...
        private void ToRunningState()
        {
            lock (_stopped)
            {
                ...
                if (_failedSince != null)
                {
                    ...
                    _logger.Info(
                        $"{GetExecutionTemplate()} recovered from the Failed state after {_failedSince?.Elapsed} and is in the Running state now"
                    );
                }
                else if (_faultedSince != null)
                {
                    ...
                    _logger.Log(
                        _faultedSince.Elapsed > _options.WarningThreshold
                            ? LogLevel.Info
                            : LogLevel.Debug,
                        () =>
        ...
        private void ToFailedState(Exception exception, out TimeSpan retryDelay)
        {
            lock (_stopped)
            {
                ...
                if (_faultedSince == null)
                {
                    ...
                    {
                        _logger.DebugException(
                            $"{GetExecutionTemplate()} is in the Faulted state now due to an exception, execution will be retried in no more than {retryDelay}",
                            exception
                        );
                    }
                }
                ...
                if (_failedSince == null && _faultedSince.Elapsed > _options.ErrorThreshold)
                {
                    ...
                    _logger.ErrorException(
                        $"{GetExecutionTemplate()} is in the Failed state now due to an exception, execution will be retried in no more than {retryDelay}",
                        exception
                    );
                    ...
                    _failedSince != null
                    && _lastException.Elapsed >= _options.StillErrorThreshold
                )
                {
                    ...
                    _logger.ErrorException(
                        $"{GetExecutionTemplate()} is still in the Failed state for {_failedSince?.Elapsed} due to an exception, will be retried in no more than {retryDelay}",
                        exception
                    );
                }
            }
