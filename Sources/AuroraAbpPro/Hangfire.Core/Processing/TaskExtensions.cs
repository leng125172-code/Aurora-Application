    ...
    internal static class TaskExtensions
    {
        ...
        )
        {
            ...
            )
            {
                try
                {
                    ...
                    logger.Error(
                        $"Actual wait time for non-canceled token was '{stopwatch.Elapsed.TotalMilliseconds}' ms instead of '{timeout.TotalMilliseconds}' ms, wait result: {waitResult}, using protective wait. Please report this to Hangfire developers."
                    );
                }
                finally
                {
        ...
        )
        {
            ...
            if (token.CanBeCanceled)
            {
                token.Register(
                    Callback,
                    Tuple.Create(registration, tcs, token),
            ...
            if (!typeInfo.IsPrimitive)
            {
                ...
                )
                {
                    ...
                    if (
                        ...
                        && typeof(Task)
                            .GetTypeInfo()
                            .IsAssignableFrom(asTask.ReturnType.GetTypeInfo())
                    )
                    {
        ...
        public static object GetTaskLikeResult(
        ...
        )
        {
            ...
            if (getAwaiter == null || getAwaiter.IsStatic || !getAwaiter.IsPublic)
                return null;
            ...
            if (getResult == null || getResult.IsStatic || !getResult.IsPublic)
                return null;
        ...
        private static void Callback(object state)
        {
            ...
            var ctx =
                (Tuple<RegisteredWaitHandle, TaskCompletionSource<bool>, CancellationToken>)state;
        ...
        private static void TrySetCanceled<T>(
