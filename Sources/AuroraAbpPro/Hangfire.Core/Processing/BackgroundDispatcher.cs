...
namespace Hangfire.Processing
{
    internal sealed class BackgroundDispatcher : IBackgroundDispatcher
    {
        ...
        {
            await _stopped
                .WaitHandle.WaitOneAsync(timeout, cancellationToken)
...
#if !NETSTANDARD1_3
                        Debug.WriteLine(
                            $"Dispatcher is stopped due to an exception, you need to restart the server manually. Please report it to Hangfire developers: {ex}"
                        );
#endif
                    }
                }
