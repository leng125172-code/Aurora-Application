...
#pragma warning restore 618
    {
        ...
        public ElectStateContext(
            [NotNull] ApplyStateContext applyContext,
            [CanBeNull] StateMachine stateMachine
        )
        {
            if (applyContext == null)
                throw new ArgumentNullException(nameof(applyContext));
            ...
            CustomData = applyContext.CustomData?.ToDictionary(
                static x => x.Key,
                static x => x.Value
            );
            ...
            set
            {
                if (value == null)
                {
                    throw new ArgumentNullException(
                        nameof(value),
                        "The CandidateState property can not be set to null."
                    );
                }
        ...
        public T GetJobParameter<T>([NotNull] string name) =>
            GetJobParameter<T>(name, allowStale: false);
