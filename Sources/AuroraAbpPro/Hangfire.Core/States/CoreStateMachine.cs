...
namespace Hangfire.States
{
    internal sealed class CoreStateMachine : IStateMachine
    {
        ...
        internal CoreStateMachine(
            [NotNull] Func<JobStorage, string, StateHandlersCollection> stateHandlersThunk
        )
        {
            if (stateHandlersThunk == null)
                throw new ArgumentNullException(nameof(stateHandlersThunk));
        ...
        public IState ApplyState(ApplyStateContext context)
        {
            ...
            {
                context.Transaction.ExpireJob(
                    context.BackgroundJob.Id,
                    context.JobExpirationTimeout
                );
            }
            else
            {
        ...
        private static StateHandlersCollection GetStateHandlers(
            JobStorage storage,
            string stateName
        )
        {
        ...
        )
        {
            public Enumerator GetEnumerator() =>
                new Enumerator(globalHandlers, storageHandlers, stateName);
            ...
            public ref struct Enumerator
            {
                ...
                public bool MoveNext()
                {
                    while (_globalEnumerator.MoveNext())
                    {
                        ...
                        if (
                            current.StateName.Equals(_stateName, StringComparison.OrdinalIgnoreCase)
                        )
                        {
                            _current = current;
                            return true;
