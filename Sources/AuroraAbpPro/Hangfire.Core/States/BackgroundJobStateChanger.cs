...
namespace Hangfire.States
{
    public class BackgroundJobStateChanger : IBackgroundJobStateChanger
    {
        ...
        internal BackgroundJobStateChanger(
        ...
        public IState ChangeState(StateChangeContext context)
        {
            ...
            {
                context.Transaction.AcquireDistributedJobLock(
            ...
            {
                distributedLock = context.Connection.AcquireDistributedJobLock(
                    context.BackgroundJobId,
                    JobLockTimeout
                );
            }
            ...
            using (distributedLock)
            {
                ...
                if (
                    context.ExpectedStates != null
                    && !context.ExpectedStates.Contains(
                        jobData.State,
                ...
                catch (JobLoadException ex)
                {
                    ...
                    if (!stateToApply.IgnoreJobLoadException)
                    {
                        ...
                        {
                            Reason =
                                $"Can not change the state to '{stateToApply.Name}': target method was not found.",
                        };
                    }
                }
                ...
                if (context.Transaction == null)
                {
                    disposableTransaction = transaction =
                        context.Connection.CreateWriteTransaction();
                }
                else
                {
                ...
                using (disposableTransaction)
                {
                    ...
                    // to the FailedState.
                    var stateMachine = context.DisableFilters
                        ? _stateMachine.InnerStateMachine
                        : _stateMachine;
                    ...
                    if (context.CompleteJob != null)
                    {
                        ...
                        else
                        {
                            throw new InvalidOperationException(
                                "Storage transaction class must inherit the "
                                    + nameof(JobStorageTransaction)
                                    + " class to use transactional acknowledge"
                            );
                        }
                    }
