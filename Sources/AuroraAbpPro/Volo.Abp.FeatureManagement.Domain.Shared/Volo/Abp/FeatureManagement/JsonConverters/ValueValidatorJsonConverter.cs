...
public class ValueValidatorJsonConverter : JsonConverter<IValueValidator>
{
    ...
    public override IValueValidator Read(
    ...
    )
    {
        ...
        var nameJsonProperty = rootElement
            .EnumerateObject()
            .FirstOrDefault(x =>
                x.Name.Equals(nameof(IValueValidator.Name), StringComparison.OrdinalIgnoreCase)
            );
        if (nameJsonProperty.Value.ValueKind == JsonValueKind.String)
        {
            ...
            var propertiesJsonProperty = rootElement
                .EnumerateObject()
                .FirstOrDefault(x =>
                    x.Name.Equals(
                        nameof(IValueValidator.Properties),
            ...
            if (propertiesJsonProperty.Value.ValueKind == JsonValueKind.Object)
            {
                ...
                var properties = propertiesJsonProperty.Value.Deserialize<
        ...
        throw new JsonException(
            $"Can't to get the {nameof(IValueValidator.Name)} property of {nameof(IValueValidator)}!"
        );
    }
    ...
    {
        foreach (
            var factory in Options.ValueValidatorFactory.Where(factory => factory.CanCreate(name))
        )
        {
        ...
        throw new ArgumentException(
            $"{nameof(IValueValidator)} named {name} was cannot be created!"
        );
    }
}
