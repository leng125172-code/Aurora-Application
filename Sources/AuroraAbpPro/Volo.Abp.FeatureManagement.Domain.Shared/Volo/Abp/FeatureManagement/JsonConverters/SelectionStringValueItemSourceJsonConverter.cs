...
public class SelectionStringValueItemSourceJsonConverter
    : JsonConverter<ISelectionStringValueItemSource>
{
    ...
    public override ISelectionStringValueItemSource Read(
    ...
    )
    {
        ...
        var itemsJsonProperty = rootElement
            .EnumerateObject()
            .FirstOrDefault(x =>
                x.Name.Equals(
                    nameof(ISelectionStringValueItemSource.Items),
        ...
        if (itemsJsonProperty.Value.ValueKind == JsonValueKind.Array)
        {
            ...
            var selectionStringValueItem =
                itemsJsonProperty.Value.Deserialize<LocalizableSelectionStringValueItem[]>(
                    _readJsonSerializerOptions
                ) ?? Array.Empty<LocalizableSelectionStringValueItem>();

            return new StaticSelectionStringValueItemSource(
                selectionStringValueItem.As<ISelectionStringValueItem[]>()
            );
        }

        throw new JsonException(
            $"Can't to get the {nameof(ISelectionStringValueItemSource.Items)} property of {nameof(ISelectionStringValueItemSource)}!"
        );
    }
