...
public class SettingManagementStore : ISettingManagementStore, ITransientDependency
{
    ...
    public virtual async Task<List<SettingValue>> GetListAsync(
        string providerName,
        string providerKey
    )
    {
    ...
    protected virtual async Task<SettingCacheItem> GetCacheItemAsync(
        string name,
        string providerName,
        string providerKey
    )
    ...
    [UnitOfWork]
    public async Task<List<SettingValue>> GetListAsync(
        string[] names,
        string providerName,
        string providerKey
    )
    ...
    protected virtual async Task<List<KeyValuePair<string, SettingCacheItem>>> GetCacheItemsAsync(
        string[] names,
        string providerName,
        string providerKey
    )
    ...
    )
    {
        var settingNames = new HashSet<string>(
            notCacheKeys.Select(GetSettingNameFormCacheKeyOrNull)
        );
        var settingDefinitions = (await SettingDefinitionManager.GetAllAsync()).Where(x =>
            settingNames.Contains(x.Name)
        );
