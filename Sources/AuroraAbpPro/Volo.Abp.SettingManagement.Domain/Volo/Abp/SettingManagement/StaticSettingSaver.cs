...
public class StaticSettingSaver : IStaticSettingSaver, ITransientDependency
{
    ...
    public async Task SaveAsync()
    {
        ...
        var settingRecords = await SettingSerializer.SerializeAsync(
            await StaticStore.GetAllAsync()
        );
        ...
        )
        {
            if (commonLockHandle == null)
            {
                ...
                throw new AbpException(
                    "Could not acquire distributed lock for saving static Settings!"
                );
            }
    ...
    private async Task<bool> UpdateChangedSettingsAsync(
    ...
    )
    {
        ...
        var settingRecordsInDatabase = (await SettingRepository.GetListAsync()).ToDictionary(x =>
            x.Name
        );
        ...
        if (SettingOptions.DeletedSettings.Any())
        {
            deletedRecords.AddRange(
                settingRecordsInDatabase.Values.Where(x =>
                    SettingOptions.DeletedSettings.Contains(x.Name)
                )
            );
        }
