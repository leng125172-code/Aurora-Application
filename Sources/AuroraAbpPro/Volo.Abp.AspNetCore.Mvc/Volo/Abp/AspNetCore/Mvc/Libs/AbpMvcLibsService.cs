...
public class AbpMvcLibsService : IAbpMvcLibsService, ITransientDependency
{
    ...
    {
        var options = context
            .ServiceProvider.GetRequiredService<IOptions<AbpMvcLibsOptions>>()
            .Value;
        if (options.CheckLibs)
        {
            ...
            if (app == null)
            {
                var logger = context.ServiceProvider.GetRequiredService<
                    ILogger<AbpMvcLibsService>
                >();
                logger.LogWarning(
                    $"The {nameof(IApplicationBuilder)} is not available. The 'CheckLibs' feature is disabled!"
                );
                return;
            }
            ...
            app.Use(
                ...
                {
                    if (
                        !httpContext.Request.IsAjax() && !await CheckLibsAsyncOnceAsync(httpContext)
                    )
                    {
        ...
        try
        {
            ...
            if (webHostEnvironment.WebRootPath.IsNullOrWhiteSpace())
            {
                logger.LogInformation(
                    "The 'WebRootPath' is not set, The 'CheckLibs' feature not needed."
                );
