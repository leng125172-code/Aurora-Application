...
public class AbpDateTimeModelBinder : IModelBinder
{
    ...
    public async Task BindModelAsync(ModelBindingContext bindingContext)
    {
        ...
        if (
            ...
            || bindingContext.Result.Model is not DateTime dateTime
        )
        {
            return;
        }
        ...
        )
        {
            try
            {
                var timezoneInfo = _timezoneProvider.GetTimeZoneInfo(
            ...
            {
                _logger.LogWarning(
                    "Could not convert DateTime with unspecified Kind using timezone '{TimeZone}'.",
                    _currentTimezoneProvider.TimeZone
                );
            }
        }
