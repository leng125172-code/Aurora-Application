...
public class AbpDateTimeModelBinderProvider : IModelBinderProvider
{
        ...
        if (modelType == typeof(DateTime))
        {
            if (context.Metadata.ContainerType == null)
            {
                if (
                    ...
                    && defaultModelMetadata.Attributes.Attributes.All(x =>
                        x.GetType() != typeof(DisableDateTimeNormalizationAttribute)
                    )
                )
                {
            ...
            {
                var dateNormalizationDisabledForClass = context.Metadata.ContainerType.IsDefined(
                    typeof(DisableDateTimeNormalizationAttribute),
                    true
                );

                var dateNormalizationDisabledForProperty = context
                    .Metadata.ContainerType.GetProperty(context.Metadata.PropertyName!)
                    ?.IsDefined(typeof(DisableDateTimeNormalizationAttribute), true);

                if (
                    !dateNormalizationDisabledForClass
    ...
    protected virtual AbpDateTimeModelBinder CreateAbpDateTimeModelBinder(
        ModelBinderProviderContext context
    )
    {
        const DateTimeStyles supportedStyles =
        ...
        return ActivatorUtilities.CreateInstance<AbpDateTimeModelBinder>(
            context.Services,
            dateTimeModelBinder
        );
    }
