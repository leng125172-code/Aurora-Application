...
public class AbpMvcAttributeValidationResultProvider : DefaultAttributeValidationResultProvider
{
    ...
    public override ValidationResult? GetOrDefault(
    ...
    {
        var resourceSource =
            _abpMvcDataAnnotationsLocalizationOptions.AssemblyResources.GetOrDefault(
                validationContext.ObjectType.Assembly
            );
        ...
        if (validationAttribute.ErrorMessage != null)
        {
            validationAttribute.ErrorMessage = _stringLocalizerFactory.Create(resourceSource)[
