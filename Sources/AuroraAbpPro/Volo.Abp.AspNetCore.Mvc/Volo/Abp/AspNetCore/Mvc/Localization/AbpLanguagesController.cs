...
public class AbpLanguagesController : AbpController
{
    ...
    [HttpGet]
    public virtual async Task<IActionResult> Switch(
        string culture,
        string uiCulture = "",
        string returnUrl = ""
    )
    {
        if (!CultureHelper.IsValidCultureCode(culture))
        {
            throw new AbpException(
                "The selected culture is not valid! Make sure you enter a valid culture code."
            );
        }
        ...
        {
            throw new AbpException(
                "The selected uiCulture is not valid! Make sure you enter a valid culture code."
            );
        }
