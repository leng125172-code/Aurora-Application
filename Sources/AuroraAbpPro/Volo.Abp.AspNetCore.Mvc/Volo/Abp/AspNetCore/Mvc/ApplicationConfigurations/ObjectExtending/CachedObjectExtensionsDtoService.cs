...
public class CachedObjectExtensionsDtoService
        ...
        ISingletonDependency
{
    ...
    public CachedObjectExtensionsDtoService(
    ...
    protected virtual ObjectExtensionsDto GenerateCacheValue()
    {
        ...
        {
            objectExtensionsDto.Modules[moduleConfig.Key] = CreateModuleExtensionDto(
                moduleConfig.Value
            );
        }
    ...
    )
    {
        ...
        {
            moduleExtensionDto.Entities[objectConfig.Key] = GetEntityExtensionDto(
                objectConfig.Value
            );
        }
    ...
    )
    {
        ...
        foreach (var propertyConfig in entityConfig.GetProperties())
        {
            ...
            entityExtensionDto.Properties[propertyConfig.Name] = CreateExtensionPropertyDto(
                propertyConfig
            );
        }
    ...
    )
    {
        ...
        foreach (
            var customConfig in propertyConfig.Configuration.Where(c => !c.Key.StartsWith("_"))
        )
        {
    ...
    protected virtual LocalizableStringDto? CreateDisplayNameDto(
        ExtensionPropertyConfiguration propertyConfig
    )
    {
    ...
    protected virtual void FillEnums(ObjectExtensionsDto objectExtensionsDto)
    {
        var enumProperties = ObjectExtensionManager
            .Instance.Modules()
            .Values.SelectMany(m => m.Entities.Values.SelectMany(e => e.GetProperties()))
        ...
        foreach (var enumProperty in enumProperties)
        {
            ...
            {
                objectExtensionsDto.Enums[
                    Nullable.GetUnderlyingType(enumProperty.Type)!.FullName + "?"
                ] = CreateExtensionEnumDto(enumProperty);
            }
            else
            {
                objectExtensionsDto.Enums[enumProperty.Type.FullName!] = CreateExtensionEnumDto(
                    enumProperty
                );
            }
        }
    ...
    protected virtual ExtensionEnumDto CreateExtensionEnumDto(
    ...
    )
    {
        ...
        var enumType =
            enumProperty.Type.IsEnum ? enumProperty.Type
