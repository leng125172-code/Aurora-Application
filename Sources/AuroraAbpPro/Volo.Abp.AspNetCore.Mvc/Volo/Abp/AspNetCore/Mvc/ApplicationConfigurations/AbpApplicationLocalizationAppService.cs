        ...
        IAbpApplicationLocalizationAppService
{
    ...
    public virtual async Task<ApplicationLocalizationDto> GetAsync(
    ...
    )
    {
        if (!CultureHelper.IsValidCultureCode(input.CultureName))
        {
            throw new AbpException(
                "The selected culture is not valid! Make sure you enter a valid culture name."
            );
        }
        ...
        using (CultureHelper.Use(input.CultureName))
        {
            var resources = LocalizationOptions
                .Resources.Values.Union(await ExternalLocalizationStore.GetResourcesAsync())
                .ToArray();
            ...
            foreach (var resource in resources)
            {
                ...
                if (localizer != null)
                {
                    ...
                    {
                        staticLocalizedStrings = (
                            await localizer.GetAllStringsAsync(
                    ...
                    foreach (var localizedString in localizedStringsWithDynamics)
                    {
                        if (input.OnlyDynamics)
                        {
                            var staticLocalizedString = staticLocalizedStrings!.GetOrDefault(
                                localizedString.Name
                            );
                            if (
                                staticLocalizedString != null
