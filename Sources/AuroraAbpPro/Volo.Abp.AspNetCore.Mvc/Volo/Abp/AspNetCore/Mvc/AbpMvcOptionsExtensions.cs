...
internal static class AbpMvcOptionsExtensions
{
    ...
    private static void AddFormatters(MvcOptions options)
    {
        ...
        var systemTextJsonOutputFormatter = options
            .OutputFormatters.Where(f => f is SystemTextJsonOutputFormatter)
            .Cast<SystemTextJsonOutputFormatter>()
            .FirstOrDefault();
        ...
        if (systemTextJsonOutputFormatter != null)
        {
            ...
            var jsonOptions = new JsonSerializerOptions(
                systemTextJsonOutputFormatter.SerializerOptions
            );
    ...
    private static void AddModelBinders(MvcOptions options)
    {
        ...
        options.ModelBinderProviders.Insert(
            1,
            new AbpExtraPropertiesDictionaryModelBinderProvider()
        );
    ...
    private static void AddMetadataProviders(MvcOptions options, IServiceCollection services)
    {
        ...
        options.ModelMetadataDetailsProviders.Add(
            new BindingSourceMetadataProvider(typeof(IRemoteStreamContent), BindingSource.FormFile)
        );
        options.ModelMetadataDetailsProviders.Add(
            new BindingSourceMetadataProvider(
                typeof(IEnumerable<IRemoteStreamContent>),
                BindingSource.FormFile
            )
        );
        options.ModelMetadataDetailsProviders.Add(
            new BindingSourceMetadataProvider(typeof(RemoteStreamContent), BindingSource.FormFile)
        );
        options.ModelMetadataDetailsProviders.Add(
            new BindingSourceMetadataProvider(
                typeof(IEnumerable<RemoteStreamContent>),
                BindingSource.FormFile
            )
        );
        options.ModelMetadataDetailsProviders.Add(
            new SuppressChildValidationMetadataProvider(typeof(IRemoteStreamContent))
        );
        options.ModelMetadataDetailsProviders.Add(
            new SuppressChildValidationMetadataProvider(typeof(RemoteStreamContent))
        );
    }
}
