...
public class ConventionalRouteBuilder : IConventionalRouteBuilder, ITransientDependency
{
    ...
    )
    {
        ...
        var url =
            $"{apiRoutePrefix}/{rootPath}/{NormalizeControllerNameCase(controllerNameInUrl, configuration)}";
        ...
        {
            if (
                TypeHelper.IsPrimitiveExtended(idParameterModel.ParameterType, includeEnums: true)
                || TypeDescriptor
                    .GetConverter(idParameterModel.ParameterType)
        ...
        if (!actionNameInUrl.IsNullOrEmpty())
        {
            ...
            //Add secondary Id
            var secondaryIds = action
                .Parameters.Where(p => p.ParameterName.EndsWith("Id", StringComparison.Ordinal))
                .ToList();
            if (
                secondaryIds.Count == 1
                && !secondaryIds[0].Attributes.Any(x => x is OptionalAttribute)
            )
            {
    ...
    protected virtual string GetApiRoutePrefix(
    ...
    protected virtual string NormalizeUrlActionName(
        string rootPath,
        string controllerName,
        ActionModel action,
        string httpMethod,
    ...
    protected virtual string NormalizeUrlControllerName(
        string rootPath,
        string controllerName,
        ActionModel action,
        string httpMethod,
    ...
    )
    {
        ...
        return configuration.UrlControllerNameNormalizer(
            new UrlControllerNameNormalizerContext(rootPath, controllerName)
        );
    }

    protected virtual string NormalizeControllerNameCase(
        string controllerName,
        ConventionalControllerSetting? configuration
    )
    {
    ...
    protected virtual string NormalizeActionNameCase(
        string actionName,
        ConventionalControllerSetting? configuration
    )
    {
    ...
    protected virtual string NormalizeIdPropertyNameCase(
    ...
    protected virtual string NormalizeSecondaryIdNameCase(
