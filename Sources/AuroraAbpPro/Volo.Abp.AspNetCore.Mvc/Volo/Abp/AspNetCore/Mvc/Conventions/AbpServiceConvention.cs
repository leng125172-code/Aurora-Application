...
public class AbpServiceConvention : IAbpServiceConvention, ITransientDependency
{
    ...
    protected virtual void ApplyForControllers(ApplicationModel application)
    {
        ...
        foreach (var controller in GetControllers(application))
        {
            ...
            {
                controller.ControllerName = controller.ControllerName.RemovePostFix(
            ...
            else
            {
                var remoteServiceAttr =
                    ReflectionHelper.GetSingleAttributeOrDefault<RemoteServiceAttribute>(
            ...
            && controllerType
                ...
                .Any(x =>
                    ...
                    && x.GetGenericTypeDefinition()
                        .FullName!.StartsWith("Volo.Abp.Http.Client.ClientProxying.ClientProxyBase")
                );
    }
    ...
    protected virtual void RemoveDuplicateControllers(ApplicationModel application)
    {
        ...
        if (Options.ControllersToRemove.Any())
        {
            ...
            if (removeControllerModels.Any())
            {
                ...
                Logger.LogInformation(
                    $"Removing the controller{(removeControllerModels.Length > 1 ? "s" : "")} {removeControllerModels.Select(c => c.ControllerType.AssemblyQualifiedName)!.JoinAsString(", ")} from the application model"
                );
            }
        }
        ...
        {
            var replaceControllersAttr =
                ReflectionHelper.GetSingleAttributeOrDefault<ReplaceControllersAttribute>(
            ...
            if (replaceControllersAttr != default)
            {
                ...
                if (replaceControllerModels.Any())
                {
                    ...
                    Logger.LogInformation(
                        $"Removing the controller{(replaceControllerModels.Length > 1 ? "s" : "")} {replaceControllersAttr.ControllerTypes.Select(c => c.AssemblyQualifiedName)!.JoinAsString(", ")} from the application model since {(replaceControllerModels.Length > 1 ? "they are" : "it is")} replaced by the controller: {controllerModel.ControllerType.AssemblyQualifiedName}"
                    );
                }
            }
        }
        ...
        foreach (var controllerModel in GetControllers(application))
        {
            ...
            var exposeServicesAttr =
                ReflectionHelper.GetSingleAttributeOrDefault<ExposeServicesAttribute>(
                    controllerModel.ControllerType
                );
            if (exposeServicesAttr!.IncludeSelf)
            {
                ...
                if (exposedControllerModels.Any())
                {
                    ...
                    Logger.LogInformation(
                        $"Removing the controller{(exposedControllerModels.Length > 1 ? "s" : "")} {exposeServicesAttr.ServiceTypes.Select(c => c.AssemblyQualifiedName)!.JoinAsString(", ")} from the application model since {(exposedControllerModels.Length > 1 ? "they are" : "it is")} replaced by the controller: {controllerModel.ControllerType.AssemblyQualifiedName}"
                    );
                }
                continue;
            }

            var baseControllerTypes = controllerModel
                .ControllerType.GetBaseClasses(typeof(Controller), includeObject: false)
            ...
            Logger.LogInformation(
                $"Removing the controller {controllerModel.ControllerType.AssemblyQualifiedName} from the application model since it replaces the controller(s): {baseControllerTypes.Select(c => c.AssemblyQualifiedName)!.JoinAsString(", ")}"
            );
        }
    ...
    protected virtual void ConfigureRemoteService(
            ...
            foreach (var prm in action.Parameters)
            {
                ...
                if (
                    !TypeHelper.IsPrimitiveExtended(
                        prm.ParameterInfo.ParameterType,
                        includeEnums: true
                    )
                )
                {
                    ...
                    {
                        prm.BindingInfo = BindingInfo.GetBindingInfo(
                            new[] { new FromBodyAttribute() }
                        );
                    }
                }
            }
    ...
    protected virtual bool CanUseFormBodyBinding(ActionModel action, ParameterModel parameter)
    {
        ...
        if (
            Options.ConventionalControllers.FormBodyBindingIgnoredTypes.Any(t =>
        ...
        foreach (var selector in action.Selectors)
        {
            ...
            foreach (var actionConstraint in selector.ActionConstraints)
            {
                ...
                if (
                    httpMethodActionConstraint.HttpMethods.All(hm =>
    ...
    {
        if (
            Options.ChangeControllerModelApiExplorerGroupName
    ...
    )
    {
        ...
        var controllerType = controller.ControllerType.AsType();
        var remoteServiceAtt = ReflectionHelper.GetSingleAttributeOrDefault<RemoteServiceAttribute>(
            controllerType.GetTypeInfo()
        );
    ...
    protected virtual void AddAbpServiceSelector(
        string rootPath,
        string controllerName,
        ActionModel action,
        ConventionalControllerSetting? configuration
    )
    {
        ...
        {
            AttributeRouteModel = CreateAbpServiceAttributeRouteModel(
                rootPath,
                controllerName,
                action,
                httpMethod,
    ...
    protected virtual string SelectHttpMethod(
        ActionModel action,
        ConventionalControllerSetting? configuration
    )
    {
    ...
    {
        return Options.ConventionalControllers.ConventionalControllerSettings.GetSettingOrNull(
            controllerType
        );
    }

    protected virtual AttributeRouteModel CreateAbpServiceAttributeRouteModel(
        string rootPath,
        string controllerName,
        ActionModel action,
        string httpMethod,
    ...
    )
    {
        return new AttributeRouteModel(
            new RouteAttribute(
                ConventionalRouteBuilder.Build(
                    rootPath,
                    controllerName,
                    action,
                    httpMethod,
    ...
    protected virtual bool IsVisibleRemoteService(Type controllerType)
    {
        ...
        var attribute = ReflectionHelper.GetSingleAttributeOrDefault<RemoteServiceAttribute>(
            controllerType
        );
        if (attribute == null)
        {
    ...
    protected virtual bool? IsVisibleRemoteServiceMethod(MethodInfo method)
    {
        ...
        return attribute.IsEnabledFor(method) && attribute.IsMetadataEnabledFor(method);
    }

    protected virtual bool IsGlobalFeatureEnabled(Type controllerType)
    {
        var attribute =
            ReflectionHelper.GetSingleAttributeOrDefault<RequiresGlobalFeatureAttribute>(
                controllerType
            );
        if (attribute == null)
        {
