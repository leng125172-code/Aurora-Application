...
public class GlobalFeatureActionFilter : IAsyncActionFilter, IAbpFilter, ITransientDependency
{
    ...
    )
    {
        ...
        {
            var logger = context.GetService<ILogger<GlobalFeatureActionFilter>>(
                NullLogger<GlobalFeatureActionFilter>.Instance
            )!;
            logger.LogWarning(
                $"The '{context.Controller.GetType().FullName}' controller needs to enable '{attribute!.Name}' feature."
            );
