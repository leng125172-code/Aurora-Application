...
public class AbpAuditActionFilter : IAsyncActionFilter, IAbpFilter, ITransientDependency
{
    ...
    )
    {
        ...
        using (
            AbpCrossCuttingConcerns.Applying(context.Controller, AbpCrossCuttingConcerns.Auditing)
        )
        {
    ...
    {
        if (
            !abpAuditingOptions.IsEnabledForIntegrationServices
            && IntegrationServiceAttribute.IsDefinedOrInherited(
                actionDescriptor.AsControllerActionDescriptor().ControllerTypeInfo
            )
        )
        {
            return false;
