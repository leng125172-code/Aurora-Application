...
public static class ValidationAttributeHelper
{
    ...
    private static readonly PropertyInfo ValidationAttributeCustomErrorMessageSetProperty =
        typeof(ValidationAttribute).GetProperty(
    ...
    {
        if (
            validationAttribute is StringLengthAttribute stringLength
            && stringLength.MinimumLength != 0
        )
        {
            var customErrorMessageSet =
                ValidationAttributeCustomErrorMessageSetProperty.GetValue(validationAttribute)
                as bool?;
        ...
        try
        {
            var errorMessageString =
                ValidationAttributeErrorMessageStringProperty.GetValue(validationAttribute)
                as string;
