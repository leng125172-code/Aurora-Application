        ...
        ITransientDependency
{
    ...
    )
    {
        ...
        foreach (var (_, module) in model.Modules)
        {
            ...
            {
                var removedController = module.Controllers.RemoveAll(x =>
                    x.Value.IsRemoteService
                    && controller.OrderBy(c => c.Value.ControllerGroupName).Skip(1).Contains(x)
                );
                foreach (var removed in removedController)
                {
                    Logger.LogInformation(
                        $"The controller named '{removed.Value.Type}' was removed from ApplicationApiDescriptionModel because it same with other controller."
                    );
                }
            }
        }
    ...
    )
    {
        ...
        Logger.LogDebug(
            $"ActionApiDescriptionModel.Create: {controllerModel.ControllerName}.{uniqueMethodName}"
        );
        ...
        else if (apiDescription.ActionDescriptor.EndpointMetadata.Any(x => x is IAuthorizeData))
        {
            ...
            var authorizeDatas = apiDescription
                .ActionDescriptor.EndpointMetadata.Where(x => x is IAuthorizeData)
                .Cast<IAuthorizeData>()
                .ToList();
            authorizeModels.AddRange(
                authorizeDatas.Select(authorizeData => new AuthorizeDataApiDescriptionModel
                {
        ...
        var interfaceType = controllerType
            .GetInterfaces()
    ...
    private void AddCustomTypesToModel(
        ApplicationApiDescriptionModel applicationModel,
        MethodInfo method
    )
    {
    ...
    private static void AddCustomTypesToModel(
        ApplicationApiDescriptionModel applicationModel,
        Type? type
    )
    {
    ...
    private static string CalculateTypeName(Type type)
    {
        ...
        var i = 0;
        var argumentList = type.GetGenericArguments().Select(_ => "T" + i++).JoinAsString(",");

        return $"{type.FullName!.Left(type.FullName!.IndexOf('`'))}<{argumentList}>";
    }

    private void AddParameterDescriptionsToModel(
        ActionApiDescriptionModel actionModel,
        MethodInfo method,
        ApiDescription apiDescription
    )
    {
        ...
        for (var i = 0; i < apiDescription.ParameterDescriptions.Count; i++)
        {
            ...
            var matchedMethodParamName =
                matchedMethodParamNames.Length > i
                    ? matchedMethodParamNames[i]
                    : parameterDescription.Name;

            actionModel.AddParameter(
                ParameterApiDescriptionModel.Create(
                    parameterDescription.Name,
    ...
    {
        var modelNameProvider = parameterInfo
            .GetCustomAttributes()
    ...
    )
    {
        ...
        var areaAttr =
            controllerType.GetCustomAttributes().OfType<AreaAttribute>().FirstOrDefault()
            ?? actionDescriptor.EndpointMetadata.OfType<AreaAttribute>().FirstOrDefault();
        if (areaAttr != null)
        {
    ...
    private string GetRemoteServiceName(Type controllerType, ConventionalControllerSetting? setting)
    {
        ...
        var remoteServiceAttr = controllerType
            .GetCustomAttributes()
            .OfType<RemoteServiceAttribute>()
            .FirstOrDefault();
    ...
    {
        var controllerNameAttribute = controllerType
            .GetCustomAttributes()
    ...
    {
        foreach (
            var controllerSetting in _abpAspNetCoreMvcOptions
