...
public class AbpDataAnnotationAutoLocalizationMetadataDetailsProvider : IDisplayMetadataProvider
{
    ...
    public AbpDataAnnotationAutoLocalizationMetadataDetailsProvider(IServiceCollection services)
    {
        ...
        _localizationOptions = services.GetRequiredServiceLazy<
    ...
    public virtual void CreateDisplayMetadata(DisplayMetadataProviderContext context)
    {
        ...
        if (
            attributes.OfType<DisplayAttribute>().Any()
        ...
        var localizer = _localizationOptions.Value.Value.DataAnnotationLocalizerProvider(
            containerType,
        ...
        {
            var key = _displayNameKeys.GetOrAdd(
                context,
                _ => GetDisplayNameKey(context, localizer)
            );
            return (key != null ? localizer[key] : null)!;
        };
    }

    protected virtual string? GetDisplayNameKey(
        ...
        */
        ...
        if (context.Key.ContainerType != null)
        {
            localizedString = localizer[
                PropertyLocalizationKeyPrefix
                    + context.Key.ContainerType.Name
                    + ":"
                    + context.Key.Name
            ];
