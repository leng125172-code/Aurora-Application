...
public class AbpAspNetCoreMvcModule : AbpModule
{
    ...
    public override void ConfigureServices(ServiceConfigurationContext context)
    {
        ...
        context.Services.PostConfigure<AbpAspNetCoreMvcOptions>(options =>
        {
            ...
            {
                options.MinifyGeneratedScript = context
                    .Services.GetHostingEnvironment()
                    .IsProduction();
            }
        });
        ...
        var abpMvcDataAnnotationsLocalizationOptions = context.Services.ExecutePreConfiguredActions(
            new AbpMvcDataAnnotationsLocalizationOptions()
        );

        context.Services.AddSingleton<IOptions<AbpMvcDataAnnotationsLocalizationOptions>>(
            new OptionsWrapper<AbpMvcDataAnnotationsLocalizationOptions>(
                abpMvcDataAnnotationsLocalizationOptions
            )
        );

        var mvcBuilder = context
            .Services.AddMvc()
            .AddDataAnnotationsLocalization(options =>
            {
                ...
                {
                    var resourceType =
                        abpMvcDataAnnotationsLocalizationOptions.AssemblyResources.GetOrDefault(
                            type.Assembly
                        );
        ...
        context.Services.TryAddEnumerable(
            ServiceDescriptor.Transient<IActionDescriptorProvider, AbpMvcActionDescriptorProvider>()
        );
        context
            .Services.AddAbpOptions<MvcOptions>()
            .Configure<IServiceProvider>(
                (mvcOptions, serviceProvider) =>
                {
                    mvcOptions.AddAbp(context.Services);

                    // serviceProvider is root service provider.
                    var stringLocalizer = serviceProvider.GetRequiredService<
                        IStringLocalizer<AbpValidationResource>
                    >();
                    mvcOptions.ModelBindingMessageProvider.SetValueIsInvalidAccessor(_ =>
                        stringLocalizer["The value '{0}' is invalid."]
                    );
                    mvcOptions.ModelBindingMessageProvider.SetNonPropertyValueMustBeANumberAccessor(
                        () =>
                            stringLocalizer["The field must be a number."]
                    );
                    mvcOptions.ModelBindingMessageProvider.SetValueMustBeANumberAccessor(value =>
        ...
        context.Services.Replace(
            ServiceDescriptor.Singleton<
                ...
                AbpMemoryPoolHttpResponseStreamWriterFactory
            >()
        );
    }
    ...
    public override void PostConfigureServices(ServiceConfigurationContext context)
    {
        ...
        DynamicProxyIgnoreTypes.Add(
            preConfigureActions
                .Configure()
                .ConventionalControllers.ConventionalControllerSettings.SelectMany(x =>
                    x.ControllerTypes
                )
                .ToArray()
        );
    ...
    private static void AddApplicationParts(ApplicationInitializationContext context)
    {
        ...
        var plugInModuleAssemblies = moduleContainer
            .Modules.Where(m => m.IsLoadedAsPlugIn)
        ...
        var controllerAssemblies = context
            .ServiceProvider.GetRequiredService<IOptions<AbpAspNetCoreMvcOptions>>()
            .Value.ConventionalControllers.ConventionalControllerSettings.Select(s => s.Assembly)
            .Distinct();

        AddToApplicationParts(partManager, controllerAssemblies);

        var additionalAssemblies = moduleContainer
            .Modules.SelectMany(m => m.GetAdditionalAssemblies())
            .Distinct();
