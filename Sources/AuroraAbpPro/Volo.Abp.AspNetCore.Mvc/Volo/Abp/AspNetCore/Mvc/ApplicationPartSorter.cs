...
public static class ApplicationPartSorter
{
    ...
    {
        var orderedModuleAssemblies = moduleContainer
            .Modules.Select((moduleDescriptor, index) => new { moduleDescriptor.Assembly, index })
            .GroupBy(x => x.Assembly)
            .ToDictionary(g => g.Key, g => g.First().index);

        var modulesAssemblies = moduleContainer.Modules.Select(x => x.Assembly).ToList();
        var sortedTypes = partManager
            .ApplicationParts.Where(x => modulesAssemblies.Contains(GetApplicationPartAssembly(x)))
            .OrderBy(x => orderedModuleAssemblies[GetApplicationPartAssembly(x)])
            .ToList();

        var sortIndex = 0;
        var sortedParts = partManager
            .ApplicationParts.Select(x =>
                modulesAssemblies.Contains(GetApplicationPartAssembly(x))
                    ? sortedTypes[sortIndex++]
                    : x
            )
            .ToList();
