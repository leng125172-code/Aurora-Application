...
public class AbpExceptionPageFilter : IAsyncPageFilter, IAbpFilter, ITransientDependency
{
    ...
    protected virtual bool ShouldHandleException(PageHandlerExecutingContext context)
    {
        ...
        if (
            context.ActionDescriptor.IsPageAction()
    ...
    protected virtual async Task HandleAndWrapException(PageHandlerExecutedContext context)
    {
        ...
        if (
            ...
            && exceptionHandlingOptions.ShouldLogException(context.Exception)
        )
        {
            var logger = context.GetService<ILogger<AbpExceptionPageFilter>>(
                NullLogger<AbpExceptionPageFilter>.Instance
            )!;
        ...
        else
        {
            ...
            else
            {
                ...
                logger.LogWarning(
                    "HTTP response has already started, cannot set headers and status code!"
                );
            }
