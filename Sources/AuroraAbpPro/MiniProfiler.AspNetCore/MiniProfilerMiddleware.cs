...
namespace StackExchange.Profiling
{
    ...
    public class MiniProfilerMiddleware
    {
        ...
        public MiniProfilerMiddleware(RequestDelegate next, IOptions<MiniProfilerOptions> options)
        {
            ...
            if (string.IsNullOrEmpty(Options.RouteBasePath))
            {
                throw new ArgumentException(
                    "RouteBasePath cannot be empty",
                    nameof(Options.RouteBasePath)
                );
            }
        }
        ...
        public async Task Invoke(HttpContext context)
        {
            ...
            if (ShouldProfile(context.Request))
            {
                ...
                var appendServerTimingHeader =
                    Options.EnableServerTimingHeader && context.Response.SupportsTrailers();
        ...
        private bool ShouldProfile(HttpRequest request)
        {
            foreach (var ignored in Options.IgnoredPaths)
            {
                if (
                    ignored != null
                    && request.Path.Value?.Contains(ignored, StringComparison.OrdinalIgnoreCase)
                        == true
                )
                {
                    return false;
        ...
        private async Task SetHeadersAndState(HttpContext context, MiniProfiler current)
        {
            try
            {
                ...
                {
                    isAuthorized = await AuthorizeRequestAsync(
                        context,
                        isList: false,
                        setResponse: false
                    );
                ...
                using (current.StepIf("Get Profiler IDs", 0.1m))
                {
                    profilerIds =
                        (
                            isAuthorized
                                ? await Options
                                    .ExpireAndGetUnviewedAsync(current.User)
                ...
                new RequestState { IsAuthorized = isAuthorized, RequestIDs = profilerIds }.Store(
                    context
                );
            }
            catch
        ...
        private async Task HandleRequest(HttpContext context, PathString subPath)
        {
            ...
            context.Response.ContentLength =
                result != null ? Encoding.UTF8.GetByteCount(result) : 0;
        ...
        private async Task<bool> AuthorizeRequestAsync(
            HttpContext context,
            bool isList,
            bool setResponse = true
        )
        {
            var req = context.Request;
            // Deny access if we a) have a configured delegate, and b) it says no
            if (
                (Options.ResultsAuthorize != null && !Options.ResultsAuthorize.Invoke(req))
                || (
                    Options.ResultsAuthorizeAsync != null
                    && !await Options.ResultsAuthorizeAsync(req)
                )
                || (
                    isList
                    && Options.ResultsListAuthorize != null
                    && !Options.ResultsListAuthorize(req)
                )
                || (
                    isList
                    && Options.ResultsListAuthorizeAsync != null
                    && !await Options.ResultsListAuthorizeAsync(req)
                )
            )
            {
                if (setResponse)
        ...
        private async Task<string> ResultsListAsync(HttpContext context)
        {
            ...
            if (
                context.Request.Query.TryGetValue("last-id", out var lastId)
                && Guid.TryParse(lastId, out var lastGuid)
            )
            {
        ...
        private async Task<string?> GetSingleProfilerResultAsync(HttpContext context)
        {
            ...
            // 优先从请求体（JSON）解析 ID，其次从 querystring 获取
            if (
                context.Request.ContentLength > 0
                && (
                    (
                        clientRequest = await JsonSerializer.DeserializeAsync<ResultRequest>(
