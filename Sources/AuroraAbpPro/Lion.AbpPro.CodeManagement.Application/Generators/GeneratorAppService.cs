...
[Authorize(policy: CodeManagementPermissions.CodeManagement.Generator.Default)]
public class GeneratorAppService : CodeManagementAppService, IGeneratorAppService
{
    ...
    /// 预览代码生成
    /// </summary>
    [Authorize(policy: CodeManagementPermissions.CodeManagement.Generator.Preview)]
    public async Task<List<TemplateTreeDto>> PreViewCodeAsync(PreViewCodeInput input)
    {
        ...
        if (input?.EntityId?.Count > 0)
        {
            context = await _projectEntityManager.GetProjectContextAsync(
                input.ProjectId,
                input.EntityId
            );
        }
        ...
        {
            context = await _projectEntityManager.GetProjectContextAsync(input.ProjectId);
        }
    ...
    {
        var treeNodes = list.Adapt<List<TreeNode>>();
        var distinctResult = _treeManager.Distinct(treeNodes);
        return distinctResult.Adapt<List<TemplateTreeDto>>();
    }

    /// <summary>
    /// 遍历模板
    /// </summary>
    private async Task<List<TemplateTreeDto>> RecursionTemplate(
        List<TemplateDetailDto> templateDetails,
        GeneratorProjectTemplateContext context,
        Guid? parentId = null
    )
    {
        ...
        foreach (var template in templates)
        {
            if (template.TemplateType == TemplateType.Folder)
            {
                ...
                tree.Add(code);
                code.Children.AddRange(
                    await RecursionTemplate(templateDetails, context, template.Id)
                );
            }
            else
            {
