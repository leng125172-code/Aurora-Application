...
public class AbpSwashbuckleDocumentFilter : IDocumentFilter
{
    ...
    {
        var actionUrls = context
            .ApiDescriptions.Select(apiDescription => apiDescription.ActionDescriptor)
            .Where(actionDescriptor =>
                !string.IsNullOrEmpty(actionDescriptor.DisplayName)
        ...
        swaggerDoc.Paths.RemoveAll(path => !actionUrls.Contains(path.Key));
        ...
        foreach (var path in swaggerDoc.Paths)
        {
            ...
            {
                tags.AddRange(
                    path.Value.Operations.SelectMany(x =>
                    {
                        return x.Value.Tags?.Select(t => t.Name ?? string.Empty)
                            ?? Array.Empty<string>();
                    })
                );
            }
    ...
    {
        var route = actionDescriptor
            .AttributeRouteInfo?.Template?.EnsureStartsWith('/')
            .Replace("?", "");
