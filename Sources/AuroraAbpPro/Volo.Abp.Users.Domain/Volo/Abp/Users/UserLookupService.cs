    ...
    where TUserRepository : IUserRepository<TUser>
{
    ...
    public async Task<TUser> FindByIdAsync(Guid id, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            externalUser = await ExternalUserLookupServiceProvider.FindByIdAsync(
                id,
                cancellationToken
            );
            if (externalUser == null)
            {
                if (localUser != null)
                {
                    ...
                    await WithNewUowAsync(() =>
                        _userRepository.DeleteAsync(localUser, cancellationToken: cancellationToken)
                    );
                }
        ...
        {
            await WithNewUowAsync(() =>
                _userRepository.UpdateAsync(localUser, cancellationToken: cancellationToken)
            );
        }
        else
        {
    ...
    public async Task<TUser> FindByUserNameAsync(
        string userName,
    ...
    )
    {
        ...
        try
        {
            externalUser = await ExternalUserLookupServiceProvider.FindByUserNameAsync(
                userName,
                cancellationToken
            );
            if (externalUser == null)
    ...
    )
    {
        if (ExternalUserLookupServiceProvider != null)
        {
            return await ExternalUserLookupServiceProvider.SearchAsync(
                sorting,
                filter,
                maxResultCount,
                skipCount,
        ...
        var localUsers = await _userRepository.SearchAsync(
            sorting,
            maxResultCount,
            skipCount,
            filter,
