...
public class RemoteSettingProvider : ISettingProvider, ITransientDependency
{
    ...
    public async Task<List<SettingValue>> GetAllAsync(string[] names)
    {
        var configuration = await ConfigurationClient.GetAsync();
        return names
            .Select(x => new SettingValue(x, configuration.Setting.Values.GetOrDefault(x)))
            .ToList();
    }
    ...
    {
        var configuration = await ConfigurationClient.GetAsync();
        return configuration.Setting.Values.Select(s => new SettingValue(s.Key, s.Value)).ToList();
    }
}
