...
public class RemoteLocalizationContributor : ILocalizationResourceContributor
{
    ...
    public void Initialize(LocalizationResourceInitializationContext context)
    {
        ...
        _applicationConfigurationClient =
            context.ServiceProvider.GetRequiredService<ICachedApplicationConfigurationClient>();
        _logger =
            context.ServiceProvider.GetService<ILogger<RemoteLocalizationContributor>>()
    ...
    protected virtual void FillInternal(
        string resourceName,
    ...
    public virtual async Task FillAsync(
        string cultureName,
        Dictionary<string, LocalizedString> dictionary
    )
    {
    ...
    protected virtual async Task FillInternalAsync(
        string resourceName,
        Dictionary<string, LocalizedString> dictionary
    )
    {
    ...
    protected virtual async Task<ApplicationLocalizationResourceDto?> GetResourceOrNullAsync(
        string resourceName
    )
    {
    ...
    {
        var resource = applicationConfigurationDto.Localization.Resources.GetOrDefault(
            resourceName
        );
        if (resource != null)
        {
        ...
        var legacyResource = applicationConfigurationDto.Localization.Values.GetOrDefault(
            resourceName
        );
        ...
        _logger.LogWarning(
            $"Could not find the localization resource {resourceName} on the remote server!"
        );
        return null;
    }
}
