...
public class HangfireBackgroundJobManager : IBackgroundJobManager, ITransientDependency
{
    ...
    )
    {
        return Task.FromResult(
            delay.HasValue
                ...
                : BackgroundJob.Enqueue<HangfireJobExecutionAdapter<TArgs>>(adapter =>
                    adapter.ExecuteAsync(GetQueueName(typeof(TArgs)), args, default)
                )
        );
    }
    ...
    {
        var queueAttribute = BackgroundJobOptions
            .Value.GetJob(argsType)
            .JobType.GetCustomAttribute<QueueAttribute>();
