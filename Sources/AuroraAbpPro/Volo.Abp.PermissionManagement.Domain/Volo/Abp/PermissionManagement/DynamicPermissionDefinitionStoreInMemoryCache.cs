        ...
        ISingletonDependency
{
    ...
    )
    {
        ...
        var resourcePermissionRecords = permissionRecords.Where(x =>
            !x.ResourceName.IsNullOrWhiteSpace()
        );
        foreach (var resourcePermissionRecord in resourcePermissionRecords)
        {
            var resourcePermission = context.AddResourcePermission(
                resourcePermissionRecord.Name,
                resourcePermissionRecord.ResourceName,
                resourcePermissionRecord.ManagementPermissionName,
                resourcePermissionRecord.DisplayName != null
                    ? LocalizableStringSerializer.Deserialize(resourcePermissionRecord.DisplayName)
                    : null,
        ...
        var permissions = permissionRecords
            .Where(x => x.ResourceName.IsNullOrWhiteSpace())
            .ToList();
        foreach (var permissionGroupRecord in permissionGroupRecords)
        {
            var permissionGroup = context.AddGroup(
                ...
                permissionGroupRecord.DisplayName != null
            ...
            var permissionRecordsInThisGroup = permissions.Where(p =>
                p.GroupName == permissionGroup.Name
            );

            foreach (
                var permissionRecord in permissionRecordsInThisGroup.Where(x =>
                    x.ParentName == null
                )
            )
            {
    ...
    {
        return ResourcePermissionDefinitions.FirstOrDefault(p =>
            p.ResourceName == resourceName && p.Name == name
        );
    }
    ...
    private void AddPermissionRecursively(
    ...
    )
    {
        var permission = permissionContainer.AddPermission(
            ...
            permissionRecord.DisplayName != null
        ...
        foreach (
            var subPermission in allPermissionRecords.Where(p =>
                p.ParentName == permissionRecord.Name
            )
        )
        {
    ...
    private void ApplyPermissionProperties(
    ...
    )
    {
        ...
        {
            var checkers = StateCheckerSerializer.DeserializeArray(
