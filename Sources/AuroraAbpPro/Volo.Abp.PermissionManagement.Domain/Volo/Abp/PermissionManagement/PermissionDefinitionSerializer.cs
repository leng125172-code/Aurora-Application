...
public class PermissionDefinitionSerializer : IPermissionDefinitionSerializer, ITransientDependency
{
    ...
    public virtual async Task<PermissionDefinitionRecord[]> SerializeAsync(
    ...
    public virtual Task<PermissionGroupDefinitionRecord> SerializeAsync(
    ...
    )
    {
        using (CultureHelper.Use(CultureInfo.InvariantCulture))
        {
            var permissionRecord = new PermissionDefinitionRecord(
                ...
                permissionGroup?.Name,
                permission.Name,
                permission.ResourceName ?? string.Empty,
                permission.ManagementPermissionName ?? string.Empty,
                permission.Parent?.Name ?? string.Empty,
    ...
    {
        return providers.Any() ? providers.JoinAsString(",") : string.Empty;
    }
    ...
    {
        return StateCheckerSerializer.Serialize(stateCheckers) ?? string.Empty;
    }
}
