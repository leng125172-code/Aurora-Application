...
public class ResourcePermissionStore : IResourcePermissionStore, ITransientDependency
{
    ...
    {
        return (
            await GetCacheItemAsync(name, resourceName, resourceKey, providerName, providerKey)
        ).IsGranted;
    }

    protected virtual async Task<ResourcePermissionGrantCacheItem> GetCacheItemAsync(
        string name,
        string resourceName,
        string resourceKey,
        string providerName,
    ...
    protected virtual async Task SetCacheItemsAsync(
        ...
        string providerKey,
        string currentName,
        ResourcePermissionGrantCacheItem currentCacheItem
    )
    {
        using (ResourcePermissionGrantRepository.DisableTracking())
        {
            ...
            Logger.LogDebug(
                $"Getting all granted resource permissions from the repository for resource name,key:{resourceName},{resourceKey} and provider name,key: {providerName},{providerKey}"
            );
            ...
            foreach (var permission in permissions)
            {
                ...
                cacheItems.Add(
                    new KeyValuePair<string, ResourcePermissionGrantCacheItem>(
                        CalculateCacheKey(
                            permission.Name,
                            resourceName,
                            resourceKey,
    ...
    protected virtual async Task<
        List<KeyValuePair<string, ResourcePermissionGrantCacheItem>>
    > GetCacheItemsAsync(
        string[] names,
        string resourceName,
        string resourceKey,
    ...
    {
        var cacheKeys = names
            .Select(x => CalculateCacheKey(x, resourceName, resourceKey, providerName, providerKey))
            .ToList();

        Logger.LogDebug(
            $"ResourcePermissionStore.GetCacheItemAsync: {string.Join(",", cacheKeys)}"
        );
        ...
        var newCacheItems = await SetCacheItemsAsync(
            resourceName,
            resourceKey,
            providerName,
            providerKey,
    ...
    )
    {
        using (ResourcePermissionGrantRepository.DisableTracking())
        {
            var permissionNames = new HashSet<string>(
                notCacheKeys.Select(GetPermissionNameFormCacheKeyOrNull)
            );
            var permissions = (await PermissionDefinitionManager.GetResourcePermissionsAsync())
            ...
            Logger.LogDebug(
                $"Getting not cache granted permissions from the repository for resource name,key:{resourceName},{resourceKey} and provider name,key: {providerName},{providerKey}"
            );
    ...
    public virtual async Task<MultiplePermissionGrantResult> GetPermissionsAsync(
        string resourceName,
        string resourceKey
    )
    {
        using (ResourcePermissionGrantRepository.DisableTracking())
        {
            ...
            var resourcePermissions = (
                ...
                .ToList();
            var permissionGrants = await ResourcePermissionGrantRepository.GetPermissionsAsync(
                resourceName,
                resourceKey
            );
    ...
    public virtual async Task<string[]> GetGrantedPermissionsAsync(
        string resourceName,
        string resourceKey
    )
    {
        var resourcePermissions = (await PermissionDefinitionManager.GetResourcePermissionsAsync())
            .Where(x => x.ResourceName == resourceName)
            .ToList();
        var grantedPermissions = await ResourcePermissionGrantRepository.GetPermissionsAsync(
            resourceName,
            resourceKey
        );
    ...
    public virtual async Task<string[]> GetGrantedResourceKeysAsync(
        string resourceName,
        string name
    )
    {
        return (await ResourcePermissionGrantRepository.GetResourceKeys(resourceName, name))
    ...
    {
        return ResourcePermissionGrantCacheItem.CalculateCacheKey(
            name,
            resourceName,
            resourceKey,
            providerName,
