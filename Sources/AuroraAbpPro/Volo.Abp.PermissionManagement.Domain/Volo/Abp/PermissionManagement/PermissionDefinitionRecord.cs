...
public class PermissionDefinitionRecord : BasicAggregateRoot<Guid>, IHasExtraProperties
{
        ...
        : base(id)
    {
        ...
        if (resourceName == null)
        {
            GroupName = Check.NotNullOrWhiteSpace(
                groupName,
                nameof(groupName),
                PermissionGroupDefinitionRecordConsts.MaxNameLength
            );
        }
        Name = Check.NotNullOrWhiteSpace(
            name,
            nameof(name),
            PermissionDefinitionRecordConsts.MaxNameLength
        );
        ResourceName = resourceName;
        ManagementPermissionName = managementPermissionName;
        ParentName = Check.Length(
            parentName,
            nameof(parentName),
            PermissionDefinitionRecordConsts.MaxNameLength
        );
        DisplayName = Check.NotNullOrWhiteSpace(
            displayName,
            nameof(displayName),
            PermissionDefinitionRecordConsts.MaxDisplayNameLength
        );
        IsEnabled = isEnabled;
        MultiTenancySide = multiTenancySide;
        Providers = providers ?? string.Empty;
        StateCheckers = stateCheckers ?? string.Empty;
    ...
    public void Patch(PermissionDefinitionRecord otherRecord)
    {
        ...
        if (Providers != otherRecord.Providers)
        {
            Providers = otherRecord.Providers ?? string.Empty;
        }

        if (StateCheckers != otherRecord.StateCheckers)
        {
            StateCheckers = otherRecord.StateCheckers ?? string.Empty;
        }
