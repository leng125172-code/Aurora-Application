    ...
    where TWorker : IBackgroundWorker
{
    ...
    public QuartzPeriodicBackgroundWorkerAdapter()
    {
        ...
        _doWorkAsyncMethod = typeof(TWorker).GetMethod(
            "DoWorkAsync",
            BindingFlags.Instance | BindingFlags.NonPublic
        );
        _doWorkMethod = typeof(TWorker).GetMethod(
            "DoWork",
            BindingFlags.Instance | BindingFlags.NonPublic
        );
    }
    ...
    public virtual void BuildWorker(IBackgroundWorker worker)
    {
        ...
        else
        {
            triggerBuilder.WithSimpleSchedule(builder =>
                builder.WithInterval(TimeSpan.FromMilliseconds(period!.Value)).RepeatForever()
            );
        }
    ...
    public override async Task Execute(IJobExecutionContext context)
    {
        ...
        switch (worker)
        {
            case AsyncPeriodicBackgroundWorkerBase asyncWorker:
            {
                ...
                {
                    await (Task)(
                        _doWorkAsyncMethod.Invoke(asyncWorker, new object[] { workerContext })!
                    );
                }
