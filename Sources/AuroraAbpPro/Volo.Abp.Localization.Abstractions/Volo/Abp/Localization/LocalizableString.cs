...
public class LocalizableString : ILocalizableString, IAsyncLocalizableString
{
    ...
    public async Task<LocalizedString> LocalizeAsync(IStringLocalizerFactory stringLocalizerFactory)
    {
        ...
        if (localizer == null)
        {
            throw new AbpException(
                $"Set {nameof(ResourceName)} or configure the default localization resource type (in the AbpLocalizationOptions)!"
            );
        }
    ...
    )
    {
        ...
        if (ResourceName != null)
        {
            var localizerByName = await stringLocalizerFactory.CreateByResourceNameOrNullAsync(
                ResourceName
            );
            if (localizerByName != null)
            {
