...
public class EfCoreNotificationSubscriptionRepository
    : EfCoreRepository<INotificationManagementDbContext, NotificationSubscription, Guid>,
        INotificationSubscriptionRepository
{
    public EfCoreNotificationSubscriptionRepository(
        [NotNull] IDbContextProvider<INotificationManagementDbContext> dbContextProvider
    )
        : base(dbContextProvider) { }

    public async Task<List<NotificationSubscription>> GetPagingListAsync(
        Guid notificationId,
        Guid? receiverUserId,
    ...
    )
    {
        return await (await GetDbSetAsync())
            ...
            .WhereIf(
                receiverUserName.IsNotNullOrWhiteSpace(),
                e => e.ReceiveUserName == receiverUserName
            )
    ...
    public async Task<NotificationSubscription> FindAsync(
        Guid receiverUserId,
        Guid notificationId,
        CancellationToken cancellationToken = default
    )
    {
        return await (await GetDbSetAsync()).FirstOrDefaultAsync(
            e => e.ReceiveUserId == receiverUserId && e.NotificationId == notificationId,
