...
public class AbpSecurityStampValidatorCallback
{
    ...
    public class SecurityStampValidatorCallback
    {
        ...
        )
        {
            ...
            var currentClaimsToKeep = context
                .CurrentPrincipal.Claims.Where(x => !newClaimTypes.Contains(x.Type))
                .ToArray();
            ...
            if (refreshingPrincipalOptions.CurrentPrincipalKeepClaimTypes.Any())
            {
                ...
                {
                    var sessionIdClaim = context.CurrentPrincipal.Claims.FirstOrDefault(x =>
                        x.Type == claimType
                    );
