...
public class MagickImageResizerContributor : IImageResizerContributor, ITransientDependency
{
    ...
    )
    {
        ...
        var memoryStream = await stream.CreateMemoryStreamAsync(
    ...
    )
    {
        if (!mimeType.IsNullOrWhiteSpace() && !CanResize(mimeType))
        {
            return Task.FromResult(
                new ImageResizeResult<byte[]>(bytes, ImageProcessState.Unsupported)
            );
        }
        ...
        return Task.FromResult(
            new ImageResizeResult<byte[]>(image.ToByteArray(), ImageProcessState.Done)
        );
    }
        ...
        switch (resizeArgs.Mode)
        {
            ...
            default:
                throw new NotSupportedException(
                    "Resize mode " + resizeArgs.Mode + "is not supported!"
                );
        }
    }

    protected virtual uint GetTargetHeight(
        ImageResizeArgs resizeArgs,
        uint min,
        uint sourceWidth,
        uint sourceHeight
    )
    {
        ...
        {
            return (uint)
                Math.Max(min, Math.Round(sourceHeight * resizeArgs.Width / (float)sourceWidth));
        }

        return resizeArgs.Height;
    }

    protected virtual uint GetTargetWidth(
        ImageResizeArgs resizeArgs,
        uint min,
        uint sourceWidth,
        uint sourceHeight
    )
    {
        ...
        {
            return (uint)
                Math.Max(min, Math.Round(sourceWidth * resizeArgs.Height / (float)sourceHeight));
        }
    ...
    protected virtual void ResizeBoxPad(MagickImage image, ImageResizeArgs resizeArgs)
    {
        ...
        var boxPadWidth =
            targetWidth > 0 ? targetWidth : (uint)Math.Round(sourceWidth * percentHeight);
        var boxPadHeight =
            targetHeight > 0 ? targetHeight : (uint)Math.Round(sourceHeight * percentWidth);
