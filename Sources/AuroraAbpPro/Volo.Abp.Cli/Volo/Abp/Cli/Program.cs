...
public class Program
{
    private static async Task Main(string[] args)
    {
        ...
        if (args.Length > 0 && args[0].Equals("mcp", StringComparison.OrdinalIgnoreCase))
        {
            Log.Logger = config
                .WriteTo.File(
                    Path.Combine(CliPaths.Log, "abp-cli-mcp-logs.txt"),
        ...
        else
        {
            Log.Logger = config
                .WriteTo.File(
                    Path.Combine(CliPaths.Log, "abp-cli-logs.txt"),
                    outputTemplate: loggerOutputTemplate
                )
                .WriteTo.Console(
        ...
        using (
            var application = AbpApplicationFactory.Create<AbpCliModule>(options =>
            {
                options.UseAutofac();
