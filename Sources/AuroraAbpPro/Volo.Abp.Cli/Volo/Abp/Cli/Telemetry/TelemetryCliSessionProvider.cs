...
{
    public TelemetryCliSessionProvider(IServiceProvider serviceProvider)
    ...
    protected override Task ExecuteAsync(ActivityContext context)
    {
        ...
        context.Current[ActivityPropertyNames.IsFirstSession] = !File.Exists(
            TelemetryPaths.ActivityStorage
        );
        context.Current["OldCli"] = true;

        if (
            context.Current.TryGetValue<Dictionary<string, object>>(
                ActivityPropertyNames.AdditionalProperties,
