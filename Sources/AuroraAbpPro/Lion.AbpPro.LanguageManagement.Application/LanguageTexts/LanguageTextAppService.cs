...
public class LanguageTextAppService : ApplicationService, ILanguageTextAppService
{
    ...
    /// 分页查询语言文本
    /// </summary>
    public virtual async Task<PagedResultDto<PageLanguageTextOutput>> PageAsync(
        PageLanguageTextInput input
    )
    {
        if (!CultureHelper.IsValidCultureCode(input.CultureName))
        {
            throw new LanguageManagementApplicationException(
                LanguageManagementErrorCodes.CultureNotValid
            );
        }
        ...
        var queryable = list.AsQueryable()
            .WhereIf(
                input.ResourceName.IsNotNullOrWhiteSpace(),
                e => e.ResourceName == input.ResourceName
            )
            .WhereIf(
                input.Filter.IsNotNullOrWhiteSpace(),
                e => e.Name.Contains(input.Filter) || e.Value.Contains(input.Filter)
            );
    ...
    protected virtual async Task<List<PageLanguageTextOutput>> FindLocalizationsAsync(
        string cultureName
    )
    {
        ...
        using (CultureHelper.Use(cultureName))
        {
            ...
            {
                var localizer = await _stringLocalizerFactory.CreateByResourceNameOrNullAsync(
                    resource
                );
                if (localizer != null)
                {
                    foreach (
                        var localizedString in await localizer.GetAllStringsAsync(
                            false,
                            false,
                            true
                        )
    ...
    {
        var localizedString = await GetLocalizedStringAsync(
            input.ResourceName,
            input.CultureName,
            input.Name
        );
        ...
        {
            throw new LanguageManagementApplicationException(
                LanguageManagementErrorCodes.LanguageTextExist
    ...
    public virtual async Task UpdateAsync(UpdateLanguageTextInput input)
    {
        ...
        if (localizedString == null)
            throw new LanguageManagementApplicationException(
                LanguageManagementErrorCodes.ResourceNotFound
            );
        if (localizedString.Value == input.Value)
            return;
    ...
    /// 获取执行语言
    /// </summary>
    protected virtual async Task<LocalizedString> GetLocalizedStringAsync(
        string resourceName,
        string cultureName,
        string name
    )
