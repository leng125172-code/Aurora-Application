...
public class ResizeImageAttribute : ActionFilterAttribute
{
    ...
    )
    {
        ...
        foreach (var (key, value) in parameters)
        {
            object? resizedValue = value switch
            {
                ...
                IRemoteStreamContent remoteStreamContent => await ResizeImageAsync(
                    remoteStreamContent,
                    imageResizer
                ),
    ...
    protected virtual async Task<IFormFile> ResizeImageAsync(
        IFormFile file,
    ...
    protected virtual async Task<IRemoteStreamContent> ResizeImageAsync(
    ...
    {
        return (
            await imageResizer.ResizeAsync(bytes, new ImageResizeArgs(Width, Height, Mode))
        ).Result;
    }
}
