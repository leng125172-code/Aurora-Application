...
public class CompressImageAttribute : ActionFilterAttribute
{
    ...
    )
    {
        ...
        var imageCompressor =
            context.HttpContext.RequestServices.GetRequiredService<IImageCompressor>();
        ...
        foreach (var (key, value) in parameters)
        {
            object? compressedValue = value switch
            {
                ...
                IRemoteStreamContent remoteStreamContent => await CompressImageAsync(
                    remoteStreamContent,
                    imageCompressor
                ),
                Stream stream => await CompressImageAsync(stream, imageCompressor),
                IEnumerable<byte> bytes => await CompressImageAsync(
                    bytes.ToArray(),
                    imageCompressor
                ),
                _ => null,
    ...
    protected virtual async Task<IFormFile> CompressImageAsync(
        IFormFile file,
    ...
    protected virtual async Task<IRemoteStreamContent> CompressImageAsync(
        IRemoteStreamContent remoteStreamContent,
        IImageCompressor imageCompressor
    )
    {
        if (
            remoteStreamContent.ContentType == null
            || !remoteStreamContent.ContentType.StartsWith("image/")
        )
        {
            return remoteStreamContent;
        }

        var result = await imageCompressor.CompressAsync(
    ...
    protected virtual async Task<Stream> CompressImageAsync(
        Stream stream,
        IImageCompressor imageCompressor
    )
    {
    ...
    protected virtual async Task<byte[]> CompressImageAsync(
        byte[] bytes,
        IImageCompressor imageCompressor
    )
    {
