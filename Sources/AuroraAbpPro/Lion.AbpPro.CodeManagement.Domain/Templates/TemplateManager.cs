...
public class TemplateManager : CodeManagementDomainService
{
    ...
    )
    {
        ...
        return list.Adapt<List<TemplateDto>>();
    }
    ...
    public async Task DeleteDetailAsync(Guid templateId, Guid templateDetailId)
    {
        ...
        foreach (
            var templateDetail in entity.TemplateDetails.Where(e => e.ParentId == templateDetailId)
        )
        {
    ...
    public async Task<TemplateDto> GetAsync(Guid templateId)
    {
        ...
        return entity.Adapt<TemplateDto>();
    }
    ...
    public async Task<List<TemplateTreeDto>> TemplateTreeAsync(Guid id)
    {
        ...
        if (template.TemplateDetails.Count == 0)
            return new List<TemplateTreeDto>();
    ...
    public async Task CopyAsync(Guid id, string name, string description)
    {
        ...
        var newTemplateDetails = CopyTemplateDetail(
            template.TemplateDetails.Adapt<List<CopyTemplateDetailDto>>(),
            newTemplate.Id
        );
        foreach (var detail in newTemplateDetails)
        {
            newTemplate.AddTemplateDetail(
                detail.NewId,
                detail.TemplateType,
                detail.ControlType,
                detail.Name,
    ...
    private List<TemplateTreeDto> RecursionTemplate(
        List<TemplateDetailDto> template,
        Guid? parentId
    )
    {
        ...
        foreach (var detail in list)
        {
            var child = new TemplateTreeDto()
            {
                ...
                Icon =
                    detail.TemplateType == TemplateType.Folder
                        ? CodeManagementConsts.AntIconFolder
                        : CodeManagementConsts.AntIconFile,
            };
    ...
    private List<CopyTemplateDetailDto> CopyTemplateDetail(
        List<CopyTemplateDetailDto> entities,
        Guid newTemplateId,
        Guid? parentId = null,
