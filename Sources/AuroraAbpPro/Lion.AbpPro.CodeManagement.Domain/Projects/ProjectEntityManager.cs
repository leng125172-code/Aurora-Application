...
public class ProjectEntityManager : CodeManagementDomainService
{
    ...
    public async Task<GeneratorProjectTemplateContext> GetProjectContextAsync(Guid projectId)
    {
        ...
        result.Project = project.Adapt<GeneratorProjectContext>();
    ...
    /// 获取上下文信息
    /// </summary>
    public async Task<GeneratorProjectTemplateContext> GetProjectContextAsync(
        Guid projectId,
        List<Guid> entityId
    )
    {
        var result = new GeneratorProjectTemplateContext();
        var entities = await _entityModelManager.FindByProjectIdAsync(projectId);
        entities = entities.Where(e => entityId.Contains(e.Id)).ToList();
        // 获取实体模型数据类型
        var dataTypes = await _dataTypeManager.ListAsync();
        // 获取实体枚举
        var enumTypes = await _enumTypeManager.ListByProjectIdAsync(projectId);
        enumTypes = enumTypes.Where(e => entityId.Contains(e.EntityModelId)).ToList();
        result.TreeEntityModels = RecursionEntity(entities, null, dataTypes, enumTypes);
        var project = await _projectManager.GetAsync(projectId);
        result.Project = project.Adapt<GeneratorProjectContext>();
        result.EntityModels = BuildEntityModelContext(entities, dataTypes, enumTypes);
    ...
    private List<GeneratorEntityModelContext> BuildEntityModelContext(
        ...
        foreach (var detail in entities)
        {
            ...
            foreach (var detailEntityModelProperty in detail.EntityModelProperties)
            {
                ...
                var property = new GeneratorEntityModelPropertyContext()
                {
                    ...
                    MinLength = detailEntityModelProperty.MinLength ?? 0,
                    ...
                    AllowSearch = detailEntityModelProperty.AllowSearch,
                    AllowAdd = detailEntityModelProperty.AllowAdd,
                    AllowEdit = detailEntityModelProperty.AllowEdit,
                };
                if (property.IsEnum)
                {
                    ...
                    if (currentEnum != null)
                    {
                        ...
                        {
                            enumType.Properties.Add(
                                new GeneratorEnumTypePropertyContext()
                                {
    ...
    private List<GeneratorTreeEntityModelContext> RecursionEntity(
        List<EntityModelDto> entities,
        Guid? parentId,
        List<DataTypeDto> dataTypes,
        List<EnumTypeDto> enumTypes
