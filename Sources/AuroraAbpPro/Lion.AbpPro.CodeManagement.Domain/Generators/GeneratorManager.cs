...
public class GeneratorManager : CodeManagementDomainService
{
    /// <summary>
    /// 模板生成
    /// </summary>
    ...
    public async Task<List<TemplateTreeDto>> RenderTemplateAsync(
        GeneratorProjectTemplateContext context,
            ...
            switch (template.ControlType)
            {
                ...
                case ControlType.Entity:
                    folder.Children.AddRange(
                        await RenderEntityAsync(template, context.Project, aggregate.EntityModels)
                    );
                    break;
                case ControlType.Enum:
                    folder.Children.AddRange(
                        await RenderEnumAsync(template, context.Project, aggregate)
                    );
                    break;
                case ControlType.Global:
                    result.Add(
                        await RenderGlobalAsync(template, context.Project, context.EntityModels)
                    );
                    break;
            }
        }
    ...
    private async Task<TemplateTreeDto> RenderAggregateAsync(
        TemplateDetailDto template,
        GeneratorProjectContext project,
        GeneratorTreeEntityModelContext treeEntityModel
    )
    {
        var fileName = await RenderFileNameAsync(
            template.Name,
            project.ProjectName,
            treeEntityModel.AggregateCode,
            treeEntityModel.Code,
            null
        );
        ...
        var result = new TemplateTreeDto()
        {
            ...
            Icon = CodeManagementConsts.AntIconFile,
            IsFolder = false,
        };
    ...
    private async Task<List<TemplateTreeDto>> RenderEntityAsync(
        TemplateDetailDto template,
        GeneratorProjectContext project,
    ...
    {
        var result = new List<TemplateTreeDto>();
        if (template.ControlType != ControlType.Entity)
            return result;
    ...
    private async Task<List<TemplateTreeDto>> RenderEnumAsync(
        TemplateDetailDto template,
        GeneratorProjectContext project,
        GeneratorTreeEntityModelContext treeEntityModelContexts
    )
    {
        var result = new List<TemplateTreeDto>();
        if (template.ControlType != ControlType.Enum)
            return result;
        ...
        foreach (var entity in treeEntityModelContexts.EntityModels)
        {
            ...
            {
                var code = await GeneratorCodeAsync(
                    template,
                    project,
                    treeEntityModelContexts,
                    enumType
                );
                result.Add(code);
            }
    ...
    private async Task<TemplateTreeDto> RenderGlobalAsync(
        TemplateDetailDto template,
        GeneratorProjectContext project,
        List<GeneratorEntityModelContext> entityModels
    )
    {
    ...
    private async Task<string> RenderFileNameAsync(
        string name,
        string projectName,
    ...
    {
        var fileName = await RenderAsync(
            name,
            new
            {
                projectName,
    ...
    public async Task<TemplateTreeDto> GeneratorFolderAsync(string name)
    {
        var result = new TemplateTreeDto()
        {
            ...
            Icon = CodeManagementConsts.AntIconFolder,
            IsFolder = true,
