...
public abstract class EmailSenderBase : IEmailSender
{
    ...
    {
        await SendAsync(
            BuildMailMessage(null, to, subject, body, isBodyHtml, additionalEmailSendingArgs)
        );
    }
    ...
    {
        await SendAsync(
            BuildMailMessage(from, to, subject, body, isBodyHtml, additionalEmailSendingArgs)
        );
    }
    ...
    )
    {
        ...
        if (additionalEmailSendingArgs != null)
        {
            ...
            {
                foreach (
                    var attachment in additionalEmailSendingArgs.Attachments.Where(x =>
                        x.File != null
                    )
                )
                {
