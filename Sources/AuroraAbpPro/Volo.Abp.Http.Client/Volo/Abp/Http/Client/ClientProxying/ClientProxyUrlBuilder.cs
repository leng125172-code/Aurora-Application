...
public class ClientProxyUrlBuilder : ITransientDependency
{
    ...
    public async Task<string> GenerateUrlWithParametersAsync(
        ActionApiDescriptionModel action,
        IReadOnlyDictionary<string, object?> methodArguments,
    ...
    )
    {
        var pathParameters = action
            .Parameters.Where(p => p.BindingSourceId == ParameterBindingSources.Path)
            .ToArray();
        ...
        foreach (var pathParameter in pathParameters.Where(p => p.Name != "apiVersion")) //TODO: Constant!
        {
            ...
            if (value == null)
            {
                ...
                else if (pathParameter.DefaultValue != null)
                {
                    urlBuilder = urlBuilder.Replace(
                        $"{{{pathParameter.Name}}}",
                        await ConvertValueToStringAsync(pathParameter.DefaultValue)
                    );
                }
                else
                {
            ...
            else
            {
                                ...
                                .Invoke(
                                    ...
                                    new object?[]
                                    {
                                        scope.ServiceProvider.GetRequiredService(
                                            HttpClientProxyingOptions.PathConverts[value.GetType()]
                                        ),
                                        action,
                                        pathParameter,
                                        value,
                ...
                urlBuilder = urlBuilder.Replace(
                    $"{{{pathParameter.Name}}}",
                    await ConvertValueToStringAsync(value)
                );
            }
        }
    }

    protected virtual async Task AddQueryStringParametersAsync(
        StringBuilder urlBuilder,
        ActionApiDescriptionModel action,
        IReadOnlyDictionary<string, object?> methodArguments,
        ApiVersionInfo apiVersion
    ...
    {
        var queryStringParameters = action
            .Parameters.Where(p =>
        ...
        foreach (var queryStringParameter in queryStringParameters)
        {
            ...
            if (HttpClientProxyingOptions.QueryStringConverts.ContainsKey(value.GetType()))
            {
                using (var scope = ServiceScopeFactory.CreateScope())
                {
                    var queryString = await (Task<string?>)
                        CallObjectToQueryStringAsyncMethod
                            ...
                            .Invoke(
                                ...
                                {
                                    scope.ServiceProvider.GetRequiredService(
                                        HttpClientProxyingOptions.QueryStringConverts[
                                            value.GetType()
                                        ]
                                    ),
                                    action,
            ...
            if (
                await AddQueryStringParameterAsync(
                    urlBuilder,
                    isFirstParam,
    ...
    protected virtual async Task<string> ObjectToQueryStringAsync<T>(
        IObjectToQueryString<T> converter,
    ...
    protected virtual async Task<string> ObjectToPathAsync<T>(
        IObjectToPath<T> converter,
        ActionApiDescriptionModel actionApiDescription,
        ParameterApiDescriptionModel parameterApiDescription,
        T value
    )
    ...
    )
    {
        if (value.GetType().IsArray || (value.GetType().IsGenericType && value is IEnumerable))
        {
            ...
            foreach (var item in (IEnumerable)value)
            {
                ...
                urlBuilder.Append(
                    name
                        + $"[{index++}]="
                        + System.Net.WebUtility.UrlEncode(await ConvertValueToStringAsync(item))
                        + "&"
                );
            }
        ...
        urlBuilder.Append(isFirstParam ? "?" : "&");
        urlBuilder.Append(
            name + "=" + System.Net.WebUtility.UrlEncode(await ConvertValueToStringAsync(value))
        );
        return true;
    }
    ...
    protected virtual Task<string> ConvertValueToStringAsync(object? value)
    {
        if (value is DateTime dateTimeValue)
        {
            ...
            return Task.FromResult(
                dateTimeValue.ToString("yyyy-MM-ddTHH:mm:ss.fffffff").TrimEnd('0').TrimEnd('.')
            );
        }
