...
public class ClientProxyApiDescriptionFinder
        ...
        ISingletonDependency
{
    ...
    protected virtual void Initialize()
    {
        ...
        var controllers = ApplicationApiDescriptionModel
            .Modules.Select(x => x.Value)
            .SelectMany(x => x.Controllers.Values)
            .ToList();
        ...
        foreach (var controller in controllers.Where(x => x.Interfaces.Any()))
        {
            ...
            {
                var actionKey =
                    $"{appServiceType}.{actionItem.Name}.{string.Join("-", actionItem.ParametersOnMethod.Select(x => x.Type))}";
    ...
    protected virtual ApplicationApiDescriptionModel GetApplicationApiDescriptionModel()
    {
        ...
        foreach (var fileInfo in fileInfoList)
        {
            using (var streamReader = new StreamReader(fileInfo.CreateReadStream()))
            {
                ...
                var subApplicationApiDescription =
                    JsonSerializer.Deserialize<ApplicationApiDescriptionModel>(content);
