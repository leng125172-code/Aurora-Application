...
internal static class TelemetryPeriod
{
    ...
    static TelemetryPeriod()
    {
        ...
        InformationSendPeriod = isTestMode ? TimeSpan.FromSeconds(15) : TimeSpan.FromDays(2);

        ActivitySendPeriod = isTestMode ? TimeSpan.FromSeconds(5) : TimeSpan.FromDays(1);
    }
    ...
    {
        var testModeVariable = Environment.GetEnvironmentVariable(
