...
public class TelemetryActivityEventBuilder : ITelemetryActivityEventBuilder, ITransientDependency
{
    ...
    public TelemetryActivityEventBuilder(
        IEnumerable<ITelemetryActivityEventEnricher> activityDataEnrichers
    )
    {
    ...
    {
        return ProxyHelper
                .GetUnProxiedType(enricher)
                .Assembly.FullName!.StartsWith(TelemetryConsts.VoloNameSpaceFilter)
            && enricher
                is not IHasParentTelemetryActivityEventEnricher<TelemetryActivityEventEnricher>;
    }
}
