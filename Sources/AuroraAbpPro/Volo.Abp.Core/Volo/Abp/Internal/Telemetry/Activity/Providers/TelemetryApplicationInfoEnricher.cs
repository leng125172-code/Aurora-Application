...
public sealed class TelemetryApplicationInfoEnricher
        ...
        IHasParentTelemetryActivityEventEnricher<TelemetrySessionInfoEnricher>
{
    ...
    protected override Task ExecuteAsync(ActivityContext context)
    {
        try
        {
            ...
            context.ExtraProperties[ActivityPropertyNames.SolutionPath] =
                projectMetaData.AbpSlnPath;
            context.Current[ActivityPropertyNames.ProjectType] =
                projectMetaData.Role ?? string.Empty;
    ...
    private static Guid? ReadSolutionIdFromSolutionPath(string solutionPath)
    {
        try
        {
            ...
            if (
                doc.RootElement.TryGetProperty("id", out var property)
                && property.TryGetGuid(out var solutionId)
            )
            {
                return solutionId;
            }
