...
public class TelemetryService : ITelemetryService, IScopedDependency
{
    ...
    public IAsyncDisposable TrackActivityAsync(
        string activityName,
    ...
    )
    {
        ...
        var context = ActivityContext.Create(
            activityName,
    ...
    private Task AddActivityAsync(ActivityContext context)
    {
        var telemetryTask = Task.Run(async () =>
        {
            ...
            var telemetryActivityEventBuilder =
                scope.ServiceProvider.GetRequiredService<ITelemetryActivityEventBuilder>();
            var telemetryActivityStorage =
                scope.ServiceProvider.GetRequiredService<ITelemetryActivityStorage>();
            var telemetryActivitySender =
                scope.ServiceProvider.GetRequiredService<ITelemetryActivitySender>();

            await BuildAndSendActivityAsync(
                context,
