...
internal static class DeviceManager
{
    ...
    private static char GetOsArchitectureOrDefault(char defaultValue = '*')
    {
        ...
        {
            return (
                (int)System.Runtime.InteropServices.RuntimeInformation.OSArchitecture
            ).ToString()[0];
        }
        catch
        {
    ...
    {
        using (
            var managementObjectSearcher = new System.Management.ManagementObjectSearcher(
        ...
        )
        {
            using (var searcherObj = managementObjectSearcher.Get())
            {
                ...
                return managementObjectEnumerator
                    .Current.GetPropertyValue("SerialNumber")
                    .ToString()!;
            }
        }
    }
    ...
    private static string GetHarddiskSerialForOsX()
    {
        ...
        command = System.Text.RegularExpressions.Regex.Replace(
            command,
            @"(\\*)" + "\"",
            @"$1$1\" + "\""
        );
