...
internal sealed class RiderDetector : SoftwareDetector
{
    ...
    public override Task<SoftwareInfo?> DetectAsync()
    {
        try
        {
            ...
            var latest = riderDirs.OrderByDescending(x => x.Version).First();
            ...
            if (File.Exists(colorsFile))
            {
                try
                {
                    ...
                    var schemeEl = doc.Descendants("global_color_scheme").FirstOrDefault();
                    var schemeName = schemeEl?.Attribute("name")?.Value;
                    if (!schemeName.IsNullOrEmpty())
                    {
                        theme =
                            schemeName.IndexOf("dark", StringComparison.OrdinalIgnoreCase) >= 0
                                ? "Dark"
                                : "Light";
                    }
                }
            ...
            return Task.FromResult<SoftwareInfo?>(
                new SoftwareInfo(Name, latest.Version?.ToString(), theme, SoftwareType.Ide)
            );
        }
        catch (Exception e)
        {
