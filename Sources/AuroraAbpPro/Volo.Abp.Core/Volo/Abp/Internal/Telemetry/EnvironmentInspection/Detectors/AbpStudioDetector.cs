...
internal sealed class AbpStudioDetector : SoftwareDetector
{
    ...
    private const string AbpStudioVersionExtensionName =
    ...
    public override Task<SoftwareInfo?> DetectAsync()
    {
        try
        {
            ...
            return Task.FromResult<SoftwareInfo?>(
                new SoftwareInfo(Name, version, uiTheme, SoftwareType.AbpStudio)
            );
        }
        catch
        {
    ...
    {
        var ideStateJsonPath = Path.Combine(TelemetryPaths.Studio, "ui", "ide-state.json");
        ...
        return doc.RootElement.TryGetProperty("theme", out var themeElement)
    ...
    private string? GetAbpStudioVersion()
    {
        ...
        if (
            doc.RootElement.TryGetProperty("Extensions", out var extensionsElement)
        ...
        )
        {
            ...
            {
                if (
                    extension.TryGetProperty("name", out var nameProp)
