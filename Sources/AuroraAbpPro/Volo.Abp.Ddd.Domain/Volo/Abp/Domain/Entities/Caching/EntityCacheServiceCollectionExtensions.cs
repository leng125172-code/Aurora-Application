...
public static class EntityCacheServiceCollectionExtensions
{
        ...
        where TEntity : Entity<TKey>
    {
        ...
        {
            options.ConfigureCache<EntityCacheItemWrapper<TEntity>>(
                cacheOptions ?? GetDefaultCacheOptions()
            );
        });
        ...
        services.Configure<AbpSystemTextJsonSerializerModifiersOptions>(options =>
        {
            options.Modifiers.Add(
                new AbpIncludeNonPublicPropertiesModifiers<TEntity, TKey>().CreateModifyAction(x =>
                    x.Id
                )
            );
        });
        ...
        where TEntityCacheItem : class
    {
        ...
        services.Configure<AbpDistributedCacheOptions>(options =>
        {
            options.ConfigureCache<EntityCacheItemWrapper<TEntityCacheItem>>(
                cacheOptions ?? GetDefaultCacheOptions()
            );
        });
    ...
    public static IServiceCollection AddEntityCache<
        TObjectMapperContext,
        TEntity,
        TEntityCacheItem,
        TKey
    >(this IServiceCollection services, DistributedCacheEntryOptions? cacheOptions = null)
        where TEntity : Entity<TKey>
        where TEntityCacheItem : class
    {
        services.TryAddTransient<
            IEntityCache<TEntityCacheItem, TKey>,
            EntityCacheWithObjectMapperContext<
                TObjectMapperContext,
                TEntity,
                TEntityCacheItem,
                TKey
