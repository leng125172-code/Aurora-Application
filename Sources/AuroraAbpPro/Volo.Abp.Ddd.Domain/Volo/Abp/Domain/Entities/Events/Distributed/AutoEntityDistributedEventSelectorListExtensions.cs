...
public static class AutoEntityDistributedEventSelectorListExtensions
{
    ...
    )
    {
        ...
        selectors.Add(
            new NamedTypeSelector(selectorName, t => t.FullName?.StartsWith(namespaceName) ?? false)
        );
    }
        ...
        where TEntity : IEntity
    {
        ...
        selectors.Add(
            new NamedTypeSelector(selectorName, t => typeof(TEntity).IsAssignableFrom(t))
        );
    }
    ...
    public static void AddAll([NotNull] this IAutoEntityDistributedEventSelectorList selectors)
    {
        ...
        selectors.Add(
            new NamedTypeSelector(AllEntitiesSelectorName, t => typeof(IEntity).IsAssignableFrom(t))
        );
    }
    ...
    )
    {
        ...
        if (selectors.Any(s => s.Name == selectorName))
        {
            throw new AbpException(
                $"There is already a selector added before with the same name: {selectorName}"
            );
        }

        selectors.Add(new NamedTypeSelector(selectorName, predicate));
    }
