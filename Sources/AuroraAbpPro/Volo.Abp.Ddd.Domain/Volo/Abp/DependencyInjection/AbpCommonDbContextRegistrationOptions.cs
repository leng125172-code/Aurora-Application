    ...
    : IAbpCommonDbContextRegistrationOptionsBuilder
{
    ...
    public IAbpCommonDbContextRegistrationOptionsBuilder ReplaceDbContext<
        TOtherDbContext,
        TTargetDbContext
    >(MultiTenancySides multiTenancySides = MultiTenancySides.Both)
    {
        return ReplaceDbContext(
            typeof(TOtherDbContext),
            typeof(TTargetDbContext),
            multiTenancySides
        );
    }
    ...
    )
    {
        ...
        ReplacedDbContextTypes[
            new MultiTenantDbContextType(otherDbContextType, multiTenancySides)
        ] = targetDbContextType;

        return this;
    }

    public IAbpCommonDbContextRegistrationOptionsBuilder AddDefaultRepositories(
    ...
    )
    {
        ...
        {
            throw new AbpException(
                $"{OriginalDbContextType.AssemblyQualifiedName} should inherit/implement {defaultRepositoryDbContextType.AssemblyQualifiedName}!"
            );
        }
    ...
    public IAbpCommonDbContextRegistrationOptionsBuilder AddDefaultRepositories<TDefaultRepositoryDbContext>(
        bool includeAllEntities = false
    )
    {
    ...
    )
    {
        ...
        Check.NotNull(
            ...
            nameof(repositoryImplementationTypeWithoutKey)
        );
