    ...
    public void PostConfigure(string? name, AbpHangfireOptions options)
    {
        ...
        if (queuesPrefix.IsNullOrWhiteSpace())
        {
            throw new AbpException(
                $"The QueuesPrefix({options.DefaultQueuePrefix}) is not valid, it must consist of lowercase letters, digits, underscore, and dash characters only."
            );
        }
        ...
        if (options.ServerOptions == null)
        {
            ...
            {
                throw new AbpException(
                    $"The maximum length of the Hangfire queue name({queue}) is {options.MaxQueueNameLength}, Please configure the AbpHangfireOptions.DefaultQueuePrefix manually."
                );
            }
        ...
        else
        {
            ...
            if (defaultQueue.IsNullOrWhiteSpace())
            {
                ...
                {
                    throw new AbpException(
                        "There is no queue defined in the Hangfire configuration!"
                    );
                }
            }
