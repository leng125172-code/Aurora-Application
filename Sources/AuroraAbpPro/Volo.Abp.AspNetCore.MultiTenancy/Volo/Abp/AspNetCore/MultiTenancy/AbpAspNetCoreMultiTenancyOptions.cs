...
public class AbpAspNetCoreMultiTenancyOptions
{
    ...
    public Func<
        ...
        Exception,
        Task<bool>
    > MultiTenancyMiddlewareErrorPageBuilder { get; set; }
    ...
    public AbpAspNetCoreMultiTenancyOptions()
    {
        ...
        MultiTenancyMiddlewareErrorPageBuilder = async (context, exception) =>
        {
            ...
            var tenantResolveResult = context
                .RequestServices.GetRequiredService<ITenantResolveResultAccessor>()
                .Result;
            if (tenantResolveResult != null)
            {
                if (
                    tenantResolveResult.AppliedResolvers.Count == 1
                    && tenantResolveResult.AppliedResolvers.Contains(
                ...
                )
                {
                    ...
                    {
                        var scheme = await context
                            .RequestServices.GetRequiredService<IAuthenticationHandlerProvider>()
                            .GetHandlerAsync(context, authenticationType);
                ...
                var options = context
                    .RequestServices.GetRequiredService<
                        IOptions<AbpAspNetCoreMultiTenancyOptions>
                    >()
                    .Value;
                if (
                    tenantResolveResult.AppliedResolvers.Contains(
                        CookieTenantResolveContributor.ContributorName
            ...
            context.Response.Headers.Append(
                "Abp-Tenant-Resolve-Error",
                HtmlEncoder.Default.Encode(exception.Message)
            );
            if (
                isCookieAuthentication
            ...
            else if (context.Request.IsAjax())
            {
                var error = new RemoteServiceErrorResponse(
                    new RemoteServiceErrorInfo(
                        exception.Message,
                        exception is BusinessException businessException
                            ? businessException.Details
                            : string.Empty
                    )
                );

                var jsonSerializerOptions = context
                ...
                var cancellationTokenProvider =
                    context.RequestServices.GetRequiredService<ICancellationTokenProvider>();
                ...
                if (resolvedContentTypeEncoding.CodePage == Encoding.UTF8.CodePage)
                {
                    try
                    {
                        ...
                        when (cancellationTokenProvider.Token.IsCancellationRequested) { }
                }
                else
                {
            ...
            else
            {
                ...
                var errorPage = new MultiTenancyMiddlewareErrorPage(
                    new MultiTenancyMiddlewareErrorPageModel(message, details!)
                );
