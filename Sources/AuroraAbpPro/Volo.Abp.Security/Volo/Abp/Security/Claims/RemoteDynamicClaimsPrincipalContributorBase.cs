    ...
    where TContributorCache : RemoteDynamicClaimsPrincipalContributorCacheBase<TContributorCache>
{
    public override async Task ContributeAsync(AbpClaimsPrincipalContributorContext context)
    {
        ...
        var dynamicClaimsCache = context
            .GetRequiredService<TContributorCache>()
            .As<TContributorCache>();
        AbpDynamicClaimCacheItem dynamicClaims;
        try
        {
            dynamicClaims = await dynamicClaimsCache.GetAsync(
                userId.Value,
                identity.FindTenantId()
            );
        }
        catch (Exception e)
        {
            ...
            logger.LogWarning(
                e,
                $"Failed to refresh remote dynamic claims cache for user: {userId.Value}"
            );
            return;
        }
