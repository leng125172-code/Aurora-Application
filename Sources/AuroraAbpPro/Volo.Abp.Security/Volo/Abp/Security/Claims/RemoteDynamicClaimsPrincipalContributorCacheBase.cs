...
public abstract class RemoteDynamicClaimsPrincipalContributorCacheBase<TContributorCache>
{
    ...
    protected RemoteDynamicClaimsPrincipalContributorCacheBase(
    ...
    public async Task<AbpDynamicClaimCacheItem> GetAsync(Guid userId, Guid? tenantId = null)
    {
        ...
        if (dynamicClaims == null)
        {
            throw new AbpException(
                $"Failed to get dynamic claims for user: {userId} from cache after refreshing, "
    ...
    protected abstract Task<AbpDynamicClaimCacheItem?> GetCacheAsync(
        Guid userId,
        Guid? tenantId = null
    );
