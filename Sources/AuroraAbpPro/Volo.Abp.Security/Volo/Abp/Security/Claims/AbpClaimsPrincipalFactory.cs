...
public class AbpClaimsPrincipalFactory : IAbpClaimsPrincipalFactory, ITransientDependency
{
    ...
    public virtual async Task<ClaimsPrincipal> CreateDynamicAsync(
        ClaimsPrincipal? existsClaimsPrincipal = null
    )
    {
    ...
    )
    {
        var claimsPrincipal =
            ...
            ?? new ClaimsPrincipal(
                new ClaimsIdentity(AuthenticationType, AbpClaimTypes.UserName, AbpClaimTypes.Role)
            );
        ...
        else
        {
            ...
            {
                var contributor = (IAbpDynamicClaimsPrincipalContributor)
                    ServiceProvider.GetRequiredService(contributorType);
                await contributor.ContributeAsync(context);
            }
        }
