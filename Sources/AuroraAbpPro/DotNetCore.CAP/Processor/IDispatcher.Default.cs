...
public class Dispatcher : IDispatcher
{
    ...
    public async ValueTask StartAsync(CancellationToken stoppingToken)
    {
        ...
        _tasksCts = CancellationTokenSource.CreateLinkedTokenSource(
            stoppingToken,
    ...
    )
    {
        ...
        var statusName =
            timeSpan <= TimeSpan.FromMinutes(1) ? StatusName.Queued : StatusName.Delayed;

        await _storage
            .ChangePublishStateAsync(message, statusName, transaction)
    ...
    )
    {
        try
        {
            ...
            {
                await WriteToChannelAsync(_receivedChannel, (message, descriptor))
                    .ConfigureAwait(false);
            }
            else
            {
                await _executor
                    .ExecuteAsync(message, descriptor, _tasksCts!.Token)
                    .ConfigureAwait(false);
            }
        }
        catch (OperationCanceledException)
    ...
    {
        var bufferSize =
            _options.SubscriberParallelExecuteThreadCount
            * _options.SubscriberParallelExecuteBufferFactor;
        ...
        _tasksCts!.Token.Register(() =>
        {
            try
            {
                ...
                _storage
                    ...
                    .GetResult();
                _logger.LogDebug(
                    "Update storage to delayed success of delayed message in memory queue!"
                );
            }
            catch (Exception e)
            {
    ...
    private async Task SendScheduledMessageDirectlyAsync(MediumMessage message)
    {
        try
        {
            ...
            {
                _logger.LogError(
                    "Delay message sending failed. MessageId: {MessageId} ",
                    message.DbId
                );
            }
        }
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "Error sending scheduled message. MessageId: {MessageId}",
                message.DbId
            );
        }
    }
    ...
    private async ValueTask SendingAsync()
    {
        ...
        {
            while (
                await _publishedChannel
                    .Reader.WaitToReadAsync(_tasksCts!.Token)
                    .ConfigureAwait(false)
            )
            {
    ...
    private async Task ProcessReceivedMessageAsync(
        (MediumMessage, ConsumerExecutorDescriptor?) messageData
    )
    {
        try
        {
