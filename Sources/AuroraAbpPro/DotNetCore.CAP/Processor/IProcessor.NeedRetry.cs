...
public class MessageNeedToRetryProcessor : IProcessor
{
    ...
    private async Task ProcessPublishedAsync(IDataStorage connection, ProcessingContext context)
    {
        ...
        var messages = await GetSafelyAsync(
                connection.GetPublishedMessagesOfNeedRetry,
                _lookbackSeconds
            )
    ...
    private async Task ProcessReceivedAsync(IDataStorage connection, ProcessingContext context)
    {
        ...
        var messages = await GetSafelyAsync(
                connection.GetReceivedMessagesOfNeedRetry,
                _lookbackSeconds
            )
            .ConfigureAwait(false);
    ...
    private async Task<IEnumerable<T>> GetSafelyAsync<T>(
    ...
    private void CheckSafeOptionsSet()
    {
        if (
            _lookbackSeconds
            < TimeSpan.FromSeconds(MinSuggestedValueForFallbackWindowLookbackSeconds)
        )
        {
            _logger.LogWarning(
                ...
                MinSuggestedValueForFallbackWindowLookbackSeconds
            );
        }
    }
}
