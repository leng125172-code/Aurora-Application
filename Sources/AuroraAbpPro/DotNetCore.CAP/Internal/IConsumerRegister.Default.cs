...
internal class ConsumerRegister : IConsumerRegister
{
    ...
    private void RegisterMessageProcessor(IConsumerClient client)
    {
        ...
        client.OnMessageCallback = async (transportMessage, sender) =>
        {
            ...
            try
            {
                ...
                var canFindSubscriber = _selector.TryGetTopicExecutor(
                    name,
                    group,
                    out var executor
                );
                try
                {
                    ...
                    var type = executor!
                        .Parameters.FirstOrDefault(x => x.IsFromCap == false)
                        ?.ParameterType;
                ...
                {
                    transportMessage.Headers[Headers.Exception] =
                ...
                if (message.HasException())
                {
                    ...
                    try
                    {
                        ...
                        _logger.ConsumerExecutedAfterThreshold(
                            message.GetId(),
                            _options.FailedRetryCount
                        );
                    }
                    catch (Exception e)
                    {
    ...
    private void WriteLog(LogMessageEventArgs logmsg)
    {
        switch (logmsg.LogType)
        {
            ...
            case MqLogType.ConsumeRetries:
                _logger.LogWarning(
                    "Kafka client consume exception, retying... --> " + logmsg.Reason
                );
                break;
            ...
            case MqLogType.ExceptionReceived:
                _logger.LogError(
                    "AzureServiceBus subscriber received an error. --> " + logmsg.Reason
                );
                break;
            ...
            case MqLogType.InvalidIdFormat:
                _logger.LogError(
                    "AmazonSQS subscriber delete inflight message failed, invalid id. --> "
                        + logmsg.Reason
                );
                break;
    ...
    )
    {
        ...
        if (
            ...
            && s_diagnosticListener.IsEnabled(CapDiagnosticListenerNames.AfterConsume)
        )
        {
    ...
    )
    {
        if (
            tracingTimestamp != null
            && s_diagnosticListener.IsEnabled(CapDiagnosticListenerNames.ErrorConsume)
        )
        {
            var now = DateTimeOffset.UtcNow.ToUnixTimeMilliseconds();
