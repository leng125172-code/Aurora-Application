...
internal class MessageSender : IMessageSender
{
    ...
    private async Task SetSuccessfulState(MediumMessage message)
    {
        ...
        await _dataStorage
            .ChangePublishStateAsync(message, StatusName.Succeeded)
    ...
    private async Task<bool> SetFailedState(MediumMessage message, Exception ex)
    {
        ...
        await _dataStorage
            .ChangePublishStateAsync(message, StatusName.Failed)
            .ConfigureAwait(false);
    ...
    )
    {
        if (
            ...
            && s_diagnosticListener.IsEnabled(CapDiagnosticListenerNames.AfterPublish)
        )
        {
    ...
    )
    {
        if (
            tracingTimestamp != null
            && s_diagnosticListener.IsEnabled(CapDiagnosticListenerNames.ErrorPublish)
        )
        {
