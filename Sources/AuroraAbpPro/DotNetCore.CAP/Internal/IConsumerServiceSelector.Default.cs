...
public class ConsumerServiceSelector : IConsumerServiceSelector
{
    ...
    )
    {
        ...
        foreach (var service in serviceCollection)
        {
            ...
            if (service.IsKeyedService)
            {
                ...
                var hasKeyedImpl =
                    service.KeyedImplementationType != null
                ...
                if (actualType == null && service.KeyedImplementationFactory != null)
                {
                    ...
                    var instance = scopeProvider.GetRequiredKeyedService(
                        service.ServiceType,
                        service.ServiceKey
                    );
            ...
            results.AddRange(
                GetTopicAttributesDescription(actualType.GetTypeInfo(), serviceTypeInfo)
            );
        }
    ...
    )
    {
        ...
        foreach (var method in typeInfo.GetRuntimeMethods())
        {
            ...
            if (topicClassAttribute is null)
                topicMethodAttributes = topicMethodAttributes.Where(x =>
                    !x.IsPartial && x.Name != null
                );
    ...
    protected virtual void SetSubscribeAttribute(TopicAttribute attribute)
    {
        ...
        {
            attribute.Group =
                $"{prefix}{attribute.Group ?? _capOptions.DefaultGroupName}.{_capOptions.Version}";
        }
    }
    ...
    private static ConsumerExecutorDescriptor? MatchUsingName(
        string key,
        IReadOnlyList<ConsumerExecutorDescriptor> executeDescriptor
    )
    {
    ...
    private ConsumerExecutorDescriptor? MatchWildcardUsingRegex(
        string key,
        IReadOnlyList<ConsumerExecutorDescriptor> executeDescriptor
    )
    {
