...
internal class SubscribeExecutor : ISubscribeExecutor
{
    ...
    )
    {
        ...
        do
        {
            var (shouldRetry, operateResult) = await ExecuteWithoutRetryAsync(
                    message,
                    descriptor,
                    cancellationToken
                )
    ...
    )
    {
        ...
        catch (Exception ex)
        {
            ...
            return (
                await SetFailedState(message, ex).ConfigureAwait(false),
    ...
    private async Task<bool> SetFailedState(MediumMessage message, Exception ex)
    {
        ...
        await _dataStorage
            .ChangeReceiveStateAsync(message, StatusName.Failed)
            .ConfigureAwait(false);
    ...
    )
    {
        ...
        try
        {
            var ret = await Invoker
                .InvokeAsync(consumerContext, cancellationToken)
                .ConfigureAwait(false);
            ...
            if (!string.IsNullOrEmpty(ret.CallbackName))
            {
                ...
                await _provider
                    .GetRequiredService<ICapPublisher>()
                    .PublishAsync(
                        ret.CallbackName,
                        ret.Result,
                        ret.CallbackHeader,
    ...
    private long? TracingBefore(Message message, MethodInfo method)
    {
        if (s_diagnosticListener.IsEnabled(CapDiagnosticListenerNames.BeforeSubscriberInvoke))
        {
            ...
            s_diagnosticListener.Write(
                CapDiagnosticListenerNames.BeforeSubscriberInvoke,
                eventData
            );
