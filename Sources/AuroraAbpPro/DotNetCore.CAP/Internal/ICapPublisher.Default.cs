...
internal class CapPublisher : ICapPublisher
{
    ...
    {
        await PublishInternalAsync(name, value, headers, null, cancellationToken)
    ...
    {
        var headers = new Dictionary<string, string?> { { Headers.CallbackName, callbackName } };
    ...
    )
    {
        if (delayTime <= TimeSpan.Zero)
        {
            throw new ArgumentException(
                "Delay time span must be greater than 0",
                nameof(delayTime)
            );
        }

        await PublishInternalAsync(name, value, headers, delayTime, cancellationToken)
            .ConfigureAwait(false);
    }
    ...
    {
        var header = new Dictionary<string, string?> { { Headers.CallbackName, callbackName } };

        await PublishDelayAsync(delayTime, name, value, header, cancellationToken)
            .ConfigureAwait(false);
    }
    ...
    {
        PublishDelayAsync(delayTime, name, value, headers)
            .ConfigureAwait(false)
            .GetAwaiter()
            .GetResult();
    }
    ...
    {
        PublishDelayAsync(delayTime, name, value, callbackName)
            .ConfigureAwait(false)
            .GetAwaiter()
            .GetResult();
    }
        ...
        try
        {
            ...
            if (Transaction?.DbTransaction == null)
            {
                var mediumMessage = await _storage
                    .StoreMessageAsync(name, message)
                    .ConfigureAwait(false);
                ...
                {
                    await _dispatcher
                        .EnqueueToScheduler(mediumMessage, publishTime)
                        .ConfigureAwait(false);
                }
                else
                {
    ...
    private static long? TracingBefore(Message message)
    {
        if (s_diagnosticListener.IsEnabled(CapDiagnosticListenerNames.BeforePublishMessageStore))
        {
            ...
            s_diagnosticListener.Write(
                CapDiagnosticListenerNames.BeforePublishMessageStore,
                eventData
            );
    ...
    private static void TracingAfter(long? tracingTimestamp, Message message)
    {
        ...
        )
        {
            ...
            s_diagnosticListener.Write(
                CapDiagnosticListenerNames.AfterPublishMessageStore,
                eventData
            );
        }
    }
    ...
    private static void TracingError(long? tracingTimestamp, Message message, Exception ex)
    {
        ...
        )
        {
            ...
            s_diagnosticListener.Write(
                CapDiagnosticListenerNames.ErrorPublishMessageStore,
                eventData
            );
        }
    }
