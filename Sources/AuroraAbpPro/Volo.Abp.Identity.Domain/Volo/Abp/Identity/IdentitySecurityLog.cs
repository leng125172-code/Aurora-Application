...
public class IdentitySecurityLog : AggregateRoot<Guid>, IMultiTenant
{
        ...
        : base(guidGenerator.Create())
    {
        ...
        TenantName = securityLogInfo.TenantName.Truncate(
            IdentitySecurityLogConsts.MaxTenantNameLength
        );

        ApplicationName = securityLogInfo.ApplicationName.Truncate(
            IdentitySecurityLogConsts.MaxApplicationNameLength
        );
        ...
        ClientIpAddress = securityLogInfo.ClientIpAddress.Truncate(
            IdentitySecurityLogConsts.MaxClientIpAddressLength
        );
        ClientId = securityLogInfo.ClientId.Truncate(IdentitySecurityLogConsts.MaxClientIdLength);
        CorrelationId = securityLogInfo.CorrelationId.Truncate(
            IdentitySecurityLogConsts.MaxCorrelationIdLength
        );
        BrowserInfo = securityLogInfo.BrowserInfo.Truncate(
            IdentitySecurityLogConsts.MaxBrowserInfoLength
        );
