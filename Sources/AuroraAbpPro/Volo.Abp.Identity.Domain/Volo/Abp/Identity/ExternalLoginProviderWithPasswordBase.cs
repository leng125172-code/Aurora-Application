...
public abstract class ExternalLoginProviderWithPasswordBase
        ...
        IExternalLoginProviderWithPassword
{
    ...
    protected override Task<ExternalLoginUserInfo> GetUserInfoAsync(string userName)
    {
        throw new NotImplementedException(
            $"{nameof(GetUserInfoAsync)} is not implemented default. It should be overriden and implemented by the deriving class!"
        );
    }

    protected abstract Task<ExternalLoginUserInfo> GetUserInfoAsync(
        string userName,
        string plainPassword
    );

    protected virtual Task<ExternalLoginUserInfo> GetUserInfoAsync(
        IdentityUser user,
        string plainPassword
    )
    {
