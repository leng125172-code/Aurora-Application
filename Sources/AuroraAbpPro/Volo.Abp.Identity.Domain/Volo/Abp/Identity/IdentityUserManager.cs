...
public class IdentityUserManager : UserManager<IdentityUser>, IDomainService
{
    ...
    /// <returns>A <see cref="IdentityResult"/> representing whether validation was successful.</returns>
    public virtual async Task<IdentityResult> CallValidatePasswordAsync(
        IdentityUser user,
        string password
    )
    {
    ...
    public virtual async Task<IdentityResult> CallUpdatePasswordHash(
        IdentityUser user,
        string newPassword,
        bool validatePassword
    )
    ...
    )
    {
        ...
        var result = await RemoveFromRolesAsync(
            user,
            currentRoleNames.Except(roleNames).Distinct()
        );
        if (!result.Succeeded)
        {
            return result;
    ...
    public virtual async Task<bool> IsInOrganizationUnitAsync(
        IdentityUser user,
        OrganizationUnit ou
    )
    {
        await UserRepository.EnsureCollectionLoadedAsync(
            user,
            u => u.OrganizationUnits,
    ...
    public virtual async Task RemoveFromOrganizationUnitAsync(
        IdentityUser user,
        OrganizationUnit ou
    )
    {
    ...
    public virtual async Task SetOrganizationUnitsAsync(
        Guid userId,
        params Guid[] organizationUnitIds
    )
    {
    ...
    {
        var maxCount = await SettingProvider.GetAsync<int>(
            IdentitySettingNames.OrganizationUnit.MaxUserMembershipCount
        );
        if (requestedCount > maxCount)
        {
            throw new BusinessException(IdentityErrorCodes.MaxAllowedOuMembership).WithData(
    ...
    )
    {
        if (includeChildren)
        {
            return await UserRepository.GetUsersInOrganizationUnitWithChildrenAsync(
        ...
        {
            return await UserRepository.GetUsersInOrganizationUnitAsync(
                organizationUnit.Id,
                CancellationToken
            );
        }
    ...
    public virtual async Task<IdentityResult> AddDefaultRolesAsync([NotNull] IdentityUser user)
    {
        ...
        foreach (
            var role in await RoleRepository.GetDefaultOnesAsync(
                cancellationToken: CancellationToken
            )
        )
        {
    ...
    public virtual async Task<bool> ShouldPeriodicallyChangePasswordAsync(IdentityUser user)
    {
        ...
        var forceUsersToPeriodicallyChangePassword = await SettingProvider.GetAsync<bool>(
        ...
        var lastPasswordChangeTime =
            ...
            ?? DateTime.SpecifyKind(user.CreationTime, DateTimeKind.Utc);
        var passwordChangePeriodDays = await SettingProvider.GetAsync<int>(
            IdentitySettingNames.Password.PasswordChangePeriodDays
        );

        return passwordChangePeriodDays > 0
            && lastPasswordChangeTime.AddDays(passwordChangePeriodDays) < DateTime.UtcNow;
    }
    ...
    public virtual async Task ResetRecoveryCodesAsync(IdentityUser user)
    {
        if (!(Store is IdentityUserStore identityUserStore))
        {
            throw new AbpException(
                $"Store is not an instance of {typeof(IdentityUserStore).AssemblyQualifiedName}"
            );
        }

        await identityUserStore.SetTokenAsync(
            user,
            await identityUserStore.GetInternalLoginProviderAsync(),
            await identityUserStore.GetRecoveryCodeTokenNameAsync(),
            string.Empty,
            CancellationToken
        );
    }
    ...
    public override async Task<IdentityResult> SetEmailAsync(IdentityUser user, string email)
    {
        ...
        if (
            !string.IsNullOrEmpty(oldMail)
            && !oldMail.Equals(email, StringComparison.OrdinalIgnoreCase)
        )
        {
    ...
    {
        var sourceRole = await RoleRepository.GetAsync(
            sourceRoleId,
            cancellationToken: CancellationToken
        );

        Logger.LogDebug($"Remove dynamic claims cache for users of role: {sourceRoleId}");
        var userIdList = await UserRepository.GetUserIdListByRoleIdAsync(
            sourceRoleId,
            cancellationToken: CancellationToken
        );
        await DynamicClaimCache.RemoveManyAsync(
            userIdList.Select(userId =>
                AbpDynamicClaimCacheItem.CalculateCacheKey(userId, sourceRole.TenantId)
            ),
            token: CancellationToken
        );

        var targetRole = targetRoleId.HasValue
            ? await RoleRepository.GetAsync(
                targetRoleId.Value,
                cancellationToken: CancellationToken
            )
            : null;
        if (targetRole != null)
        {
            ...
            userIdList = await UserRepository.GetUserIdListByRoleIdAsync(
                targetRoleId.Value,
                cancellationToken: CancellationToken
            );
            await DynamicClaimCache.RemoveManyAsync(
                userIdList.Select(userId =>
                    AbpDynamicClaimCacheItem.CalculateCacheKey(userId, targetRole.TenantId)
                ),
                token: CancellationToken
            );
        }
    ...
    public virtual async Task UpdateOrganizationAsync(
        Guid sourceOrganizationId,
        Guid? targetOrganizationId
    )
    {
        var sourceOrganization = await OrganizationUnitRepository.GetAsync(
            sourceOrganizationId,
            cancellationToken: CancellationToken
        );

        Logger.LogDebug(
            $"Remove dynamic claims cache for users of organization: {sourceOrganizationId}"
        );
        var userIdList = await OrganizationUnitRepository.GetMemberIdsAsync(
            sourceOrganizationId,
        ...
        await DynamicClaimCache.RemoveManyAsync(
            userIdList.Select(userId =>
                AbpDynamicClaimCacheItem.CalculateCacheKey(userId, sourceOrganization.TenantId)
            ),
            token: CancellationToken
        );

        var targetOrganization = targetOrganizationId.HasValue
            ? await OrganizationUnitRepository.GetAsync(
        ...
        {
            Logger.LogDebug(
                $"Remove dynamic claims cache for users of organization: {targetOrganizationId}"
            );
            userIdList = await OrganizationUnitRepository.GetMemberIdsAsync(
                targetOrganizationId.Value,
                includeChildren: true,
                cancellationToken: CancellationToken
            );
            await DynamicClaimCache.RemoveManyAsync(
                userIdList.Select(userId =>
                    AbpDynamicClaimCacheItem.CalculateCacheKey(userId, targetOrganization.TenantId)
                ),
                token: CancellationToken
            );
        }

        await UserRepository.UpdateOrganizationAsync(
            sourceOrganizationId,
    ...
    public virtual async Task<bool> ValidateUserNameAsync(string userName, Guid? userId = null)
    {
        ...
        if (
            !string.IsNullOrEmpty(Options.User.AllowedUserNameCharacters)
            && userName.Any(c => !Options.User.AllowedUserNameCharacters.Contains(c))
        )
        {
            return false;
        }
    ...
    public virtual Task<string> GetRandomUserNameAsync(int length)
    {
        ...
        {
            randomUserName += allowedUserNameCharacters[
                random.Next(0, allowedUserNameCharacters.Length)
            ];
        }
    ...
    public virtual async Task<string> GetUserNameFromEmailAsync(string email)
    {
        ...
        else if (Options.User.AllowedUserNameCharacters.Where(char.IsDigit).Distinct().Count() >= 4)
        {
            ...
            var numbers = Options
                .User.AllowedUserNameCharacters.Where(char.IsDigit)
        ...
        Logger.LogError(
            $"Could not get a valid user name for the given email address: {email}, allowed characters: {Options.User.AllowedUserNameCharacters}, tried {maxTryCount} times."
        );
        throw new AbpIdentityResultException(
            IdentityResult.Failed(ErrorDescriber.InvalidUserName(userName))
        );
    }
    ...
    public virtual async Task<IdentityUser> FindSharedUserByEmailAsync(string email)
    {
        ...
        using (CurrentTenant.Change(null))
        {
            using (DataFilter.Disable<IMultiTenant>())
            {
                ...
                var hostusers = await UserRepository.GetUsersByNormalizedEmailAsync(
                    normalizedEmail,
                    cancellationToken: CancellationToken
                );
                //host user first
                var hostUser =
                    hostusers.FirstOrDefault(x => x.TenantId == null)
                    ?? hostusers.FirstOrDefault(x => x.TenantId != Guid.Empty)
                    ?? hostusers.FirstOrDefault();
                if (hostUser == null)
                {
                    return null;
                }
    ...
    public virtual async Task<IdentityUser> FindSharedUserByNameAsync(string userName)
    {
        ...
        using (CurrentTenant.Change(null))
        {
            using (DataFilter.Disable<IMultiTenant>())
            {
                ...
                var hostusers = await UserRepository.GetUsersByNormalizedUserNameAsync(
                    normalizeduserName,
                    cancellationToken: CancellationToken
                );
                //host user first
    ...
    public virtual async Task<IdentityUser> FindSharedUserByLoginAsync(
    ...
    )
    {
        ...
        using (CurrentTenant.Change(null))
        {
            using (DataFilter.Disable<IMultiTenant>())
            {
                var hostusers = await UserRepository.GetUsersByLoginAsync(
                    loginProvider,
                    providerKey,
                    cancellationToken: CancellationToken
                );
    ...
    public virtual async Task<IdentityUser> FindSharedUserByPasskeyIdAsync(byte[] credentialId)
    {
        ...
        using (CurrentTenant.Change(null))
        {
            using (DataFilter.Disable<IMultiTenant>())
            {
                var hostusers = await UserRepository.GetUsersByPasskeyIdAsync(
                    credentialId,
                    cancellationToken: CancellationToken
                );
                //host user first
