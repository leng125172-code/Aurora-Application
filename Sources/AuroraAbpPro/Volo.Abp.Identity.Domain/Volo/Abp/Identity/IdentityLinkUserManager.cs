...
public class IdentityLinkUserManager : DomainService
{
    ...
    )
    {
        ...
        {
            var users = await IdentityLinkUserRepository.GetListAsync(
                linkUserInfo,
            ...
            var allUsers = await IdentityLinkUserRepository.GetListAsync(
                batchSize,
                cancellationToken: cancellationToken
            );
    ...
    )
    {
        ...
        {
            if (
                sourceLinkUser.UserId == targetLinkUser.UserId
    ...
    )
    {
        using (CurrentTenant.Change(null))
        {
            ...
            {
                return (
                    await GetListAsync(sourceLinkUser, true, cancellationToken: cancellationToken)
                ).Any(x =>
                    x.SourceTenantId == targetLinkUser.TenantId
                        && x.SourceUserId == targetLinkUser.UserId
                    || x.TargetTenantId == targetLinkUser.TenantId
                        && x.TargetUserId == targetLinkUser.UserId
                );
            }
            return await IdentityLinkUserRepository.FindAsync(
                    sourceLinkUser,
                    targetLinkUser,
                    cancellationToken
                ) != null;
    ...
    )
    {
        using (CurrentTenant.Change(null))
        {
            ...
            var linkedUser = await IdentityLinkUserRepository.FindAsync(
                sourceLinkUser,
                targetLinkUser,
                cancellationToken
            );
