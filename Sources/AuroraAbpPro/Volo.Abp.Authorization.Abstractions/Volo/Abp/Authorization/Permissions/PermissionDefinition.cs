        ...
        ICanAddChildPermission
{
        ...
        : this(name, displayName, multiTenancySide, isEnabled)
    {
        ...
        ManagementPermissionName = Check.NotNull(
            managementPermissionName,
            nameof(managementPermissionName)
        );
    }
    ...
    )
    {
        if (ResourceName != null)
        {
            throw new AbpException(
                $"Resource permission cannot have child permissions. Resource: {ResourceName}"
            );
        }

        var child = new PermissionDefinition(name, displayName, multiTenancySide, isEnabled)
        {
            Parent = this,
        };

        child[PermissionDefinitionContext.KnownPropertyNames.CurrentProviderName] = this[
