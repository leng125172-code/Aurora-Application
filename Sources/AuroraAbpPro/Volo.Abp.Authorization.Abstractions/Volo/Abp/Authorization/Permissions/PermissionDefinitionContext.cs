...
public class PermissionDefinitionContext : IPermissionDefinitionContext
{
    ...
    )
    {
        ...
        if (Groups.ContainsKey(name))
        {
            throw new AbpException(
                $"There is already an existing permission group with name: {name}"
            );
        }
    ...
    )
    {
        ...
        {
            throw new AbpException(
                $"There is already an existing resource permission with name: {name} for resource: {resourceName}"
            );
        }
    ...
    public virtual PermissionDefinition? GetResourcePermissionOrNull(
    ...
    public virtual void RemoveResourcePermission(
        [NotNull] string resourceName,
        [NotNull] string name
    )
    {
        ...
        {
            throw new AbpException(
                $"Not found resource permission with name: {name} for resource: {resourceName}"
            );
        }
