...
public class BundleConfigurationCollection
{
    ...
    private readonly ConcurrentDictionary<
        string,
        List<Action<BundleConfiguration>>
    > _lazyBundleConfigurationActions;
    ...
    public BundleConfigurationCollection()
    {
        ...
        _lazyBundleConfigurationActions =
            new ConcurrentDictionary<string, List<Action<BundleConfiguration>>>();
    ...
    )
    {
        if (!TryAdd(bundleName, configureAction))
        {
            throw new AbpException(
                $"There is already a bundle added with given {nameof(bundleName)}: {bundleName}"
            );
        }
    ...
    private BundleConfiguration CreateBundle(
        string bundleName,
        Action<BundleConfiguration>? configureAction
    )
    {
    ...
    public BundleConfigurationCollection Configure(
        [NotNull] string bundleName,
        [NotNull] Action<BundleConfiguration> configureAction
    )
    {
        Check.NotNull(bundleName, nameof(bundleName));
        ...
        {
            var configurationActions = _lazyBundleConfigurationActions.GetOrAdd(
                bundleName,
                () => new List<Action<BundleConfiguration>>()
            );
    ...
    /// <param name="configureAction">Configure action</param>
    /// <returns>Returns this object for chained calls.</returns>
    public BundleConfigurationCollection ConfigureAll(
        [NotNull] Action<BundleConfiguration> configureAction
    )
    {
        Check.NotNull(configureAction, nameof(configureAction));
    ...
    public BundleConfiguration Get([NotNull] string bundleName)
    {
        ...
        {
            throw new AbpException(
                $"There is no bundle with given {nameof(bundleName)}: {bundleName}"
            );
        }
