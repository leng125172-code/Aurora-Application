...
public class BundleContributorCollection
{
    ...
    public void Replace(
        [NotNull] Type sourceContributorType,
        [NotNull] Type destContributorType,
    ...
    )
    {
        ...
        {
            _contributors.ReplaceOne(
                x => x.GetType() == sourceContributorType,
    ...
    public void RemoveBundleFile(string[] fileNames)
    {
        var contributors = _contributors
            .Where(x =>
                x is BundleFileContributor bundleContributor
                && bundleContributor.Files.Any(f =>
                    fileNames.Any(name =>
                        name.Equals(f.FileName, StringComparison.OrdinalIgnoreCase)
                    )
                )
            )
        ...
        {
            contributor.Files.RemoveAll(x =>
                fileNames.Any(name => name.Equals(x.FileName, StringComparison.OrdinalIgnoreCase))
            );
        }
    }
    ...
    private IEnumerable<Type> GetDirectDependencies(Type contributorType)
    {
        ...
        return dependsOnAttributes.SelectMany(a => a.GetDependedTypes());
    }
    ...
    private void AddInstanceToContributors(Type contributorType)
    {
        if (!typeof(IBundleContributor).IsAssignableFrom(contributorType))
        {
            throw new AbpException(
                $"Given {nameof(contributorType)} ({contributorType.AssemblyQualifiedName}) should implement the {typeof(IBundleContributor).AssemblyQualifiedName} interface!"
            );
        }
        ...
        {
            throw new AbpException(
                $"Can not instantiate {contributorType.AssemblyQualifiedName}",
                ex
            );
        }
    }
