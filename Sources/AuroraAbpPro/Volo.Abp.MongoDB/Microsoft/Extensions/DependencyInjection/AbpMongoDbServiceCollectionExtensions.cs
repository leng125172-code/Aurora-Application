...
public static class AbpMongoDbServiceCollectionExtensions
{
    ...
    ) //Created overload instead of default parameter
        where TMongoDbContext : AbpMongoDbContext
    {
        ...
        var replacedDbContextTypes = typeof(TMongoDbContext)
        ...
        foreach (var dbContextType in replacedDbContextTypes)
        {
            options.ReplaceDbContext(
                dbContextType.Type,
                multiTenancySides: dbContextType.MultiTenancySide
            );
        }
        ...
        foreach (var entry in options.ReplacedDbContextTypes)
        {
            ...
            {
                var multiTenantDbContextType = new MultiTenantDbContextType(
                    originalDbContextType,
                    entry.Key.MultiTenancySide
                );
