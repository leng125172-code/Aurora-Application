    ...
    where TMongoDbContext : IAbpMongoDbContext
{
    ...
    public virtual async Task<TMongoDbContext> GetDbContextAsync(
    ...
    )
    {
        ...
        if (databaseApi == null)
        {
            databaseApi = new MongoDbDatabaseApi(
                await CreateDbContextAsync(unitOfWork, mongoUrl, databaseName, cancellationToken)
            );
    ...
    )
    {
        ...
        var activeTransaction =
            unitOfWork.FindTransactionApi(transactionApiKey) as MongoDbTransactionApi;
        var dbContext = unitOfWork.ServiceProvider.GetRequiredService<TMongoDbContext>();
        ...
        if (activeTransaction?.SessionHandle == null)
        {
            ...
            unitOfWork.AddTransactionApi(
                transactionApiKey,
                new MongoDbTransactionApi(session, CancellationTokenProvider)
            );
        ...
        else
        {
            dbContext
                .ToAbpMongoDbContext()
                .InitializeDatabase(database, client, activeTransaction.SessionHandle);
        }
