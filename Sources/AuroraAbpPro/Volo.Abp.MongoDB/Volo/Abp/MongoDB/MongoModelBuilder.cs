...
public class MongoModelBuilder : IMongoModelBuilder
{
    ...
    public virtual MongoDbContextModel Build(AbpMongoDbContext dbContext)
    {
        lock (SyncObj)
        {
            var useAbpClockHandleDateTime = dbContext
                .LazyServiceProvider.LazyGetRequiredService<IOptions<AbpMongoDbOptions>>()
                .Value.UseAbpClockHandleDateTime;
            ...
            foreach (var entityModel in entityModels.Values)
            {
                ...
                {
                    var dateTimePropertyInfos = entityModel
                    ...
                    dateTimePropertyInfos.ForEach(property =>
                    {
                        var disableDateTimeNormalization =
                            entityModel.EntityType.IsDefined(
                        ...
                        if (property.PropertyType == typeof(DateTime?))
                        {
                            map.MapProperty(property.Name)
                                .SetSerializer(
                                    new NullableSerializer<DateTime>().WithSerializer(
                                        new AbpMongoDbDateTimeSerializer(
                                            GetDateTimeKind(dbContext),
                ...
                var createCollectionOptions = entityModel
                    .As<IHasCreateCollectionOptions>()
            ...
            foreach (var baseClass in baseClasses)
            {
                if (!BsonClassMap.IsClassMapRegistered(baseClass))
                {
                    ...
                    if (useAbpClockHandleDateTime)
                    {
                        var dateTimePropertyInfos = baseClass
                            .GetProperties(
                                BindingFlags.DeclaredOnly
                                    | BindingFlags.Instance
                                    | BindingFlags.NonPublic
    ...
    {
        return dbContext
            .LazyServiceProvider.LazyGetRequiredService<IOptions<AbpClockOptions>>()
            .Value.Kind;
    }
    ...
    )
    {
        ...
        var model = (IMongoEntityModelBuilder)
            _entityModelBuilders.GetOrAdd(
                entityType,
                () =>
    ...
    protected virtual void CreateCollectionIndexes(
        AbpMongoDbContext dbContext,
        string collectionName,
        Action<IMongoIndexManager<BsonDocument>>? indexesAction = null
    )
    {
