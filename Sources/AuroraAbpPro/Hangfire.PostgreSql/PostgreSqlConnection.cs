...
namespace Hangfire.PostgreSql
{
    ...
    public class PostgreSqlConnection : JobStorageConnection
    {
      ...
      ";
            ...
            return _storage.UseTransaction(
                _dedicatedConnection,
                (connection, transaction) =>
                {
                    string jobId = connection
                        .QuerySingle<long>(
                            createJobSql,
                            new
                            {
                                InvocationData = JsonParameter.GetParameterValue(
                                    SerializationHelper.Serialize(invocationData)
                                ),
                                Arguments = JsonParameter.GetParameterValue(
                                    invocationData.Arguments,
                                    JsonParameter.ValueType.Array
                                ),
                                CreatedAt = createdAt,
      ...
      ";
            ...
            SqlJob jobData = _storage.UseConnection(
                _dedicatedConnection,
                connection =>
                    connection.QuerySingleOrDefault<SqlJob>(
                        sql,
                        new { Id = Convert.ToInt64(id, CultureInfo.InvariantCulture) }
                    )
            );
            ...
            InvocationData invocationData = SerializationHelper.Deserialize<InvocationData>(
                jobData.InvocationData
            );
      ...
      ";
            ...
            SqlState sqlState = _storage.UseConnection(
                _dedicatedConnection,
                connection =>
                    connection.QuerySingleOrDefault<SqlState>(
                        sql,
                        new { JobId = Convert.ToInt64(jobId, CultureInfo.InvariantCulture) }
                    )
            );
      ...
      ";
        ...
        public override string GetJobParameter(string id, string name)
        {
            ...
            return _storage.UseConnection(
                _dedicatedConnection,
                connection =>
                    connection.QuerySingleOrDefault<string>(
                        query,
                        new { Id = Convert.ToInt64(id, CultureInfo.InvariantCulture), Name = name }
                    )
            );
        }
        ...
        )
        {
            ...
            if (toScore < fromScore)
            {
                throw new ArgumentException(
                    $"The '{nameof(toScore)}' value must be higher or equal to the '{nameof(fromScore)}' value."
                );
            }
        ...
        )
        {
            ...
            {
                throw new ArgumentException(
                    $"The '{nameof(count)}' value must be greater than zero (0)."
                );
            }
      ...
      ";
            ...
            while (true)
            {
                ...
                if (executionTimer.Elapsed > _options.TransactionSynchronisationTimeout)
                {
                    throw new TimeoutException(
                        "SetRangeInHash experienced timeout while trying to execute transaction"
                    );
                }
            }
        }
        ...
        public override Dictionary<string, string> GetAllEntriesFromHash(string key)
        {
            ...
            Dictionary<string, string> result = _storage.UseConnection(
                _dedicatedConnection,
                connection =>
                    connection
                        .Query<SqlHash>(
            ...
            _storage.UseConnection(
                ...
                connection =>
                    connection.Execute(
                        ...
                        {
                            Id = serverId,
                            Data = JsonParameter.GetParameterValue(
      ...
      ";
            ...
            int affectedRows = _storage.UseConnection(
                _dedicatedConnection,
                connection => connection.Execute(query, new { Id = serverId })
            );
        ...
        public override long GetSetCount(string key)
        {
            ...
            return _storage.UseConnection(
                _dedicatedConnection,
                connection => connection.QuerySingleOrDefault<long>(query, new { Key = key })
            );
        }
        ...
        public override List<string> GetAllItemsFromList(string key)
        {
            ...
            return _storage.UseConnection(
                _dedicatedConnection,
                connection => connection.Query<string>(query, new { Key = key }).AsList()
            );
        }
        ...
        public override long GetCounter(string key)
        {
            ...
            return _storage.UseConnection(
                _dedicatedConnection,
                connection => connection.QuerySingleOrDefault<long?>(query, new { Key = key }) ?? 0
            );
        }
        ...
        public override TimeSpan GetListTtl(string key)
        {
            ...
            DateTime? result = _storage.UseConnection(
                _dedicatedConnection,
                connection => connection.QuerySingleOrDefault<DateTime?>(query, new { Key = key })
            );
        ...
        public override long GetHashCount(string key)
        {
            ...
            return _storage.UseConnection(
                _dedicatedConnection,
                connection => connection.QuerySingle<long>(query, new { Key = key })
            );
        }
        ...
        public override string GetValueFromHash(string key, string name)
        {
            ...
            return _storage.UseConnection(
                _dedicatedConnection,
                connection =>
                    connection.QuerySingleOrDefault<string>(query, new { Key = key, Field = name })
            );
        }
        ...
        private IDisposable AcquireLock(string resource, TimeSpan timeout)
        {
            ...
            if (!_lockedResources.ContainsKey(resource))
            {
                try
                {
                    PostgreSqlDistributedLock.Acquire(
                        _dedicatedConnection,
                        resource,
                        timeout,
                        _options
