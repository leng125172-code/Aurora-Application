...
namespace Hangfire.PostgreSql
{
    public class PostgreSqlStorageOptions
    {
        ...
        public TimeSpan QueuePollInterval
        {
            ...
            set
            {
                ThrowIfValueIsLowerThan(
                    _minimumQueuePollInterval,
                    value,
                    nameof(QueuePollInterval)
                );
        ...
        private void ThrowIfValueIsLowerThan(TimeSpan minValue, TimeSpan value, string fieldName)
        {
            ...
            {
                string message =
                    $"The {fieldName} property value seems to be too low ({value}, lower than suggested minimum of {minValue}). Consider increasing it. If you really need to have such a low value, please set {nameof(PostgreSqlStorageOptions)}.{nameof(AllowUnsafeValues)} to true.";
        ...
        private static void ThrowIfValueIsNotPositive(int value, string fieldName)
        {
            if (value <= 0)
            {
                throw new ArgumentException(
                    $"The {fieldName} property value should be positive. Given: {value}."
                );
            }
        }
    }
