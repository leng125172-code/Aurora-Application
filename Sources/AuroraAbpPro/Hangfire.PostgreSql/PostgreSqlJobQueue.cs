...
namespace Hangfire.PostgreSql
{
    ...
    public class PostgreSqlJobQueue : IPersistentJobQueue
    {
      ...
      ";
        ...
        [NotNull]
        internal IFetchedJob Dequeue_Transaction(
            string[] queues,
      ...
      ";
            ...
            WaitHandle[] nextFetchIterationWaitHandles = new[]
            {
                ...
                .Concat(_queueEventRegistry.GetWaitHandles(queues))
                .ToArray();
            ...
            do
            {
                ...
                Utils.Utils.TryExecute(
                    () =>
                    {
                        ...
                        try
                        {
                            using NpgsqlTransaction trx = connection.BeginTransaction(
                                IsolationLevel.ReadCommitted
                            );
                            FetchedJob jobToFetch = connection.QuerySingleOrDefault<FetchedJob>(
                                fetchJobSql,
        ...
        [NotNull]
        internal IFetchedJob Dequeue_UpdateCount(
            string[] queues,
            CancellationToken cancellationToken
        )
        {
      ...
      ";
            ...
            do
            {
                ...
                FetchedJob jobToFetch = _storage.UseConnection(
                    null,
                    connection =>
                        connection.QuerySingleOrDefault<FetchedJob>(
                            jobToFetchSql,
                            new { Queues = queues.ToList() }
                        )
                );
                ...
                {
                    WaitHandle.WaitAny(
                        new[] { cancellationToken.WaitHandle, SignalDequeue, JobQueueNotification },
                        _storage.Options.QueuePollInterval
                    );
                ...
                {
                    markJobAsFetched = _storage.UseConnection(
                        null,
                        connection =>
                            connection.QuerySingleOrDefault<FetchedJob>(
                                markJobAsFetchedSql,
