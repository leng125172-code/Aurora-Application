...
namespace Hangfire.PostgreSql
{
    ...
    public class PostgreSqlMonitoringApi : IMonitoringApi
    {
        ...
        )
        {
            ...
            _queueProviders =
                queueProviders ?? throw new ArgumentNullException(nameof(queueProviders));
        }
        ...
        public JobList<ProcessingJobDto> ProcessingJobs(int from, int count)
        {
            return GetJobs(
                ...
                (_, job, stateData) =>
                    ...
                    {
                        Job = job,
                        ServerId = stateData.TryGetValue("ServerId", out string serverId)
                            ? serverId
        ...
        public IList<ServerDto> Servers()
        {
            ...
            {
                List<(Entities.Server Server, ServerData Data)> servers = connection
                    .Query<Entities.Server>(
                        $@"SELECT * FROM ""{_storage.Options.SchemaName}"".""server"""
                    )
                    .AsEnumerable()
                    .Select(server =>
                        (server, SerializationHelper.Deserialize<ServerData>(server.Data))
                    )
                    .ToList();

                List<ServerDto> result = servers
                    .Select(item => new ServerDto
                    {
                        Name = item.Server.Id,
            ...
            return GetJobs(
                ...
                (_, job, stateData) =>
                    new SucceededJobDto
                    {
                        ...
                        TotalDuration =
                            stateData.ContainsKey("PerformanceDuration")
                            && stateData.TryGetValue("Latency", out string latency)
                                ? long.Parse(stateData["PerformanceDuration"])
                                    + (long?)long.Parse(latency)
                                : null,
                        SucceededAt = JobHelper.DeserializeNullableDateTime(
                            stateData["SucceededAt"]
                        ),
                    }
            );
        }
        ...
        public IList<QueueWithTopEnqueuedJobsDto> Queues()
        {
            ...
            {
                IEnumerable<long> enqueuedJobIds = tuple.Monitoring.GetEnqueuedJobIds(
                    tuple.Queue,
                    0,
                    5
                );
                EnqueuedAndFetchedCountDto counters = tuple.Monitoring.GetEnqueuedAndFetchedCount(
                    tuple.Queue
                );
        ...
        ";
                ...
                Dictionary<string, string> parameters = multi
                    .Read<JobParameter>()
                    .ToDictionary(x => x.Name, x => x.Value);
                List<StateHistoryDto> history = multi
                    .Read<SqlState>()
                    .ToList()
        ...
        ";
                ...
                {
                    Dictionary<string, long> countByStates = multi
                        .Read<(string StateName, long Count)>()
                        .ToDictionary(x => x.StateName, x => x.Count);

                    long GetCountIfExists(string name)
                    {
                        return countByStates.TryGetValue(name, out long stateCount)
                            ? stateCount
                            : 0;
                    }
        ...
        private Dictionary<DateTime, long> GetHourlyTimelineStats(string type)
        {
            ...
            Dictionary<string, DateTime> keyMaps = dates.ToDictionary(
        ...
        private Dictionary<DateTime, long> GetTimelineStats(IDictionary<string, DateTime> keyMaps)
        {
            ...
            Dictionary<string, long> valuesMap = UseConnection(connection =>
                connection
                    .Query<KeyCount>(query, new { Keys = keyMaps.Keys.ToList() })
                    .AsList()
      ...
      ";
            ...
            List<SqlJob> jobs = UseConnection(connection =>
                connection.Query<SqlJob>(enqueuedJobsSql, new { JobIds = jobIds.ToList() }).AsList()
            );
            ...
            return DeserializeJobs(
                ...
                (sqlJob, job, stateData) =>
                    new EnqueuedJobDto
                    {
                        ...
                        EnqueuedAt =
                            sqlJob.StateName == EnqueuedState.StateName
        ...
        private long GetNumberOfJobsByStateName(string stateName)
        {
            ...
            return UseConnection(connection =>
                connection.QuerySingle<long>(sqlQuery, new { StateName = stateName })
            );
        }
      ...
      ";
        ...
        )
        {
            ...
            foreach (SqlJob job in jobs)
            {
                ...
                {
                    Dictionary<string, string> deserializedData = SerializationHelper.Deserialize<
                        Dictionary<string, string>
                    >(job.StateData);
                    SafeDictionary<string, string> stateData =
                    ...
                    dto = selector(
                        job,
                        DeserializeJob(job.InvocationData, job.Arguments),
                        stateData
                    );
                }
      ...
      ";
            ...
            List<SqlJob> jobs = UseConnection(connection =>
                connection.Query<SqlJob>(fetchedJobsSql, new { JobIds = jobIds.ToList() }).AsList()
            );

            Dictionary<string, FetchedJobDto> result = jobs.ToDictionary(
                job => job.Id.ToString(),
