...
namespace Hangfire.PostgreSql
{
    public class PostgreSqlStorage : JobStorage
    {
        ...
        [Obsolete(
            "Will be removed in 2.0, please use the overload with IConnectionFactory argument"
        )]
        ...
        public PostgreSqlStorage(
            ...
            : this(new NpgsqlConnectionFactory(connectionString, options, connectionSetup), options)
        { }
        ...
        public PostgreSqlStorage(
            ...
            : this(new ExistingNpgsqlConnectionFactory(existingConnection, options), options) { }

        public PostgreSqlStorage(IConnectionFactory connectionFactory)
            : this(connectionFactory, new PostgreSqlStorageOptions()) { }
...
#pragma warning restore CS0618
        ...
        public override void WriteOptionsToLog(ILog logger)
        {
            ...
            logger.InfoFormat(
                "    Use sliding invisibility timeout: {0}.",
                Options.UseSlidingInvisibilityTimeout
            );
        }
            ...
            try
            {
                ...
                if (Options.EnableLongPolling && !connection.SupportsNotifications())
                {
                    throw new InvalidOperationException(
                        "Long polling is supported only with PostgreSQL version 11 or higher."
                    );
                }
        ...
        private void TryInitializeStorage(bool isStartup)
        {
            ...
            while (attempts < maxAttempts)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    TimeSpan delay = ComputeBackoffDelay(
                        attempts,
                        Options.StartupConnectionBaseDelay,
                        Options.StartupConnectionMaxDelay
                    );
        ...
        private static TimeSpan ComputeBackoffDelay(
            int attempt,
            TimeSpan baseDelay,
            TimeSpan maxDelay
        )
        ...
        internal void UseTransaction(
            ...
            Func<TransactionScope> transactionScopeFactory
        )
        {
            UseTransaction(
                dedicatedConnection,
        ...
        internal T UseTransaction<T>(
            DbConnection dedicatedConnection,
            Func<DbConnection, DbTransaction, T> func,
            Func<TransactionScope> transactionScopeFactory
        )
        {
            return UseConnection(
        ...
        {
            return TransactionHelpers.CreateTransactionScope(
                isolationLevel,
                Options.EnableTransactionScopeEnlistment,
                timeout
            );
        }

        private static System.Data.IsolationLevel? ConvertIsolationLevel(
            IsolationLevel? isolationLevel
        )
        {
        ...
        internal void UseConnection(
            DbConnection dedicatedConnection,
            [InstantHandle] Action<DbConnection> action
        )
        {
            UseConnection(
                dedicatedConnection,
        ...
        private bool IsExistingConnection(IDbConnection connection)
        {
            return connection != null
                && _connectionFactory is ExistingNpgsqlConnectionFactory
                && ReferenceEquals(connection, _connectionFactory.GetOrCreateConnection());
        }
