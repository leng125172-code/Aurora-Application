...
namespace Hangfire.PostgreSql
{
    public sealed class PostgreSqlDistributedLock
    {
        ...
        private static void Log(string resource, string message, Exception ex)
        {
            ...
            _logger.Log(
                isConcurrencyError ? LogLevel.Trace : LogLevel.Warn,
                () => $"{resource}: {message}",
                ex
            );
        }
        ...
        private static class LockHandler
        {
            ...
            )
            {
                ...
                Func<IDbConnection, string, string, bool> tryLock =
                    options.UseNativeDatabaseTransactions
                ...
                while (tryAcquireLock)
                {
                    ...
                    {
                        int sleepDuration = (int)(
                            timeout.TotalMilliseconds - lockAcquiringTime.ElapsedMilliseconds
                        );
            ...
            public static bool TryRemoveLock(
                string resource,
                IDbConnection connection,
                PostgreSqlStorageOptions options,
                bool onlyExpired
            )
            {
                ...
                try
                {
                    ...
                    DateTime timeout = onlyExpired
                        ? DateTime.UtcNow - options.DistributedLockTimeout
                        : DateTime.MaxValue;

                    int rowsAffected = connection.Execute(
                        $@"DELETE FROM ""{options.SchemaName}"".""lock"" WHERE ""resource"" = @Resource AND ""acquired"" < @Timeout",
                        new { Resource = resource, Timeout = timeout },
                        trx
                    );
        ...
        private static class TransactionLockHandler
        {
            public static bool TryLock(IDbConnection connection, string schemaName, string resource)
            {
              ...
              ",
                        new { Resource = resource, Acquired = DateTime.UtcNow },
                        trx
                    );
                    trx?.Commit();
