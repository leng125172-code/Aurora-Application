...
namespace Hangfire.PostgreSql
{
    public static class PostgreSqlObjectsInstaller
    {
        ...
        public static void Install(NpgsqlConnection connection, string schemaName = "hangfire")
        {
            ...
            do
            {
                try
                {
                    ...
                    {
                        string commandText =
                            $@"{script}; UPDATE ""{schemaName}"".""schema"" SET ""version"" = @Version WHERE ""version"" = @PreviousVersion";
        ...
        private static bool VersionAlreadyApplied(
        ...
        )
        {
            try
            {
                using NpgsqlCommand command = new(
                    $@"SELECT true ""VersionAlreadyApplied"" FROM ""{schemaName}"".""schema"" WHERE ""version"" >= $1",
                    connection
                );
