...
use aurora_core_service::actor::{StationHandle, StationRunState};
...
async fn restart_requires_local_acknowledgement_before_start() {
    ...
    let station = StationHandle::spawn("main", Arc::clone(&journal));
    ...
    assert_eq!(journal.pending_count().expect("outbox count"), 2);
}
